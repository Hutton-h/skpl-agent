    ...
    async def _load_single_skill(self, skill_root: str) -> Skill | None:
        ...
        try:
            ...
            if not name or not description:
                logger.warning('SKILL.md in %s is missing required fields (name or description). Skipping.', skill_root)
                return None
            skill = Skill(name=str(name), description=str(description), dir=skill_root, markdown=content.content, updated_at=updated_at)
            self._cache[skill_root] = skill
            return skill
        except Exception as e:
            logger.warning('Failed to load skill from %s: %s', skill_root, str(e))
            return None
    ...
    async def list_skills(self) -> list[Skill]:
        ...
        try:
            if not await aiofiles.ospath.isdir(self.directory):
                logger.warning('Skill directory %s does not exist.', self.directory)
                return []
            ...
            logger.warning('Failed to list skills from directory %s: %s', self.directory, str(e))
            return []