...
from ..message import Msg, TextBlock, HintBlock, DataBlock, ToolCallBlock, ToolResultBlock, ThinkingBlock, URLSource, Base64Source

class _OllamaFormatterBase(FormatterBase, ABC):
    """Base class for Ollama formatters, providing shared data block
    formatting logic."""

    def _format_ollama_data_block(self, block: DataBlock) -> str | None:
        ...
        if not media_type.startswith('image/'):
            logger.warning('Ollama only supports image data, got %s, skipped.', media_type)
            return None
...
class OllamaChatFormatter(_OllamaFormatterBase):
    ...
    async def format(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        for msg in msgs:
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    if content_parts or images:
                        msg_flush = {'role': msg.role, 'content': '\n'.join(content_parts)}
                        if images:
                    ...
                    if isinstance(block.hint, str):
                        ...
                        for sub in block.hint:
                            if isinstance(sub, TextBlock):
                                ...
                                formatted_sub = self._format_ollama_data_block(sub)
                                if formatted_sub:
                                    hint_images.append(formatted_sub)
                        if hint_text_parts or hint_images:
                            hint_msg: dict[str, Any] = {'role': 'user', 'content': '\n'.join(hint_text_parts)}
                            if hint_images:
                    ...
                    messages.append({'role': msg.role, 'content': '\n'.join(content_parts) if content_parts else '', 'tool_calls': [{'function': {'name': block.name, 'arguments': _json_loads_with_repair(block.input or '{}')}}]})
                    content_parts = []
                    images = []
                    ...
                    messages.append({'role': 'tool', 'tool_name': block.name, 'content': textual_output})
                    if multimodal_data:
                        user_images = []
                        user_content_parts = []
                        ...
                        user_msg = {'role': 'user', 'content': '\n'.join(user_content_parts) if user_content_parts else textual_output}
                        if user_images:
            ...
            if content_parts or images:
                msg_ollama: dict[str, Any] = {'role': msg.role, 'content': '\n'.join(content_parts) if content_parts else ''}
                if images:
...
class OllamaMultiAgentFormatter(_OllamaFormatterBase):
    ...
    async def _format_tool_sequence(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        return await OllamaChatFormatter(input_types=self.input_types).format(msgs)
    ...
    async def _format_agent_message(self, msgs: list[Msg], is_first: bool) -> list[dict[str, Any]]:
        ...
        for msg in msgs:
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    logger.warning('Unsupported block type %s in agent message, skipped.', type(block))
            if msg_text_parts:
        ...
        if conversation_blocks and is_first:
            if conversation_blocks[0].get('text'):
                ...
                conversation_blocks.insert(0, {'text': self.conversation_history_prompt + '<history>\n'})
            if conversation_blocks[-1].get('text'):
                conversation_blocks[-1]['text'] += '\n</history>'
            else:
                conversation_blocks.append({'text': '</history>'})
        conversation_blocks_text = '\n'.join((conversation_block.get('text', '') for conversation_block in conversation_blocks))
        user_message: dict[str, Any] = {'role': 'user', 'content': conversation_blocks_text}
        if images:
    ...
    async def _format_system_message(msg: Msg) -> dict[str, Any]:
        ...
        for block in msg.get_content_blocks():
            if isinstance(block, TextBlock):
                ...
                logger.warning('Unsupported block type %s in system message, skipped.', type(block))
