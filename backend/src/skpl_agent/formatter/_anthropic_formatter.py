...
from ..message import Msg, TextBlock, ThinkingBlock, HintBlock, DataBlock, ToolCallBlock, ToolResultBlock, URLSource, Base64Source
...
class _AnthropicFormatterBase(FormatterBase, ABC):
    ...
    async def _format_messages(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        for msg in msgs:
            ...
            for block in msg.get_content_blocks():
                if has_tool_result and content_blocks and (not isinstance(block, ToolResultBlock)):
                ...
                if isinstance(block, TextBlock):
                    ...
                    redacted_data = getattr(block, 'redacted_thinking_data', None)
                    if redacted_data is not None:
                        content_blocks.append({'type': 'redacted_thinking', 'data': redacted_data})
                    else:
                        signature = getattr(block, 'signature', None)
                        if signature:
                            content_blocks.append({'type': 'thinking', 'thinking': block.thinking, 'signature': signature})
                        else:
                            logger.debug('Dropping ThinkingBlock without signature; Anthropic requires a valid signature.')
                    ...
                    if isinstance(block.hint, str):
                        messages.append({'role': 'user', 'content': [{'type': 'text', 'text': block.hint}]})
                    else:
                        ...
                        if hint_parts:
                            messages.append({'role': 'user', 'content': hint_parts})
                elif isinstance(block, DataBlock):
                    formatted_block = self._format_anthropic_data_block(block)
                    if formatted_block:
                        content_blocks.append(formatted_block)
                elif isinstance(block, ToolCallBlock):
                    content_blocks.append({'type': 'tool_use', 'id': block.id, 'name': block.name, 'input': _json_loads_with_repair(block.input or '{}')})
                    ...
                    if isinstance(output, str):
                        if output:
                            tool_result_content.append({'type': 'text', 'text': output})
                    else:
                        for out_block in output:
                            if isinstance(out_block, TextBlock):
                                if out_block.text:
                                    tool_result_content.append({'type': 'text', 'text': out_block.text})
                            elif isinstance(out_block, DataBlock):
                                fmt_block = self._format_anthropic_data_block(out_block)
                                if fmt_block:
                                    ...
                                    if isinstance(source, URLSource):
                                        fallback = f'[{main_type} file returned, URL: {source.url}]'
                                    else:
                                        fallback = f'[{main_type} file returned, type: {source.media_type}]'
                                    tool_result_content.append({'type': 'text', 'text': fallback})
                    if not tool_result_content:
                        tool_result_content.append({'type': 'text', 'text': '(empty tool output)'})
                    content_blocks.append({'type': 'tool_result', 'tool_use_id': block.id, 'content': tool_result_content})
    ...
    def _format_anthropic_data_block(self, block: DataBlock) -> dict[str, Any] | None:
        ...
        if not any((fnmatch.fnmatch(media_type, pattern) for pattern in self.supported_input_media_types)):
            logger.warning('Media type %s is not supported, skipped.', media_type)
            return None
        if not media_type.startswith('image/'):
            logger.warning('Anthropic only supports image data, got %s, skipped.', media_type)
            return None
    ...
    def _format_image_source(source: URLSource | Base64Source) -> dict[str, Any]:
        ...
        if isinstance(source, Base64Source):
            ...
            if url.startswith('file://'):
                ...
                return {'type': 'image', 'source': {'type': 'base64', 'media_type': source.media_type, 'data': data}}
            else:
...
class AnthropicChatFormatter(_AnthropicFormatterBase):
    ...
    """
    input_types: list[str] = Field(default_factory=lambda : ['text/plain', 'image/*'], description='The supported input types. Defaults to ``["text/plain", "image/*"]``.')
...
class AnthropicMultiAgentFormatter(_AnthropicFormatterBase):
    ...
    async def format(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        async for (typ, group) in self._group_messages(msgs[start_index:]):
            match typ:
                case 'tool_sequence':
                    formatted_msgs.extend(await self._format_messages(group))
                case 'agent_message':
                    formatted_msgs.extend(await self._format_agent_message(group, is_first_agent_message))
                    is_first_agent_message = False
        return formatted_msgs

    async def _format_agent_message(self, msgs: list[Msg], is_first: bool) -> list[dict[str, Any]]:
        ...
        for msg in msgs:
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    if formatted_block:
                        if accumulated_text:
                            conversation_blocks.append({'type': 'text', 'text': '\n'.join(accumulated_text)})
                            accumulated_text = []
        ...
        if conversation_blocks and is_first:
            if conversation_blocks[0].get('text'):
                conversation_blocks[0]['text'] = self.conversation_history_prompt + '<history>\n' + conversation_blocks[0]['text']
            else:
                conversation_blocks.insert(0, {'type': 'text', 'text': self.conversation_history_prompt + '<history>\n'})
            if conversation_blocks[-1].get('text'):
                conversation_blocks[-1]['text'] += '\n</history>'
            else:
                conversation_blocks.append({'type': 'text', 'text': '</history>'})
        if conversation_blocks:
            return [{'role': 'user', 'content': conversation_blocks}]
        return []
