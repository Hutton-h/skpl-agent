...
from ..message import Msg, TextBlock, DataBlock, ThinkingBlock, HintBlock, ToolCallBlock, ToolResultBlock
...
class DeepSeekChatFormatter(FormatterBase):
    ...
    """
    input_types: list[str] = Field(default_factory=lambda : ['text/plain'], description='The supported input types. Defaults to ``["text/plain"]`` (DeepSeek does not support multimodal input).')
    ...
    async def format(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        for msg in msgs:
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    if content_blocks or tool_calls or reasoning_content_blocks:
                        ...
                        msg_flush_hint: dict[str, Any] = {'role': msg.role, 'content': content_text or (None if tool_calls else '')}
                        if msg.role == 'assistant':
                            msg_flush_hint['reasoning_content'] = '\n'.join(reasoning_content_blocks) if reasoning_content_blocks else ''
                        if tool_calls:
                    ...
                    if isinstance(block.hint, str):
                        ...
                        for sub in block.hint:
                            if isinstance(sub, TextBlock):
                                ...
                                hint_text_parts.append(f'[{sub.source.media_type} attached, not supported by this provider]')
                        if hint_text_parts:
                    ...
                    if content_blocks or tool_calls or reasoning_content_blocks:
                        content_text = '\n'.join((b.get('text', '') for b in content_blocks))
                        msg_flush: dict[str, Any] = {'role': msg.role, 'content': content_text or (None if tool_calls else '')}
                        if msg.role == 'assistant':
            ...
            content_msg = '\n'.join((b.get('text', '') for b in content_blocks))
            msg_deepseek: dict[str, Any] = {'role': msg.role, 'content': content_msg or (None if tool_calls else '')}
            if msg.role == 'assistant':
                msg_deepseek['reasoning_content'] = '\n'.join(reasoning_content_blocks) if reasoning_content_blocks else ''
            if tool_calls:
                msg_deepseek['tool_calls'] = tool_calls
            if msg_deepseek['content'] or msg_deepseek.get('tool_calls') or reasoning_content_blocks:
...
class DeepSeekMultiAgentFormatter(FormatterBase):
    ...
    async def _format_tool_sequence(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        return await DeepSeekChatFormatter(input_types=self.input_types).format(msgs)
    ...
    async def _format_agent_message(self, msgs: list[Msg], is_first: bool=True) -> list[dict[str, Any]]:
        ...
        if conversation_blocks_text:
            formatted_msgs.append({'role': 'user', 'content': conversation_blocks_text})
        return formatted_msgs
