...
from ..message import Msg, TextBlock, ThinkingBlock, ToolCallBlock, ToolResultBlock, DataBlock, URLSource, Base64Source, HintBlock
...
class XAIChatFormatter(FormatterBase):
    ...
    """
    input_types: list[str] = Field(default=['text/plain', 'image/jpeg', 'image/png'], description='The supported input types. Defaults to ``["text/plain", "image/jpeg", "image/png"]``.')

    async def format(self, msgs: list[Msg], **kwargs: Any) -> List[Any]:
        ...
        Returns:
            `list[Any]`:
                ...
                ``chat.append()``.
        """
        from xai_sdk.chat import assistant, image, system, tool_result, user, chat_pb2
        ...
        for msg in msgs:
            ...
            if msg.role == 'system':
                ...
                for block in blocks:
                    if isinstance(block, ThinkingBlock):
                        ...
                        if isinstance(block.hint, str):
                            ...
                            hint_args = self._xai_user_args_from_blocks(block.hint, image)
                            if hint_args:
                        ...
                        if block.source.media_type.startswith('image/'):
                            if isinstance(block.source, URLSource):
                                ...
                                if url_str.startswith('file://'):
                                    ...
                                    content_args.append(image(f'data:{block.source.media_type};base64,{encoded}'))
                                else:
                                    content_args.append(image(url_str))
                            elif isinstance(block.source, Base64Source):
                                content_args.append(image(f'data:{block.source.media_type};base64,{block.source.data}'))
                        else:
                            logger.warning('Unsupported media type %s for xAI API. Only image/jpeg and image/png are supported. This block will be skipped.', block.source.media_type)
                    else:
                        logger.warning('Unsupported block type %s in user message, skipped.', type(block).__name__)
                if content_args:
                    xai_messages.append(user(*content_args))
                ...
                for block in blocks:
                    if isinstance(block, ToolResultBlock):
                        if pending_tool_calls:
                            ...
                            msg_proto.role = chat_pb2.MessageRole.Value('ROLE_ASSISTANT')
                            if pending_text:
                            ...
                            for tc in pending_tool_calls:
                                ...
                                proto_tc.id = tc.id
                                proto_tc.type = chat_pb2.ToolCallType.Value('TOOL_CALL_TYPE_CLIENT_SIDE_TOOL')
                            ...
                            pending_text = []
                        output_text = self._extract_result_text(block.output)
                        xai_messages.append(tool_result(output_text, tool_call_id=block.id))
                ...
                logger.warning("Unsupported message role '%s', skipped.", msg.role)
        return xai_messages

    def _xai_user_args_from_blocks(self, blocks: list, image: Any) -> list:
        ...
        for sub in blocks:
            if isinstance(sub, TextBlock):
                ...
                if not sub.source.media_type.startswith('image/'):
                    logger.warning('Unsupported media type %s for xAI API. Only image/jpeg and image/png are supported. This hint sub-block will be skipped.', sub.source.media_type)
                    continue
                if isinstance(sub.source, URLSource):
                    ...
                    if url_str.startswith('file://'):
                        ...
                        with open(local_path, 'rb') as f:
                            encoded = base64.b64encode(f.read()).decode('utf-8')
                        args.append(image(f'data:{sub.source.media_type};base64,{encoded}'))
                    else:
                        args.append(image(url_str))
                elif isinstance(sub.source, Base64Source):
                    args.append(image(f'data:{sub.source.media_type};base64,{sub.source.data}'))
        return args
    ...
    async def format(self, msgs: list[Msg], **kwargs: Any) -> List[Any]:
        ...
        async for (typ, group) in self._group_messages(msgs[start_index:]):
            if typ == 'tool_sequence':
                xai_messages.extend(await XAIChatFormatter(input_types=self.input_types).format(group))
            elif typ == 'agent_message':
                history_text = self._build_history_text(group, is_first=is_first_agent_message)
                if history_text:
