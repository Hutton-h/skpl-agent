...
from ..message import Msg, TextBlock, ThinkingBlock, ToolResultBlock, URLSource, DataBlock, ToolCallBlock, Base64Source, HintBlock
...
class _DashScopeFormatterBase(FormatterBase, ABC):
    ...
    def supported_input_media_types(self) -> list[str]:
        ...
        return [t for t in self.input_types if t not in ('text/plain', 'application/x-thinking')]
    ...
    def _format_dashscope_data_block(self, block: DataBlock) -> dict[str, Any] | None:
        ...
        """
        if not any((fnmatch(block.source.media_type, pattern) for pattern in self.supported_input_media_types)):
            logger.warning('Unsupported media type %s for DashScope API. Supported types: %s. This block will be skipped.', block.source.media_type, ', '.join(self.supported_input_media_types))
            return None
        ...
        logger.warning('Unsupported main media type %s for DashScope API. This block will be skipped.', main_type)
        return None

    @staticmethod
    def _format_image_source(source: URLSource | Base64Source) -> dict[str, Any]:
    ...
    @staticmethod
    def _format_video_source(source: URLSource | Base64Source) -> dict[str, Any]:
    ...
    @staticmethod
    def _format_audio_source(source: URLSource | Base64Source) -> dict[str, Any]:
        ...
        if isinstance(source, Base64Source):
            ...
            return {'type': 'input_audio', 'input_audio': {'data': source.data, 'format': fmt}}
        if isinstance(source, URLSource):
            ...
            if url_str.startswith('file://'):
                ...
                return {'type': 'input_audio', 'input_audio': {'data': url_str, 'format': fmt}}
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    if content_blocks or tool_calls or thinking_parts:
                        msg_openai: dict[str, Any] = {'role': msg.role, 'content': content_blocks or None}
                        if tool_calls:
                    ...
                    if isinstance(block.hint, str):
                        formatted_msgs.append({'role': 'user', 'content': [{'type': 'text', 'text': block.hint}]})
                    else:
                        hint_parts: list[dict] = []
                        for sub in block.hint:
                            if isinstance(sub, TextBlock):
                                hint_parts.append({'type': 'text', 'text': sub.text})
                            elif isinstance(sub, DataBlock):
                                formatted_sub = self._format_dashscope_data_block(sub)
                                if formatted_sub:
                                    hint_parts.append(formatted_sub)
                        if hint_parts:
                            formatted_msgs.append({'role': 'user', 'content': hint_parts})
                    ...
                    if content_blocks or tool_calls or thinking_parts:
                        msg_flush: dict[str, Any] = {'role': msg.role, 'content': content_blocks or None}
                        if tool_calls:
                    ...
                    formatted_msgs.append({'role': 'tool', 'tool_call_id': block.id, 'content': textual_output, 'name': block.name})
                    if multimodal_data:
                        promo_content = []
                        for item in multimodal_data:
                            if isinstance(item, TextBlock):
                                promo_content.append({'type': 'text', 'text': item.text})
                            elif isinstance(item, DataBlock):
                                fmt_item = self._format_dashscope_data_block(item)
                                if fmt_item is not None:
                                    promo_content.append(fmt_item)
                        if promo_content:
                            formatted_msgs.append({'role': 'user', 'content': promo_content})
                else:
                    logger.warning('Unsupported block type %s in the message, skipped.', type(block))
            msg_dashscope: dict[str, Any] = {'role': msg.role, 'content': content_blocks or None}
            if tool_calls:
                msg_dashscope['tool_calls'] = tool_calls
            if thinking_parts:
                msg_dashscope['reasoning_content'] = '\n'.join(thinking_parts)
            if msg_dashscope['content'] or msg_dashscope.get('tool_calls') or msg_dashscope.get('reasoning_content'):
...
class DashScopeMultiAgentFormatter(_DashScopeFormatterBase):
    ...
    .. tip:: Telling the assistant's name in the system prompt is very
     ...
     is playing as.
    """
    conversation_history_prompt: str = Field(description='The conversation history prompt.', default='# Conversation History\nThe content between <history></history> tags contains your conversation history\n')
    ...
    async def format(self, msgs: list[Msg]) -> list[dict]:
        ...
        async for (typ, group) in self._group_messages(msgs[start_index:]):
            match typ:
                ...
                case 'agent_message':
                    formatted_msgs.extend(await self._format_agent_message(group, is_first_agent_message))
                    is_first_agent_message = False
        return formatted_msgs

    async def _format_tool_sequence(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        """Given a sequence of tool call/result messages, format them into
        the required format for the DashScope API."""
        return await DashScopeChatFormatter(input_types=self.input_types).format(msgs)

    async def _format_agent_message(self, msgs: list[Msg], is_first: bool=True) -> list[dict[str, Any]]:
        ...
        if conversation_blocks:
            if conversation_blocks[0].get('text'):
                conversation_blocks[0]['text'] = conversation_history_prompt + '<history>\n' + conversation_blocks[0]['text']
            else:
                conversation_blocks.insert(0, {'text': conversation_history_prompt + '<history>\n'})
            if conversation_blocks[-1].get('text'):
                conversation_blocks[-1]['text'] += '\n</history>'
            else:
                conversation_blocks.append({'text': '</history>'})
        conversation_blocks_text = '\n'.join((b.get('text', '') for b in conversation_blocks))
        content_list: list[dict[str, Any]] = []
        if conversation_blocks_text:
            content_list.append({'type': 'text', 'text': conversation_blocks_text})
    ...
    async def _format_system_message(msg: Msg) -> dict[str, Any]:
        ...
        return {'role': 'system', 'content': msg.get_text_content()}