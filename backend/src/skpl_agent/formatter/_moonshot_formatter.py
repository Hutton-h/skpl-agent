...
from ..message import Msg, URLSource, Base64Source, TextBlock, DataBlock, ThinkingBlock, HintBlock, ToolCallBlock, ToolResultBlock

def _moonshot_format_image_source(source: URLSource | Base64Source) -> dict[str, Any]:
    ...
    *Preserved Thinking* feature works correctly in multi-turn conversations.
    ...
    async def format(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        while i < len(msgs):
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, ThinkingBlock):
                    ...
                    if content_blocks or tool_calls or reasoning_parts:
                        msg_moonshot = {'role': msg.role, 'name': msg.name, 'content': content_blocks or None}
                    ...
                    if content_blocks or tool_calls or reasoning_parts:
                        msg_flush = {'role': msg.role, 'name': msg.name, 'content': content_blocks or None}
                        if msg.role == 'assistant':
            ...
            if tool_calls:
                msg_moonshot['tool_calls'] = tool_calls
            if msg_moonshot['content'] or msg_moonshot.get('tool_calls') or reasoning_parts:
                messages.append(msg_moonshot)
            i += 1
        return messages
...
class MoonshotMultiAgentFormatter(_OpenAIFormatterBase):
    ...
    *Preserved Thinking* conversations.
    ...
    async def format(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        async for (typ, group) in self._group_messages(msgs[start_index:]):
            if typ == 'tool_sequence':
                formatted_msgs.extend(await MoonshotChatFormatter(input_types=self.input_types).format(group))
    ...
    async def _format_tool_sequence(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        return await MoonshotChatFormatter(input_types=self.input_types).format(msgs)
    ...
    async def _format_agent_message(self, msgs: list[Msg], is_first: bool=True) -> list[dict[str, Any]]:
        ...
        if history_text:
            history_text = conversation_history_prompt + '<history>\n' + history_text + '\n</history>'
