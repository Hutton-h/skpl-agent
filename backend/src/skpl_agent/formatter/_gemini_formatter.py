...
class _GeminiFormatterBase(FormatterBase, ABC):
    ...
    def _format_gemini_data_block(self, block: DataBlock) -> dict[str, Any] | None:
    ...
    @staticmethod
    def _format_media_source(source: URLSource | Base64Source) -> dict[str, Any]:
        ...
        if isinstance(source, Base64Source):
            return {'inline_data': {'data': source.data, 'mime_type': source.media_type}}
            ...
            if url.startswith('file://'):
                ...
                return {'inline_data': {'data': data, 'mime_type': source.media_type}}
            else:
...
class GeminiChatFormatter(_GeminiFormatterBase):
    ...
    """
    input_types: list[str] = Field(default_factory=lambda : ['text/plain', 'image/*', 'audio/*', 'video/*'], description='The supported input types. Defaults to ``["text/plain", "image/*", "audio/*", "video/*"]``.')
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    if isinstance(block.hint, str):
                        messages.append({'role': 'user', 'parts': [{'text': block.hint}]})
                    else:
                        ...
                        for sub in block.hint:
                            if isinstance(sub, TextBlock):
                                ...
                                formatted_sub = self._format_gemini_data_block(sub)
                                if formatted_sub:
                                    hint_parts.append(formatted_sub)
                        if hint_parts:
                            messages.append({'role': 'user', 'parts': hint_parts})
                elif isinstance(block, DataBlock):
                    formatted = self._format_gemini_data_block(block)
                    if formatted:
                        parts.append(formatted)
                elif isinstance(block, ToolCallBlock):
                    parts.append({'function_call': {'id': block.id, 'name': block.name, 'args': _json_loads_with_repair(block.input or '{}')}})
                    ...
                    messages.append({'role': 'user', 'parts': [{'function_response': {'id': block.id, 'name': block.name, 'response': {'output': textual_output}}}]})
                    if multimodal_data:
                        promo_parts = []
                        for item in multimodal_data:
                            if isinstance(item, TextBlock):
                                ...
                                fmt_item = self._format_gemini_data_block(item)
                                if fmt_item is not None:
                                    promo_parts.append(fmt_item)
                        if promo_parts:
                            messages.append({'role': 'user', 'parts': promo_parts})
                else:
...
class GeminiMultiAgentFormatter(_GeminiFormatterBase):
    ...
    async def _format_tool_sequence(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        return await GeminiChatFormatter(input_types=self.input_types).format(msgs)
    ...
    async def _format_agent_message(self, msgs: list[Msg], is_first: bool=True) -> list[dict[str, Any]]:
        ...
        if conversation_parts:
            if conversation_parts[0].get('text'):
                conversation_parts[0]['text'] = conversation_history_prompt + '<history>\n' + conversation_parts[0]['text']
            else:
                conversation_parts.insert(0, {'text': conversation_history_prompt + '<history>\n'})
            if conversation_parts[-1].get('text'):
                conversation_parts[-1]['text'] += '\n</history>'
            else:
                conversation_parts.append({'text': '</history>'})
            formatted_msgs.append({'role': 'user', 'parts': conversation_parts})
        return formatted_msgs

    @staticmethod
    async def _format_system_message(msg: Msg) -> dict[str, Any]:
        """Format system message for the Gemini API."""
        return {'role': 'user', 'parts': [{'text': msg.get_text_content()}]}