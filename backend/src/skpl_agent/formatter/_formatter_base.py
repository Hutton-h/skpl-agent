...
from ..message import Msg, DataBlock, TextBlock, URLSource, Base64Source
...
class FormatterBase(BaseModel):
    ...
    def convert_tool_result_to_string(self, output: str | List[TextBlock | DataBlock]) -> tuple[str, list[TextBlock | DataBlock]]:
        ...
        for block in output:
            if isinstance(block, TextBlock):
                ...
                if any((fnmatch(block.source.media_type, _) for _ in self.supported_input_media_types)):
                    identifier = shortuuid.uuid()
                    textual_output.append(f'<system-reminder>A(n) {main_type} file is returned and will be presented to you with the identifier [{identifier}].</system-reminder>')
                    multimodal_data.extend([TextBlock(text=f'- {identifier} ({main_type} file): '), block])
                elif isinstance(block.source, URLSource):
                    textual_output.append(f'<system-reminder>A(n) {main_type} file is returned and can be accessed at the URL: {block.source.url}.</system-reminder>')
                    ...
                    with tempfile.NamedTemporaryFile(suffix=extension, delete=False) as temp_file:
                        ...
                        textual_output.append(f'<system-reminder>A(n) {main_type} file is returned and saved locally at: {temp_file.name}.</system-reminder>')
        if multimodal_data:
            multimodal_data = [TextBlock(text='<system-reminder>The multimodal data and their identifiers are listed as follows:'), *multimodal_data, TextBlock(text='</system-reminder>')]
    ...
    async def _group_messages(msgs: list[Msg]) -> AsyncGenerator:
        ...
        for msg in msgs:
            if group_type is None:
                if msg.get_content_blocks('tool_call') or msg.get_content_blocks('tool_result'):
            ...
            if group_type == 'tool_sequence':
                if msg.has_content_blocks('tool_call') or msg.has_content_blocks('tool_result'):
                    group.append(msg)
                else:
