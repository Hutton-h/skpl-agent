...
from ..message import Msg, URLSource, TextBlock, DataBlock, Base64Source, ToolCallBlock, ToolResultBlock, HintBlock, ThinkingBlock

class _OpenAIFormatterBase(FormatterBase, ABC):
    """Base class for OpenAI formatters, providing shared data block
    formatting logic."""

    def _format_openai_data_block(self, block: DataBlock) -> dict[str, Any] | None:
        ...
        if not any((fnmatch(block.source.media_type, pattern) for pattern in self.supported_input_media_types)):
            logger.warning('Unsupported media type %s for OpenAI API. Supported types: %s. This block will be skipped.', block.source.media_type, ', '.join(self.supported_input_media_types))
            return None
        ...
        logger.warning('Unsupported main media type %s for OpenAI API. This block will be skipped.', main_type)
        return None

    def _format_image_source(self, source: URLSource | Base64Source) -> dict[str, Any]:
    ...
    def _format_audio_source(source: URLSource | Base64Source) -> dict[str, Any]:
        ...
        if isinstance(source, Base64Source):
            ...
            if media_type not in ['audio/wav', 'audio/mp3']:
                raise TypeError(f'Unsupported audio media type: {media_type}, only audio/wav and audio/mp3 are supported.')
            return {'type': 'input_audio', 'input_audio': {'data': source.data, 'format': media_type.split('/')[-1]}}
        if isinstance(source, URLSource):
            ...
            if url_str.startswith('file://'):
                ...
                if extension not in ['wav', 'mp3']:
                    raise TypeError(f'Unsupported audio file extension: {extension}, wav and mp3 are supported.')
                with open(local_path, 'rb') as f:
            ...
            return {'type': 'input_audio', 'input_audio': {'data': data, 'format': extension}}
...
class OpenAIChatFormatter(_OpenAIFormatterBase):
    ...
    """
    input_types: list[str] = Field(default_factory=lambda : ['text/plain', 'image/*', 'audio/*'], description='The supported input types. Defaults to ``["text/plain", "image/*", "audio/*"]``.')
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    if content_blocks or tool_calls:
                        msg_openai = {'role': msg.role, 'name': msg.name, 'content': content_blocks or None}
                        if tool_calls:
                    ...
                    if isinstance(block.hint, str):
                        ...
                        for sub in block.hint:
                            if isinstance(sub, TextBlock):
                                ...
                                formatted_sub = self._format_openai_data_block(sub)
                    ...
                    if content_blocks or tool_calls:
                        msg_openai_flush = {'role': msg.role, 'name': msg.name, 'content': content_blocks or None}
                        if tool_calls:
                    ...
                    messages.append({'role': 'tool', 'tool_call_id': block.id, 'content': textual_output, 'name': block.name})
                    if multimodal_data:
                        promo_content = []
                        for item in multimodal_data:
                            if isinstance(item, TextBlock):
                                ...
                                fmt_item = self._format_openai_data_block(item)
                                if fmt_item is not None:
                                    promo_content.append(fmt_item)
                        if promo_content:
                            messages.append({'role': 'user', 'name': 'system-reminder', 'content': promo_content})
...
class OpenAIMultiAgentFormatter(_OpenAIFormatterBase):
    ...
    async def _format_tool_sequence(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        return await OpenAIChatFormatter(input_types=self.input_types).format(msgs)
