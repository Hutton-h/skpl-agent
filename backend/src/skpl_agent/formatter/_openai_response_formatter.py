...
from ..message import Msg, TextBlock, DataBlock, ToolCallBlock, ToolResultBlock, HintBlock, ThinkingBlock
...
class _OpenAIResponseFormatterBase(_OpenAIFormatterBase, ABC):
    ...
    """
    input_types: list[str] = Field(default_factory=lambda : ['text/plain', 'image/*'], description='The supported input types. Defaults to ``["text/plain", "image/*"]``. Audio is not supported by the Responses API.')

    def _format_response_data_block(self, block: DataBlock) -> dict[str, Any] | None:
        ...
        * ``input_audio`` → skipped (the Responses API does not support
        ...
        if media_type.split('/', 1)[0] == 'audio':
            logger.warning('Audio input is not supported by the OpenAI Responses API. Use OpenAIChatModel with an audio-capable model instead. This audio block will be skipped.')
            return None
        base_result = self._format_openai_data_block(block)
        if base_result is None:
            return None
        if base_result.get('type') == 'image_url':
            return {'type': 'input_image', 'image_url': base_result['image_url']['url']}
        return base_result
        ...
        while i < len(msgs):
            ...
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    content_parts.append({'type': text_type, 'text': block.text})
                    ...
                    if isinstance(block.hint, str):
                        items.append({'role': 'user', 'content': [{'type': 'input_text', 'text': block.hint}]})
                    else:
                        hint_parts: list[dict] = []
                        for sub in block.hint:
                            if isinstance(sub, TextBlock):
                                hint_parts.append({'type': 'input_text', 'text': sub.text})
                            elif isinstance(sub, DataBlock):
                                formatted_sub = self._format_response_data_block(sub)
                                if formatted_sub is not None:
                                    hint_parts.append(formatted_sub)
                        if hint_parts:
                            items.append({'role': 'user', 'content': hint_parts})
                elif isinstance(block, ThinkingBlock):
                    reasoning_item_id = getattr(block, 'reasoning_item_id', None)
                    if reasoning_item_id:
                        ...
                        items.append({'type': 'reasoning', 'id': reasoning_item_id, 'summary': summary, 'content': []})
                elif isinstance(block, ToolCallBlock):
                    function_calls.append({'type': 'function_call', 'call_id': block.id, 'name': block.name, 'arguments': block.input})
                    ...
                    items.append({'type': 'function_call_output', 'call_id': block.id, 'output': textual_output})
                    if multimodal_data:
                        promo_content = []
                        for item in multimodal_data:
                            if isinstance(item, TextBlock):
                                promo_content.append({'type': 'input_text', 'text': item.text})
                            elif isinstance(item, DataBlock):
                                fmt_item = self._format_response_data_block(item)
                                if fmt_item is not None:
                                    promo_content.append(fmt_item)
                        if promo_content:
                            items.append({'role': 'user', 'content': promo_content})
                else:
...
class OpenAIResponseMultiAgentFormatter(_OpenAIResponseFormatterBase):
    ...
    async def _format_tool_sequence(self, msgs: list[Msg]) -> list[dict[str, Any]]:
        ...
        """
        return await OpenAIResponseFormatter(input_types=self.input_types).format(msgs)
    ...
    async def _format_agent_message(self, msgs: list[Msg], is_first: bool=True) -> list[dict[str, Any]]:
        ...
        """
        conversation_history_prompt = self.conversation_history_prompt if is_first else ''
        ...
        wrapped = conversation_history_prompt + '<history>\n' + history_text + '\n</history>'
        content_list: list[dict[str, Any]] = [{'type': 'input_text', 'text': wrapped}]
