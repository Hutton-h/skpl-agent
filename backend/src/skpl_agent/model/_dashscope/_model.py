...
class DashScopeChatModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        temperature: float | None = Field(default=None, title='Temperature', description='The temperature for the LLM output.', ge=0, lt=2)
        top_p: float | None = Field(default=None, title='Top P', description='The top P value for the LLM output.', gt=0, le=1)
        top_k: int | None = Field(default=None, title='Top K', description='The top K value for the LLM output.', gt=0, le=100)
        parallel_tool_calls: bool = Field(default=True, title='Parallel Tool Calls', description='If enable parallel tool calls for the LLM output.')
    ...
    def __init__(self, credential: DashScopeCredential, model: str, parameters: 'DashScopeChatModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=131072, formatter: FormatterBase | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`DashScopeChatModel.Parameters | None`, defaults to             `None`):
    ...
    async def _call_api(self, model_name: str, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        """
        import openai
        client = openai.AsyncClient(**{'api_key': self.credential.api_key.get_secret_value(), 'base_url': self.credential.base_url, **self.client_kwargs})
        formatted_messages = await self.formatter.format(messages)
        request_kwargs: dict[str, Any] = {'model': model_name, 'messages': formatted_messages, 'stream': self.stream}
        ...
        if self.parameters.voice is not None:
            request_kwargs['audio'] = {'voice': self.parameters.voice, 'format': 'pcm16'}
            ...
            async for chunk in stream:
                ...
                if chunk.usage:
                    ...
                    if ptd and hasattr(ptd, 'cached_tokens'):
                        ...
                        cache_read = 0
                    usage = ChatUsage(input_tokens=u.prompt_tokens or 0, output_tokens=u.completion_tokens or 0, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=cache_read)
                ...
                if getattr(delta, 'reasoning_content', None):
                    delta_res.append_thinking(block_id=thinking_id, thinking=delta.reasoning_content)
                if getattr(delta, 'content', None):
                    delta_res.append_text(block_id=text_id, text=delta.content)
    ...
    def _parse_completion_response(self, start_datetime: datetime, response: ChatCompletion) -> ChatResponse:
        ...
        if response.usage:
            ...
            if ptd and hasattr(ptd, 'cached_tokens'):
                ...
                cache_read = 0
            usage = ChatUsage(input_tokens=u.prompt_tokens, output_tokens=u.completion_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=cache_read)
    ...
    def _format_tools(self, tools: list[dict] | None, tool_choice: ToolChoice | None) -> tuple[list[dict] | None, str | dict | None]:
        ...
        if tools:
            for value in tools:
                if not isinstance(value, dict) or 'type' not in value or value['type'] != 'function' or ('function' not in value):
                    raise ValueError(f"Each schema must be a dict with 'type' as 'function' and 'function' key, got {value}")
            fmt_tools = tools
        if not tool_choice:
            return (fmt_tools, None)
        mode = tool_choice.mode
        if mode not in _TOOL_CHOICE_LITERAL_MODES:
            return (fmt_tools, {'type': 'function', 'function': {'name': mode}})
        if mode == 'required':
            warnings.warn(f"'{mode}' is not fully supported by DashScope API. It will be converted to 'auto'.", DeprecationWarning, stacklevel=2)
