...
class DeepSeekChatModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        thinking_enable: bool = Field(default=False, title='Thinking', description='Whether to enable thinking mode. When enabled, the model outputs a chain-of-thought reasoning before the final answer via the reasoning_content field.')
        reasoning_effort: Literal['high', 'max'] | None = Field(default=None, title='Reasoning Effort', description='Controls the depth of reasoning in thinking mode. Supported values: high (default), max. For compatibility, low/medium map to high, xhigh maps to max.')
    ...
    def __init__(self, credential: DeepSeekCredential, model: str, parameters: 'DeepSeekChatModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=65536, formatter: FormatterBase | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`DeepSeekChatModel.Parameters | None`, defaults to             `None`):
    ...
    async def _call_api(self, model_name: str, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **generate_kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        thinking_type = 'enabled' if self.parameters.thinking_enable else 'disabled'
    ...
    async def _parse_stream_response(self, start_datetime: datetime, response: AsyncStream) -> AsyncGenerator[ChatResponse, None]:
        ...
        async with response as stream:
            async for chunk in stream:
                ...
                if chunk.usage:
                    u = chunk.usage
                    usage = ChatUsage(input_tokens=u.prompt_tokens, output_tokens=u.completion_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(u, 'prompt_cache_hit_tokens', 0))
