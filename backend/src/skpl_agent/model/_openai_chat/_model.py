...
from ...message import Msg, ThinkingBlock, ToolCallBlock, TextBlock, DataBlock, Base64Source
...
class OpenAIChatModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        max_tokens: int | None = Field(default=None, title='Max Tokens', description='The maximum number of tokens for the LLM output.', gt=0)
        thinking_enable: bool = Field(default=False, title='Thinking', description='Whether to enable reasoning for reasoning models (e.g. o3, o4-mini, gpt-5.5). Use reasoning_effort to control the depth of reasoning.')
        reasoning_effort: Literal['none', 'minimal', 'low', 'medium', 'high', 'xhigh'] | None = Field(default=None, title='Reasoning Effort', description='Controls the depth of reasoning for reasoning models (e.g. o3, o4-mini, gpt-5.5). Supported values are model-dependent and may include: none, minimal, low, medium, high, xhigh.')
        temperature: float | None = Field(default=None, title='Temperature', description='The temperature for the LLM output.', ge=0, le=2)
        top_p: float | None = Field(default=None, title='Top P', description='The top P value for the LLM output.', gt=0, le=1)
        parallel_tool_calls: bool = Field(default=True, title='Parallel Tool Calls', description='Whether to enable parallel tool calls.')
        voice: str | None = Field(default=None, title='Voice', description="Voice for audio output on omni-style models (e.g. ``gpt-audio-mini``). Setting this implicitly asks the model to speak its response — ``modalities`` is filled in automatically. Supported voices vary by model — see the model card's ``voice.suggestions``. Any value the API accepts works — the suggestions are convenience-only. Leave unset for text-only responses.")
    type: Literal['openai_chat'] = 'openai_chat'
    'The type of the chat model.'

    def __init__(self, credential: OpenAICredential, model: str, parameters: 'OpenAIChatModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=128000, formatter: FormatterBase | None=None, client_kwargs: dict[str, Any] | None=None, extra_body: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`OpenAIChatModel.Parameters | None`, defaults to             `None`):
        ...
        """
        super().__init__(credential=credential, model=model, parameters=parameters or self.Parameters(), stream=stream, max_retries=max_retries, retry_delay=retry_delay, context_size=context_size)
    ...
    def _get_retryable_exceptions(cls) -> tuple[Type[Exception], ...]:
        import openai
        return (openai.APIConnectionError, openai.APITimeoutError, openai.RateLimitError, openai.InternalServerError)

    async def _call_api(self, model_name: str, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **generate_kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        """
        import openai
        client = openai.AsyncClient(**{'api_key': self.credential.api_key.get_secret_value(), 'organization': self.credential.organization, 'base_url': self.credential.base_url, **self.client_kwargs})
        formatted_messages = await self.formatter.format(messages)
        kwargs: dict[str, Any] = {'model': model_name, 'messages': formatted_messages, 'stream': self.stream}
        ...
        if self.parameters.voice is not None:
            kwargs['audio'] = {'voice': self.parameters.voice, 'format': 'pcm16'}
    ...
    async def _parse_stream_response(self, start_datetime: datetime, response: AsyncStream) -> AsyncGenerator[ChatResponse, None]:
        ...
        async with response as stream:
            async for chunk in stream:
                ...
                if chunk.usage:
                    ...
                    usage = ChatUsage(input_tokens=u.prompt_tokens, output_tokens=u.completion_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(details, 'cached_tokens', 0) if details else 0)
                ...
                if isinstance(delta_thinking, str) and delta_thinking:
                    delta_res.append_thinking(block_id=thinking_id, thinking=delta_thinking)
                ...
                if delta_audio is not None:
                    if isinstance(delta_audio, dict):
                        ...
                        transcript_chunk = delta_audio.get('transcript', '') or ''
                    else:
                        audio_chunk = getattr(delta_audio, 'data', '') or ''
                        transcript_chunk = getattr(delta_audio, 'transcript', '') or ''
                    if transcript_chunk:
                    ...
                    if audio_chunk:
                        ...
                        if not audio_header_sent:
                            ...
                            payload = pcm_bytes
                        delta_res.append_data_block(block_id=audio_id, data=payload, media_type='audio/wav')
                if delta_text:
                    delta_res.append_text(block_id=text_id, text=delta_text)
                for tool_call in getattr(delta, 'tool_calls', None) or []:
                    ...
                    if index not in tool_call_mapping:
                        tool_call_mapping[index] = (tool_call.id, delta_name or 'unknown')
                    (stored_id, stored_name) = tool_call_mapping[index]
                    delta_res.append_tool_call(block_id=tool_call.id or stored_id, name=delta_name or stored_name, input=delta_args or '')
                if delta_res.content or usage:
                    delta_res.usage = usage
                    yield delta_res

    def _parse_completion_response(self, start_datetime: datetime, response: ChatCompletion, audio_format: str='wav') -> ChatResponse:
        ...
        """
        content_blocks: List[TextBlock | ToolCallBlock | ThinkingBlock | DataBlock] = []
        if response.choices:
            ...
            if audio_obj is not None:
                if isinstance(audio_obj, dict):
                    ...
                    audio_transcript = getattr(audio_obj, 'transcript', '') or ''
                if not choice.message.content and audio_transcript:
                    content_blocks.append(TextBlock(text=audio_transcript))
                if audio_data:
                    content_blocks.append(DataBlock(source=Base64Source(data=audio_data, media_type=f'audio/{audio_format}')))
            for tool_call in choice.message.tool_calls or []:
                content_blocks.append(ToolCallBlock(id=tool_call.id, name=tool_call.function.name, input=tool_call.function.arguments))
        usage = None
        if response.usage:
            u = response.usage
            details = getattr(u, 'prompt_tokens_details', None)
            usage = ChatUsage(input_tokens=u.prompt_tokens, output_tokens=u.completion_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(details, 'cached_tokens', 0) if details else 0)
        resp_kwargs: dict[str, Any] = {'content': content_blocks, 'is_last': True, 'usage': usage}
        response_id = getattr(response, 'id', None)
        if response_id:
            resp_kwargs['id'] = response_id
        return ChatResponse(**resp_kwargs)

    async def _call_api_with_structured_output(self, model_name: str, messages: list[Msg], structured_model: Type[BaseModel] | dict, tool_choice: ToolChoice | None=None, **kwargs: Any) -> StructuredResponse:
        ...
        import openai
        try:
            return await super()._call_api_with_structured_output(model_name=model_name, messages=messages, structured_model=structured_model, tool_choice=tool_choice, **kwargs)
        except openai.BadRequestError as e:
            if 'tool_choice' not in str(e):
                raise
            warnings.warn(f"Forced tool_choice rejected by provider ({e}), retrying with tool_choice='auto'.", stacklevel=2)
            return await super()._call_api_with_structured_output(model_name=model_name, messages=messages, structured_model=structured_model, tool_choice=ToolChoice(mode='auto'), **kwargs)

    def _format_tools(self, tools: list[dict] | None, tool_choice: ToolChoice | None) -> tuple[list[dict] | None, str | dict | None]:
    ...
    @staticmethod
    def _flatten_tool_schemas(tools: list[dict]) -> list[dict]:
