...
def _sanitize_schema_for_gemini(schema: Any) -> Any:
    ...
    for key in ['properties', 'patternProperties', '$defs']:
        if key in schema and isinstance(schema[key], dict):
            schema[key] = {k: _sanitize_schema_for_gemini(v) for (k, v) in schema[key].items()}
...
class GeminiChatModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        thinking_enable: bool = Field(default=False, title='Thinking', description='Whether to enable thinking output.')
        thinking_budget: int | None = Field(default=None, title='Thinking Budget', description='The thinking budget in tokens.', gt=0)
    ...
    def __init__(self, credential: GeminiCredential, model: str, parameters: 'GeminiChatModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=1048576, formatter: FormatterBase | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`GeminiChatModel.Parameters | None`, defaults to             `None`):
    ...
    async def _call_api(self, model_name: str, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **config_kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        client = genai.Client(**{'api_key': self.credential.api_key.get_secret_value(), **self.client_kwargs})
        ...
        if self.parameters.thinking_enable:
            config['thinking_config'] = {'include_thoughts': True, 'thinking_budget': self.parameters.thinking_budget or 1024}
        else:
            config['thinking_config'] = {'include_thoughts': False, 'thinking_budget': 0}
        (fmt_tools, fmt_tool_choice) = self._format_tools(tools, tool_choice)
        if fmt_tools is not None:
            config['tools'] = fmt_tools
        if fmt_tool_choice is not None:
            config['tool_config'] = fmt_tool_choice
        kwargs: dict[str, Any] = {'model': model_name, 'contents': formatted_messages, 'config': config}
        start_datetime = datetime.now()
        if self.stream:
            response = await client.aio.models.generate_content_stream(**kwargs)
            return self._parse_stream_response(start_datetime, response, client)
        response = await client.aio.models.generate_content(**kwargs)
        return self._parse_completion_response(start_datetime, response)

    async def _parse_stream_response(self, start_datetime: datetime, response: Any, _client: Any=None) -> AsyncGenerator[ChatResponse, None]:
        ...
        async for chunk in response:
            ...
            if chunk.candidates and chunk.candidates[0].content and chunk.candidates[0].content.parts:
                for part in chunk.candidates[0].content.parts:
                    if part.text:
                        if part.thought:
                            delta_res.append_thinking(block_id=thinking_id, thinking=part.text)
                        else:
                            delta_res.append_text(block_id=text_id, text=part.text)
                    if part.function_call:
                        if part.thought_signature:
                            call_id = base64.b64encode(part.thought_signature).decode('utf-8')
                        else:
                            call_id = part.function_call.id or _generate_id()
                        delta_res.append_tool_call(block_id=call_id, name=part.function_call.name, input=json.dumps(part.function_call.args or {}, ensure_ascii=False))
            usage = self._extract_usage(chunk.usage_metadata, start_datetime)
            if delta_res.content or usage:
                delta_res.usage = usage
                yield delta_res

    def _parse_completion_response(self, start_datetime: datetime, response: GenerateContentResponse) -> ChatResponse:
            ...
            for part in response.candidates[0].content.parts:
                ...
                if part.function_call:
                    ...
                    if part.thought_signature:
                        ...
                        call_id = part.function_call.id or _generate_id()
                    content_blocks.append(ToolCallBlock(id=call_id, name=part.function_call.name, input=json.dumps(keyword_args, ensure_ascii=False)))
        usage = self._extract_usage(response.usage_metadata, start_datetime)
        return ChatResponse(id=getattr(response, 'response_id', None) or _generate_id(), content=content_blocks, is_last=True, usage=usage)

    def _extract_usage(self, usage_metadata: Any, start_datetime: datetime) -> ChatUsage | None:
        ...
        if prompt_tokens is not None and total_tokens is not None:
            return ChatUsage(input_tokens=prompt_tokens, output_tokens=total_tokens - prompt_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(usage_metadata, 'cached_content_token_count', 0))
        return None
    ...
    def _format_tools(self, tools: list[dict] | None, tool_choice: ToolChoice | None) -> tuple[list[dict] | None, dict | None]:
        ...
        if tools:
            ...
            for schema in tools:
                ...
                if 'parameters' in func:
                    func['parameters'] = _sanitize_schema_for_gemini(_flatten_json_schema(func['parameters']))
        ...
        if mode not in _TOOL_CHOICE_LITERAL_MODES:
            fmt_choice: dict = {'function_calling_config': {'mode': 'ANY', 'allowed_function_names': [mode]}}
            return (fmt_tools, fmt_choice)
        mode_mapping = {'auto': 'AUTO', 'none': 'NONE', 'required': 'ANY'}
        fmt_choice = {'function_calling_config': {'mode': mode_mapping[mode]}}
        return (fmt_tools, fmt_choice)