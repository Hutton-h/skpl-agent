...
class OpenAIResponseModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        max_tokens: int | None = Field(default=None, title='Max Tokens', description='The maximum number of output tokens.', gt=0)
    ...
    def __init__(self, credential: OpenAICredential, model: str, parameters: 'OpenAIResponseModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=200000, formatter: FormatterBase | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`OpenAIResponseModel.Parameters | None`, defaults             to `None`):
    ...
    async def _call_api(self, model_name: str, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **generate_kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        api_kwargs: dict[str, Any] = {'model': model_name, 'input': formatted_messages, 'stream': self.stream}
        ...
        api_kwargs.update({k: v for (k, v) in generate_kwargs.items() if k not in _RESPONSES_UNSUPPORTED_KWARGS})
    ...
    async def _parse_stream_response(self, start_datetime: datetime, response: 'AsyncStream[ResponseStreamEvent]') -> AsyncGenerator[ChatResponse, None]:
        ...
        async for event in response:
            ...
            if event_type == 'response.reasoning_summary_text.delta':
                delta_res.append_thinking(event.delta, block_id=thinking_id)
            elif event_type == 'response.output_text.delta':
                delta_res.append_text(event.delta, block_id=text_id)
            elif event_type == 'response.output_item.added':
                item = event.item
                if getattr(item, 'type', None) == 'function_call':
                    tool_call_mapping[item.id] = (item.call_id, getattr(item, 'name', '') or 'unknown')
            elif event_type == 'response.function_call_arguments.delta':
                item_id = event.item_id
                if item_id in tool_call_mapping:
                    (call_id, name) = tool_call_mapping[item_id]
                    delta_res.append_tool_call(block_id=call_id, name=name, input=event.delta or '')
            elif event_type == 'response.completed':
                resp = event.response
                if resp.usage:
                    u = resp.usage
                    details = getattr(u, 'input_tokens_details', None)
                    usage = ChatUsage(input_tokens=u.input_tokens, output_tokens=u.output_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(details, 'cached_tokens', 0) if details else 0)
                for output_item in getattr(resp, 'output', []):
                    if getattr(output_item, 'type', None) == 'reasoning':
                        ...
                        if reasoning_item_id:
                            delta_res.append_thinking(thinking='', block_id=thinking_id, reasoning_item_id=reasoning_item_id)
            if delta_res.content or usage:
                delta_res.usage = usage
                yield delta_res

    def _parse_completion_response(self, start_datetime: datetime, response: 'Response') -> ChatResponse:
        ...
        for item in response.output:
            ...
            if item_type == 'reasoning':
                ...
                combined_summary = ' '.join((getattr(s, 'text', '') for s in getattr(item, 'summary', []) if getattr(s, 'text', '')))
                if combined_summary or reasoning_item_id:
                    content_blocks.append(ThinkingBlock(type='thinking', thinking=combined_summary, reasoning_item_id=reasoning_item_id))
            elif item_type == 'message':
                for part in getattr(item, 'content', []):
                    if getattr(part, 'type', None) == 'output_text':
                        content_blocks.append(TextBlock(type='text', text=part.text))
            elif item_type == 'function_call':
                content_blocks.append(ToolCallBlock(id=item.call_id, name=item.name, input=getattr(item, 'arguments', '') or '{}'))
        usage = None
        if response.usage:
            u = response.usage
            details = getattr(u, 'input_tokens_details', None)
    ...
    def _format_tools(self, tools: list[dict] | None, tool_choice: ToolChoice | None) -> tuple[list[dict] | None, str | dict | None]:
        ...
        fmt_tools = None
        if tools:
            fmt_tools = [{'type': 'function', **tool['function']} for tool in tools]
        if not tool_choice:
        ...
        if tool_choice.tools:
            return (fmt_tools, {'type': 'allowed_tools', 'mode': mode, 'tools': [{'type': 'function', 'name': name} for name in tool_choice.tools]})
