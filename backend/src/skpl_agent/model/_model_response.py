...
from ..message import TextBlock, ToolCallBlock, ThinkingBlock, DataBlock, Base64Source
from ..types import JSONSerializableObject

class FinishedReason(str, Enum):
...
class ChatResponse(DictMixin):
    ...
    finished_reason: FinishedReason = field(default=FinishedReason.COMPLETED)
    'The finished reason of the chat response, available when `is_last`\n    is `True`.'
    metadata: dict[str, JSONSerializableObject] = field(default_factory=lambda : {})
    ...
    def append_text(self, text: str, block_id: str | None=None) -> Self:
        ...
        for block in self.content:
            if isinstance(block, TextBlock) and (block_id is None or block_id == block.id):
                block.text += text
                return self
    ...
    def append_thinking(self, thinking: str, block_id: str | None=None, **extra_fields: Any) -> Self:
        ...
        """
        for block in self.content:
            if isinstance(block, ThinkingBlock) and (block_id is None or block_id == block.id):
    ...
    def append_tool_call(self, block_id: str, name: str, input: str, **extra_fields: Any) -> Self:
            ...
            if isinstance(block, ToolCallBlock) and block.id == block_id:
                ...
                for (key, value) in extra_fields.items():
                    if value is not None:
                        setattr(block, key, value)
                return self
        block = ToolCallBlock(id=block_id, name=name, input=input)
        for (key, value) in extra_fields.items():
            if value is not None:
                setattr(block, key, value)
        assert isinstance(self.content, list)
    ...
    def append_data_block(self, block_id: str, data: bytes, media_type: str, name: str | None=None) -> Self:
        ...
        """
        for block in self.content:
            if isinstance(block, DataBlock) and block.id == block_id and isinstance(block.source, Base64Source) and (block.source.media_type == media_type):
                old_bytes = base64.b64decode(block.source.data) if block.source.data else b''
                block.source.data = base64.b64encode(old_bytes + data).decode('ascii')
                return self
        self.content.append(DataBlock(id=block_id, source=Base64Source(data=base64.b64encode(data).decode('ascii'), media_type=media_type), name=name))
        return self
        ...
        for block in self.content:
            if block.id in new_block_dict:
                ...
                if isinstance(block, ThinkingBlock):
                    ...
                    if not (isinstance(block.source, Base64Source) and isinstance(delta_block.source, Base64Source) and (block.source.media_type == delta_block.source.media_type)):
                        block.source = delta_block.source
                    elif block.source.media_type.startswith('audio/'):
                        old_bytes = base64.b64decode(block.source.data) if block.source.data else b''
                        delta_bytes = base64.b64decode(delta_block.source.data) if delta_block.source.data else b''
                        block.source.data = base64.b64encode(old_bytes + delta_bytes).decode('ascii')
                    else:
                        block.source.data = delta_block.source.data
        if new_block_dict:
            self.content.extend((block.model_copy(deep=True) for block in new_block_dict.values()))
