...
from ..message import Msg, TextBlock, UserMsg, ToolCallBlock, ThinkingBlock, ToolResultBlock, DataBlock, HintBlock
...
class ChatModelBase:
    ...
    def __init__(self, credential: CredentialBase, model: str, parameters: BaseModel, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=32768) -> None:
    ...
    def list_models(cls, custom_yaml_dir: str | None=None) -> list[ModelCard]:
        ...
        for yaml_file in yaml_files:
            try:
                card = ModelCard.from_yaml(yaml_path=str(yaml_file), parameter_class=cls.Parameters)
                model_cards.append(card)
            except Exception as e:
                logger.warning('Warning: Failed to load %s: %s', yaml_file, str(e))
                continue
        return model_cards

    async def __call__(self, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                res = await self._call_api(self.model, messages=messages, tools=tools, tool_choice=tool_choice, **kwargs)
                break
            except asyncio.CancelledError:
                return ChatResponse(content=[], is_last=True, finished_reason=FinishedReason.INTERRUPTED)
            except Exception as e:
                if not isinstance(e, retryable):
                    raise
                last_error = e
                if attempt < self.max_retries:
                    logger.warning('Attempt %d failed for model %s: %s. Retrying in %.1fs...', attempt + 1, self.model, str(e), self.retry_delay)
                    await asyncio.sleep(self.retry_delay)
                else:
                    logger.warning('All %d attempt(s) failed for model %s.', self.max_retries + 1, self.model)
        else:
            if last_error is not None:
                raise last_error
            raise RuntimeError(f'Failed to call model {self.model} after {self.max_retries + 1} retries.')
    ...
    def _validate_tool_choice(self, tool_choice: ToolChoice | None, tools: list[dict] | None) -> None:
        ...
        available_functions = [tool['function']['name'] for tool in tools or []]
        tool_names = tool_choice.tools
        if tool_names is not None:
            for name in tool_names:
                if name not in available_functions:
                    raise ValueError(f"Invalid tool name '{name}' in tool_choice.tools. Available tools: {', '.join(sorted(available_functions))}")
        if mode not in _TOOL_CHOICE_LITERAL_MODES:
            validation_scope = tool_names if tool_names else available_functions
            if mode not in validation_scope:
                raise ValueError(f"Invalid tool name '{mode}' in tool_choice.mode. " + (f"Available tools in tool_choice.tools: {', '.join(sorted(tool_names))}" if tool_names is not None else f"Available tools: {', '.join(sorted(available_functions))}"))
        ...
        for msg in messages:
            for block in msg.get_content_blocks():
                if isinstance(block, TextBlock):
                    ...
                    logger.warning('Unknown block type %s in token counting, skipping.', type(block))
        if tools:
    ...
    async def generate_structured_output(self, messages: list[Msg], structured_model: Type[BaseModel] | dict, **kwargs: Any) -> StructuredResponse:
        ...
        """
        if len(messages) == 0:
            raise ValueError('The input messages cannot be empty for the `generate_structured_output` method.')
        retryable = tuple(self._get_retryable_exceptions())
        last_error: Exception | None = None
        for attempt in range(self.max_retries + 1):
            try:
                return await self._call_api_with_structured_output(self.model, messages=messages, structured_model=structured_model, **kwargs)
            except Exception as e:
                if not isinstance(e, retryable):
                    raise
                last_error = e
        ...
        if last_error is not None:
            raise last_error
        raise RuntimeError(f'Failed to generate structured output after {self.max_retries + 1} retries.')
    ...
    async def _call_api_with_structured_output(self, model_name: str, messages: list[Msg], structured_model: Type[BaseModel] | dict, tool_choice: ToolChoice | None=None, **kwargs: Any) -> StructuredResponse:
        ...
        instruction = f"<system-reminder>Now you **MUST** call the tool named '{func_name}' to generate the structured output required by the user. DON'T do anything else.</system-reminder>"
        copied_messages = deepcopy(messages)
        if copied_messages[-1].role == 'user':
            copied_messages[-1].content = copied_messages[-1].get_content_blocks() + [TextBlock(text=instruction)]
        else:
            copied_messages.append(UserMsg(name='user', content=[TextBlock(text=instruction)]))
        res = await self._call_api(model_name=model_name, messages=copied_messages, tools=[{'type': 'function', 'function': {'name': func_name, 'description': 'Call this function to generate structured output required by the user.', 'parameters': input_schema}}], tool_choice=tool_choice, **kwargs)
        ...
        if completed_response is None or not completed_response.content:
            raise RuntimeError(f'Failed to get the completed response from model {model_name}.')
        ...
        if structured_output is None:
            raise RuntimeError('Failed to generate structured output for model.')
        if isinstance(structured_model, dict):
            ...
            raise ValueError(f'The structured_model is expected to be a subclass of Pydantic.BaseModel or a dict, but got {type(structured_model)}.')
        return StructuredResponse(id=completed_response.id, created_at=completed_response.created_at, content=structured_output, usage=completed_response.usage, finished_reason=completed_response.finished_reason)