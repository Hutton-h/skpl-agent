...
class XAIChatModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        thinking_enable: bool = Field(default=False, title='Thinking', description='Whether to enable reasoning for models that support extended thinking (e.g. ``grok-3-mini``). Use reasoning_effort to control the depth of reasoning.')
        reasoning_effort: Literal['low', 'medium', 'high'] | None = Field(default=None, title='Reasoning Effort', description="Controls the depth of reasoning for models that support extended thinking (e.g. ``grok-3-mini``). Set to ``'low'``, ``'medium'``, or ``'high'`` to enable reasoning with the corresponding effort level. ``None`` disables reasoning.")
        temperature: float | None = Field(default=None, title='Temperature', description='The temperature for the LLM output.', ge=0, le=2)
        'The sampling temperature.'
        top_p: float | None = Field(default=None, title='Top P', description='The top-p nucleus sampling value.', gt=0, le=1)
        'The top-p sampling parameter.'
    type: Literal['xai_chat'] = 'xai_chat'
    'The type of the chat model.'

    def __init__(self, credential: XAICredential, model: str, parameters: 'XAIChatModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=131072, formatter: XAIChatFormatter | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`XAIChatModel.Parameters | None`, defaults to             `None`):
    ...
    async def _call_api(self, model_name: str, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **generate_kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        """
        from xai_sdk import AsyncClient
        client = AsyncClient(**{'api_key': self.credential.api_key.get_secret_value(), 'api_host': self.credential.api_host, **self.client_kwargs})
    ...
    def _format_tools(self, tools: list[dict] | None, tool_choice: ToolChoice | None) -> tuple[list | None, Any]:
        ...
        if tools:
            ...
            for t in tools:
                if t.get('type') == 'function' and 'function' in t:
                    fn = t['function']
                    xai_tools.append(tool(name=fn['name'], description=fn.get('description', ''), parameters=fn.get('parameters', {})))
        if not tool_choice:
    ...
    async def _parse_stream_response(self, start_datetime: datetime, chat: Any, client: AsyncClient) -> AsyncGenerator[ChatResponse, None]:
        ...
        try:
            async for (response, chunk) in chat.stream():
                ...
                if delta_thinking:
                    delta_res.append_thinking(delta_thinking, block_id=thinking_id)
            ...
            await client.close()
        trailing = ChatResponse(content=[], is_last=False, id=response_id)
        if last_response is not None:
            for tc in last_response.tool_calls or []:
                trailing.append_tool_call(block_id=tc.id, name=tc.function.name, input=tc.function.arguments)
        if last_response is not None and last_response.usage is not None:
            u = last_response.usage
            trailing.usage = ChatUsage(input_tokens=u.prompt_tokens, output_tokens=u.completion_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(u, 'cached_prompt_text_tokens', 0))
    ...
    def _parse_completion_response(self, start_datetime: datetime, response: Response) -> ChatResponse:
        ...
        if response.reasoning_content:
            content_blocks.append(ThinkingBlock(thinking=response.reasoning_content))
        if response.content:
            content_blocks.append(TextBlock(text=response.content))
        for tc in response.tool_calls or []:
            content_blocks.append(ToolCallBlock(id=tc.id, name=tc.function.name, input=tc.function.arguments))
        usage = None
        if response.usage is not None:
            u = response.usage
            usage = ChatUsage(input_tokens=u.prompt_tokens, output_tokens=u.completion_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(u, 'cached_prompt_text_tokens', 0))
