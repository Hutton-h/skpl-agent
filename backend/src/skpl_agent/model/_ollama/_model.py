...
class OllamaChatModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        thinking_enable: bool = Field(default=False, title='Thinking', description='Whether to enable thinking (for models like qwen3, deepseek-r1).')
    ...
    def __init__(self, credential: OllamaCredential | None=None, model: str='', parameters: 'OllamaChatModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=32768, formatter: FormatterBase | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`OllamaChatModel.Parameters | None`, defaults to             `None`):
        ...
        super().__init__(credential=resolved_credential, model=model, parameters=parameters or self.Parameters(), stream=stream, max_retries=max_retries, retry_delay=retry_delay, context_size=context_size)
    ...
    def _get_retryable_exceptions(cls) -> tuple[Type[Exception], ...]:
        import httpx
        return (httpx.ConnectError, httpx.ReadTimeout, httpx.RemoteProtocolError)
    ...
    async def _call_api(self, model_name: str, messages: list[Msg], tools: list[dict] | None=None, tool_choice: ToolChoice | None=None, **generate_kwargs: Any) -> ChatResponse | AsyncGenerator[ChatResponse, None]:
        ...
        """
        import ollama
        client = ollama.AsyncClient(**{'host': self.credential.host, **self.client_kwargs})
    ...
    def _format_tools(self, tools: list[dict] | None, tool_choice: ToolChoice | None) -> tuple[list[dict] | None, None]:
        ...
        if tool_choice:
            logger.warning('Ollama ignores tool_choice.mode; tool_choice.tools is still applied to filter tool schemas.')
        return (tools, None)

    async def _parse_stream_response(self, start_datetime: datetime, response: Any) -> AsyncGenerator[ChatResponse, None]:
        ...
        async for chunk in response:
            ...
            if getattr(msg, 'thinking', None):
                delta_res.append_thinking(block_id=thinking_id, thinking=msg.thinking)
            if msg.content:
                delta_res.append_text(block_id=text_id, text=msg.content)
            for (idx, tool_call) in enumerate(msg.tool_calls or []):
                delta_res.append_tool_call(block_id=f'{idx}_{tool_call.function.name}', name=tool_call.function.name, input=json.dumps(tool_call.function.arguments, ensure_ascii=False))
            current_time = (datetime.now() - start_datetime).total_seconds()
            delta_res.usage = ChatUsage(input_tokens=getattr(chunk, 'prompt_eval_count', 0) or 0, output_tokens=getattr(chunk, 'eval_count', 0) or 0, time=current_time)
            yield delta_res

    async def _parse_completion_response(self, start_datetime: datetime, response: OllamaChatResponse) -> ChatResponse:
        ...
        for (idx, tool_call) in enumerate(response.message.tool_calls or []):
            content_blocks.append(ToolCallBlock(id=f'{idx}_{tool_call.function.name}', name=tool_call.function.name, input=json.dumps(tool_call.function.arguments, ensure_ascii=False)))
        usage = None
        prompt_eval = getattr(response, 'prompt_eval_count', None)
        eval_count = getattr(response, 'eval_count', None)
        if prompt_eval is not None and eval_count is not None:
            usage = ChatUsage(input_tokens=prompt_eval, output_tokens=eval_count, time=(datetime.now() - start_datetime).total_seconds())
        return ChatResponse(id=getattr(response, 'id', None) or _generate_id(), content=content_blocks, is_last=True, usage=usage)