...
class MoonshotChatModel(ChatModelBase):
    ...
    class Parameters(BaseModel):
        ...
        thinking_enable: bool = Field(default=False, title='Thinking', description='Whether to enable thinking mode. For kimi-k2-thinking, thinking is always enabled. For kimi-k2.6, thinking is enabled by default but can be disabled.')
        reasoning_effort: Literal['low', 'high', 'max'] | None = Field(default=None, title='Reasoning Effort', description="The reasoning effort level for kimi-k3. Supports 'low', 'high', and 'max' (default 'max').")
        temperature: float | None = Field(default=None, title='Temperature', description='The temperature for the LLM output.', ge=0, le=1)
    ...
    def __init__(self, credential: MoonshotCredential, model: str, parameters: 'MoonshotChatModel.Parameters | None'=None, stream: bool=True, max_retries: int=3, retry_delay: float=1.0, context_size: int=131072, formatter: FormatterBase | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`MoonshotChatModel.Parameters | None`, defaults to             `None`):
    ...
    async def _parse_stream_response(self, start_datetime: datetime, response: AsyncStream) -> AsyncGenerator[ChatResponse, None]:
        ...
        async with response as stream:
            async for chunk in stream:
                ...
                if chunk.usage:
                    u = chunk.usage
                    usage = ChatUsage(input_tokens=u.prompt_tokens, output_tokens=u.completion_tokens, time=(datetime.now() - start_datetime).total_seconds(), cache_input_tokens=getattr(u, 'cached_tokens', 0))
