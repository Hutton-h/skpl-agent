...
class ApproxTokenChunker(ChunkerBase):
    ...
    async def chunk(self, sections: list[Section]) -> list[Chunk]:
        ...
        for section in sections:
            ...
            chunks.extend((Chunk(content=content, source=section.source, chunk_index=0, total_chunks=0, metadata=dict(section.metadata)) for content in contents))
    ...
    def _split_text(self, text: str) -> list[str]:
        ...
        while start < len(text):
            end = bisect_right(byte_offsets, byte_offsets[start] + chunk_bytes) - 1
            end = max(end, start + 1)
            pieces.append(text[start:end])
            if end >= len(text):
                break
            next_start = bisect_right(byte_offsets, byte_offsets[end] - overlap_bytes) - 1
