...
class KnowledgeBase:
    ...
    def __init__(self, name: str, description: str, embedding_model: EmbeddingModelBase, vector_store: VectorStoreBase, collection: str, metadata_filter: dict | None=None) -> None:
    ...
    async def ensure_collection(self) -> None:
        ...
        if not await self._vector_store.has_collection(self._collection):
            await self._vector_store.create_collection(self._collection, dimensions=self._embedding_model.dimensions)
        self._collection_ready = True

    async def search(self, queries: list[str | TextBlock | DataBlock], top_k: int=5, score_threshold: float | None=None) -> list[VectorSearchResult]:
        ...
        results_per_query = await asyncio.gather(*(self._vector_store.search(collection=self._collection, query_vector=vector, top_k=top_k, metadata_filter=self._metadata_filter) for vector in response.embeddings))
        ...
        for results in results_per_query:
            for result in results:
                ...
                if key not in best or result.score > best[key].score:
                    best[key] = result
        merged = sorted(best.values(), key=lambda result: result.score, reverse=True)
        return merged[:top_k]

    async def insert_document(self, chunks: list[Chunk], document_id: str | None=None, document_metadata: dict | None=None) -> str:
        ...
        await self.ensure_collection()
        for chunk in chunks:
            chunk.metadata = {**(document_metadata or {}), **chunk.metadata, **(self._metadata_filter or {})}
        response = await self._embedding_model([chunk.content for chunk in chunks])
        if len(response.embeddings) != len(chunks):
            raise RuntimeError(f'Embedding model returned {len(response.embeddings)} vectors for {len(chunks)} chunks.')
        records = [VectorRecord(vector=vector, document_id=document_id, chunk=chunk) for (vector, chunk) in zip(response.embeddings, chunks)]
    ...
    async def delete_document(self, document_id: str) -> None:
        ...
        """
        await self.ensure_collection()
        await self._vector_store.delete(self._collection, document_id)
    ...
    async def list_documents(self) -> list['DocumentSummary']:
        ...
        """
        await self.ensure_collection()
        return await self._vector_store.list_documents(self._collection, metadata_filter=self._metadata_filter)