...
class QdrantStore(VectorStoreBase):
    ...
    def __init__(self, location: str | None=None, url: str | None=None, path: str | None=None, api_key: str | None=None, distance: Literal['Cosine', 'Dot', 'Euclid', 'Manhattan']='Cosine', client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            distance (`Literal["Cosine", "Dot", "Euclid", "Manhattan"]`,              defaults to ``"Cosine"``):
    ...
    def get_client(self) -> 'AsyncQdrantClient':
        ...
        if self._client is None:
            from qdrant_client import AsyncQdrantClient
            self._client = AsyncQdrantClient(location=self._location, url=self._url, path=self._path, api_key=self._api_key, **self._client_kwargs)
        return self._client
    ...
    async def create_collection(self, name: str, dimensions: int) -> None:
        ...
        if await client.collection_exists(name):
            return
        await client.create_collection(collection_name=name, vectors_config=models.VectorParams(size=dimensions, distance=models.Distance(self._distance)))
    ...
    async def insert(self, collection: str, records: list[VectorRecord]) -> None:
        ...
        if not records:
            return
        await self.get_client().upsert(collection_name=collection, points=[models.PointStruct(id=str(uuid.uuid4()), vector=record.vector, payload={'document_id': record.document_id, 'chunk': record.chunk.model_dump(mode='json')}) for record in records])
    ...
    async def delete(self, collection: str, document_id: str) -> None:
        ...
        """
        from qdrant_client import models
        await self.get_client().delete(collection_name=collection, points_selector=models.FilterSelector(filter=models.Filter(must=[models.FieldCondition(key='document_id', match=models.MatchValue(value=document_id))])))
    ...
    async def search(self, collection: str, query_vector: list[float], top_k: int=5, metadata_filter: dict[str, Any] | None=None) -> list[VectorSearchResult]:
        ...
        """
        response = await self.get_client().query_points(collection_name=collection, query=query_vector, limit=top_k, with_payload=True, query_filter=self._build_metadata_filter(metadata_filter))
        return [VectorSearchResult(score=point.score, document_id=point.payload['document_id'], chunk=Chunk.model_validate(point.payload['chunk'])) for point in response.points]
    ...
    async def list_documents(self, collection: str, metadata_filter: dict[str, Any] | None=None) -> list[DocumentSummary]:
        ...
        offset: Any = None
        while True:
            (points, next_offset) = await client.scroll(collection_name=collection, scroll_filter=query_filter, limit=256, offset=offset, with_payload=True, with_vectors=False)
            for point in points:
    ...
    @staticmethod
    def _build_metadata_filter(metadata_filter: dict[str, Any] | None) -> Any:
        ...
        if not metadata_filter:
            return None
        from qdrant_client import models
        return models.Filter(must=[models.FieldCondition(key=f'chunk.metadata.{key}', match=models.MatchValue(value=value)) for (key, value) in metadata_filter.items()])