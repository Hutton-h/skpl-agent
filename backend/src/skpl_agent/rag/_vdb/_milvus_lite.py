...
from ._vector_store import DocumentSummary, VectorRecord, VectorSearchResult, VectorStoreBase
...
class MilvusLiteStore(VectorStoreBase):
    ...
    def __init__(self, uri: str='./agentscope_milvus_lite.db', metric_type: Literal['COSINE', 'IP', 'L2']='COSINE', index_type: str='AUTOINDEX', client_kwargs: dict[str, Any] | None=None, batch_size: int=256) -> None:
        ...
        Args:
            uri (`str`, defaults to ``"./agentscope_milvus_lite.db"``):
                ...
                endpoint URI.
            metric_type (`Literal["COSINE", "IP", "L2"]`, defaults to              ``"COSINE"``):
    ...
    async def create_collection(self, name: str, dimensions: int) -> None:
        ...
        def _create() -> None:
            ...
            schema.add_field(field_name='chunk', datatype=DataType.JSON)
            schema.add_field(field_name='metadata', datatype=DataType.JSON)
            index_params = client.prepare_index_params()
            index_params.add_index(field_name='vector', index_type=self._index_type, metric_type=self._metric_type)
    ...
    async def delete_collection(self, name: str) -> None:
        ...
        await asyncio.to_thread(self.get_client().drop_collection, collection_name=name)

    async def has_collection(self, name: str) -> bool:
        """Check whether a collection exists."""
        return await asyncio.to_thread(self.get_client().has_collection, collection_name=name)
    ...
    async def insert(self, collection: str, records: list[VectorRecord]) -> None:
        ...
        for start in range(0, len(records), self._batch_size):
            ...
            await asyncio.to_thread(self.get_client().insert, collection_name=collection, data=[{'id': str(uuid.uuid4()), 'vector': record.vector, 'document_id': record.document_id, 'chunk': record.chunk.model_dump(mode='json'), 'metadata': record.chunk.metadata} for record in batch])

    async def delete(self, collection: str, document_id: str) -> None:
        """Delete all records belonging to one source document."""
        await asyncio.to_thread(self.get_client().delete, collection_name=collection, filter=self._build_document_filter(document_id))

    async def search(self, collection: str, query_vector: list[float], top_k: int=5, metadata_filter: dict[str, Any] | None=None) -> list[VectorSearchResult]:
        ...
        """
        response = await asyncio.to_thread(self.get_client().search, collection_name=collection, data=[query_vector], anns_field='vector', limit=top_k, filter=self._build_metadata_filter(metadata_filter), output_fields=['document_id', 'chunk'], search_params={'metric_type': self._metric_type})
        hits = response[0] if response else []
        return [VectorSearchResult(score=self._extract_score(hit, self._metric_type), document_id=hit['entity']['document_id'], chunk=Chunk.model_validate(hit['entity']['chunk'])) for hit in hits]

    async def list_documents(self, collection: str, metadata_filter: dict[str, Any] | None=None) -> list[DocumentSummary]:
        ...
        """
        client = self.get_client()
        rows = await asyncio.to_thread(self._query_all_rows, client, collection, metadata_filter)
        ...
        for row in rows:
            ...
            if summary is None:
                ...
                summaries[doc_id] = DocumentSummary(document_id=doc_id, source=chunk_payload.get('source', ''), chunk_count=1, metadata=dict(chunk_payload.get('metadata', {})))
            else:
                summary.chunk_count += 1
        return list(summaries.values())

    def _query_all_rows(self, client: 'MilvusClient', collection: str, metadata_filter: dict[str, Any] | None) -> list[dict[str, Any]]:
        ...
        if hasattr(client, 'query_iterator'):
            iterator = client.query_iterator(collection_name=collection, filter=expr, output_fields=output_fields, batch_size=self._batch_size)
            try:
                rows = []
                while True:
                    batch = iterator.next()
        ...
        offset = 0
        while True:
            batch = client.query(collection_name=collection, filter=expr, output_fields=output_fields, limit=self._batch_size, offset=offset)
            if not batch:
                break
            rows.extend(batch)
    ...
    @staticmethod
    def _build_metadata_filter(metadata_filter: dict[str, Any] | None) -> str:
        """Translate a flat ``{key: value}`` filter into a Milvus expr."""
        if not metadata_filter:
            return ''
        return ' and '.join((f'metadata[{json.dumps(key, ensure_ascii=False)}] == {json.dumps(value, ensure_ascii=False)}' for (key, value) in metadata_filter.items()))

    @staticmethod
    def _extract_score(hit: dict[str, Any], metric_type: Literal['COSINE', 'IP', 'L2']) -> float:
    ...
    def _is_local_db_uri(uri: str) -> bool:
        ...
        return not uri.startswith(('http://', 'https://')) and os.path.splitext(uri)[1] == '.db'
