...
class MongoDBStore(VectorStoreBase):
    """Vector store backend backed by `MongoDB Vector Search <https://www.mongodb.com/docs/atlas/atlas-vector-search/vector-search-overview/>`_.
    ...
    def __init__(self, uri: str, database: str, distance: Literal['cosine', 'euclidean', 'dotProduct']='cosine', index_name: str='vector_index', filter_fields: list[str] | None=None, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            distance (`Literal["cosine", "euclidean", "dotProduct"]`,              defaults to ``"cosine"``):
    ...
    def get_client(self) -> 'AsyncMongoClient':
        ...
        if self._client is None:
            from pymongo import AsyncMongoClient
            self._client = AsyncMongoClient(self._uri, **self._client_kwargs)
        return self._client
    ...
    async def create_collection(self, name: str, dimensions: int) -> None:
        ...
        col = self._col(name)
        fields: list[dict[str, Any]] = [{'type': 'vector', 'path': 'vector', 'numDimensions': dimensions, 'similarity': self._distance}]
        fields.extend(({'type': 'filter', 'path': field_path} for field_path in self._filter_fields))
        model = SearchIndexModel(definition={'fields': fields}, name=self._index_name, type='vectorSearch')
    ...
    async def _wait_for_index_ready(self, collection: Any, collection_name: str, timeout: float=30.0) -> None:
        ...
        while time.monotonic() < deadline:
            async for index in await collection.list_search_indexes(self._index_name):
                if index.get('queryable'):
                    return
                break
            await asyncio.sleep(0.5)
        raise TimeoutError(f'Vector search index {self._index_name!r} on collection {collection_name!r} was not queryable within {timeout}s')
    ...
    async def insert(self, collection: str, records: list[VectorRecord]) -> None:
        ...
        operations = [ReplaceOne({'_id': f'{record.document_id}_{record.chunk.chunk_index}'}, {'_id': f'{record.document_id}_{record.chunk.chunk_index}', 'document_id': record.document_id, 'vector': record.vector, 'chunk': record.chunk.model_dump(mode='json')}, upsert=True) for record in records]
    ...
    async def search(self, collection: str, query_vector: list[float], top_k: int=5, metadata_filter: dict[str, Any] | None=None) -> list[VectorSearchResult]:
        ...
        col = self._col(collection)
        vector_search: dict[str, Any] = {'index': self._index_name, 'path': 'vector', 'queryVector': query_vector, 'numCandidates': max(100, top_k * 20), 'limit': top_k}
        filter_expr = self._build_metadata_filter(metadata_filter)
        if filter_expr is not None:
            vector_search['filter'] = filter_expr
        pipeline = [{'$vectorSearch': vector_search}, {'$project': {'document_id': 1, 'chunk': 1, 'score': {'$meta': 'vectorSearchScore'}}}]
        results: list[VectorSearchResult] = []
        async for doc in await col.aggregate(pipeline):
            results.append(VectorSearchResult(score=float(doc['score']), document_id=doc['document_id'], chunk=Chunk.model_validate(doc['chunk'])))
        return results

    @staticmethod
    def _build_metadata_filter(metadata_filter: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        if not metadata_filter:
            return None
        return {'$and': [{f'chunk.metadata.{key}': {'$eq': value}} for (key, value) in metadata_filter.items()]}
    ...
    async def list_documents(self, collection: str, metadata_filter: dict[str, Any] | None=None) -> list[DocumentSummary]:
        ...
        if metadata_filter:
            pipeline.append({'$match': {f'chunk.metadata.{key}': value for (key, value) in metadata_filter.items()}})
        pipeline.append({'$group': {'_id': '$document_id', 'source': {'$first': '$chunk.source'}, 'metadata': {'$first': '$chunk.metadata'}, 'chunk_count': {'$sum': 1}}})
        summaries: list[DocumentSummary] = []
        async for row in await self._col(collection).aggregate(pipeline):
            summaries.append(DocumentSummary(document_id=row['_id'], source=row['source'], chunk_count=row['chunk_count'], metadata=row['metadata'] or {}))
        return summaries