...
class ElasticsearchStore(VectorStoreBase):
    ...
    def __init__(self, hosts: str | list[str], *, num_candidates: int=100, refresh: bool | Literal['wait_for']='wait_for', client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            refresh (`bool | Literal["wait_for"]`, defaults to             ``"wait_for"``):
        ...
        """
        if num_candidates <= 0 or num_candidates > 10000:
    ...
    def get_client(self) -> 'AsyncElasticsearch':
        ...
        if self._client is None:
            from elasticsearch import AsyncElasticsearch
            self._client = AsyncElasticsearch(self._hosts, **self._client_kwargs)
        return self._client
    ...
    async def create_collection(self, name: str, dimensions: int) -> None:
        ...
        if await self.has_collection(name):
            return
        await self.get_client().indices.create(index=name, mappings={'dynamic': False, 'properties': {'vector': {'type': 'dense_vector', 'dims': dimensions, 'index': True, 'similarity': 'cosine'}, 'document_id': {'type': 'keyword'}, 'chunk': {'type': 'object', 'enabled': False}, 'metadata': {'type': 'object', 'dynamic': 'runtime'}}})
    ...
    async def insert(self, collection: str, records: list[VectorRecord]) -> None:
        ...
        for record in records:
            operations.extend([{'index': {'_index': collection, '_id': self._record_id(record)}}, {'vector': record.vector, 'document_id': record.document_id, 'chunk': record.chunk.model_dump(mode='json'), 'metadata': record.chunk.metadata}])
        response = await self.get_client().bulk(operations=operations, refresh=self._refresh)
        if response.get('errors'):
            failures = [item for item in response.get('items', []) if next(iter(item.values())).get('error')]
            raise RuntimeError(f'Elasticsearch bulk insert failed for {len(failures)} record(s)')

    async def delete(self, collection: str, document_id: str) -> None:
        """Delete every chunk belonging to one source document."""
        await self.get_client().delete_by_query(index=collection, query={'term': {'document_id': document_id}}, conflicts='proceed', refresh=self._refresh is not False)
    ...
    async def search(self, collection: str, query_vector: list[float], top_k: int=5, metadata_filter: dict[str, Any] | None=None) -> list[VectorSearchResult]:
        ...
        num_candidates = min(max(self._num_candidates, top_k), 10000)
        knn: dict[str, Any] = {'field': 'vector', 'query_vector': query_vector, 'k': top_k, 'num_candidates': num_candidates}
        filters = self._metadata_filters(metadata_filter)
        if filters:
            knn['filter'] = filters
        response = await self.get_client().search(index=collection, size=top_k, knn=knn, source_includes=['document_id', 'chunk'])
        return [VectorSearchResult(score=2.0 * float(hit['_score']) - 1.0, document_id=hit['_source']['document_id'], chunk=Chunk.model_validate(hit['_source']['chunk'])) for hit in response['hits']['hits']]
    ...
    async def list_documents(self, collection: str, metadata_filter: dict[str, Any] | None=None) -> list[DocumentSummary]:
        ...
        while True:
            composite: dict[str, Any] = {'size': 500, 'sources': [{'document_id': {'terms': {'field': 'document_id'}}}]}
            if after is not None:
                composite['after'] = after
            response = await self.get_client().search(index=collection, size=0, query=self._filter_query(metadata_filter), aggs={'documents': {'composite': composite, 'aggs': {'sample': {'top_hits': {'size': 1, '_source': ['chunk']}}}}})
            ...
            for bucket in buckets:
                ...
                summaries.append(DocumentSummary(document_id=bucket['key']['document_id'], source=chunk.source, chunk_count=bucket['doc_count'], metadata=chunk.metadata))
    ...
    def _record_id(record: VectorRecord) -> str:
        ...
        raw = f'{record.document_id}\x00{record.chunk.chunk_index}'
        return hashlib.sha256(raw.encode('utf-8')).hexdigest()

    @staticmethod
    def _metadata_filters(metadata_filter: dict[str, Any] | None) -> list[dict[str, Any]]:
        """Translate exact metadata matches into Elasticsearch filters."""
        return [{'term': {f'metadata.{key}': value}} for (key, value) in (metadata_filter or {}).items()]

    @classmethod
    def _filter_query(cls, metadata_filter: dict[str, Any] | None) -> dict[str, Any]:
