...
def _extract_images_from_worksheet(worksheet: Any, filename: str) -> list[tuple[int, DataBlock]]:
    ...
    for img in worksheet._images:
        try:
            ...
            images.append((row_index, DataBlock(source=Base64Source(media_type=media_type, data=b64_data), name=filename)))
...
class ExcelParser(ParserBase):
    ...
    """
    supported_media_types: list[str] = ['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel']
    ...
    def __init__(self, include_sheet_names: bool=True, include_cell_coordinates: bool=False, include_image: bool=False, separate_sheet: bool=False, table_format: Literal['markdown', 'json']='markdown') -> None:
        ...
        if table_format not in ('markdown', 'json'):
            raise ValueError(f"The table_format must be one of 'markdown' or 'json', got {table_format!r}.")
    ...
    async def parse(self, file: bytes | str, filename: str) -> list[Section]:
        ...
        try:
            ...
            raise ImportError('Please install pandas to use the Excel parser. You can install it by `pip install pandas` (or `pip install agentscope[rag]`).') from e
        ...
        try:
            if self.include_image:
                try:
                    ...
                    logger.warning('openpyxl not available, image extraction disabled.')
                except Exception as e:
                    logger.warning('Failed to load workbook for images: %s', e)
            sections: list[Section] = []
            for sheet_name in excel_file.sheet_names:
                sheet_sections = self._parse_sheet(excel_file, sheet_name, filename, workbook)
    ...
    def _parse_sheet(self, excel_file: Any, sheet_name: str, filename: str, workbook: Any=None) -> list[Section]:
        ...
        if table_text:
            sheet_sections.append(Section(content=TextBlock(text=table_text), source=filename, metadata={'sheet': sheet_name}))
        if self.include_image and workbook is not None:
            try:
                worksheet = workbook[sheet_name]
                images = _extract_images_from_worksheet(worksheet, filename)
                images.sort(key=lambda x: x[0])
                for (_row, block) in images:
                    sheet_sections.append(Section(content=block, source=filename, metadata={'sheet': sheet_name, 'media_type': block.source.media_type}))
            except Exception as e:
                logger.warning("Failed to extract images from sheet '%s': %s", sheet_name, e)
        return sheet_sections

    @staticmethod
    def _merge_text_sections(sections: list[Section], filename: str) -> list[Section]:
        ...
        result: list[Section] = []
        if text_parts:
            result.append(Section(content=TextBlock(text='\n'.join(text_parts)), source=filename, metadata={}))
        result.extend(non_text)
        return result

    def _table_to_markdown(self, table_data: list[list[str]], sheet_name: str) -> str:
    ...
    def _table_to_json(self, table_data: list[list[str]], sheet_name: str) -> str:
        """Render table data as JSON with optional sheet header and cell
        coordinates."""
        parts: list[str] = []
        if self.include_sheet_names:
            parts.append(f'Sheet: {sheet_name}')
        parts.append('<system-info>A table loaded as a JSON array:</system-info>')
        for (ri, row) in enumerate(table_data):
            if self.include_cell_coordinates:
                row_dict = {f'{_get_excel_column_name(ci)}{ri + 1}': cell for (ci, cell) in enumerate(row)}
