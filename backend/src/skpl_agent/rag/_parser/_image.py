...
class ImageParser(ParserBase):
    ...
    """
    supported_media_types: list[str] = ['image/png', 'image/jpeg', 'image/gif', 'image/bmp', 'image/webp']
    ...
    async def parse(self, file: bytes | str, filename: str) -> list[Section]:
        ...
        return [Section(content=DataBlock(source=Base64Source(media_type=media_type, data=data), name=filename), source=filename, metadata={'media_type': media_type})]