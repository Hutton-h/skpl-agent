...
class PDFParser(ParserBase):
    ...
    async def parse(self, file: bytes | str, filename: str) -> list[Section]:
        ...
        try:
            ...
            raise ImportError('Please install pypdf to use the PDF parser. You can install it by `pip install pypdf` (or `pip install agentscope[rag]`).') from e
        try:
            reader = PdfReader(io.BytesIO(file))
        except PdfReadError as e:
            raise ValueError(f'Failed to parse {filename!r} as PDF: {e}') from e
        sections: list[Section] = []
        for (page_idx, page) in enumerate(reader.pages, start=1):
            text = page.extract_text() or ''
            sections.append(Section(content=TextBlock(text=text), source=filename, metadata={'page': page_idx}))
        return sections