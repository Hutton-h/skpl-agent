...
class TextParser(ParserBase):
    ...
    """
    supported_media_types: list[str] = ['text/plain', 'text/markdown', 'text/csv', 'text/html', 'text/x-rst', 'application/json', 'application/xml', 'application/x-yaml']
    ...
    def supported_extensions(cls) -> list[str]:
        ...
        ``application/x-yaml``.
        """
        return ['.csv', '.htm', '.html', '.json', '.markdown', '.md', '.rst', '.txt', '.xml', '.yaml', '.yml']
        ...
        if isinstance(file, str):
            if os.path.isfile(file):
                ...
                try:
                    ...
                    raise ValueError(f'Failed to decode {filename!r} as {self.encoding!r}: {e}') from e
            else:
                text = file
        else:
            try:
