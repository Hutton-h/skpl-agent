...
from ._utils import _guess_image_media_type, _table_to_json, _table_to_markdown
...
class PPTParser(ParserBase):
    ...
    """
    supported_media_types: list[str] = ['application/vnd.openxmlformats-officedocument.presentationml.presentation']
    ...
    def __init__(self, include_image: bool=True, separate_table: bool=False, table_format: Literal['markdown', 'json']='markdown', slide_prefix: str | None='<slide index={index}>', slide_suffix: str | None='</slide>') -> None:
    ...
    async def parse(self, file: bytes | str, filename: str) -> list[Section]:
        ...
        try:
            ...
            raise ImportError('Please install python-pptx to use the PowerPoint parser. You can install it by `pip install python-pptx` (or `pip install agentscope[rag]`).') from e
        try:
            prs = Presentation(io.BytesIO(file))
        except Exception as e:
            raise ValueError(f'Failed to parse {filename!r} as PPTX: {e}') from e
        sections: list[Section] = []
        for (slide_idx, slide) in enumerate(prs.slides):
            sections.extend(self._parse_slide(slide, slide_idx, filename))
        return sections

    def _parse_slide(self, slide: Any, slide_idx: int, filename: str) -> list[Section]:
        """Walk one slide and return its ordered sections."""
        slide_no = slide_idx + 1
        prefix = self.slide_prefix.format(index=slide_no) if self.slide_prefix is not None else ''
        ...
        def flush_text() -> None:
            if not text_buffer:
                return
            slide_sections.append(Section(content=TextBlock(text='\n'.join(text_buffer)), source=filename, metadata={'slide': slide_no}))
            text_buffer.clear()
        if prefix:
        ...
        for shape in slide.shapes:
            if self.include_image:
                ...
                if image_bytes is not None:
                    ...
                    slide_sections.append(Section(content=DataBlock(source=Base64Source(media_type=media_type, data=data), name=filename), source=filename, metadata={'slide': slide_no, 'media_type': media_type}))
                    continue
            if getattr(shape, 'has_table', False):
                try:
                    rows = _extract_table_rows(shape.table)
                except Exception as e:
                    logger.warning('Failed to extract table from slide %d: %s', slide_no, e)
                    continue
                rendered = _table_to_markdown(rows) if self.table_format == 'markdown' else _table_to_json(rows)
                if not rendered:
                    continue
                if self.separate_table:
                    flush_text()
                    slide_sections.append(Section(content=TextBlock(text=rendered), source=filename, metadata={'slide': slide_no}))
                else:
                    text_buffer.append(rendered)
                continue
            if getattr(shape, 'has_text_frame', False):
                try:
                    parts = [para.text.strip() for para in shape.text_frame.paragraphs if para.text.strip()]
                except Exception as e:
                    logger.warning('Failed to extract text from shape in slide %d: %s', slide_no, e)
                    continue
                if parts:
                    text_buffer.append('\n'.join(parts))
