    ...
    for tr in table._element.findall(qn('w:tr')):
        ...
        for tc in tr.findall(qn('w:tc')):
            ...
            if tc_pr is not None:
                ...
                if gs is not None:
                    span = int(gs.get(qn('w:val'), gs.get('val', '1')))
                    row_data.extend([''] * (span - 1))
        table_data.append(row_data)
    return table_data

def _extract_image_blocks(para: DocxParagraph, filename: str) -> list[DataBlock]:
    ...
    for drawing in para._element.findall('.//' + qn('w:drawing')):
        for blip in drawing.findall('.//' + qn('a:blip')):
            ...
            try:
                ...
                media_type = part.content_type or _guess_image_media_type(part.blob)
                blocks.append(DataBlock(source=Base64Source(media_type=media_type, data=base64.b64encode(part.blob).decode('utf-8')), name=filename))
...
class WordParser(ParserBase):
    ...
    """
    supported_media_types: list[str] = ['application/vnd.openxmlformats-officedocument.wordprocessingml.document']
    ...
    def __init__(self, include_image: bool=True, separate_table: bool=False, table_format: Literal['markdown', 'json']='markdown') -> None:
    ...
    async def parse(self, file: bytes | str, filename: str) -> list[Section]:
        ...
        try:
            ...
            raise ImportError('Please install python-docx to use the Word parser. You can install it by `pip install python-docx` (or `pip install agentscope[rag]`).') from e
        ...
        def flush_text() -> None:
            if not text_buffer:
                return
            sections.append(Section(content=TextBlock(text='\n'.join(text_buffer)), source=filename, metadata={}))
            text_buffer.clear()
        for element in doc.element.body:
            if isinstance(element, CT_P):
                ...
                if self.include_image:
                    ...
                    has_pict = bool(para._element.findall('.//' + qn('w:pict')))
                    if has_drawing or has_pict:
                        flush_text()
                        for block in _extract_image_blocks(para, filename):
                            sections.append(Section(content=block, source=filename, metadata={'media_type': block.source.media_type}))
            elif isinstance(element, CT_Tbl):
                table_data = _extract_table_data(Table(element, doc))
                rendered = _table_to_markdown(table_data) if self.table_format == 'markdown' else _table_to_json(table_data)
                if not rendered:
                    continue
                if self.separate_table:
                    flush_text()
                    sections.append(Section(content=TextBlock(text=rendered), source=filename, metadata={}))
                else:
