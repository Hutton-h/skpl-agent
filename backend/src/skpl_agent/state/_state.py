...
from ..message import TextBlock, DataBlock, Msg, ToolCallBlock, ToolCallState, ToolResultBlock, HintBlock
...
class ToolContext(BaseModel):
    ...
    async def cache_file(self, file_path: str, lines: list[str]) -> None:
        ...
        try:
            ...
            return
        new_entry_bytes = sum((len(line.encode('utf-8')) for line in lines)) / 1024
        self.read_file_cache = [entry for entry in self.read_file_cache if entry.file_path != file_path]
        ...
        while self.read_file_cache and current_size + new_entry_bytes > self.max_cache_bytes:
            removed = self.read_file_cache.pop(0)
            current_size -= removed.bytes
        self.read_file_cache.append(ReadCacheEntry(lines=lines, updated_at=updated_at, bytes=new_entry_bytes, file_path=file_path))

    async def clean_file_cache(self, reserved_file_paths: set[str] | None=None) -> None:
        ...
        self.read_file_cache = [entry for entry in self.read_file_cache if entry.file_path in reserved_file_paths]
...
class ReplyContext(BaseModel):
    ...
    def _serialize_structured_schema(self, value: Type[BaseModel] | dict | None) -> dict | None:
...
class AgentState(BaseModel):
    ...
    def _migrate_legacy_reply_fields(cls, data: Any) -> Any:
        ...
        if isinstance(data, dict) and ('reply_id' in data or 'cur_iter' in data):
            data = dict(data)
    ...
    def append_context(self, name: str, blocks: list[TextBlock | DataBlock | HintBlock | ToolCallBlock | ToolResultBlock]) -> None:
        ...
        """
        if self.context and self.context[-1].role == 'assistant' and (self.context[-1].name == name) and (self.context[-1].id == self.reply_id):
            self.context[-1].content.extend(blocks)
        else:
            self.context.append(Msg(id=self.reply_id, role='assistant', name=name, content=blocks))
    ...
    def get_awaiting_tool_calls(self, name: str) -> list[ToolCallBlock]:
        ...
        return [tc for tc in last_msg.get_content_blocks('tool_call') if tc.state == ToolCallState.ASKING or (tc.state == ToolCallState.SUBMITTED and tc.id not in result_ids)]