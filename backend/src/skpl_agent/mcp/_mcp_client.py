...
class MCPClient(BaseModel):
    ...
    """
    name: str = Field(title='MCP Name', description='The MCP name.')
    is_stateful: bool = Field(title='Stateful', description='Whether this is a stateful connection that requires explicit connect() and close(). STDIO MCP must be stateful. HTTP MCP can be either stateful or stateless.')
    mcp_config: StdioMCPConfig | HttpMCPConfig = Field(discriminator='type', title='MCP Config', description='The MCP server configuration.')
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if not re.fullmatch('[a-zA-Z0-9_-]+', self.name):
            raise ValueError(f"MCPClient name '{self.name}' contains characters not allowed by LLM providers (only [a-zA-Z0-9_-] are permitted). Please rename it.")
        ...
        if self.enable_tools is not None:
            if not isinstance(self.enable_tools, list) or any((not isinstance(_, str) for _ in self.enable_tools)):
                raise ValueError(f'Enable tools should be a list of strings, but got {self.enable_tools}.')
        if self.disable_tools is not None:
            if not isinstance(self.disable_tools, list) or any((not isinstance(_, str) for _ in self.disable_tools)):
                raise ValueError(f'Disable tools should be a list of strings, but got {self.disable_tools}.')
        if self.enable_tools is not None and self.disable_tools is not None:
            intersection = set(self.enable_tools).intersection(set(self.disable_tools))
            if len(intersection) != 0:
                raise ValueError(f'The tools in enable_tools and disable_tools should not overlap, but got {intersection}.')
    ...
    def _initialize_client(self) -> None:
        ...
        if self.mcp_config.type == 'stdio_mcp':
            ...
            self._client = stdio_client(StdioServerParameters(command=config.command, args=config.args or [], env=config.env, cwd=str(config.cwd) if config.cwd else None, encoding='utf-8', encoding_error_handler=config.encoding_error_handler))
    ...
    async def connect(self) -> None:
        ...
        if not self.is_stateful:
            logger.debug("Stateless MCP '%s' does not require explicit connect.", self.name)
            return
        if self._is_connected:
            raise RuntimeError(f"MCP '{self.name}' is already connected. Call close() before reconnecting.")
    ...
    async def close(self, ignore_errors: bool=True) -> None:
        ...
        """
        if not self.is_stateful:
            logger.debug("Stateless MCP '%s' does not require explicit close.", self.name)
            return
        ...
        try:
            ...
            if not ignore_errors:
                raise e
            logger.warning("Error closing MCP '%s': %s", self.name, str(e))
        finally:
            self._stack = None
            self._session = None
    ...
    async def get_tool(self, name: str) -> MCPTool:
        ...
        if not self.is_stateful:
            return MCPTool(mcp_name=self.name, tool=target_tool, client_gen=self._get_client_gen, timeout=self.execution_timeout)
        else:
            self._validate_connection()
            return MCPTool(mcp_name=self.name, tool=target_tool, session=self._session, timeout=self.execution_timeout)
    ...
    def _validate_connection(self) -> None:
        ...
        if not self._session:
            raise RuntimeError(f"MCP '{self.name}' session is not initialized. Call connect() first.")