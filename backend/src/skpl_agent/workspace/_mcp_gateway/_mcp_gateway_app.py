...
from skpl_agent.mcp import MCPClient
...
def _build_app(state: _State, auth_token: str | None=None, instance_nonce: str | None=None) -> FastAPI:
    ...
    async def _call_tool(name: str, tool: str, request: Request) -> dict[str, Any]:
        client = state.clients.get(name)
        if client is None:
            raise HTTPException(404, f'{name!r} not found')
        body = await request.json()
...
async def _connect_initial(state: _State, server_cfgs: list[dict[str, Any]]) -> None:
    ...
    for cfg in server_cfgs:
        ...
        if client.name in state.clients:
            if client.is_stateful and client.is_connected:
                await client.close()
            raise ValueError(f'Duplicated server name in config: {client.name!r}')
        state.clients[client.name] = client
        print(f'[gateway] connected {client.name!r}', flush=True)

async def _run(config_path: str, port: int, auth_token: str | None=None, instance_nonce: str | None=None) -> None:
    """Read config, connect upstreams, start uvicorn, clean up on exit."""
    with open(config_path, encoding='utf-8') as f:
        servers = json.load(f)
    if not isinstance(servers, list):
        raise ValueError(f'config file must be a JSON list of MCPClient specs, got {type(servers).__name__}')
    state = _State()
    await _connect_initial(state, servers)
    app = _build_app(state, auth_token=auth_token, instance_nonce=instance_nonce)
    print(f'[gateway] serving {len(state.clients)} MCPs on :{port}', flush=True)
    import uvicorn
    uvi_cfg = uvicorn.Config(app, host='127.0.0.1', port=port, log_level='info')
...
def main() -> None:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description='In-workspace MCP gateway (FastAPI)')
    parser.add_argument('--config', required=True)
    parser.add_argument('--port', type=int, default=5600)
    parser.add_argument('--auth-token')
    parser.add_argument('--instance-nonce')
    args = parser.parse_args()
    asyncio.run(_run(args.config, args.port, auth_token=args.auth_token, instance_nonce=args.instance_nonce))
