...
from ._gateway_shim import BODY_INLINE_LIMIT, SANDBOX_TMP_DIR, SHIM_SCRIPT
if TYPE_CHECKING:
...
class GatewayMCPTool(ToolBase):
    ...
    def __init__(self, mcp_name: str, tool: mcp.types.Tool, gateway: 'GatewayClient') -> None:
    ...
    async def check_permissions(self, *_args: Any, **_kwargs: Any) -> PermissionDecision:
        ...
        if self.is_read_only:
            return PermissionDecision(behavior=PermissionBehavior.ALLOW, message='This is a read-only MCP tool. Allowing execution.')
        return PermissionDecision(behavior=PermissionBehavior.ASK, message='MCP tools must be explicitly allowed by the user.')
    ...
    async def __call__(self, **kwargs: Any) -> ToolChunk:
        ...
        """
        (status, body) = await self._gateway.exec_request('POST', f'/mcps/{self.mcp_name}/tools/{self._tool.name}', body={'arguments': kwargs})
        if status >= 400:
            return ToolChunk(content=[{'type': 'text', 'text': _safe_detail(status, body)}], state=ToolResultState.ERROR)
        payload = json.loads(body)
        chunk_dict = payload.get('chunk')
        if chunk_dict is None:
            raise RuntimeError(f'gateway returned no chunk for {self.name!r}')
...
class GatewayMCPClient(MCPClient):
    ...
    * ``list_raw_tools`` / ``get_tool`` fetch and wrap upstream tools.
    ...
    def attach(self, gateway: 'GatewayClient', *, connected: bool=False) -> None:
    ...
    async def connect(self) -> None:
        ...
        if self._is_connected:
            raise RuntimeError(f'MCP {self.name!r} is already connected. Call close() before reconnecting.')
        assert self._gateway is not None
        body = self.model_dump(mode='json')
        (status, resp_body) = await self._gateway.exec_request('POST', '/mcps', body=body)
        if status >= 400:
            raise RuntimeError(f'gateway failed to add MCP {self.name!r}: {_safe_detail(status, resp_body)}')
        self._is_connected = True
    ...
    async def close(self, ignore_errors: bool=True) -> None:
        ...
        if not self._is_connected:
            if ignore_errors:
                return
            raise RuntimeError(f'MCP {self.name!r} is not connected. Call connect() first.')
        assert self._gateway is not None
        try:
            (status, resp_body) = await self._gateway.exec_request('DELETE', f'/mcps/{self.name}')
            if status >= 400 and (not ignore_errors):
                raise RuntimeError(f'gateway failed to remove MCP {self.name!r}: {_safe_detail(status, resp_body)}')
        except Exception:
    ...
    async def list_raw_tools(self) -> list[mcp.types.Tool]:
        ...
        """
        assert self._gateway is not None
        (status, body) = await self._gateway.exec_request('GET', f'/mcps/{self.name}/tools')
        if status >= 400:
            raise RuntimeError(f'gateway failed to list tools for MCP {self.name!r}: {_safe_detail(status, body)}')
        ...
        if self.disable_tools is not None:
            raw_tools = [t for t in raw_tools if t.name not in self.disable_tools]
        return raw_tools
    ...
    async def get_tool(self, name: str) -> GatewayMCPTool:
        ...
        raise ValueError(f'Tool {name!r} not found in MCP {self.name!r}.')
    ...
    def _wrap_tool(self, tool: mcp.types.Tool) -> GatewayMCPTool:
        ...
        """
        assert self._gateway is not None
        return GatewayMCPTool(mcp_name=self.name, tool=tool, gateway=self._gateway)
...
class GatewayClient:
    ...
    def __init__(self, backend: 'BackendBase', gateway_port: int, *, timeout: float | None=None, inline_limit: int=BODY_INLINE_LIMIT, tmp_dir: str=SANDBOX_TMP_DIR, gateway_log_path: str | None=None, auth_token: str | None=None, instance_nonce: str | None=None) -> None:
    ...
    async def health(self) -> bool:
        ...
        """
        try:
            (status, body) = await self.exec_request('GET', '/health', include_auth=False)
        except Exception:
            return False
        if status != 200:
            return False
        ...
        return isinstance(nonce, str) and nonce.isascii() and self.instance_nonce.isascii() and secrets.compare_digest(nonce, self.instance_nonce)
    ...
    async def list_mcps(self) -> list[GatewayMCPClient]:
        ...
        if status >= 400:
            raise RuntimeError(f'gateway failed to list MCPs: {_safe_detail(status, body)}')
        specs = json.loads(body)
        return [self.make_client(spec, connected=True) for spec in specs]

    def make_client(self, spec: dict[str, Any], *, connected: bool=False) -> GatewayMCPClient:
    ...
    async def exec_request(self, method: str, path: str, *, body: Any=None, include_auth: bool=True) -> tuple[int, bytes]:
        ...
        if body is not None:
            ...
            await self.backend.write_file(body_file, json.dumps(body, ensure_ascii=False).encode('utf-8'))
        try:
            try:
                result: 'ExecResult' = await self.backend.exec_shell(['python3', '-c', SHIM_SCRIPT, method, f'http://127.0.0.1:{self.gateway_port}{path}', body_file, str(self.inline_limit), self.tmp_dir, self.auth_token or '' if include_auth else ''], timeout=self.timeout)
            finally:
            ...
            if result.exit_code != 0:
                raise RuntimeError(f"gateway shim exited with {result.exit_code}: {result.stderr.decode(errors='replace')[:500]}")
            try:
                env = json.loads(result.stdout)
            except Exception as e:
                raise RuntimeError(f'gateway shim produced non-JSON stdout: {result.stdout[:200]!r}') from e
            status = int(env['status'])
            if status == -1:
                raise RuntimeError(f"gateway request failed: {env.get('error', 'unknown error')}")
            if 'body_file' in env:
    ...
    async def _diagnose_failure(self, method: str, path: str, exc: BaseException) -> None:
        ...
        if healthy:
            return
        logger.error('Gateway unreachable during %s %s: %s. Probing /health failed. Attempting to tail gateway log at %r ...', method, path, exc, self.gateway_log_path)
        if self.gateway_log_path is None:
            return
        try:
            log_bytes = await self.backend.read_file(self.gateway_log_path)
        except Exception as read_exc:
            logger.error('Failed to read gateway log at %r: %s', self.gateway_log_path, read_exc)
            return
        tail = log_bytes[-self._log_tail_bytes:].decode(errors='replace')
        logger.error('Gateway log tail (last %d bytes of %r):\n%s', len(tail), self.gateway_log_path, tail)
