...
class BubblewrapBackend(BackendBase):
    ...
    def __init__(self, *, host_workdir: str, host_tmpdir: str, host_cache_dir: str | None=None, workdir: str=SANDBOX_WORKDIR, share_net: bool=True, env: dict[str, str] | None=None) -> None:
        ...
        os.makedirs(host_workdir_path, mode=448, exist_ok=True)
        os.makedirs(host_tmpdir_path, mode=448, exist_ok=True)
        if workdir_created:
            os.chmod(host_workdir_path, 448)
        if tmpdir_created:
            os.chmod(host_tmpdir_path, 448)
        self._host_workdir = os.path.realpath(host_workdir_path)
        self._host_tmpdir = os.path.realpath(host_tmpdir_path)
        self._host_workdir_identity = self._directory_identity(self._host_workdir, label='host_workdir')
        self._host_tmpdir_identity = self._directory_identity(self._host_tmpdir, label='host_tmpdir')
        mount_sources: list[tuple[str, str]] = [('host_workdir', self._host_workdir), ('host_tmpdir', self._host_tmpdir)]
        ...
        if cache_dir is not None:
            ...
            os.makedirs(cache_dir, mode=448, exist_ok=True)
            if cache_created:
                os.chmod(cache_dir, 448)
            if not os.path.isdir(cache_dir):
                raise ValueError('host_cache_dir must be a directory.')
            assert self._host_cache_dir is not None
            self._host_cache_identity = self._directory_identity(self._host_cache_dir, label='host_cache_dir')
    ...
    async def exec_shell(self, command: list[str], *, cwd: str | None=None, timeout: float | None=None) -> ExecResult:
        ...
        try:
            process = await self.start_process(command, cwd=cwd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        ...
        try:
            ...
            await asyncio.shield(self._terminate_process_tree(process, grace=1.0))
            raise
        return ExecResult(exit_code=process.returncode or 0, stdout=stdout, stderr=stderr)

    async def start_process(self, command: list[str], *, cwd: str | None=None, stdin: Any=asyncio.subprocess.DEVNULL, stdout: Any=asyncio.subprocess.DEVNULL, stderr: Any=asyncio.subprocess.DEVNULL) -> asyncio.subprocess.Process:
        ...
        return await asyncio.create_subprocess_exec(*argv, stdin=stdin, stdout=stdout, stderr=stderr, **kwargs)

    @staticmethod
    async def _terminate_process_tree(process: asyncio.subprocess.Process, *, grace: float) -> None:
    ...
    async def read_file(self, path: str) -> bytes:
        ...
        result = await self.exec_shell(['sh', '-c', 'resolved=$(realpath -e -- "$1") || exit 66; case "$resolved" in /workspace|/workspace/*|/tmp|/tmp/*) cat -- "$resolved" ;; *) exit 65 ;; esac', 'sh', sandbox_path])
        if result.exit_code == 66:
            raise FileNotFoundError(f'not found in Bubblewrap sandbox: {path}')
        if result.exit_code == 65:
            raise PermissionError(f'path escapes writable Bubblewrap mounts: {path}')
        if not result.ok():
            raise RuntimeError(f"Bubblewrap read_file failed (exit {result.exit_code}): {result.stderr.decode(errors='replace')}")
        return result.stdout

    async def write_file(self, path: str, data: bytes) -> None:
        """Write raw bytes, refusing every symbolic-link final component."""
        sandbox_path = self._sandbox_path_for(path)
        result = await self._exec_with_input(['sh', '-c', 'parent=$(dirname -- "$1") && resolved_parent=$(realpath -m -- "$parent") || exit 65; case "$resolved_parent" in /workspace|/workspace/*|/tmp|/tmp/*) ;; *) exit 65 ;; esac; mkdir -p -- "$resolved_parent" && target="$resolved_parent/$(basename -- "$1")" && [ ! -L "$target" ] || exit 65; cat > "$target"', 'sh', sandbox_path], data)
        if result.exit_code == 65:
            raise PermissionError(f'path escapes writable Bubblewrap mounts: {path}')
        if not result.ok():
            raise RuntimeError(f"Bubblewrap write_file failed (exit {result.exit_code}): {result.stderr.decode(errors='replace')}")

    async def _exec_with_input(self, command: list[str], data: bytes, *, cwd: str | None=None, timeout: float | None=None) -> ExecResult:
        """Run a sandbox command with ``data`` piped to stdin."""
        try:
            process = await self.start_process(command, cwd=cwd, stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        except (FileNotFoundError, NotADirectoryError, OSError) as exc:
            return ExecResult(exit_code=127, stdout=b'', stderr=str(exc).encode('utf-8'))
        try:
    ...
    def _bwrap_argv(self, command: list[str], *, cwd: str | None) -> list[str]:
        """Build the ``bwrap`` argv for one sandboxed process."""
        self._validate_mount_sources()
        args = ['bwrap', '--die-with-parent', '--new-session', '--unshare-all', '--proc', '/proc', '--dev', '/dev', '--bind', self._host_workdir, SANDBOX_WORKDIR, '--bind', self._host_tmpdir, SANDBOX_TMPDIR, '--tmpfs', '/run', '--dir', '/var', '--tmpfs', '/var/tmp']
        if self._host_cache_dir is not None:
            args.extend(['--bind', self._host_cache_dir, SANDBOX_CACHE_DIR])
        if self._share_net:
    ...
    def _validate_mount_sources(self) -> None:
        ...
        for (label, path, expected_identity) in (('host_workdir', self._host_workdir, self._host_workdir_identity), ('host_tmpdir', self._host_tmpdir, self._host_tmpdir_identity)):
            try:
                identity = self._directory_identity(path, label=label)
            except ValueError as exc:
                raise RuntimeError(f'{label} was removed or replaced before execution.') from exc
            if identity != expected_identity:
                raise RuntimeError(f'{label} was replaced before execution.')
        if self._host_cache_dir is None:
            return
        try:
            identity = self._directory_identity(self._host_cache_dir, label='host_cache_dir')
        except ValueError as exc:
            raise RuntimeError('host_cache_dir was removed or replaced before execution.') from exc
        if identity != self._host_cache_identity:
            raise RuntimeError('host_cache_dir was replaced before execution.')

    @staticmethod
    def _directory_identity(path: str, *, label: str) -> tuple[int, int]:
    ...
    def _base_env(self) -> dict[str, str]:
        ...
        path_parts = [f'{SANDBOX_WORKDIR}/.agentscope/.venv/bin', f'{SANDBOX_WORKDIR}/.agentscope/bin', '/usr/local/sbin', '/usr/local/bin', '/usr/sbin', '/usr/bin', '/sbin', '/bin']
        return {'HOME': SANDBOX_WORKDIR, 'TMPDIR': SANDBOX_TMPDIR, 'UV_CACHE_DIR': f'{SANDBOX_CACHE_DIR}/uv', 'XDG_CACHE_HOME': f'{SANDBOX_CACHE_DIR}/xdg', 'PIP_CACHE_DIR': f'{SANDBOX_CACHE_DIR}/pip', 'PATH': ':'.join(path_parts), 'LANG': 'C.UTF-8', 'LC_ALL': 'C.UTF-8', 'PYTHONUNBUFFERED': '1'}
    ...
    def _readonly_etc_mounts() -> list[str]:
        ...
        args = ['--dir', '/etc']
        for path in ('/etc/resolv.conf', '/etc/hosts', '/etc/nsswitch.conf', '/etc/passwd', '/etc/group'):
            if os.path.exists(path):
                args.extend(['--ro-bind', path, path])
        for path in ('/etc/alternatives', '/etc/ssl', '/etc/pki', '/etc/ca-certificates'):
            if os.path.exists(path):
                args.extend(['--ro-bind', path, path])
        return args
    ...
    def _sandbox_path_for(self, path: str) -> str:
        ...
        for sandbox_root in (SANDBOX_WORKDIR, SANDBOX_TMPDIR):
            ...
            if normalized.startswith(prefix):
                return normalized
        raise PermissionError(f'Bubblewrap file access is limited to {SANDBOX_WORKDIR!r} and {SANDBOX_TMPDIR!r}: {path!r}')