...
from .._utils import _GATEWAY_BASE_REQUIREMENTS, _read_gateway_script_bytes, _read_glob_helper_bytes
from ._bubblewrap_backend import BubblewrapBackend
from ._constants import BWRAP_SMOKE_PROBE_ARGV, DEFAULT_GATEWAY_PORT, GATEWAY_HOME, SANDBOX_CACHE_DIR, SANDBOX_WORKDIR
...
class BubblewrapWorkspace(SandboxedWorkspaceBase):
    ...
    def __init__(self, *, workspace_id: str | None=None, host_workdir: str | None=None, host_cache_dir: str | None=None, gateway_port: int | None=DEFAULT_GATEWAY_PORT, share_net: bool=True, env: dict[str, str] | None=None, extra_pip: list[str] | None=None, instructions: str=_DEFAULT_INSTRUCTIONS, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None) -> None:
        ...
        if not share_net:
            raise ValueError('BubblewrapWorkspace currently requires share_net=True because its TCP MCP gateway must be reachable across separate bwrap executions.')
        ...
        super().__init__(workspace_id=workspace_id, default_mcps=default_mcps, skill_paths=skill_paths)
        ...
        self._host_cache_dir_input = os.path.abspath(host_cache_dir) if host_cache_dir else None
        ...
        self.host_workdir = os.path.abspath(host_workdir) if host_workdir else ''
        self._tmpdir: tempfile.TemporaryDirectory[str] | None = None
        self._host_cache_dir = os.path.abspath(host_cache_dir) if host_cache_dir else ''
    ...
    def _validate_gateway_port(gateway_port: int | None) -> None:
        ...
        if gateway_port is None:
            return
        if isinstance(gateway_port, bool) or not isinstance(gateway_port, int) or (not 1 <= gateway_port <= 65535):
            raise ValueError('gateway_port must be None or an integer from 1 to 65535.')
    ...
    async def _provision_backend(self) -> None:
        ...
        os.makedirs(self.host_workdir, mode=448, exist_ok=True)
        if not os.path.isdir(self.host_workdir):
            raise ValueError('host_workdir must be a directory.')
        if workdir_created:
            os.chmod(self.host_workdir, 448)
        if self._host_workdir_input is None and self._host_cache_dir_input is None:
            self._owned_cache_dir = tempfile.TemporaryDirectory(prefix='agentscope-bwrap-cache-')
            self._host_cache_dir = self._owned_cache_dir.name
        else:
            self._host_cache_dir = self._resolve_host_cache_dir()
        self._tmpdir = tempfile.TemporaryDirectory()
        self._backend = BubblewrapBackend(host_workdir=self.host_workdir, host_tmpdir=self._tmpdir.name, host_cache_dir=self._host_cache_dir, workdir=SANDBOX_WORKDIR, share_net=self.share_net, env=self.env)
    ...
    def _resolve_host_cache_dir(self) -> str:
        ...
        if self._host_cache_dir_input is not None:
            ...
            key = hashlib.blake2b(workdir.encode('utf-8'), digest_size=16).hexdigest()
            cache_root = os.path.join(os.path.dirname(workdir), '.agentscope-bwrap-cache')
            self._ensure_cache_directory(cache_root, make_private=True)
        ...
        if common in (workdir, cache_dir):
            raise ValueError('host_cache_dir must not overlap host_workdir because it is used as a Bubblewrap bind source.')
        self._ensure_cache_directory(path, make_private=make_private)
        return os.path.realpath(path)

    @staticmethod
    def _ensure_cache_directory(path: str, *, make_private: bool) -> None:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.exception('Bubblewrap cleanup after init failure failed')
            raise
    ...
    async def _setup_mcp_gateway(self) -> None:
        ...
        if not await self._bootstrap_is_ready():
            logger.info('BubblewrapWorkspace: bootstrapping or repairing workspace_id=%r', self.workspace_id)
            for cmd in self._bootstrap_commands():
                result = await backend.exec_shell(['sh', '-c', cmd], timeout=self._bootstrap_cmd_timeout)
                if not result.ok():
                    raise RuntimeError(f"BubblewrapWorkspace bootstrap failed (exit {result.exit_code}) for: {cmd!r}\nstderr: {result.stderr.decode(errors='replace')}\nstdout: {result.stdout.decode(errors='replace')}")
        await backend.write_file(self._glob_helper_path, _read_glob_helper_bytes())
        await backend.write_file(self._gateway_script, _read_gateway_script_bytes())
        ...
        for attempt in range(max_attempts):
            ...
            await self._stop_gateway_process()
            self._rotate_gateway_credentials()
            launch_cmd = f'exec {shlex.quote(self._gateway_python)} -I -u {shlex.quote(self._gateway_script)} --config {shlex.quote(self._mcp_file)} --port {gateway_port} --auth-token {shlex.quote(self._gateway_token)} --instance-nonce {shlex.quote(self._gateway_nonce)} > {shlex.quote(self._gateway_log)} 2>&1'
            self._gateway_process = await backend.start_process(['sh', '-c', launch_cmd], cwd=SANDBOX_WORKDIR)
            self._gateway = GatewayClient(backend=backend, gateway_port=gateway_port, timeout=30.0, gateway_log_path=self._gateway_log, auth_token=self._gateway_token, instance_nonce=self._gateway_nonce)
    ...
    async def _bootstrap_is_ready(self) -> bool:
        ...
        result = await self.get_backend().exec_shell(['sh', '-c', 'test -f "$1" && test -x "$2" && "$2" --version >/dev/null 2>&1 && "$2" -I -c \'import agentscope, fastapi, uvicorn, mcp\' >/dev/null 2>&1 && "$3" --version >/dev/null 2>&1 && "$4" --version >/dev/null 2>&1', 'sh', self._gateway_script, self._gateway_python, f'{self._gateway_home}/bin/uv', f'{self._gateway_home}/bin/rg'], timeout=30.0)
        return result.ok()
    ...
    async def _wait_gateway_health(self, timeout: float) -> bool:
        ...
        while asyncio.get_event_loop().time() < deadline:
            if self._gateway_process is not None and self._gateway_process.returncode is not None:
                return False
    ...
    async def _probe_bubblewrap() -> None:
        ...
        try:
            proc = await asyncio.create_subprocess_exec(*BWRAP_SMOKE_PROBE_ARGV, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, **kwargs)
            (stdout, stderr) = await asyncio.wait_for(proc.communicate(), timeout=5.0)
        except asyncio.TimeoutError as exc:
            if proc is not None:
                await BubblewrapBackend._terminate_process_tree(proc, grace=1.0)
            raise RuntimeError('BubblewrapWorkspace bwrap smoke probe timed out.') from exc
        except (FileNotFoundError, NotADirectoryError, OSError) as exc:
            raise RuntimeError("BubblewrapWorkspace requires Linux and a working 'bwrap' executable.") from exc
        except BaseException:
            if proc is not None:
                await asyncio.shield(BubblewrapBackend._terminate_process_tree(proc, grace=1.0))
            raise
        if proc.returncode != 0:
            detail = (stderr or stdout).decode('utf-8', errors='replace')
            raise RuntimeError(f"BubblewrapWorkspace requires Linux and a working 'bwrap' executable. Smoke probe failed: {detail}")

    def _bootstrap_commands(self) -> list[str]:
        """Return user-space provisioning commands for Bubblewrap."""
        pip_pkgs = [*_GATEWAY_BASE_REQUIREMENTS, *self.extra_pip]
        ...
        return [f'mkdir -p {shlex.quote(bin_dir)}', f'if ! {uv_path} --version >/dev/null 2>&1; then rm -f {uv_path}; {self._install_uv_script(bin_dir)}; fi', f'if ! {python_path} --version >/dev/null 2>&1; then rm -rf {shlex.quote(self._gateway_venv)}; {uv_path} venv {shlex.quote(self._gateway_venv)}; fi', f'{uv_path} pip install --python {python_path} {pip_args}', f'if ! {rg_path} --version >/dev/null 2>&1; then rm -f {rg_path}; {self._install_ripgrep_script()}; fi', f'{rg_path} --version', f"{uv_path} pip install --python {python_path} --no-deps 'agentscope'"]
    ...
    def _install_uv_script(bin_dir: str) -> str:
        ...
        return f'set -eu; tmp_installer=$(mktemp); cleanup_installer() {{ rm -f "$tmp_installer"; }}; trap cleanup_installer EXIT INT TERM; curl -LsSf --retry 5 --retry-delay 2 --retry-all-errors --connect-timeout 15 --max-time 60 --retry-max-time 180 -o "$tmp_installer" https://astral.sh/uv/install.sh; env UV_INSTALL_DIR={quoted_bin_dir} INSTALLER_NO_MODIFY_PATH=1 sh "$tmp_installer"; rm -f "$tmp_installer"; trap - EXIT INT TERM'
    ...
    def _install_ripgrep_script(self) -> str:
        ...
        version = '14.1.0'
        base_url = f'https://github.com/BurntSushi/ripgrep/releases/download/{version}'
        curl_release_asset = 'curl -fL --retry 5 --retry-delay 2 --retry-all-errors --connect-timeout 15 --max-time 90 --retry-max-time 180'
        x86_64_sha256 = 'f84757b07f425fe5cf11d87df6644691c644a5cd2348a2c670894272999d3ba7'
        aarch64_sha256 = 'c8c210b99844fbf16b7a36d1c963e8351bca5ff2dd7c788f5fba4ac18ba8c60d'
        return f'set -eu; arch=$(uname -m); case "$arch" in x86_64|amd64) asset=ripgrep-{version}-x86_64-unknown-linux-musl.tar.gz; sha256={x86_64_sha256} ;; aarch64|arm64) asset=ripgrep-{version}-aarch64-unknown-linux-gnu.tar.gz; sha256={aarch64_sha256} ;; *) echo "unsupported ripgrep architecture: $arch" >&2; exit 1 ;; esac; mkdir -p {cache_dir} {bin_dir}; cd {cache_dir}; if ! printf "%s  %s\\n" "$sha256" "$asset" | sha256sum -c - >/dev/null 2>&1; then tmp_asset=$(mktemp "${{asset}}.tmp.XXXXXX"); cleanup_asset() {{ rm -f "$tmp_asset"; }}; trap cleanup_asset EXIT INT TERM; {curl_release_asset} -o "$tmp_asset" {shlex.quote(base_url)}/"$asset"; printf "%s  %s\\n" "$sha256" "$tmp_asset" | sha256sum -c -; mv -f "$tmp_asset" "$asset"; trap - EXIT INT TERM; fi; tmp=$(mktemp -d); tar -xzf "$asset" -C "$tmp"; tmp_rg=$(mktemp {bin_dir}/.rg.tmp.XXXXXX); cleanup_rg() {{ rm -f "$tmp_rg"; }}; trap cleanup_rg EXIT INT TERM; cp "$tmp"/ripgrep-*/rg "$tmp_rg"; chmod +x "$tmp_rg"; mv -f "$tmp_rg" {bin_dir}/rg; trap - EXIT INT TERM; rm -rf "$tmp"'
    ...
    async def _stop_gateway_process(self) -> None:
        ...
        try:
            if proc.returncode is None:
                await BubblewrapBackend._terminate_process_tree(proc, grace=5.0)
        finally:
    ...
    async def _raise_gateway_timeout(self, timeout: float) -> None:
        ...
        code = self._gateway_process.returncode if self._gateway_process is not None else None
        raise RuntimeError(f'gateway did not become healthy within {timeout}s (process returncode={code}). Tail of {self._gateway_log}:\n{tail}')