...
from ._constants import DEFAULT_GATEWAY_PORT, DEFAULT_IMAGE, GATEWAY_HOME, POD_WORKDIR, SYSTEM_DEPS, _k8s_safe_name
...
class K8sWorkspace(SandboxedWorkspaceBase):
    ...
    def __init__(self, *, workspace_id: str | None=None, kubeconfig: str | None=None, namespace: str='agentscope', image: str=DEFAULT_IMAGE, image_pull_policy: str='IfNotPresent', image_pull_secrets: list[str] | None=None, resources: dict[str, Any] | None=None, node_selector: dict[str, str] | None=None, tolerations: list[dict[str, Any]] | None=None, service_account: str | None=None, gateway_port: int=DEFAULT_GATEWAY_PORT, extra_pip: list[str] | None=None, storage_class: str | None=None, storage_size: str='1Gi', delete_pvc_on_close: bool=False, env: dict[str, str] | None=None, instructions: str=DEFAULT_WORKSPACE_INSTRUCTIONS, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None) -> None:
        ...
        self.instructions = instructions.format(backend='Kubernetes-based', workdir=self.workdir)
    ...
    async def _provision_backend(self) -> None:
        ...
        self._backend = K8sBackend(api_client=self._api_client, namespace=self._namespace, pod_name=self._pod_name, container_name='workspace', workdir=POD_WORKDIR)
    ...
    async def _teardown_backend(self) -> None:
        ...
        if self._v1 is not None and self._pod_name:
            try:
                await self._v1.delete_namespaced_pod(self._pod_name, self._namespace)
            except Exception as e:
                logger.warning('K8sWorkspace: Pod delete failed: %s', e)
            if self._delete_pvc_on_close:
                try:
                    await self._v1.delete_namespaced_persistent_volume_claim(self._pod_name, self._namespace)
                except Exception as e:
                    logger.warning('K8sWorkspace: PVC delete failed: %s', e)
    ...
    async def _ensure_namespace(self) -> None:
        ...
        try:
            ...
            if e.status == 404:
                from kubernetes_asyncio import client as k8s_client
                ns = k8s_client.V1Namespace(metadata=k8s_client.V1ObjectMeta(name=self._namespace))
    ...
    async def _ensure_pvc(self) -> None:
        ...
        try:
            pvc = await self._v1.read_namespaced_persistent_volume_claim(pvc_name, self._namespace)
            if pvc.metadata and pvc.metadata.deletion_timestamp is not None:
                logger.info('K8sWorkspace: PVC %r is being deleted, waiting...', pvc_name)
    ...
    async def _create_pvc(self, pvc_name: str) -> None:
        """Create a new PVC."""
        from kubernetes_asyncio import client as k8s_client
        spec_kwargs: dict[str, Any] = {'access_modes': ['ReadWriteOnce'], 'resources': k8s_client.V1VolumeResourceRequirements(requests={'storage': self._storage_size})}
        if self._storage_class is not None:
            spec_kwargs['storage_class_name'] = self._storage_class
        pvc = k8s_client.V1PersistentVolumeClaim(metadata=k8s_client.V1ObjectMeta(name=pvc_name, namespace=self._namespace, labels={'app.kubernetes.io/managed-by': 'agentscope', 'skpl_agent.workspace.id': self.workspace_id}), spec=k8s_client.V1PersistentVolumeClaimSpec(**spec_kwargs))
        await self._v1.create_namespaced_persistent_volume_claim(self._namespace, pvc)

    async def _wait_pvc_deleted(self, pvc_name: str, timeout: float=60.0) -> None:
        """Poll until the PVC is fully deleted."""
        from kubernetes_asyncio.client.rest import ApiException
        deadline = asyncio.get_event_loop().time() + timeout
        while asyncio.get_event_loop().time() < deadline:
            try:
                await self._v1.read_namespaced_persistent_volume_claim(pvc_name, self._namespace)
            except ApiException as e:
                if e.status == 404:
                    return
                raise
            await asyncio.sleep(1.0)
        raise RuntimeError(f'PVC {pvc_name!r} did not finish deleting within {timeout}s')
    ...
    async def _ensure_pod(self) -> None:
        ...
        try:
            pod = await self._v1.read_namespaced_pod(self._pod_name, self._namespace)
            phase = pod.status.phase if pod.status else None
            deletion_ts = pod.metadata.deletion_timestamp if pod.metadata is not None else None
            if deletion_ts is not None:
                logger.info('K8sWorkspace: Pod %r is being deleted, waiting to recreate', self._pod_name)
            ...
            if phase in _REBUILD_PHASES:
                logger.info('K8sWorkspace: Pod %r is %s, deleting and recreating', self._pod_name, phase)
                try:
                    await self._v1.delete_namespaced_pod(self._pod_name, self._namespace)
                except ApiException:
                    pass
                ...
                logger.warning('K8sWorkspace: Pod %r has unexpected phase %r, deleting and recreating', self._pod_name, phase)
                try:
                    await self._v1.delete_namespaced_pod(self._pod_name, self._namespace)
                except ApiException:
                    pass
    ...
    async def _create_pod(self) -> None:
        ...
        container_env = None
        if self.env:
            container_env = [k8s_client.V1EnvVar(name=k, value=v) for (k, v) in self.env.items()]
        container = k8s_client.V1Container(name='workspace', image=self._image, image_pull_policy=self._image_pull_policy, command=['sleep', 'infinity'], working_dir=POD_WORKDIR, ports=[k8s_client.V1ContainerPort(container_port=self.gateway_port)], resources=k8s_client.V1ResourceRequirements(**self._resources) if self._resources else None, volume_mounts=[k8s_client.V1VolumeMount(name='workspace-data', mount_path=POD_WORKDIR)], env=container_env)
        volumes = [k8s_client.V1Volume(name='workspace-data', persistent_volume_claim=k8s_client.V1PersistentVolumeClaimVolumeSource(claim_name=self._pod_name))]
        spec_kwargs: dict[str, Any] = {'restart_policy': 'OnFailure', 'containers': [container], 'volumes': volumes}
        if self._node_selector:
            spec_kwargs['node_selector'] = self._node_selector
        if self._tolerations:
            spec_kwargs['tolerations'] = [k8s_client.V1Toleration(**t) for t in self._tolerations]
        if self._service_account:
            spec_kwargs['service_account_name'] = self._service_account
        if self._image_pull_secrets:
            spec_kwargs['image_pull_secrets'] = [k8s_client.V1LocalObjectReference(name=s) for s in self._image_pull_secrets]
        pod = k8s_client.V1Pod(metadata=k8s_client.V1ObjectMeta(name=self._pod_name, namespace=self._namespace, labels={'app.kubernetes.io/managed-by': 'agentscope', 'skpl_agent.workspace': 'true', 'skpl_agent.workspace.id': self.workspace_id}), spec=k8s_client.V1PodSpec(**spec_kwargs))
    ...
    async def _wait_pod_running(self, timeout: float=120.0) -> None:
        ...
        """
        _TERMINAL_WAITING_REASONS = frozenset({'ImagePullBackOff', 'ErrImagePull', 'InvalidImageName', 'CrashLoopBackOff'})
        _UNSCHEDULABLE_TYPES = frozenset({'PodScheduled'})
        deadline = asyncio.get_event_loop().time() + timeout
        delay = 0.5
        while asyncio.get_event_loop().time() < deadline:
            pod = await self._v1.read_namespaced_pod(self._pod_name, self._namespace)
            ...
            if phase in ('Failed', 'Unknown'):
                raise RuntimeError(f'Pod {self._pod_name!r} entered {phase} state')
                ...
                for cs in pod.status.container_statuses or []:
                    if cs.state and cs.state.waiting:
                        ...
                        if reason in _TERMINAL_WAITING_REASONS:
                            ...
                            raise RuntimeError(f'Pod {self._pod_name!r} container is stuck: {msg}')
                for cond in pod.status.conditions or []:
                    if cond.type in _UNSCHEDULABLE_TYPES and cond.status == 'False' and (cond.reason == 'Unschedulable'):
                        raise RuntimeError(f'Pod {self._pod_name!r} is unschedulable: {cond.message}')
            await asyncio.sleep(delay)
            delay = min(delay * 1.5, 3.0)
        raise RuntimeError(f'Pod {self._pod_name!r} did not become Running within {timeout}s')
    ...
    async def _wait_pod_deleted(self, timeout: float=30.0) -> None:
        ...
        while asyncio.get_event_loop().time() < deadline:
            try:
                await self._v1.read_namespaced_pod(self._pod_name, self._namespace)
            except ApiException as e:
                if e.status == 404:
                    return
                raise
            await asyncio.sleep(1.0)
        raise RuntimeError(f'Pod {self._pod_name!r} did not finish deleting within {timeout}s')
    ...
    def _bootstrap_commands(self) -> list[str]:
        ...
        return [f'apt-get update -qq && apt-get install -y --no-install-recommends {sys_deps} && rm -rf /var/lib/apt/lists/*', 'curl -LsSf https://astral.sh/uv/install.sh | env UV_INSTALL_DIR=/usr/local/bin INSTALLER_NO_MODIFY_PATH=1 sh', f'uv venv {self._gateway_venv}', f'uv pip install --python {self._gateway_python} {pip_args}', f"uv pip install --python {self._gateway_python} --no-deps 'agentscope'"]