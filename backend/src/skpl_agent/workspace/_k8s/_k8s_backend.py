...
class K8sBackend(BackendBase):
    ...
    def __init__(self, api_client: Any, namespace: str, pod_name: str, container_name: str, workdir: str) -> None:
    ...
    async def exec_shell(self, command: list[str], *, cwd: str | None=None, timeout: float | None=None) -> ExecResult:
        ...
        wrapped = ['sh', '-c', f'cd {shlex.quote(effective_cwd)} && exec "$@"', '--', *command]
        ...
        async def _run() -> ExecResult:
            async with WsApiClient(self._api_client.configuration) as ws_api:
                ...
                ws = await v1_ws.connect_get_namespaced_pod_exec(self._pod_name, self._namespace, command=wrapped, container=self._container_name, stderr=True, stdin=False, stdout=True, tty=False, _preload_content=False)
                            ...
                            if channel == 1:
                                ...
                                try:
                                    ...
                                    if status.get('status') != 'Success':
                                        exit_code = int(status.get('details', {}).get('causes', [{}])[0].get('message', '1'))
                                except (json.JSONDecodeError, ValueError, IndexError, KeyError):
                                    exit_code = 1
                        else:
                            break
                return ExecResult(exit_code=exit_code, stdout=b''.join(stdout_parts), stderr=b''.join(stderr_parts))
        if timeout is None:
            return await _run()
        try:
    ...
    async def read_file(self, path: str) -> bytes:
        ...
        result = await self.exec_shell(['tar', 'cf', '-', '-C', dirname, basename], cwd='/')
        if not result.ok():
            stderr_text = result.stderr.decode(errors='replace')
            if 'No such file' in stderr_text or 'not found' in stderr_text:
                raise FileNotFoundError(f'not found in Pod: {path}')
            raise FileNotFoundError(f'not found in Pod: {path} (tar stderr: {stderr_text})')
        try:
            tar = tarfile.open(fileobj=io.BytesIO(result.stdout), mode='r')
        except tarfile.ReadError as exc:
            raise FileNotFoundError(f'not found in Pod: {path}') from exc
        try:
    ...
    async def write_file(self, path: str, data: bytes) -> None:
        ...
        async with WsApiClient(self._api_client.configuration) as ws_api:
            v1_ws = k8s_client.CoreV1Api(api_client=ws_api)
            ws = await v1_ws.connect_get_namespaced_pod_exec(self._pod_name, self._namespace, command=['tar', 'xf', '-', '-C', parent], container=self._container_name, stderr=True, stdin=True, stdout=True, tty=False, _preload_content=False)
            stderr_parts: list[bytes] = []
            exit_code = 0
            async with ws as sock:
                await sock.send_bytes(bytes([0]) + tar_bytes)
                ...
                async for msg in sock:
                    ...
                    if channel == 2:
                        ...
                        try:
                            ...
                            exit_code = 1
            if exit_code != 0:
                stderr_text = b''.join(stderr_parts).decode(errors='replace')
                raise RuntimeError(f'write_file to {path!r} failed: tar xf exited {exit_code}: {stderr_text}')