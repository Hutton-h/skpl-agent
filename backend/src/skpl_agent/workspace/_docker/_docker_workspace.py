...
from ._make_dockerfile import CONTAINER_WORKDIR, DEFAULT_BASE_IMAGE, DEFAULT_GATEWAY_PORT, GATEWAY_HOME, prepare_build_context
...
class DockerWorkspace(SandboxedWorkspaceBase):
    ...
    def __init__(self, *, workspace_id: str | None=None, base_image: str=DEFAULT_BASE_IMAGE, host_workdir: str | None=None, node_version: str | None=None, extra_pip: list[str] | None=None, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, instructions: str=DEFAULT_WORKSPACE_INSTRUCTIONS, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, **kwargs: Any) -> None:
        ...
        if 'workdir' in kwargs:
            logger.warning("DockerWorkspace parameter 'workdir' is deprecated, use 'host_workdir' instead.")
        ...
        self.instructions = instructions.format(backend='Docker-based', workdir=self.workdir)
    ...
    async def _teardown_backend(self) -> None:
        ...
        if self._container is not None:
            if self.host_workdir is not None and sys.platform == 'linux' and (self._backend is not None):
                try:
                    await self._backend.exec_shell(['chown', '-R', f'{os.getuid()}:{os.getgid()}', CONTAINER_WORKDIR], timeout=10.0)
                except Exception:
                    pass
            try:
    ...
    async def _build_or_reuse_image(self) -> None:
        ...
        Raises:
            RuntimeError: If a build error message comes through the
                docker stream.
        """
        (ctx_dir, tag, _) = prepare_build_context(base_image=self.base_image, gateway_home=GATEWAY_HOME, container_workdir=CONTAINER_WORKDIR, node_version=self.node_version, extra_pip=self.extra_pip)
        self._image_tag = tag
        try:
            try:
            ...
            tar_buf.seek(0)
            stream = self._client.images.build(fileobj=tar_buf, encoding='identity', tag=tag, stream=True, rm=True)
            tail: list[str] = []
            tail_max = 200
    ...
    async def _create_and_start_container(self) -> None:
        ...
        * The gateway port stays internal — calls run from inside the
        ...
        """
        config: dict[str, Any] = {'Image': self._image_tag, 'Cmd': ['sleep', 'infinity'], 'WorkingDir': CONTAINER_WORKDIR, 'Labels': {'skpl_agent.workspace': 'true', 'skpl_agent.workspace.id': self.workspace_id}}
        if self.env:
        ...
        if self.host_workdir is not None:
            ...
            host_config['Binds'] = [f'{os.path.abspath(self.host_workdir)}:{CONTAINER_WORKDIR}:rw']
        config['HostConfig'] = host_config
        self._container = await self._client.containers.create_or_replace(name=f'as_ws_{self.workspace_id}', config=config)
