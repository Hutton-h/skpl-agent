...
_TEMPLATE_PKG = 'skpl_agent.workspace._docker'
...
def render_dockerfile(*, base_image: str=DEFAULT_BASE_IMAGE, gateway_home: str=GATEWAY_HOME, container_workdir: str=CONTAINER_WORKDIR, node_version: str | None=None, install_agentscope_block: str='') -> str:
    ...
    """
    if node_version:
        nf_raw = _read_template(_DOCKERFILE_NODE_FROM_TEMPLATE).format(node_version=node_version)
        ...
        node_from_block = ''
        node_copy_block = ''
    return _read_template(_DOCKERFILE_TEMPLATE).format(base_image=base_image, gateway_home=gateway_home, container_workdir=container_workdir, node_from_block=node_from_block, node_copy_block=node_copy_block, install_agentscope_block=install_agentscope_block.rstrip() + '\n')
...
def compute_image_tag(dockerfile_text: str, copy_files: dict[str, bytes]) -> str:
...
def prepare_build_context(*, base_image: str=DEFAULT_BASE_IMAGE, gateway_home: str=GATEWAY_HOME, container_workdir: str=CONTAINER_WORKDIR, node_version: str | None=None, extra_pip: list[str] | None=None) -> tuple[Path, str, dict[str, bytes]]:
    ...
    dockerfile_text = render_dockerfile(base_image=base_image, gateway_home=gateway_home, container_workdir=container_workdir, node_version=node_version, install_agentscope_block=install_block)
    ...
    copy_files: dict[str, bytes] = {'requirements.txt': requirements_text.encode('utf-8'), '_mcp_gateway_app.py': gateway_script_bytes, '_glob_helper.py': glob_helper_bytes}
    ...
    (ctx_dir / 'requirements.txt').write_bytes(copy_files['requirements.txt'])
