    ...
    async def exec_shell(self, command: list[str], *, cwd: str | None=None, timeout: float | None=None) -> ExecResult:
        ...
        *command* is an argv list executed via the container ``exec``
        ...
        async def _run() -> ExecResult:
            exec_obj = await self._container.exec(cmd=command, workdir=cwd or self._workdir)
            ...
            if code is None:
                code = -1
            return ExecResult(exit_code=int(code), stdout=b''.join(stdout_parts), stderr=b''.join(stderr_parts))
        if timeout is None:
            return await _run()
        try:
