...
from ._constants import DEFAULT_GATEWAY_PORT, DEFAULT_TIMEOUT, GATEWAY_HOME_NAME, METADATA_WORKSPACE_ID_KEY
...
class DaytonaWorkspace(SandboxedWorkspaceBase):
    ...
    def __init__(self, *, workspace_id: str | None=None, api_key: str='', api_url: str='', target: str='', timeout_seconds: int=DEFAULT_TIMEOUT, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, sandbox_metadata: dict[str, str] | None=None, extra_pip: list[str] | None=None, instructions: str=DEFAULT_WORKSPACE_INSTRUCTIONS, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, os_user: str | None=None) -> None:
    ...
    async def get_instructions(self) -> str:
        ...
        """
        return self.instructions.format(backend='Daytona-based', workdir=self.workdir or '<unknown>')
    ...
    async def _attach_or_create_sandbox(self) -> None:
        ...
        if existing is not None:
            ...
            return
        self._sandbox = await client.create(self._create_params(), timeout=self.timeout_seconds)
    ...
    async def _find_existing_sandbox(self) -> Any:
        ...
        client = await self._get_daytona_client()
        query = ListSandboxesQuery(labels={METADATA_WORKSPACE_ID_KEY: self.workspace_id}, states=[SandboxState.STARTED, SandboxState.STOPPED, SandboxState.STARTING, SandboxState.STOPPING, SandboxState.ERROR, SandboxState.PAUSING, SandboxState.PAUSED, SandboxState.RESUMING])
        candidates: list[Any] = []
        try:
            ...
            return None
        usable = [sandbox for sandbox in candidates if self._is_candidate_usable(sandbox)]
        if not usable:
            return None
        if len(usable) > 1:
            logger.warning('DaytonaWorkspace: %d sandboxes match workspace_id=%r; attaching to most recent', len(usable), self.workspace_id)
    ...
    def _is_candidate_usable(self, sandbox: Any) -> bool:
        ...
        return state in {'started', 'stopped', 'starting', 'stopping', 'pausing', 'paused', 'resuming'}
    ...
    async def _ensure_existing_sandbox_ready(self) -> None:
        ...
        if state == 'error':
            ...
            await self._sandbox.wait_for_sandbox_stop(timeout=self.timeout_seconds)
            await self._sandbox.start(timeout=self.timeout_seconds)
        elif state in {'starting', 'resuming'}:
            await self._sandbox.wait_for_sandbox_start(timeout=self.timeout_seconds)
    ...
    def _create_params(self) -> Any:
        ...
        labels = {METADATA_WORKSPACE_ID_KEY: self.workspace_id, **self.sandbox_metadata}
        kwargs: dict[str, Any] = {'labels': labels, 'public': False}
        if self.env:
    ...
    async def _derive_sdk_paths(self) -> None:
        ...
        self._gateway_home = posixpath.join(self._user_home, GATEWAY_HOME_NAME)
    ...
    def _bootstrap_commands(self) -> list[str]:
        ...
        return ['sudo apt-get update -qq && sudo apt-get install -y --no-install-recommends ripgrep && sudo rm -rf /var/lib/apt/lists/*', f'curl -LsSf https://astral.sh/uv/install.sh | env UV_INSTALL_DIR={shlex.quote(uv_install_dir)} INSTALLER_NO_MODIFY_PATH=1 sh', f'{shlex.quote(self._uv_bin)} venv {shlex.quote(self._gateway_venv)}', f'{shlex.quote(self._uv_bin)} pip install --python {shlex.quote(self._gateway_python)} {pip_args}', f"{shlex.quote(self._uv_bin)} pip install --python {shlex.quote(self._gateway_python)} --no-deps 'agentscope'"]
...
def _candidate_sort_key(sandbox: Any) -> str:
    ...
    return str(sandbox.last_activity_at or sandbox.updated_at or sandbox.created_at or sandbox.id)