...
class WorkspaceBase:
    ...
    def __init__(self, *, workspace_id: str | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None) -> None:
        ...
        self.skill_paths = [_normalize_local_path(path) for path in skill_paths or []]
        self._mcps = []
    ...
    def _skills_dir(self) -> str:
        ...
        return self.get_backend().join_path(self.workdir, DEFAULT_SKILLS_DIR)

    @property
    def _sessions_dir(self) -> str:
        """``${workdir}/sessions`` — per-session offload files."""
        return self.get_backend().join_path(self.workdir, DEFAULT_SESSIONS_DIR)
    ...
    def get_backend(self) -> BackendBase:
        ...
        if self._backend is None:
            raise RuntimeError(f'{type(self).__name__} has no active backend. Initialize the workspace before requesting its backend.')
    ...
    async def list_tools(self) -> list[ToolBase]:
        ...
        return [Bash(cwd=self.workdir, backend=backend), Edit(backend=backend), Glob(**glob_kwargs), Grep(backend=backend), Read(backend=backend), Write(backend=backend)]
    ...
    async def _save_mcp_file(self) -> None:
        ...
        if backend is None:
            return
        payload = json.dumps([m.model_dump(mode='json') for m in self._mcps], indent=2, ensure_ascii=False).encode('utf-8')
        try:
            await backend.write_file(self._mcp_file, payload)
        except Exception as e:
            logger.warning('Failed to save MCP file at %s: %s', self._mcp_file, e)
    ...
    async def offload_tool_result(self, session_id: str, tool_result: ToolResultBlock) -> str:
        ...
        while await backend.file_exists(path):
            path = backend.join_path(base, f'tool_result-{tool_result.id}({index}).txt')
            index += 1
        parts: list[str] = []
        if isinstance(tool_result.output, str):
            ...
            for block in tool_result.output:
                if isinstance(block, TextBlock):
                    ...
                    parts.append(f"<data url='{url}' name='{block.name}' media_type='{block.source.media_type}'/>")
    ...
    async def _offload_data_block(self, block: DataBlock) -> DataBlock:
        ...
        *base64* text rather than the decoded bytes lets a second
        ...
        if not await backend.file_exists(path):
            await backend.write_file(path, base64.b64decode(block.source.data))
        return DataBlock(id=block.id, name=block.name, source=URLSource(url=AnyUrl(self._path_to_file_uri(path)), media_type=block.source.media_type))
        ...
        for md_path in entries:
            ...
            try:
                ...
                if not name or not desc:
                    continue
                skills.append(Skill(name=str(name), description=str(desc), dir=backend.dirname(md_path), markdown=doc.content or '', updated_at=0.0))
            except Exception as e:
    ...
    async def add_skill(self, skill_path: str) -> None:
        ...
        if not os.path.isfile(skill_md):
            raise ValueError(f'Invalid skill at {skill_path!r}: SKILL.md not found')
        backend = self.get_backend()
        async with self._skill_lock:
            ...
            if await backend.file_exists(remote_dir):
                raise ValueError(f'Skill directory {dir_name!r} already exists in {self._skills_dir}')
            buf = io.BytesIO()
            ...
            result = await backend.exec_shell(['python3', '-c', _EXTRACT_TAR_SHIM, tmp_path, self._skills_dir])
            if not result.ok():
                raise RuntimeError(f"Failed to extract skill {dir_name!r}: {result.stderr.decode('utf-8', 'replace')}")
