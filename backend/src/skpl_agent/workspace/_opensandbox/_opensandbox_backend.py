...
class OpenSandboxBackend(BackendBase):
    ...
    async def exec_shell(self, command: list[str], *, cwd: str | None=None, timeout: float | None=None) -> ExecResult:
        ...
        *command* is an argv list. The OpenSandbox ``commands.run`` API
        ...
        try:
            ...
            return ExecResult(exit_code=int(getattr(res, 'exit_code', 0) or 0), stdout=self._execution_stream_bytes(res, 'stdout'), stderr=self._execution_stream_bytes(res, 'stderr'))
        except Exception as e:
            return ExecResult(exit_code=-1, stdout=b'', stderr=str(e).encode('utf-8'))
    ...
    async def read_file(self, path: str) -> bytes:
        ...
        try:
            ...
            if self._is_not_found_error(exc):
                raise FileNotFoundError(f'not found in OpenSandbox sandbox: {path}') from exc
            raise
        return data
    ...
    def _make_write_entry(path: str, data: bytes) -> Any:
        ...
        try:
            ...
            return WriteEntry(path=path, data=data, mode=420)
        except Exception:
            return _WriteEntry(path=path, data=data, mode=420)
    ...
    def _make_run_opts(cwd: str, timeout: float | None) -> Any:
        ...
        try:
            ...
            return RunCommandOpts(working_directory=cwd, timeout=timedelta(seconds=timeout) if timeout is not None else None)
        except Exception:
            return type('RunCommandOpts', (), {'working_directory': cwd, 'timeout': timedelta(seconds=timeout) if timeout is not None else None})()
    ...
    def _execution_stream_bytes(res: Any, stream: str) -> bytes:
        ...
        for part in parts:
            if text and part:
                previous_is_tight = not text.endswith(('\n', '\r')) and (not text[-1].isspace())
                next_is_tight = not part.startswith(('\n', '\r')) and (not part[0].isspace())
