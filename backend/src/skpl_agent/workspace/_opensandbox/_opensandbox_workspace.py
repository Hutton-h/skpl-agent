...
from ._constants import DEFAULT_GATEWAY_PORT, DEFAULT_IMAGE, BOOTSTRAP_COMMAND_TIMEOUT, DEFAULT_REQUEST_TIMEOUT, DEFAULT_TIMEOUT, GATEWAY_HOME, METADATA_WORKSPACE_ID_KEY, SANDBOX_WORKDIR
from ._opensandbox_backend import OpenSandboxBackend
if TYPE_CHECKING:
    from opensandbox import Sandbox
    from opensandbox.config.connection import ConnectionConfig
    from opensandbox.models.sandboxes import NetworkPolicy, SandboxInfo
...
class OpenSandboxWorkspace(SandboxedWorkspaceBase):
    ...
    def __init__(self, *, workspace_id: str | None=None, image: str=DEFAULT_IMAGE, api_key: str='', domain: str='', protocol: Literal['http', 'https']='http', request_timeout_seconds: float | None=DEFAULT_REQUEST_TIMEOUT, timeout_seconds: int=DEFAULT_TIMEOUT, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, sandbox_metadata: dict[str, str] | None=None, resource: dict[str, str] | None=None, entrypoint: list[str] | None=None, network_policy: NetworkPolicy | None=None, extra_pip: list[str] | None=None, instructions: str=DEFAULT_WORKSPACE_INSTRUCTIONS, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None) -> None:
    ...
    async def _teardown_backend(self) -> None:
        ...
        if self._sandbox is not None:
            ...
            try:
                ...
                logger.warning('OpenSandboxWorkspace: local close failed: %s', exc)
            self._sandbox = None
    ...
    async def get_instructions(self) -> str:
        ...
        """
        return self.instructions.format(backend='OpenSandbox', workdir=self.workdir)
    ...
    async def _find_existing_sandbox(self) -> SandboxInfo | None:
        ...
        sandbox_filter = SandboxFilter(states=[SandboxState.RUNNING, SandboxState.PAUSED], metadata={METADATA_WORKSPACE_ID_KEY: self.workspace_id})
        try:
            ...
            try:
                ...
                logger.warning('OpenSandboxWorkspace: manager close failed: %s', exc)
        candidates = infos.sandbox_infos
        if not candidates:
            return None
        if len(candidates) > 1:
            logger.warning('OpenSandboxWorkspace: %d sandboxes match workspace_id=%r; attaching to most recent', len(candidates), self.workspace_id)
    ...
    async def _create_sandbox(self) -> Sandbox:
        ...
        from opensandbox import Sandbox
        kwargs: dict = {'image': self.image, 'connection_config': self._connection_config(), 'metadata': {**self.sandbox_metadata, METADATA_WORKSPACE_ID_KEY: self.workspace_id}, 'timeout': timedelta(seconds=self.timeout_seconds), 'ready_timeout': timedelta(seconds=self.timeout_seconds)}
        if self.env:
            kwargs['env'] = self.env
        if self.resource:
    ...
    async def _attach_existing_sandbox(self, info: SandboxInfo) -> Sandbox:
        ...
        if state == 'paused':
            return await Sandbox.resume(sandbox_id=info.id, connection_config=self._connection_config(), resume_timeout=timedelta(seconds=self.timeout_seconds))
        if state == 'running':
            return await Sandbox.connect(sandbox_id=info.id, connection_config=self._connection_config(), connect_timeout=timedelta(seconds=self.timeout_seconds))
        raise RuntimeError(f'OpenSandbox sandbox {info.id!r} is not attachable (state={state!r})')
    ...
    async def _wait_until_running(self, timeout: float=30.0) -> None:
        ...
        while asyncio.get_event_loop().time() < deadline:
            try:
                ...
                logger.debug('OpenSandboxWorkspace: %s probe error (will retry): %s', probe_name, exc)
            await asyncio.sleep(delay)
            delay = min(delay * 1.5, 1.0)
        raise RuntimeError(f'OpenSandbox sandbox did not become ready within {timeout}s (workspace_id={self.workspace_id!r})')
