...
class LocalWorkspace(WorkspaceBase):
    ...
    def __init__(self, *, workdir: str, workspace_id: str | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, instructions: str=DEFAULT_WORKSPACE_INSTRUCTIONS) -> None:
        ...
        Args:
            ...
            instructions (`str`, defaults to             `_DEFAULT_WORKSPACE_INSTRUCTIONS`):
        ...
        self.instructions = instructions.format(backend='local', workdir=self.workdir)
        self.default_mcps: list[MCPClient] = list(default_mcps or [])
        self.skill_paths: list[str] = [_normalize_local_path(path) for path in skill_paths or []]
    ...
    async def list_tools(self) -> list[ToolBase]:
        ...
        from ..tool import Bash, Edit, Glob, Grep, PowerShell, Read, RunPython, Write
        ...
        return [
            ...
            RunPython(workspace_dir=self.workdir),
    ...
    async def initialize(self) -> None:
        ...
        if await self._backend.file_exists(mcp_file):
            ...
            for m in raw_list:
                try:
                    ...
                    logger.warning("Skipping invalid MCP entry '%s': %s", m.get('name', '?'), e)
        else:
        ...
        for mcp in self._mcps:
            if mcp.is_stateful and (not mcp.is_connected):
                try:
                    ...
                    logger.warning("Failed to connect stateful MCP '%s': %s, removing.", mcp.name, e)
                    failed.append(mcp)
        for mcp in failed:
        ...
        existing_agent_names: set[str] = {e['skill_name'] for e in existing.values()}
        ...
        for skill_path in self.skill_paths:
            ...
            if skill_hash in existing_hashes:
                logger.info("Skill '%s' (hash: %s...) already exists, skipping", raw_name, skill_hash[:8])
                continue
            # Default-skill upgrade: a skill with the same agent-facing name
            # already exists under a different hash. Remove the stale copy
            # (directory + index entry) so the updated default replaces it
            # in place instead of being seeded as a "name (1)" duplicate.
            stale_dirs = [d for d, e in existing.items() if e['skill_name'] == raw_name]
            for stale_dir in stale_dirs:
                stale_path = os.path.join(skills_dir, stale_dir)
                try:
                    if os.path.isdir(stale_path) and os.path.realpath(stale_path).startswith(os.path.realpath(skills_dir) + os.sep):
                        await asyncio.to_thread(shutil.rmtree, stale_path, True)
                except Exception as e:
                    logger.warning("Failed to remove stale skill dir %s: %s", stale_path, str(e))
                stale_entry = existing.pop(stale_dir, None)
                if stale_entry is not None:
                    existing_hashes.discard(stale_entry['hash'])
                    existing_agent_names.discard(stale_entry['skill_name'])
                    existing_dir_names.discard(stale_dir)
                updated = True
            agent_name = raw_name
            counter = 1
            ...
            if not os.path.realpath(dest_path).startswith(os.path.realpath(skills_dir) + os.sep):
                logger.warning("Skill '%s' resolves outside skills_dir, skipping", raw_name)
                continue
            try:
                await asyncio.to_thread(shutil.copytree, skill_path, dest_path, dirs_exist_ok=False)
            except Exception as e:
                logger.warning("Failed to copy skill '%s' from %s: %s", raw_name, skill_path, str(e))
                continue
            logger.info("Copied skill '%s' (agent name: '%s') from %s to %s", raw_name, agent_name, skill_path, dest_path)
        ...
        if updated:
            ...
            skills_file['skills_dir_mtime'] = mtime if mtime is not None else 0.0
    ...
    async def _load_skills_file(self, skills_dir: str) -> _SkillsFile:
        ...
        try:
            ...
            return _SkillsFile(skills_dir_mtime=float(data.get('skills_dir_mtime', 0.0)), skills=data.get('skills', {}))
        except Exception as e:
            logger.warning('Failed to load .skills from %s: %s', path, str(e))
            return {'skills_dir_mtime': 0.0, 'skills': {}}

    async def _save_skills_file(self, skills_dir: str, data: _SkillsFile) -> None:
        ...
        path = os.path.join(skills_dir, '.skills')
        try:
            await self._backend.write_file(path, json.dumps(data, indent=2, ensure_ascii=False).encode('utf-8'))
        except Exception as e:
            logger.warning('Failed to save .skills to %s: %s', path, str(e))

    async def _validate_skill(self, skill_path: str) -> tuple[str, str, str] | None:
        ...
        try:
            if not await self._backend.file_exists(skill_md_path):
                logger.warning('Invalid skill at %s: SKILL.md not found', skill_path)
                return None
            ...
            if not name or not description:
                logger.warning('Invalid skill at %s: SKILL.md missing required fields (name or description)', skill_path)
                return None
            return (str(name), str(description), content_str)
        except Exception as e:
            logger.warning('Failed to validate skill at %s: %s', skill_path, str(e))
            return None

    async def _validate_and_hash_skill(self, skill_path: str) -> tuple[str, str, str] | None:
        ...
        skill_hash = hashlib.sha256(skill_md_content.encode('utf-8')).hexdigest()
            ...
            for mcp in self._mcps:
                if mcp.is_stateful and mcp.is_connected:
                    try:
                        ...
                        logger.warning('Failed to close MCP %r when closing local workspace: %s', mcp.name, e)
        self.is_alive = False
            ...
            for mcp in self._mcps:
                if mcp.is_stateful and mcp.is_connected:
                    try:
                        ...
                        logger.warning('MCP %r close failed during reset: %s', mcp.name, e)
            self._mcps = []
            mcp_file = os.path.join(self.workdir, '.mcp')
    ...
    async def list_skills(self) -> list[Skill]:
        ...
        async with self._skill_lock:
            ...
            if current_mtime != skills_file['skills_dir_mtime']:
                skills_file = await self._reconcile_skills_dir(skills_dir, skills_file, current_mtime)
            tasks = [self._load_single_skill(os.path.join(skills_dir, dir_name), entry['skill_name']) for (dir_name, entry) in skills_file['skills'].items()]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            skills: list = []
            for (dir_name, result) in zip(skills_file['skills'], results):
                if isinstance(result, Exception):
                    logger.warning('Failed to load skill from %s: %s', dir_name, str(result))
                elif result is not None:
                    skills.append(result)
            return skills

    async def _reconcile_skills_dir(self, skills_dir: str, skills_file: _SkillsFile, current_mtime: float) -> _SkillsFile:
        ...
        for removed in indexed_dirs - actual_dirs:
            logger.info("Skill directory '%s' removed, updating index", removed)
        ...
        for new_dir in actual_dirs - indexed_dirs:
            ...
            (_, raw_name, skill_hash) = result
            if skill_hash in existing_hashes:
                logger.info("Manually added skill '%s' already tracked by hash, skipping", new_dir)
                continue
            agent_name = raw_name
            counter = 1
            while agent_name in existing_agent_names:
            ...
            existing_hashes.add(skill_hash)
            updated = True
            logger.info("Manually added skill '%s' indexed as agent name '%s'", new_dir, agent_name)
        skills_file['skills'] = existing
    ...
    async def _load_single_skill(self, skill_dir: str, skill_name: str) -> Skill | None:
        ...
        try:
            ...
            if not description:
                ...
                return None
            return Skill(name=skill_name, description=str(description), dir=skill_dir, markdown=content.content, updated_at=updated_at)
        except Exception as e:
            logger.warning('Failed to load skill from %s: %s', skill_dir, str(e))
            return None
    ...
    async def add_skill(self, skill_path: str) -> None:
        ...
        async with self._skill_lock:
            ...
            result = await self._validate_and_hash_skill(skill_path)
            if result is None:
                raise ValueError(f"Invalid skill at {skill_path!r}: missing or malformed SKILL.md (requires 'name' and 'description' fields).")
            (_, raw_name, skill_hash) = result
            skills_file = await self._load_skills_file(skills_dir)
            existing: dict[str, _SkillEntry] = skills_file['skills']
            existing_hashes: set[str] = {e['hash'] for e in existing.values()}
            ...
            dest_path = os.path.join(skills_dir, dir_name)
            if not os.path.realpath(dest_path).startswith(os.path.realpath(skills_dir) + os.sep):
                raise ValueError(f'Skill path {skill_path!r} resolves outside skills_dir.')
            await asyncio.to_thread(shutil.copytree, skill_path, dest_path, dirs_exist_ok=False)
            logger.info("Copied skill '%s' (agent name: '%s') from %s to %s", raw_name, agent_name, skill_path, dest_path)
    ...
    async def remove_skill(self, name: str) -> None:
        ...
        async with self._skill_lock:
            if not await self._backend.is_dir(skills_dir):
                logger.warning('Skills directory does not exist; cannot remove skill %r', name)
                return
            skills_file = await self._load_skills_file(skills_dir)
            existing: dict[str, _SkillEntry] = skills_file['skills']
            ...
            if await self._backend.is_dir(skill_dir_path):
                ...
                logger.info("Removed skill '%s' from %s", name, skill_dir_path)
            else:
                logger.warning('Skill directory %r not found on disk; removing index entry', skill_dir_path)
