...
from ._constants import DEFAULT_GATEWAY_PORT, DEFAULT_TEMPLATE, DEFAULT_TIMEOUT, GATEWAY_HOME, METADATA_WORKSPACE_ID_KEY, SANDBOX_WORKDIR
...
class E2BWorkspace(SandboxedWorkspaceBase):
    ...
    def __init__(self, *, workspace_id: str | None=None, template: str=DEFAULT_TEMPLATE, api_key: str='', domain: str='', timeout_seconds: int=DEFAULT_TIMEOUT, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, sandbox_metadata: dict[str, str] | None=None, extra_pip: list[str] | None=None, instructions: str=DEFAULT_WORKSPACE_INSTRUCTIONS, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None) -> None:
        ...
        self.instructions = instructions.format(backend='E2B-based', workdir=self.workdir)
    ...
    async def _attach_or_create_sandbox(self) -> None:
        ...
        if existing is not None:
            self._sandbox = await AsyncSandbox.connect(sandbox_id=existing.sandbox_id, timeout=self.timeout_seconds, **api_opts)
        else:
            merged_metadata = {METADATA_WORKSPACE_ID_KEY: self.workspace_id, **self.sandbox_metadata}
            create_kwargs: dict[str, Any] = {'template': self.template, 'timeout': self.timeout_seconds, 'metadata': merged_metadata, **api_opts}
            if self.env:
    ...
    async def _wait_until_running(self, timeout: float=30.0) -> None:
        ...
        while asyncio.get_event_loop().time() < deadline:
            try:
                ...
                logger.debug('E2BWorkspace: is_running probe error (will retry): %s', e)
            await asyncio.sleep(delay)
            delay = min(delay * 1.5, 1.0)
        raise RuntimeError(f'E2B sandbox did not become ready within {timeout}s (workspace_id={self.workspace_id!r})')
    ...
    async def _find_existing_sandbox(self) -> Any:
        ...
        query = SandboxQuery(metadata={METADATA_WORKSPACE_ID_KEY: self.workspace_id}, state=[SandboxState.PAUSED, SandboxState.RUNNING])
        candidates: list[Any] = []
        ...
        while paginator.has_next:
            try:
                ...
                logger.warning('E2BWorkspace: list sandboxes failed: %s', e)
                break
            candidates.extend(page)
        if not candidates:
            return None
        if len(candidates) > 1:
            logger.warning('E2BWorkspace: %d sandboxes match workspace_id=%r; attaching to most recent', len(candidates), self.workspace_id)
