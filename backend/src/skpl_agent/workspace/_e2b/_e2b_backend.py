    ...
    async def exec_shell(self, command: list[str], *, cwd: str | None=None, timeout: float | None=None) -> ExecResult:
        ...
        *command* is an argv list. The E2B ``commands.run`` API takes a
        ...
        try:
            ...
            return ExecResult(exit_code=int(res.exit_code or 0), stdout=(res.stdout or '').encode('utf-8'), stderr=(res.stderr or '').encode('utf-8'))
        except CommandExitException as e:
            return ExecResult(exit_code=int(e.exit_code or 1), stdout=(e.stdout or '').encode('utf-8'), stderr=(e.stderr or '').encode('utf-8'))
