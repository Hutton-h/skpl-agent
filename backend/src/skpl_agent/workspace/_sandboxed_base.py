...
from ._utils import DEFAULT_GATEWAY_LOG, DEFAULT_GATEWAY_SCRIPT, DEFAULT_GATEWAY_VENV, DEFAULT_GLOB_HELPER_SCRIPT, _read_gateway_script_bytes, _read_glob_helper_bytes
...
class SandboxedWorkspaceBase(WorkspaceBase):
    ...
    def _gateway_venv(self) -> str:
        ...
        return self.get_backend().join_path(self._gateway_home, DEFAULT_GATEWAY_VENV)

    @property
    def _gateway_python(self) -> str:
        """Sandbox-side path of the gateway venv python interpreter."""
        return self.get_backend().join_path(self._gateway_venv, 'bin', 'python')

    @property
    def _gateway_script(self) -> str:
        """Sandbox-side path of the gateway entry script."""
        return self.get_backend().join_path(self._gateway_home, DEFAULT_GATEWAY_SCRIPT)

    @property
    def _gateway_log(self) -> str:
        """Sandbox-side path of the gateway stdout/stderr log."""
        return self.get_backend().join_path(self._gateway_home, DEFAULT_GATEWAY_LOG)

    @property
    def _glob_helper_path(self) -> str:
        """Standalone glob helper script used by the builtin Glob tool."""
        return self.get_backend().join_path(self._gateway_home, DEFAULT_GLOB_HELPER_SCRIPT)

    def __init__(self, *, workspace_id: str | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None) -> None:
    ...
    async def initialize(self) -> None:
        ...
        """
        logger.info('Initialize workspace (id=%s) from %s ...', self.workspace_id, self.__class__.__name__)
        if self.is_alive:
            return
        await self._provision_backend()
        assert self._backend is not None, '_provision_backend must set self._backend before returning'
        await self._ensure_workspace_layout()
        await self._setup_mcp_gateway()
        await self._setup_skills()
        self.is_alive = True
        logger.info('Finished initializing workspace (id=%s) from %s.', self.workspace_id, self.__class__.__name__)
    ...
    async def reset(self) -> None:
        ...
        async with self._mcp_lock, self._skill_lock:
            ...
            self._mcps = []
            for path in (self._sessions_dir, self._data_dir, self._skills_dir):
    ...
    async def add_mcp(self, mcp_client: MCPClient) -> None:
        ...
        async with self._mcp_lock:
            if any((m.name == mcp_client.name for m in self._mcps)):
                raise ValueError(f'MCP {mcp_client.name!r} already exists in workspace.')
    ...
    async def _ensure_workspace_layout(self) -> None:
        ...
        backend = self.get_backend()
        await backend.exec_shell(['mkdir', '-p', self.workdir, self._data_dir, self._skills_dir, self._sessions_dir, self._gateway_home], cwd='/')
        payload = json.dumps([m.model_dump(mode='json') for m in self.default_mcps], indent=2, ensure_ascii=False).encode('utf-8')
        if await backend.file_exists(self._mcp_file):
            try:
                ...
                logger.warning('%s: %s is corrupted (%s); reseeding defaults', type(self).__name__, self._mcp_file, e)
            else:
                if isinstance(parsed, list):
                    return
                logger.warning('%s: %s does not contain a JSON list (got %s); reseeding defaults', type(self).__name__, self._mcp_file, type(parsed).__name__)
    ...
    async def _setup_mcp_gateway(self) -> None:
        ...
        if not await backend.file_exists(self._gateway_script):
            logger.info('%s: bootstrapping workspace_id=%r', type(self).__name__, self.workspace_id)
            for cmd in self._bootstrap_commands():
                r = await backend.exec_shell(['sh', '-c', cmd], timeout=self._bootstrap_cmd_timeout)
                if not r.ok():
                    raise RuntimeError(f"{type(self).__name__} bootstrap failed (exit {r.exit_code}) for: {cmd!r}\nstderr: {r.stderr.decode(errors='replace')}\nstdout: {r.stdout.decode(errors='replace')}")
        ...
        await backend.exec_shell(['sh', '-c', "pkill -f '[_]mcp_gateway_app.py' || true"])
        launch_cmd = f'nohup {shlex.quote(self._gateway_python)} -u {shlex.quote(self._gateway_script)} --config {shlex.quote(self._mcp_file)} --port {self.gateway_port} > {shlex.quote(self._gateway_log)} 2>&1 &'
        await backend.exec_shell(['sh', '-c', launch_cmd])
        self._gateway = GatewayClient(backend=backend, gateway_port=self.gateway_port, timeout=30.0, gateway_log_path=self._gateway_log)
        health_timeout = 30.0
        ...
        while asyncio.get_event_loop().time() < deadline:
            ...
            raise RuntimeError(f'gateway did not become healthy within {health_timeout}s. Tail of {self._gateway_log}:\n{tail}')
