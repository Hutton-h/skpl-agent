...
class MiddlewareBase:
    ...
    **Transformer Pattern Hook** (sequential pipeline):
    ...
    async def on_model_call(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., Awaitable['ChatResponse' | AsyncGenerator['ChatResponse', None]]]) -> 'ChatResponse' | AsyncGenerator['ChatResponse', None]:
        ...
        """
        raise RuntimeError(f'{type(self).__name__} does not implement on_model_call')

    async def on_compress_context(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., Awaitable[None]]) -> None:
