...
from ..event import DataBlockDeltaEvent, DataBlockEndEvent, DataBlockStartEvent, TextBlockDeltaEvent, TextBlockEndEvent
            ...
            async for evt in next_handler(**input_kwargs):
                ...
                if isinstance(evt, TextBlockDeltaEvent):
                    ...
                    if self.tts.realtime and evt.delta:
                        ...
                        async for audio_evt in self._emit_chunk(agent, tts_res, audio_block_id, audio_media_type):
                    ...
                    if self.tts.realtime:
                        ...
                        async for audio_evt in self._emit_synth_result(agent, res, audio_block_id, audio_media_type):
                            if isinstance(audio_evt, DataBlockStartEvent):
                                audio_block_id = audio_evt.block_id
                                audio_media_type = audio_evt.media_type
                            yield audio_evt
                    ...
                    if audio_block_id is not None:
                        yield DataBlockEndEvent(reply_id=agent.state.reply_id, block_id=audio_block_id)
                    audio_block_id = None
                    audio_media_type = None

    async def _emit_synth_result(self, agent: 'Agent', res: TTSResponse | AsyncGenerator[TTSResponse, None], audio_block_id: str | None, audio_media_type: str | None) -> AsyncGenerator:
        """Normalize ``synthesize()`` returns (single response or async
        generator) into a stream of ``DATA_BLOCK_*`` events."""
        if isinstance(res, AsyncGenerator):
            async for chunk in res:
                async for ae in self._emit_chunk(agent, chunk, audio_block_id, audio_media_type):
                    if isinstance(ae, DataBlockStartEvent):
                        audio_block_id = ae.block_id
                        audio_media_type = ae.media_type
                    yield ae
        else:
            async for ae in self._emit_chunk(agent, res, audio_block_id, audio_media_type):
                yield ae

    @staticmethod
    async def _emit_chunk(agent: 'Agent', tts_res: TTSResponse | None, audio_block_id: str | None, audio_media_type: str | None) -> AsyncGenerator:
        ...
        if audio_block_id is None:
            ...
            yield DataBlockStartEvent(reply_id=agent.state.reply_id, block_id=audio_block_id, media_type=media_type)
        yield DataBlockDeltaEvent(reply_id=agent.state.reply_id, block_id=audio_block_id, data=data, media_type=audio_media_type)