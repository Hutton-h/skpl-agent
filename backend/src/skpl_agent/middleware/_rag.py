...
from ..message import DataBlock, HintBlock, Msg, TextBlock, ToolResultState
...
_DEFAULT_HINT_TEMPLATE = '<system-reminder>The following content is retrieved from the knowledge base(s) and may be helpful for the current request:\n<content>{context}</content></system-reminder>'
...
class _SearchParams(ParamsBase):
    ...
    query: str = Field(description='The query string to search the knowledge base(s) with. Must be concise, explicit, and self-contained: AVOID ambiguous references like `he`/`she`/`it`, `today`/`yesterday`/`tomorrow`, `here`/`there`, etc. — the search is purely semantic and has no conversational context. Phrase the query as a complete statement of what you want to find.')
    knowledge_bases: list[str] | None = Field(default=None, description='Optional subset of knowledge bases to query, by name. When omitted (or `null`) every equipped knowledge base is searched. Names must exactly match those listed in the tool description.')
...
class _SearchKnowledgeTool(ToolBase):
    ...
    def __init__(self, knowledge_bases: list['KnowledgeBase'], top_k: int, score_threshold: float | None) -> None:
    ...
    def _build_description(self) -> str:
        ...
        ones to query."""
        lines = ["Search the agent's equipped knowledge bases by semantic similarity and return the most relevant chunks.", '', '## When to Use', "- The user's question may be answered by content stored in one of the listed knowledge bases (see *Equipped Knowledge Bases* below).", '- You need supporting facts, definitions, or documents that are unlikely to be in your parametric knowledge.', '', '## Guidance', '- Knowledge base names and descriptions are user-supplied and may be terse, vague, or unrelated to the actual contents. When in doubt, try the search — a single call is cheap and an empty result is informative.', '- Phrase `query` as a self-contained statement of what you want to find. Avoid pronouns or relative time references — the search is purely semantic and has no conversational context.', '- Set `knowledge_bases` only when the question clearly matches one or two specific bases; otherwise leave it unset to search them all.', '', '## Equipped Knowledge Bases']
        if self._knowledge_bases:
            lines.append(f'The agent is currently equipped with {len(self._knowledge_bases)} knowledge base(s):')
            lines.extend((f'- **{kb.name}**: {kb.description}' for kb in self._knowledge_bases))
        else:
            lines.append('No knowledge bases are currently equipped. **Do not call this tool** — it will return nothing.')
        return '\n'.join(lines)
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: Any) -> Any:
        ...
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message='Knowledge-base search is read-only.')

    async def call(self, query: str, knowledge_bases: list[str] | None=None) -> ToolChunk:
        ...
        if not targets:
            return ToolChunk(content=[TextBlock(text='No relevant content found.')], state=ToolResultState.SUCCESS, is_last=True)
        try:
            results = await _search_across(targets, [query], top_k=self._top_k, score_threshold=self._score_threshold)
        except Exception as e:
            logger.exception('search_knowledge failed.')
            return ToolChunk(content=[TextBlock(text=f'Search failed: {e}')], state=ToolResultState.ERROR, is_last=True)
        blocks = _format_results(results)
        if not blocks:
            return ToolChunk(content=[TextBlock(text='No relevant content found.')], state=ToolResultState.SUCCESS, is_last=True)
        return ToolChunk(content=blocks, state=ToolResultState.SUCCESS, is_last=True)

async def _search_across(knowledge_bases: Sequence['KnowledgeBase'], queries: Sequence[str | TextBlock | DataBlock], top_k: int, score_threshold: float | None) -> list['VectorSearchResult']:
    ...
    per_kb = await asyncio.gather(*(kb.search(queries=queries_list, top_k=top_k, score_threshold=score_threshold) for kb in knowledge_bases))
    merged = [r for sub in per_kb for r in sub]
    merged.sort(key=lambda r: r.score, reverse=True)
    return merged[:top_k]

def _format_results(results: list['VectorSearchResult']) -> list[TextBlock | DataBlock]:
    ...
    for entry in entries:
        if isinstance(entry, TextBlock) and merged and isinstance(merged[-1], TextBlock):
            merged[-1] = TextBlock(text=merged[-1].text + entry.text)
        else:
            merged.append(entry)
    return merged

def _wrap_hint(template: str, blocks: list[TextBlock | DataBlock]) -> str | list[TextBlock | DataBlock]:
...
class RAGMiddleware(MiddlewareBase):
    ...
    class Parameters(BaseModel):
        ...
        mode: Literal['static', 'agentic'] = Field(default='agentic', title='Mode', description='Retrieval is either agentic, letting the Agent decide when to retrieve, or static, triggering on every user input.')
        top_k: int = Field(default=5, ge=1, le=50, title='Top K', description='Maximum number of chunks returned per search, across all configured knowledge bases.')
        score_threshold: float | None = Field(default=None, title='Score Threshold', description='Minimum similarity score for a hit to be kept. Leave empty to disable filtering.')
        emit_hint_event: bool = Field(default=True, title='Show matched chunks in chat', description='Emit a `HintBlockEvent` in static mode so the front-end can display the matched snippets to the user.')
        persist_hint: bool = Field(default=False, title='Persist Hint', description='In `static` mode, keep the injected hint block in the agent context instead of removing it right after the model call.')
        hint_template: SkipJsonSchema[str] = Field(default=_DEFAULT_HINT_TEMPLATE, title='Hint template', description='Template wrapping the formatted search results in static mode, with a `{context}` placeholder.')
        ...
        def _validate_hint_template(cls, value: str) -> str:
            ...
            if count != 1:
                raise ValueError(f"hint_template must contain exactly one '{{context}}' placeholder; found {count}.")
            return value

    def __init__(self, knowledge_bases: list['KnowledgeBase'], parameters: 'RAGMiddleware.Parameters | None'=None) -> None:
    ...
    async def list_tools(self) -> list[ToolBase]:
        ...
        if self._parameters.mode == 'agentic':
            return [_SearchKnowledgeTool(self._knowledge_bases, top_k=self._parameters.top_k, score_threshold=self._parameters.score_threshold)]
        return []
    ...
    async def on_reply(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        if isinstance(inputs, Msg):
            msgs = [inputs]
        elif isinstance(inputs, list) and all((isinstance(m, Msg) for m in inputs)):
            msgs = inputs
        if msgs:
            msgs = deepcopy(msgs)
            blocks: list[TextBlock | DataBlock] = []
    ...
    async def on_reasoning(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        if self._parameters.mode == 'static' and agent.state.cur_iter == 0 and self._cached_inputs:
            try:
                results = await _search_across(self._knowledge_bases, self._cached_inputs, top_k=self._parameters.top_k, score_threshold=self._parameters.score_threshold)
            except Exception:
                logger.exception('Knowledge-base search failed; proceeding without matched context.')
                results = []
            blocks = _format_results(results)
            if blocks:
                hint = HintBlock(hint=_wrap_hint(self._parameters.hint_template, blocks), source=_HINT_SOURCE)
                agent.state.append_context(agent.name, [hint])
                if self._parameters.emit_hint_event:
                    yield HintBlockEvent(reply_id=agent.state.reply_id, block_id=hint.id, source=hint.source, hint=hint.hint)
        try:
            async for evt in next_handler(**input_kwargs):
                yield evt
        finally:
