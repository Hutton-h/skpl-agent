...
_DEFAULT_HINT_MESSAGE = '<system-reminder>You have reached the maximum token budget set by the user. Now you MUST wrap up immediately and provide a final concluding response without invoking any tools.</system-reminder>'
...
class ReplyBudgetControlMiddleware(MiddlewareBase):
    ...
    def __init__(self, token_budget: float, input_token_weight: float=1, output_token_weight: float=1, hint_message: str=_DEFAULT_HINT_MESSAGE) -> None:
    ...
    async def on_reply(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        async for event in next_handler(**input_kwargs):
            if isinstance(event, ReplyStartEvent):
                ...
                agent.state.middle_context[middleware_key].pop(event.reply_id, None)
            elif isinstance(event, ModelCallEndEvent):
                if middleware_key not in agent.state.middle_context:
                    agent.state.middle_context[middleware_key] = {}
                agent.state.middle_context[middleware_key][event.reply_id] += self.input_token_weight * event.input_tokens + self.output_token_weight * event.output_tokens
            yield event
    ...
    async def on_reasoning(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        if used >= self.token_budget:
            ...
            if len(agent.state.context) > 0 and agent.state.context[-1].role == 'assistant' and (agent.state.context[-1].name == agent.name):
                agent.state.context[-1].content.append(hint_block)
            else:
                agent.state.context.append(AssistantMsg(id=agent.state.reply_id, name=agent.name, content=[hint_block]))
