...
from ....message import AssistantMsg, Msg, SystemMsg, UserMsg
...
class _AsyncBridge:
    ...
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(target=self._loop.run_forever, name='mem0-agentscope-bridge', daemon=True)
        self._thread.start()
...
class AgentScopeLLM(LLMBase):
    ...
    def __init__(self, config: BaseLlmConfig | dict | None=None) -> None:
        """Initialize the AgentScope LLM for mem0."""
        super().__init__(config)
        if self.config.model is None:
            raise ValueError('AgentScopeLLM requires `model` in the config to be an AgentScope ChatModelBase instance.')
        if not isinstance(self.config.model, ChatModelBase):
            raise TypeError(f'AgentScopeLLM `model` must be a ChatModelBase, got {type(self.config.model).__name__}.')
        self._agentscope_model: ChatModelBase = self.config.model
        self._bridge = _AsyncBridge()

    def generate_response(self, messages: list[dict[str, str]], response_format: Any | None=None, tools: list[dict] | None=None, tool_choice: str='auto') -> str | dict:
        ...
        if not as_messages:
            raise ValueError('AgentScopeLLM received no usable messages (empty list or all roles unrecognized).')
        response = self._bridge.run(_await_chat(self._agentscope_model, as_messages, tools))
        return _parse_chat_response(response, has_tool=bool(tools))

async def _await_chat(model: ChatModelBase, messages: list[Msg], tools: list[dict] | None) -> 'ChatResponse':
    ...
    if isinstance(result, AsyncGenerator):
        ...
        async for chunk in result:
            last = chunk
        if last is None:
            raise RuntimeError('AgentScope streaming model yielded no chunks.')
        return last
    return result

def _convert_messages_to_agentscope(messages: list[dict[str, str]]) -> list[Msg]:
...
def _parse_chat_response(response: 'ChatResponse', has_tool: bool) -> str | dict:
    ...
    for block in response.content or []:
        ...
        if block_type == 'text':
            ...
            try:
                ...
                arguments = raw_input
            tool_parts.append({'name': block.name, 'arguments': arguments})
...
class AgentScopeEmbedding(EmbeddingBase):
    ...
    def __init__(self, config: BaseEmbedderConfig | dict | None=None) -> None:
        if isinstance(config, dict):
            config = BaseEmbedderConfig(**config)
        super().__init__(config)
        if self.config.model is None:
            raise ValueError('AgentScopeEmbedding requires `model` in the config to be an AgentScope EmbeddingModelBase instance.')
        if not isinstance(self.config.model, EmbeddingModelBase):
            raise TypeError(f'AgentScopeEmbedding `model` must be an EmbeddingModelBase, got {type(self.config.model).__name__}.')
        self._agentscope_model: EmbeddingModelBase = self.config.model
        self._bridge = _AsyncBridge()

    def embed(self, text: str | list[str], memory_action: str | None=None) -> list[float]:
        ...
        if not response.embeddings:
            raise RuntimeError('AgentScope embedding model returned no embeddings.')
        return response.embeddings[0]
_AGENTSCOPE_PROVIDER = 'agentscope'

def build_mem0_config(*, chat_model: ChatModelBase | None=None, embedding_model: EmbeddingModelBase | None=None, mem0_config: Any | None=None) -> Any:
    ...
    if mem0_config is None:
        if chat_model is None or embedding_model is None:
            raise ValueError('build_mem0_config requires `chat_model` and `embedding_model` when `mem0_config` is not given.')
        return MemoryConfig(llm=llm_cfg_cls(provider=_AGENTSCOPE_PROVIDER, config={'model': chat_model}), embedder=emb_cfg_cls(provider=_AGENTSCOPE_PROVIDER, config={'model': embedding_model}))
    if chat_model is not None:
        mem0_config.llm = llm_cfg_cls(provider=_AGENTSCOPE_PROVIDER, config={'model': chat_model})
    if embedding_model is not None:
        mem0_config.embedder = emb_cfg_cls(provider=_AGENTSCOPE_PROVIDER, config={'model': embedding_model})
    return mem0_config
...
def _register_agentscope_provider() -> None:
    ...
    LlmFactory.provider_to_class[_AGENTSCOPE_PROVIDER] = (f'{__name__}.AgentScopeLLM', BaseLlmConfig)
    EmbedderFactory.provider_to_class[_AGENTSCOPE_PROVIDER] = f'{__name__}.AgentScopeEmbedding'
