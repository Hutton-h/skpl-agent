...
class _Mem0MemoryToolBase(ToolBase):
    ...
    def __init__(self, mw: 'Mem0Middleware') -> None:
        self._mw = mw

    async def check_permissions(self, *_args: Any, **_kwargs: Any) -> PermissionDecision:
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message='auto-allowed: mem0 long-term memory tool')

class _SearchMemoryTool(_Mem0MemoryToolBase):
    """Agent-callable mem0 search tool."""
    name: str = 'search_memory'
    description: str = 'Retrieve memories based on short, targeted search keywords. Each keyword is issued as an independent query; results are merged and deduplicated.'
    ...
    async def __call__(self, keywords: list[str], limit: int=5) -> ToolChunk:
        ...
        try:
            per_keyword = await asyncio.gather(*[self._mw._async_search(kw, user_id=user_id, agent_id=search_agent_id, top_k=limit) for kw in keywords])
...
class _AddMemoryTool(_Mem0MemoryToolBase):
    ...
    name: str = 'add_memory'
    description: str = 'Record important, durable information that may be useful later. Only the provided content is persisted; thinking is retained in the tool result for auditability.'
    input_schema: dict[str, Any] = {'type': 'object', 'properties': {'thinking': {'type': 'string', 'description': 'Reasoning about why this information is worth remembering. This is not persisted to mem0.'}, 'content': {'type': 'array', 'items': {'type': 'string'}, 'description': 'Specific facts to remember. Each item should be a complete, standalone sentence.'}}, 'required': ['thinking', 'content']}
    ...
    async def __call__(self, thinking: str, content: list[str]) -> ToolChunk:
        ...
        text = '\n'.join(content)
        try:
            result = await self._mw._async_add_with_fallback(text, user_id=user_id, agent_id=agent_id)
        except Exception as e:
            return _error_chunk(f'Error recording memory: {e}')
        rationale = f' (rationale: {thinking})' if thinking else ''
        return _text_chunk(f'Successfully recorded to memory{rationale} → {result}')

def _build_memory_tools(mw: 'Mem0Middleware') -> list[ToolBase]:
    ...
    that state.
    """
    return [_SearchMemoryTool(mw), _AddMemoryTool(mw)]

def _text_chunk(message: str) -> ToolChunk:
    """Wrap a text message as a normal tool chunk."""
    return ToolChunk(content=[TextBlock(type='text', text=message)])
