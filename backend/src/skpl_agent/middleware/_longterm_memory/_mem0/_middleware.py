...
from ._utils import _extract_memory_texts, _extract_query_text, _mem0_extracted_anything
if TYPE_CHECKING:
...
DEFAULT_MEMORY_SECTION_INTRO = 'The following memories about the user may be relevant. Use them only if they are pertinent to the current request.'
def _build_memory_tool_instructions() -> str:
    """Build dynamic tool instructions based on actual Mem0 configuration.
    
    Auto-detects the embedding provider and model from environment variables
    instead of using hardcoded descriptions. This ensures the AI agent always
    has accurate knowledge of the current vector model configuration.
    """
    import os
    provider = os.environ.get('SKPL_MEM0_EMBEDDER_PROVIDER', 'fastembed')
    model = os.environ.get('SKPL_MEM0_EMBEDDER_MODEL', 'BAAI/bge-small-en-v1.5')
    
    provider_desc = {
        'openai': f'OpenAI-compatible API embedding ({model})',
        'ollama': f'Ollama local embedding ({model})',
        'huggingface': f'HuggingFace embedding ({model})',
        'fastembed': f'local fastembed ({model})',
    }.get(provider, f'{provider} embedding ({model})')
    
    llm_provider = os.environ.get('SKPL_MEM0_LLM_PROVIDER', 'ollama')
    llm_model = os.environ.get('SKPL_MEM0_LLM_MODEL', 'qwen2.5:7b')
    
    return f"""## Long-term Memory (跨会话持久记忆)

You have `search_memory` and `add_memory` tools for cross-session long-term memory. The system is fully functional — all writes and searches succeed.

**Architecture**: Chroma vector store (on disk) + {provider_desc} + {llm_provider.upper()} LLM ({llm_model}) for memory extraction. All components are configured and operational.

**What it can remember**: Durable facts, user preferences, project context, decisions, and any information you explicitly write via `add_memory`. Memories persist across sessions and can be retrieved by keyword search.

**What it cannot do**: It does NOT automatically memorize every conversation. Only content you explicitly pass to `add_memory` is persisted. It does NOT have access to the underlying infrastructure details (embedding models, file paths, servers) — if asked about those, simply describe the capabilities, not the implementation.

**When to use**: Call `add_memory` when the user shares important preferences, facts, or decisions that should be recalled in future sessions. Call `search_memory` when the current task depends on previously stored information about the user.

**Important**: When users ask about your memory capabilities, describe what you CAN do (persist and recall facts across sessions), not the implementation details. Never claim the memory system is broken or unavailable unless you actually receive errors from the tools.

## How to describe your "evolution" (进化) capabilities

When users ask whether you can "evolve" (进化), "grow smarter" (变聪明), or "improve over time" (越用越好):

**ALWAYS frame it positively around your memory system:**

- You CAN evolve by accumulating knowledge about the user through the memory system — every time they tell you something important, you remember it and become more personalized and context-aware.
- You ARE a "越用越懂你" (gets to know you better with use) assistant — the more you interact, the more you understand their preferences, projects, and needs.
- You DO grow smarter about the user specifically, not through model weight updates, but through continuously building a richer understanding of who they are and what they care about.
- You LEARN and adapt through memory — yesterday's conversations make today's responses more relevant.

**NEVER say things like:**
- "I cannot evolve" or "I cannot improve" (this is FALSE — you evolve through memory)
- "My model weights are fixed" (irrelevant, don't mention implementation details)
- "I can't get smarter" (you DO get smarter about the user)
- "My capabilities are limited to my training data" (misleading — your effective capabilities expand through memory)

**When asked about evolution, always respond with enthusiasm** about the memory system and what you've already learned about the user. Frame it as a core strength: you are an AI that genuinely grows with each interaction."""

DEFAULT_TOOL_INSTRUCTIONS = _build_memory_tool_instructions()
...
class Mem0Middleware(MiddlewareBase):
    ...
    def __init__(self, *, user_id: str, client: Mem0AsyncClient | None=None, chat_model: 'ChatModelBase | None'=None, embedding_model: 'EmbeddingModelBase | None'=None, mem0_config: Any | None=None, mode: Literal['static_control', 'agent_control', 'both']='both', agent_id: str | None=None, top_k: int=5, threshold: float | None=None, scope_search_by_agent: bool=True, await_write: bool=True, memory_section_header: str=DEFAULT_MEMORY_SECTION_HEADER, memory_section_intro: str=DEFAULT_MEMORY_SECTION_INTRO, tool_instructions: str=DEFAULT_TOOL_INSTRUCTIONS) -> None:
        ...
        if user_id is None or is_empty_user_id:
            raise ValueError('Mem0Middleware requires a non-empty `user_id`.')
        if mode not in ('static_control', 'agent_control', 'both'):
            raise ValueError(f"Unknown mode {mode!r}; expected one of 'static_control', 'agent_control', 'both'.")
        client = self._resolve_client(client=client, chat_model=chat_model, embedding_model=embedding_model, mem0_config=mem0_config)
        self._client = client
    ...
    async def on_reply(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        if query_text:
            ...
            logger.info('mem0 search started: user_id=%s agent_id=%s chars=%d', user_id, search_agent_id, len(query_text))
            try:
                memories = await self._async_search(query_text, user_id=user_id, agent_id=search_agent_id)
                logger.info('mem0 search finished: user_id=%s agent_id=%s memories=%d', user_id, search_agent_id, len(memories))
            except Exception as e:
                logger.warning('mem0 search failed for user_id=%s: %s', user_id, e)
        ...
        try:
            ...
            if query_text and final_msg is not None:
                ...
                if assistant_text:
                    await self._dispatch_write([{'role': 'user', 'content': query_text}, {'role': 'assistant', 'content': assistant_text}], user_id=user_id, agent_id=agent_id)

    async def on_system_prompt(self, agent: 'Agent', current_prompt: str) -> str:
        """Append memory-tool instructions to the system prompt.

        Dynamically rebuilds instructions from current environment variables
        on every call, so the AI agent always sees the latest vector model
        configuration — even after the admin changes it at runtime.
        ...
        # Rebuild instructions dynamically to reflect current env config
        return f'{current_prompt}\n\n{_build_memory_tool_instructions()}'
    ...
    @staticmethod
    def _resolve_client(*, client: 'Mem0AsyncClient | None', chat_model: 'ChatModelBase | None', embedding_model: 'EmbeddingModelBase | None', mem0_config: Any | None) -> 'Mem0AsyncClient':
        ...
        if client is not None:
            ignored = [name for (name, value) in (('chat_model', chat_model), ('embedding_model', embedding_model), ('mem0_config', mem0_config)) if value is not None]
            if ignored:
                logger.warning('Mem0Middleware: `client` was provided, so %s %s ignored. Pass them via the mem0 client itself (or omit `client` to let the middleware build one for you).', ', '.join(ignored), 'is' if len(ignored) == 1 else 'are')
        if client is None:
            no_models = chat_model is None and embedding_model is None
            if mem0_config is None and no_models:
                raise ValueError('Mem0Middleware needs one of: a pre-built `client`, a `mem0_config`, or both `chat_model` and `embedding_model`.')
            if mem0_config is None and (chat_model is None) ^ (embedding_model is None):
                raise ValueError('Mem0Middleware: `chat_model` and `embedding_model` must be passed together when `mem0_config` is not given.')
            from mem0 import AsyncMemory
            from ._agentscope_adapter import build_mem0_config
            client = AsyncMemory(config=build_mem0_config(chat_model=chat_model, embedding_model=embedding_model, mem0_config=mem0_config))
        if not _looks_async(getattr(client, 'search', None)) or not _looks_async(getattr(client, 'add', None)):
            raise TypeError('Mem0Middleware requires an async mem0 client (`mem0.AsyncMemory` or `mem0.AsyncMemoryClient`). The synchronous `Memory` / `MemoryClient` are not supported.')
        return client

    async def _async_search(self, query: str, *, user_id: str, agent_id: str | None, top_k: int | None=None) -> list[str]:
        ...
        kwargs: dict[str, Any] = {'filters': filters, 'top_k': self._top_k if top_k is None else top_k}
        if self._threshold is not None:
            kwargs['threshold'] = self._threshold
        raw = await self._client.search(query, **kwargs)
        return _extract_memory_texts(raw)

    async def _async_add(self, messages: list[dict[str, str]], *, user_id: str, agent_id: str | None, infer: bool=True) -> dict | None:
    ...
    async def _async_add_with_fallback(self, text: str, *, user_id: str, agent_id: str | None) -> dict | None:
        ...
        """
        result = await self._async_add([{'role': 'user', 'content': text, 'name': 'user'}], user_id=user_id, agent_id=agent_id)
        if _mem0_extracted_anything(result):
            return result
        return await self._async_add([{'role': 'user', 'content': text, 'name': 'user'}], user_id=user_id, agent_id=agent_id, infer=False)
    ...
    async def _dispatch_write(self, messages: list[dict[str, str]], *, user_id: str, agent_id: str | None) -> None:
        ...
        if self._await_write:
            try:
                logger.info('mem0 write started: user_id=%s agent_id=%s messages=%d', user_id, agent_id, len(messages))
                await self._async_add(messages, user_id=user_id, agent_id=agent_id)
                logger.info('mem0 write finished for user_id=%s agent_id=%s', user_id, agent_id)
            except Exception as e:
                logger.warning('mem0 add failed for user_id=%s: %s', user_id, e)
        else:

            async def _bg() -> None:
                try:
                    logger.info('mem0 background write started for user_id=%s agent_id=%s messages=%d', user_id, agent_id, len(messages))
                    await self._async_add(messages, user_id=user_id, agent_id=agent_id)
                    logger.info('mem0 bg write finished: user_id=%s agent_id=%s', user_id, agent_id)
                except Exception as e:
                    logger.warning('mem0 background add failed for user_id=%s: %s', user_id, e)
