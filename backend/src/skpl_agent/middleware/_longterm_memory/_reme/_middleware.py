...
_MEMORY_SECTION_INTRO = 'The following memories about the user may be relevant. Use them only if they are pertinent to the current request.'
_TOOL_INSTRUCTIONS = '## Long-term memory\n\nYou have a `memory_search` tool available. Use it whenever the current conversation may depend on a durable fact from a past session (a preference, a name, a prior decision). Recording memory is handled automatically; there is no add tool.'
...
class ReMeMiddleware(MiddlewareBase):
    ...
    class Parameters(BaseModel):
        ...
        chat_model: ChatModelBase | None = Field(default=None, title='Chat Model', description="AgentScope chat model injected into ReMe's default LLM component, fixed for the lifetime of the embedded app. When `None`, ReMe uses the LLM from its own config/credentials. Needed for `auto_memory` write-back.")
        embedding_model: EmbeddingModelBase | None = Field(default=None, title='Embedding Model', description="AgentScope embedding model injected into ReMe's default embedding component, fixed for the lifetime of the embedded app. ReMe starts this component eagerly (it is wired into the file store even when search is keyword-only), so injecting a model bypasses ReMe building its own from credentials. When `None`, ReMe uses the embedding backend from its own config/credentials.")
        mode: Literal['static_control', 'agent_control', 'both'] = Field(default='both', title='Retrieval Mode', description='How the agent retrieves from ReMe (write-back runs automatically in every mode). `static_control`: search and inject before each reply, no tool. `agent_control`: expose the `memory_search` tool, no auto-retrieval. `both`: auto-retrieve/inject and expose the tool.')
        top_k: int = Field(default=5, title='Top K', description='Max number of memories retrieved per search, and the default `limit` advertised by the `memory_search` tool.')

    def __init__(self, *, workspace_dir: str='.reme', config: str='default', parameters: Parameters | None=None) -> None:
    ...
    def _build_app(self) -> Any:
        ...
        try:
            ...
            raise ImportError('ReMeMiddleware requires the `reme-ai` package. Install it with `pip install "agentscope[memory-reme]"` (or `pip install reme-ai`).') from e
        app_kwargs: dict[str, Any] = {'config': self._config, 'workspace_dir': self._workspace_dir, 'enable_logo': False, 'log_to_console': False}
        if self._parameters.embedding_model is not None:
            app_kwargs['components'] = {'file_store': {'default': {'embedding_store': 'default'}}}
    ...
    async def _ensure_started(self) -> None:
        ...
        if not self._started:
            if self._parameters.chat_model is not None:
                await self._app.update_component(_AS_LLM_COMPONENT, _AS_DEFAULT, model=self._parameters.chat_model)
            if self._parameters.embedding_model is not None:
                await self._app.update_component(_AS_EMBEDDING_COMPONENT, _AS_DEFAULT, model=self._parameters.embedding_model)
    ...
    async def on_reply(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        if self._parameters.mode != 'agent_control' and query_text:
            self._retrieval_tasks[session_id] = asyncio.create_task(self._search(query_text))
        ...
        try:
            ...
            if task is not None:
                try:
                    ...
                    pass
            increment = [m for m in agent.state.context if isinstance(m, Msg) and m.id not in pre_ids and (getattr(m, 'name', None) != _MEMORY_MSG_NAME)]
            if query_text and any((m.role == 'assistant' and m.get_text_content() for m in increment)):
    ...
    async def _run_job(self, name: str, **kwargs: Any) -> Any:
        ...
        if getattr(response, 'success', True) is False:
            raise RuntimeError(f"ReMe {name!r} failed: {getattr(response, 'answer', '')}")
        return response

    async def _search(self, query: str, *, limit: int | None=None) -> list[str]:
        """Search ReMe and return the retrieved memory texts."""
        response = await self._run_job(_SEARCH_JOB, query=query, limit=self._parameters.top_k if limit is None else limit)
        return _extract_memory_texts(getattr(response, 'metadata', {}))

    async def _write_back(self, messages: list[Msg], session_id: str | None) -> None:
        ...
        if not session_id:
            ...
            return
        try:
            await self._run_job(_AUTO_MEMORY_JOB, messages=[m.model_dump(mode='json') for m in messages], session_id=session_id)
    ...
    def _build_memory_message(memories: list[str]) -> Msg:
        ...
        content = f'{_MEMORY_SECTION_HEADER}\n{_MEMORY_SECTION_INTRO}\n{bullets}'
        return AssistantMsg(name=_MEMORY_MSG_NAME, content=[HintBlock(hint=content)])