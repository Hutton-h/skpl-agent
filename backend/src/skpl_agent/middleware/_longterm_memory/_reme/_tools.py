    ...
    async def check_permissions(self, *_args: Any, **_kwargs: Any) -> PermissionDecision:
        ...
        Returns:
            `PermissionDecision`:
                An ``ALLOW`` decision.
        """
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message='auto-allowed: ReMe long-term memory tool')
...
class _MemorySearchTool(_ReMeMemoryToolBase):
    ...
    description: str = 'Retrieve memories from past conversations relevant to a query.'
    ...
    def __init__(self, mw: 'ReMeMiddleware') -> None:
        ...
        """
        super().__init__(mw)
        self.input_schema: dict[str, Any] = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': "What to retrieve from memory — for example a person's name, a preference, or a past decision."}, 'limit': {'type': 'integer', 'description': 'Maximum number of memories to retrieve.', 'default': mw._parameters.top_k}}, 'required': ['query']}
