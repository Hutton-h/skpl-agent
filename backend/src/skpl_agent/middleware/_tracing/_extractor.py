...
_CLASS_NAME_MAP = {'dashscope': ProviderNameValues.DASHSCOPE, 'openai': ProviderNameValues.OPENAI, 'anthropic': ProviderNameValues.ANTHROPIC, 'gemini': ProviderNameValues.GCP_GEMINI, 'ollama': ProviderNameValues.OLLAMA, 'deepseek': ProviderNameValues.DEEPSEEK, 'xai': ProviderNameValues.XAI, 'moonshot': ProviderNameValues.MOONSHOT}
_BASE_URL_PROVIDER_MAP = [('api.openai.com', ProviderNameValues.OPENAI), ('dashscope', ProviderNameValues.DASHSCOPE), ('deepseek', ProviderNameValues.DEEPSEEK), ('moonshot', ProviderNameValues.MOONSHOT), ('generativelanguage.googleapis.com', ProviderNameValues.GCP_GEMINI), ('openai.azure.com', ProviderNameValues.AZURE_AI_OPENAI), ('amazonaws.com', ProviderNameValues.AWS_BEDROCK), ('api.x.ai', ProviderNameValues.XAI)]
...
def _get_common_attributes(session_id: str='') -> Dict[str, str]:
    ...
    """
    return {SpanAttributes.GEN_AI_CONVERSATION_ID: session_id if session_id else '[no_session_id]'}
...
def _get_provider_name(instance: 'ChatModelBase') -> str:
    ...
    prefix_key = classname.removesuffix('ChatModel').removesuffix('MultiAgentModel').removesuffix('ResponseModel').lower()
...
def _get_tool_definitions(tools: list[dict[str, Any]] | None, tool_choice: 'ToolChoice | None') -> str | None:
    ...
    try:
        ...
        for tool in tools:
            ...
            flat_tool = {'type': tool.get('type', 'function'), 'name': func_def.get('name'), 'description': func_def.get('description'), 'parameters': func_def.get('parameters')}
...
def _get_llm_request_attributes(instance: 'ChatModelBase', kwargs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    attributes = {SpanAttributes.GEN_AI_OPERATION_NAME: OperationNameValues.CHAT, SpanAttributes.GEN_AI_PROVIDER_NAME: _get_provider_name(instance), SpanAttributes.GEN_AI_REQUEST_MODEL: instance.model, SpanAttributes.GEN_AI_REQUEST_TEMPERATURE: kwargs.get('temperature'), SpanAttributes.GEN_AI_REQUEST_TOP_P: kwargs.get('p') or kwargs.get('top_p'), SpanAttributes.GEN_AI_REQUEST_TOP_K: kwargs.get('top_k'), SpanAttributes.GEN_AI_REQUEST_MAX_TOKENS: kwargs.get('max_tokens'), SpanAttributes.GEN_AI_REQUEST_PRESENCE_PENALTY: kwargs.get('presence_penalty'), SpanAttributes.GEN_AI_REQUEST_FREQUENCY_PENALTY: kwargs.get('frequency_penalty'), SpanAttributes.GEN_AI_REQUEST_STOP_SEQUENCES: kwargs.get('stop_sequences'), SpanAttributes.GEN_AI_REQUEST_SEED: kwargs.get('seed')}
    tool_definitions = _get_tool_definitions(tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'))
    if tool_definitions:
...
def _get_llm_span_name(attributes: Dict[str, str]) -> str:
    ...
    """
    return f'{attributes[SpanAttributes.GEN_AI_OPERATION_NAME]} {attributes[SpanAttributes.GEN_AI_REQUEST_MODEL]}'

def _get_llm_output_messages(chat_response: ChatResponse | None) -> list[dict[str, Any]]:
    ...
    try:
        if not isinstance(chat_response, ChatResponse):
            return [{'role': 'assistant', 'parts': [{'type': 'text', 'content': str(chat_response)}], 'finish_reason': 'unknown'}]
        parts = []
        finish_reason = 'stop'
        for block in chat_response.content:
            ...
            if part:
                parts.append(part)
        output_message = {'role': 'assistant', 'parts': parts, 'finish_reason': finish_reason}
        return [output_message]
    except Exception:
        return [{'role': 'assistant', 'parts': [{'type': 'text', 'content': '<error processing response>'}], 'finish_reason': 'error'}]

def _get_llm_response_attributes(chat_response: ChatResponse | None) -> Dict[str, Any]:
    ...
    Returns:
        `Dict[str, Any]`:
            ...
            messages.
    """
    attributes = {SpanAttributes.GEN_AI_RESPONSE_ID: getattr(chat_response, 'id', 'unknown_id'), SpanAttributes.GEN_AI_RESPONSE_FINISH_REASONS: '["stop"]'}
    if hasattr(chat_response, 'usage') and chat_response.usage:
        usage = chat_response.usage
        attributes[SpanAttributes.GEN_AI_USAGE_INPUT_TOKENS] = usage.input_tokens
        attributes[SpanAttributes.GEN_AI_USAGE_OUTPUT_TOKENS] = usage.output_tokens
        cache_input = usage.cache_input_tokens
        if cache_input:
            attributes[SpanAttributes.AGENTSCOPE_CACHE_INPUT_TOKENS] = cache_input
        cache_creation = usage.cache_creation_input_tokens
        if cache_creation:
            attributes[SpanAttributes.AGENTSCOPE_CACHE_CREATION_INPUT_TOKENS] = cache_creation
    output_messages = _get_llm_output_messages(chat_response)
    if output_messages:
        attributes[SpanAttributes.GEN_AI_OUTPUT_MESSAGES] = _serialize_to_str(output_messages)
    return attributes

def _get_agent_messages(msg: Msg | list[Msg]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for m in msg:
            ...
            for block in m.get_content_blocks():
                ...
                if part:
                    parts.append(part)
            formatted_msg = {'role': m.role, 'parts': parts, 'name': m.name, 'finish_reason': 'stop'}
            formatted_msgs.append(formatted_msg)
        return formatted_msgs
    except Exception:
        try:
            single = msg[0] if isinstance(msg, list) else msg
            return [{'role': single.role, 'parts': [{'type': 'text', 'content': str(single.content) if single.content else ''}], 'name': single.name, 'finish_reason': 'stop'}]
        except Exception:
            return []

def _get_agent_request_attributes(instance: 'Agent', kwargs: Dict[str, Any]) -> Dict[str, str]:
    ...
    """
    attributes = {SpanAttributes.GEN_AI_OPERATION_NAME: OperationNameValues.INVOKE_AGENT, SpanAttributes.GEN_AI_AGENT_NAME: instance.name, SpanAttributes.GEN_AI_AGENT_DESCRIPTION: inspect.getdoc(instance.__class__) or 'No description available'}
    inputs = kwargs.get('inputs')
    if inputs is not None:
        if isinstance(inputs, (Msg, list)):
            input_messages = _get_agent_messages(inputs)
            attributes[SpanAttributes.GEN_AI_INPUT_MESSAGES] = _serialize_to_str(input_messages)
...
def _get_agent_span_name(attributes: Dict[str, str]) -> str:
    ...
    """
    return f'{attributes[SpanAttributes.GEN_AI_OPERATION_NAME]} {attributes[SpanAttributes.GEN_AI_AGENT_NAME]}'

def _get_agent_response_attributes(agent_response: Msg) -> Dict[str, str]:
    ...
    """
    attributes = {SpanAttributes.GEN_AI_OUTPUT_MESSAGES: _serialize_to_str(_get_agent_messages(agent_response))}
    return attributes

def _get_tool_request_attributes(instance: 'Toolkit', tool_call: ToolCallBlock) -> Dict[str, str]:
    ...
    Returns:
        `Dict[str, str]`:
            ...
            arguments.
    """
    attributes = {SpanAttributes.GEN_AI_OPERATION_NAME: OperationNameValues.EXECUTE_TOOL}
    if tool_call:
        ...
        if tool_name:
            ...
            if registered is not None:
                ...
                if description:
                    attributes[SpanAttributes.GEN_AI_TOOL_DESCRIPTION] = description
    return attributes
...
def _get_tool_span_name(attributes: Dict[str, str]) -> str:
    ...
    """
    return f'{attributes[SpanAttributes.GEN_AI_OPERATION_NAME]} {attributes[SpanAttributes.GEN_AI_TOOL_NAME]}'

def _get_tool_response_attributes(tool_response: Any) -> Dict[str, str]:
    ...
    """
    attributes = {SpanAttributes.GEN_AI_TOOL_CALL_RESULT: _serialize_to_str(tool_response)}
    return attributes