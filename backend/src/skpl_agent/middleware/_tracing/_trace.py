...
import json
from typing import Any, AsyncGenerator, Callable, Awaitable, Union, TypeVar, TYPE_CHECKING
import aioitertools
from opentelemetry import trace as otel_trace
from opentelemetry.trace import StatusCode
from .._base import MiddlewareBase
from ...event import ExternalExecutionResultEvent, RequireExternalExecutionEvent, RequireUserConfirmEvent, ReplyStartEvent
from ...message import Msg, ToolCallBlock
from ...model import ChatModelBase
from ._attributes import SpanAttributes, OperationNameValues
from ._extractor import _get_common_attributes, _get_agent_request_attributes, _get_agent_span_name, _get_agent_response_attributes, _get_llm_request_attributes, _get_llm_span_name, _get_llm_response_attributes, _get_tool_request_attributes, _get_tool_span_name, _get_tool_response_attributes
...
async def _trace_async_generator_wrapper(res: AsyncGenerator[T, None], span: 'Span') -> AsyncGenerator[T, None]:
...
class TracingMiddleware(MiddlewareBase):
    ...
    async def on_reply(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        tracer = _get_tracer()
        request_attributes = _get_agent_request_attributes(agent, input_kwargs)
        span_name = _get_agent_span_name(request_attributes)
        with tracer.start_as_current_span(name=span_name, attributes={**request_attributes, **common_attrs}, end_on_exit=False) as span:
            ...
            if isinstance(event_arg, ExternalExecutionResultEvent):
                for result in event_arg.execution_results:
                    tool_attrs: dict[str, Any] = {SpanAttributes.GEN_AI_OPERATION_NAME: OperationNameValues.EXECUTE_TOOL, SpanAttributes.GEN_AI_TOOL_CALL_ID: result.id, SpanAttributes.GEN_AI_TOOL_NAME: result.name, SpanAttributes.AGENTSCOPE_IS_EXTERNAL_EXECUTION: True, **common_attrs}
                    if result.output is not None:
                        tool_attrs[SpanAttributes.GEN_AI_TOOL_CALL_RESULT] = _serialize_to_str(result.output)
                    with tracer.start_as_current_span(name=f'{OperationNameValues.EXECUTE_TOOL} {result.name}', attributes=tool_attrs):
                        pass
            has_error = False
            ...
            try:
                ...
                if reply_id:
                    span.set_attribute(SpanAttributes.AGENTSCOPE_REPLY_ID, reply_id)
                if hitl_pending:
                    span.set_attribute(SpanAttributes.AGENTSCOPE_HITL_PENDING_TOOLS, json.dumps(hitl_pending, ensure_ascii=False))
                if external_pending:
                    span.set_attribute(SpanAttributes.AGENTSCOPE_EXTERNAL_EXECUTION_PENDING_TOOLS, json.dumps(external_pending, ensure_ascii=False))
                if has_error and error_exc is not None:
                    _set_span_error_status(span, error_exc)
                else:
                    if last_msg is not None:
                        span.set_attributes(_get_agent_response_attributes(last_msg))
                    _set_span_success_status(span)

    async def on_model_call(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., Awaitable['ChatResponse' | AsyncGenerator['ChatResponse', None]]]) -> Union['ChatResponse', AsyncGenerator['ChatResponse', None]]:
        if not _check_tracing_enabled():
        ...
        if not isinstance(model, ChatModelBase):
            return await next_handler(**input_kwargs)
        tracer = _get_tracer()
        combined_kwargs = {**getattr(model, 'generate_kwargs', {}), **input_kwargs}
        request_attributes = _get_llm_request_attributes(model, combined_kwargs)
        span_name = _get_llm_span_name(request_attributes)
        with tracer.start_as_current_span(name=span_name, attributes={**request_attributes, **_get_common_attributes(agent.state.session_id)}, end_on_exit=False) as span:
            try:
    ...
    async def on_acting(self, agent: 'Agent', input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        if not isinstance(tool_call, ToolCallBlock):
            ...
            return
        tracer = _get_tracer()
        request_attributes = _get_tool_request_attributes(agent.toolkit, tool_call)
            ...
            try:
                ...
                if not has_error:
                    if last_item is not None:
                        span.set_attributes(_get_tool_response_attributes(last_item))
                    _set_span_success_status(span)