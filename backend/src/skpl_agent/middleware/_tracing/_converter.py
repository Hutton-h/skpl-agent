...
from ...message import ContentBlock, TextBlock, ThinkingBlock, ToolCallBlock, ToolResultBlock, DataBlock, Base64Source, URLSource
...
def _convert_media_block(source: Base64Source | URLSource) -> Dict[str, Any] | None:
    ...
    if isinstance(source, URLSource):
        return {'type': 'uri', 'uri': str(source.url), 'modality': modality}
    if isinstance(source, Base64Source):
        return {'type': 'blob', 'content': source.data, 'media_type': source.media_type, 'modality': modality}
    return None
...
def _convert_block_to_part(block: ContentBlock) -> Dict[str, Any] | None:
    ...
    if isinstance(block, TextBlock):
        ...
        part = {'type': 'tool_call', 'id': block.id, 'name': block.name, 'arguments': arguments}
        ...
        if isinstance(output, (list, dict)):
            ...
            result = str(output)
        part = {'type': 'tool_call_response', 'id': block.id, 'response': result}
