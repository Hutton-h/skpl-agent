...
from ..deps import get_chat_service, get_current_user_id, get_message_bus, get_resource_access_service, get_session_service, get_storage, get_workspace_manager
from ._schema import CreateSessionRequest, CreateSessionResponse, InterruptSessionResponse, ListMessagesResponse, ListSessionsResponse, SessionStatusResponse, SessionView, TeamDetailResponse, TeamMemberView, UpdateSessionRequest
from ..message_bus import MessageBus, MessageBusKeys
from .._service import AgentView, ResourceAccessService, ChatService, SessionService, SessionProjection, SubagentHitlProjector
from ..storage import ChatModelConfig, SessionKnowledgeConfig, TTSModelConfig, SessionConfig, SessionRecord, StorageBase, TeamRecord
from ...message import ToolCallState
from ..storage._utils import _ensure_team_members
from ...event import CustomEvent
from ..workspace_manager import WorkspaceManagerBase

async def _build_team_detail(storage: StorageBase, user_id: str, team: TeamRecord) -> TeamDetailResponse:
    ...
    if leader_session is not None:
        leader_record = await storage.get_agent(user_id, leader_session.agent_id)
        if leader_record is not None:
            leader_agent = AgentView.model_validate({**leader_record.model_dump(), 'editable': leader_record.user_id == user_id})
    ...
    for member in await _ensure_team_members(storage, user_id, team):
        ...
        if agent is None:
            continue
        members.append(TeamMemberView(agent=AgentView.model_validate({**agent.model_dump(), 'editable': member.owner_id == user_id}), session_id=member.session_id))
    return TeamDetailResponse(team=team, leader_agent=leader_agent, members=members)
session_router = APIRouter(prefix='/sessions', tags=['sessions'], responses={404: {'description': 'Not found'}})

async def _ensure_credential_exists(access: ResourceAccessService, user_id: str, config: ChatModelConfig | TTSModelConfig | None) -> None:
    ...
    if config is None:
        return
    await access.get_resource(user_id, ResourceKind.CREDENTIAL, config.credential_id)

async def _ensure_knowledge_bases_exist(access: ResourceAccessService, user_id: str, config: SessionKnowledgeConfig | None) -> None:
...
@session_router.get('/', response_model=ListSessionsResponse, summary='List sessions for an agent')
async def list_sessions(agent_id: str=Query(description='Filter sessions by agent ID.'), user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), access: ResourceAccessService=Depends(get_resource_access_service), message_bus: MessageBus=Depends(get_message_bus)) -> ListSessionsResponse:
    ...
    for session in sessions:
        ...
        if session.team_id:
            ...
            if team_record is not None:
                team_detail = await _build_team_detail(storage, user_id, team_record)
        views.append(SessionView(session=session, is_running=await message_bus.is_locked(MessageBusKeys.session_lock(session.id)), team=team_detail))
    return ListSessionsResponse(sessions=views, total=len(views))

@session_router.post('/', response_model=CreateSessionResponse, status_code=status.HTTP_201_CREATED, summary='Create a new session')
async def create_session(body: CreateSessionRequest, user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), workspace_manager: WorkspaceManagerBase=Depends(get_workspace_manager), access: ResourceAccessService=Depends(get_resource_access_service)) -> CreateSessionResponse:
    ...
    await _ensure_credential_exists(access, user_id, body.fallback_chat_model_config)
    await _ensure_credential_exists(access, user_id, body.tts_model_config)
    await _ensure_knowledge_bases_exist(access, user_id, body.knowledge_config)
    resolved_workspace_id = body.workspace_id or workspace_manager.assign_workspace_id(user_id=user_id, agent_id=body.agent_id, session_id=_generate_id())
    session_record = await storage.upsert_session(user_id=user_id, agent_id=body.agent_id, config=SessionConfig(workspace_id=resolved_workspace_id, chat_model_config=body.chat_model_config, fallback_chat_model_config=body.fallback_chat_model_config, tts_model_config=body.tts_model_config, knowledge_config=body.knowledge_config, **{'name': body.name} if body.name is not None else {}))
    return CreateSessionResponse(session_id=session_record.id)

@session_router.delete('/{session_id}', status_code=status.HTTP_204_NO_CONTENT, summary='Delete a session')
async def delete_session(session_id: str, agent_id: str=Query(description='Agent the session belongs to.'), user_id: str=Depends(get_current_user_id), session_service: SessionService=Depends(get_session_service)) -> None:
    ...
    """
    deleted = await session_service.delete_session(user_id, agent_id, session_id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Session '{session_id}' not found.")

@session_router.post('/{session_id}/interrupt', status_code=status.HTTP_202_ACCEPTED, summary='Interrupt a running or HITL-parked chat run for a session', responses={404: {'description': 'Session not found.'}})
async def interrupt_session(session_id: str, agent_id: str=Query(description='Agent the session belongs to.'), user_id: str=Depends(get_current_user_id), chat_service: ChatService=Depends(get_chat_service)) -> InterruptSessionResponse:
...
@session_router.patch('/{session_id}', response_model=SessionRecord, summary='Update a session')
async def update_session(session_id: str, body: UpdateSessionRequest, agent_id: str=Query(description='Agent the session belongs to.'), user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), access: ResourceAccessService=Depends(get_resource_access_service)) -> SessionRecord:
    ...
    if body.permission_mode is not None:
        ...
        updated_state = existing.state.model_copy(update={'permission_context': updated_ctx})
    config_updates = body.model_dump(exclude_unset=True, exclude={'permission_mode'})
    return await storage.upsert_session(user_id=user_id, agent_id=agent_id, config=SessionConfig.model_validate({**existing.config.model_dump(mode='json'), **config_updates}), state=updated_state, session_id=session_id)

@session_router.get('/{session_id}/messages', response_model=ListMessagesResponse, summary='List messages for a session')
async def list_messages(session_id: str, agent_id: str=Query(description='Agent the session belongs to.'), before: str | None=Query(None, description='A message ID used as the pagination cursor. Omit to get the latest page. Provide a message ID from a previous response to load older messages.'), offset: int | None=Query(None, deprecated=True, description='**Deprecated.** Use ``before`` for cursor-based pagination. This parameter is always ignored.'), limit: int=Query(50, ge=1, le=200, description='Max messages.'), user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), message_bus: MessageBus=Depends(get_message_bus)) -> ListMessagesResponse:
    ...
    if offset is not None:
        extra['offset'] = offset
    (messages, has_more) = await storage.list_messages(user_id, session_id, limit=limit, before=before, **extra)
    return ListMessagesResponse(messages=messages, is_running=await message_bus.is_locked(MessageBusKeys.session_lock(session_id)), has_more=has_more)

@session_router.get('/{session_id}/status', response_model=SessionStatusResponse, summary="Probe the session's high-level status")
async def get_session_status(session_id: str, agent_id: str=Query(description='Agent the session belongs to.'), user_id: str=Depends(get_current_user_id), session_service: SessionService=Depends(get_session_service)) -> SessionStatusResponse:
    ...
    """
    session_status = await session_service.get_session_status(user_id, agent_id, session_id)
    if session_status is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Session '{session_id}' not found.")
    return SessionStatusResponse(session_id=session_id, status=session_status)
_HEARTBEAT_INTERVAL_SECS = 30

async def _worker_still_asking(storage: StorageBase, user_id: str, worker_agent_id: str, worker_session_id: str, reply_id: str) -> bool:
    ...
    if last_msg.role != 'assistant' or last_msg.id != reply_id:
        return False
    return any((tc.state in (ToolCallState.ASKING, ToolCallState.SUBMITTED) for tc in last_msg.get_content_blocks('tool_call')))

@session_router.get('/{session_id}/stream', summary="Subscribe to a session's event stream (SSE)", response_description='Server-Sent Events stream of AgentEvent objects')
async def stream_session_events(session_id: str, agent_id: str=Query(description='Agent the session belongs to.'), user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), message_bus: MessageBus=Depends(get_message_bus)) -> StreamingResponse:
    ...
    async def _sse_generator() -> AsyncGenerator[str, None]:
        try:
            for (_entry_id, event) in await message_bus.log_read(MessageBusKeys.session_events(session_id), max_count=MessageBusKeys.SESSION_REPLAY_MAX_LEN):
            ...
            for payload in await projection.list(session_id, SubagentHitlProjector.KIND):
                if not await _worker_still_asking(storage, user_id, payload['worker_agent_id'], payload['worker_session_id'], payload['reply_id']):
                    await projection.delete(session_id, SubagentHitlProjector.KIND, SubagentHitlProjector.entry_id(payload['worker_session_id'], payload['reply_id']))
                    continue
                custom = CustomEvent(name=SubagentHitlProjector.EVT_REQUIRE, value=payload)
                data = json.dumps(custom.model_dump(mode='json'), ensure_ascii=False)
                yield f'data: {data}\n\n'
            ...
            async def _feeder() -> None:
                ...
                """
                try:
                    async for evt in message_bus.subscribe(MessageBusKeys.session_events(session_id)):
                        await queue.put({k: v for (k, v) in evt.items() if k != '_entry_id'})
                except asyncio.CancelledError:
                    pass
                except Exception:
                    import logging
                    _log = logging.getLogger(__name__)
                    _log.exception("SSE feeder task unhandled exception for session %s", session_id)
                finally:
            ...
            import logging
            _log = logging.getLogger(__name__)
            _log.exception("SSE generator unhandled exception for session %s", session_id)
            error_event = json.dumps({"type": "error", "message": "Internal stream error, please reconnect"}, ensure_ascii=False)
