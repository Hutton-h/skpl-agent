...
from ...storage import EmbeddingModelConfig, KnowledgeDocumentRecord, KnowledgeDocumentStatus
...
class CreateKnowledgeBaseRequest(BaseModel):
    ...
    description: str = Field(default='', description='Free-form description shown in the UI.')
    embedding_model_config: EmbeddingModelConfig = Field(description='Embedding model used both at indexing and at query time. Cannot be changed after creation — switching would invalidate every previously inserted vector.')
    is_public: bool = Field(default=False, description='Whether this knowledge base is publicly visible to all users. Only admins can set this to True.')

class CreateKnowledgeBaseResponse(BaseModel):
    """Response body after creating a knowledge base."""
    knowledge_base_id: str = Field(description='Server-assigned knowledge base identifier.')
...
class ListKnowledgeBasesResponse(BaseModel):
    ...
    knowledge_bases: list[KnowledgeBaseView] = Field(description='All knowledge bases owned by the caller.')
...
class KnowledgeDocumentView(BaseModel):
    ...
    content_type: str | None = Field(default=None, description='IANA media type recorded at upload time, if any.')
    status: KnowledgeDocumentStatus = Field(description='Current lifecycle state of the document.')
    error: str | None = Field(default=None, description="Human-readable failure reason when ``status == 'error'``.")
    chunk_count: int = Field(default=0, description='Number of chunks indexed so far.')
    ...
    def from_record(cls, record: KnowledgeDocumentRecord) -> 'KnowledgeDocumentView':
        ...
        """
        return cls(id=record.id, filename=record.data.filename, size=record.data.size, content_type=record.data.content_type, status=record.status, error=record.data.error, chunk_count=record.data.chunk_count, created_at=record.created_at, updated_at=record.updated_at)

class ListKnowledgeDocumentsResponse(BaseModel):
    """Response body for listing documents inside a knowledge base."""
    documents: list[KnowledgeDocumentView] = Field(description='One view per registered document.')
    total: int = Field(description='Total number of returned items.')

class ListKnowledgeDocumentStatusResponse(BaseModel):
    """Response body for batch document-status polling."""
    items: list[KnowledgeDocumentView] = Field(description='Subset of the requested documents that still exist. Missing ids are silently omitted — clients may legitimately ask about a document that was deleted between two polls.')

class UploadKnowledgeDocumentResponse(BaseModel):
    """Response body after uploading a document into a knowledge base."""
    document_id: str = Field(description='Server-assigned document identifier.')
    filename: str = Field(description='The original filename of the uploaded document.')
    status: KnowledgeDocumentStatus = Field(description="Lifecycle state immediately after upload — always ``'pending'`` in the happy path; surfaced so the client can seed its progress tracker without an extra round-trip.")
...
class SearchKnowledgeBaseResponse(BaseModel):
    ...
    results: list[VectorSearchResult] = Field(description='Matched chunks ordered by descending similarity score.')
...
class KbEmbeddingProvider(BaseModel):
    ...
    """
    credential: CredentialView = Field(description='The credential record exposing these models.')
    models: list[EmbeddingModelCard] = Field(description="Embedding model cards available under this credential, filtered to those compatible with the manager's dimension policy.")
...
class ListKbEmbeddingModelsResponse(BaseModel):
    ...
    """
    providers: list[KbEmbeddingProvider] = Field(description='One entry per credential that has at least one compatible embedding model.')
    policy: DimensionPolicy = Field(description='The dimension policy used to filter the cards; surfaced verbatim so the UI can explain *why* models were filtered.')
...
class KbMiddlewareParametersSchemaResponse(BaseModel):
    ...
    """
    parameter_schema: dict = Field(description='JSON Schema produced by `RAGMiddleware.Parametersmodel_json_schema()`.  Shaped identically to the `parameter_schema` field on `ModelCard`.')
...
class ListSupportedContentTypesResponse(BaseModel):
    ...
    """
    media_types: list[str] = Field(description='Union of IANA media types every registered parser claims to handle.  Deduplicated and sorted.')
    extensions: list[str] = Field(description='Filename extensions (each starting with `.`) every registered parser claims to handle.  Deduplicated and sorted.  Derived from `mimetypes` by the base parser; subclasses may override the default.')