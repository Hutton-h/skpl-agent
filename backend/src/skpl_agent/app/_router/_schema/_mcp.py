...
class MCPBase(BaseModel):
    ...
    name: str = Field(title='MCP Name', description='The unique name to identify this MCP configuration.')
    connection_scope: ConnectionScope = Field(title='Connection Scope', description='The connection scope and lifecycle strategy.')
    mcp_config: StdioMCPConfig | HttpMCPConfig = Field(discriminator='type', title='MCP Config', description='The base MCP server configuration.')
    ...
    def validate_config(self) -> None:
        ...
        """
        if self.mcp_config.type == 'stdio_mcp' and self.connection_scope == ConnectionScope.EPHEMERAL:
            raise ValueError("STDIO MCP does not support ephemeral mode. Use 'shared' or 'isolated' instead.")
...
class MCPUpdateRequest(BaseModel):
    ...
    """
    connection_scope: ConnectionScope | None = Field(default=None, description='New connection scope.')
    mcp_config: StdioMCPConfig | HttpMCPConfig | None = Field(default=None, discriminator='type', description='New MCP server configuration.')
...
class MCPResponse(MCPBase):
    ...
    updated_at: float = Field(description='Last-updated timestamp (Unix epoch).')

class ListMCPsResponse(BaseModel):
    """Response model for listing MCP configurations."""
    mcps: list[MCPResponse] = Field(description='List of MCP configurations.')
