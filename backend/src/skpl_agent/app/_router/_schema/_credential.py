...
class CreateCredentialResponse(BaseModel):
    ...
    credential_id: str = Field(description='Server-assigned credential identifier.')
...
class ListCredentialsResponse(BaseModel):
    ...
    credentials: list[CredentialView] = Field(description='Credential records.')
...
class ListCredentialSchemasResponse(BaseModel):
    ...
    schemas: list[dict] = Field(description='JSON schemas for all registered credential types.')