...
from ..deps import get_current_user_id, get_resource_access_service, get_scheduler_manager, get_session_service, get_storage
from ._schema import CreateScheduleRequest, CreateScheduleResponse, ListSchedulesResponse, ScheduleSessionsResponse, UpdateScheduleRequest
from .._service import ResourceAccessService, SessionService
from ..storage import StorageBase, ScheduleData, ScheduleRecord, ScheduleSource
schedule_router = APIRouter(prefix='/schedule', tags=['schedule'], responses={404: {'description': 'Not found'}})

@schedule_router.get('/', response_model=ListSchedulesResponse, summary='List all schedules')
async def list_schedules(user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage)) -> ListSchedulesResponse:
...
@schedule_router.post('/', response_model=CreateScheduleResponse, status_code=status.HTTP_201_CREATED, summary='Create a new schedule')
async def create_schedule(body: CreateScheduleRequest, user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), access: ResourceAccessService=Depends(get_resource_access_service), scheduler: SchedulerManager=Depends(get_scheduler_manager)) -> CreateScheduleResponse:
    ...
    await access.get_resource(user_id, ResourceKind.CREDENTIAL, body.chat_model_config.credential_id)
    record = ScheduleRecord(user_id=user_id, agent_id=body.agent_id, data=ScheduleData(name=body.name, description=body.description, cron_expression=body.cron_expression, timezone=body.timezone, enabled=body.enabled, stateful=body.stateful, permission_mode=body.permission_mode, chat_model_config=body.chat_model_config, source=ScheduleSource.USER, started_at=datetime.now()))
...
@schedule_router.patch('/{schedule_id}', response_model=ScheduleRecord, summary='Update a schedule')
async def update_schedule(schedule_id: str, body: UpdateScheduleRequest, user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), scheduler: SchedulerManager=Depends(get_scheduler_manager)) -> ScheduleRecord:
    ...
    updated_record = existing.model_copy(update={'data': updated_data, 'updated_at': datetime.now()})
...
@schedule_router.delete('/{schedule_id}', status_code=status.HTTP_204_NO_CONTENT, summary='Delete a schedule')
async def delete_schedule(schedule_id: str, user_id: str=Depends(get_current_user_id), session_service: SessionService=Depends(get_session_service), scheduler: SchedulerManager=Depends(get_scheduler_manager)) -> None:
...
@schedule_router.get('/{schedule_id}/sessions', response_model=ScheduleSessionsResponse, summary='List execution sessions for a schedule')
async def list_schedule_sessions(schedule_id: str, user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage)) -> ScheduleSessionsResponse:
