...
from ..deps import get_chat_run_registry, get_chat_service, get_current_user_id, get_message_bus
...
@chat_router.post('/', response_model=ChatTriggerResponse, summary='Trigger a chat run (fire-and-forget)')
async def chat(request: ChatRequest, user_id: str=Depends(get_current_user_id), chat_service: ChatService=Depends(get_chat_service), chat_run_registry: ChatRunRegistry=Depends(get_chat_run_registry), message_bus: MessageBus=Depends(get_message_bus)) -> ChatTriggerResponse:
    ...
    """
    if isinstance(request.input, (UserConfirmResultEvent, ExternalExecutionResultEvent)):
        run_session_id = request.session_id
        run_agent_id = request.agent_id
        target = await SubagentHitlProjector.resolve(SessionProjection(message_bus), request.session_id, request.input.reply_id)
        if target is not None:
            run_session_id = target['worker_session_id']
            run_agent_id = target['worker_agent_id']
        await enqueue_run_trigger(message_bus, user_id=user_id, session_id=run_session_id, agent_id=run_agent_id, kind=MessageBusKeys.WAKEUP_KIND_RESUME, inputs=request.input)
        return ChatTriggerResponse(status='started', session_id=run_session_id)
    try:
        chat_run_registry.spawn(chat_service.run(user_id=user_id, session_id=request.session_id, agent_id=request.agent_id, input_msg=request.input), session_id=request.session_id)
