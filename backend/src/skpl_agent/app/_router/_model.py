...
model_router = APIRouter(prefix='/model', tags=['model'], responses={404: {'description': 'Not found'}})

@model_router.get('/', response_model=ListModelsResponse, summary='List all candidate models under the given credential type')
async def list_models(body: ListModelsRequest=Depends(), user_id: str = Depends(get_current_user_id)) -> ListModelsResponse:
    ...
    if credential_cls is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Provider '{body.provider}' not found.")
