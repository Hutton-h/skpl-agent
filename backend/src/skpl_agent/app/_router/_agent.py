...
from ..deps import get_current_user_id, get_resource_access_service, get_session_service, get_storage
from ._schema import AgentSchemaResponse, AgentSchemaV2Response, ListAgentsResponse, CreateAgentRequest, CreateAgentResponse, UpdateAgentRequest
from .._service import AgentView, ResourceAccessService, SessionService
from ..storage import StorageBase, AgentData, AgentRecord
agent_router = APIRouter(prefix='/agent', tags=['agent'], responses={404: {'description': 'Not found'}})

@agent_router.get('/schema', response_model=AgentSchemaResponse, deprecated=True, summary='[Deprecated] Legacy sectioned schema — use /schema/v2')
async def get_agent_schema() -> AgentSchemaResponse:
    ...
    identity = {'type': 'object', 'title': 'Identity', 'properties': {k: v for (k, v) in agent_schema.get('properties', {}).items() if k in identity_keys}, 'required': [r for r in agent_schema.get('required', []) if r in identity_keys]}
    context_schema = ContextConfig.model_json_schema()
    context_schema.get('properties', {}).pop('summary_schema', None)
    return AgentSchemaResponse(identity=identity, context_config=context_schema, react_config=ReActConfig.model_json_schema())

@agent_router.get('/schema/v2', response_model=AgentSchemaV2Response, summary='Full AgentData JSON Schema for the agent form')
...
@agent_router.get('/', response_model=ListAgentsResponse, summary='List all agents')
async def list_agents(user_id: str=Depends(get_current_user_id), access: ResourceAccessService=Depends(get_resource_access_service)) -> ListAgentsResponse:
...
@agent_router.post('/', response_model=CreateAgentResponse, status_code=status.HTTP_201_CREATED, summary='Create a new agent')
async def create_agent(body: CreateAgentRequest, user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage)) -> CreateAgentResponse:
    ...
    """
    try:
        data = AgentData(name=body.name, system_prompt=body.system_prompt, context_config=body.context_config, react_config=body.react_config, plan_config=body.plan_config, invite_config=body.invite_config)
...
@agent_router.patch('/{agent_id}', response_model=AgentView, summary='Update an agent')
async def update_agent(agent_id: str, body: UpdateAgentRequest, user_id: str=Depends(get_current_user_id), storage: StorageBase=Depends(get_storage), access: ResourceAccessService=Depends(get_resource_access_service)) -> AgentView:
    ...
    """
    (owner_id, existing) = await access.resolve_for_edit(user_id, ResourceKind.AGENT, agent_id)
    updates = body.model_dump(exclude_none=True)
    try:
        updated_data = AgentData.model_validate({**existing.data.model_dump(), **updates})
    except ValidationError as exc:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=exc.errors()) from exc
    updated_agent = existing.model_copy(update={'data': updated_data, 'updated_at': datetime.now()})
    await storage.upsert_agent(owner_id, updated_agent)
    return AgentView.model_validate({**updated_agent.model_dump(), 'editable': True})

@agent_router.delete('/{agent_id}', status_code=status.HTTP_204_NO_CONTENT, summary='Delete an agent')
async def delete_agent(agent_id: str, user_id: str=Depends(get_current_user_id), session_service: SessionService=Depends(get_session_service), access: ResourceAccessService=Depends(get_resource_access_service)) -> None:
    ...
    Raises:
        `HTTPException`: 404 if the agent is not visible to the caller;
            403 if visible but only readable.
    """
    (owner_id, _) = await access.resolve_for_edit(user_id, ResourceKind.AGENT, agent_id)
