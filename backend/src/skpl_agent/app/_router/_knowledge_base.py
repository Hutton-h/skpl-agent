...
from ..deps import get_current_user_id, get_knowledge_base_manager, get_knowledge_base_service, get_knowledge_parsers, get_resource_access_service
...
from .._auth.router import _get_jwt_claims
from ..rag.knowledge_base_manager import KnowledgeBaseManagerBase
from .._service import KnowledgeBaseService, KnowledgeBaseView, ResourceAccessService
from ...middleware import RAGMiddleware
from ...rag import ParserBase
knowledge_base_router = APIRouter(prefix='/knowledge_bases', tags=['knowledge_bases'], responses={404: {'description': 'Not found'}})

@knowledge_base_router.get('/embedding_models', response_model=ListKbEmbeddingModelsResponse, summary='List embedding models compatible with the KB dimension policy')
async def list_kb_embedding_models(user_id: str=Depends(get_current_user_id), access: 'ResourceAccessService'=Depends(get_resource_access_service), manager: 'KnowledgeBaseManagerBase'=Depends(get_knowledge_base_manager)) -> ListKbEmbeddingModelsResponse:
    ...
    for credential in credentials:
        ...
        if not filtered:
            continue
        providers.append(KbEmbeddingProvider(credential=credential, models=filtered))
    return ListKbEmbeddingModelsResponse(providers=providers, policy=policy)

@knowledge_base_router.get('/middleware/parameters_schema', response_model=KbMiddlewareParametersSchemaResponse, summary="JSON Schema for the KB middleware's tunable parameters")
async def get_kb_middleware_parameters_schema(_: str=Depends(get_current_user_id)) -> KbMiddlewareParametersSchemaResponse:
    ...
    """
    return KbMiddlewareParametersSchemaResponse(parameter_schema=RAGMiddleware.Parameters.model_json_schema())

@knowledge_base_router.get('/supported_content_types', response_model=ListSupportedContentTypesResponse, summary='List file types the configured parsers can ingest')
async def list_supported_content_types(_: str=Depends(get_current_user_id), parsers: list[ParserBase] | dict[str, ParserBase]=Depends(get_knowledge_parsers)) -> ListSupportedContentTypesResponse:
    ...
    return ListSupportedContentTypesResponse(media_types=sorted(media_types), extensions=sorted(extensions))

@knowledge_base_router.post('/', response_model=CreateKnowledgeBaseResponse, status_code=status.HTTP_201_CREATED, summary='Create a new knowledge base')
async def create_knowledge_base(body: CreateKnowledgeBaseRequest, user_id: str=Depends(get_current_user_id), claims=Depends(_get_jwt_claims), service: 'KnowledgeBaseService'=Depends(get_knowledge_base_service)) -> CreateKnowledgeBaseResponse:
    ...
    Args:
        ...
        user_id (`str`):
            Injected authenticated user ID.
        claims (`JWTClaims`):
            Injected JWT claims for role-based authorization.
    ...
    Raises:
        `HTTPException`:
            403 if is_public is True and the user is not an admin.
    """
    if body.is_public and claims.role != "admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admins can create public knowledge bases")
    record = await service.create_knowledge_base(user_id=user_id, name=body.name, description=body.description, embedding_model_config=body.embedding_model_config, is_public=body.is_public)
    return CreateKnowledgeBaseResponse(knowledge_base_id=record.id)

@knowledge_base_router.get('/', response_model=ListKnowledgeBasesResponse, summary="List the caller's knowledge bases")
async def list_knowledge_bases(user_id: str=Depends(get_current_user_id), access: 'ResourceAccessService'=Depends(get_resource_access_service)) -> ListKnowledgeBasesResponse:
...
async def delete_knowledge_base(knowledge_base_id: str=Path(description='The knowledge base id.'), user_id: str=Depends(get_current_user_id), service: 'KnowledgeBaseService'=Depends(get_knowledge_base_service)) -> None:
...
@knowledge_base_router.get('/{knowledge_base_id}/documents', response_model=ListKnowledgeDocumentsResponse, summary='List documents registered in a knowledge base')
async def list_knowledge_documents(knowledge_base_id: str=Path(description='The knowledge base id.'), user_id: str=Depends(get_current_user_id), service: 'KnowledgeBaseService'=Depends(get_knowledge_base_service)) -> ListKnowledgeDocumentsResponse:
    ...
    return ListKnowledgeDocumentsResponse(documents=views, total=len(views))

@knowledge_base_router.get('/{knowledge_base_id}/documents/status', response_model=ListKnowledgeDocumentStatusResponse, summary='Batch-query indexing status of one or more documents')
async def list_knowledge_document_status(knowledge_base_id: str=Path(description='The knowledge base id.'), ids: str=Query(description='Comma-separated list of document ids to query. Missing ids are silently omitted from the response.'), user_id: str=Depends(get_current_user_id), service: 'KnowledgeBaseService'=Depends(get_knowledge_base_service)) -> ListKnowledgeDocumentStatusResponse:
    ...
    records = await service.get_document_status(user_id, knowledge_base_id, document_ids)
    return ListKnowledgeDocumentStatusResponse(items=[KnowledgeDocumentView.from_record(r) for r in records])

@knowledge_base_router.post('/{knowledge_base_id}/documents', response_model=UploadKnowledgeDocumentResponse, status_code=status.HTTP_201_CREATED, summary='Upload a document into a knowledge base')
async def upload_knowledge_document(knowledge_base_id: str=Path(description='The knowledge base id.'), file: UploadFile=File(description='The document to index (PDF, TXT, Markdown, …).'), content_type: str | None=Form(default=None, description='Override the IANA media type used to route the upload. Defaults to the type guessed from the filename.'), user_id: str=Depends(get_current_user_id), service: 'KnowledgeBaseService'=Depends(get_knowledge_base_service)) -> UploadKnowledgeDocumentResponse:
    ...
    """
    record = await service.register_document(user_id=user_id, knowledge_base_id=knowledge_base_id, filename=file.filename or 'uploaded_file', stream=file.file, size=file.size or 0, content_type=content_type or file.content_type)
    return UploadKnowledgeDocumentResponse(document_id=record.id, filename=record.data.filename, status=record.status)

@knowledge_base_router.delete('/{knowledge_base_id}/documents/{document_id}', status_code=status.HTTP_204_NO_CONTENT, summary='Delete a document from a knowledge base')
async def delete_knowledge_document(knowledge_base_id: str=Path(description='The knowledge base id.'), document_id: str=Path(description='The document id.'), user_id: str=Depends(get_current_user_id), service: 'KnowledgeBaseService'=Depends(get_knowledge_base_service)) -> None:
...
@knowledge_base_router.post('/{knowledge_base_id}/search', response_model=SearchKnowledgeBaseResponse, summary='Search a knowledge base by natural-language query')
async def search_knowledge_base(body: SearchKnowledgeBaseRequest, knowledge_base_id: str=Path(description='The knowledge base id.'), user_id: str=Depends(get_current_user_id), service: 'KnowledgeBaseService'=Depends(get_knowledge_base_service)) -> SearchKnowledgeBaseResponse:
    ...
    """
    try:
        results = await service.search(user_id=user_id, knowledge_base_id=knowledge_base_id, query=body.query, top_k=body.top_k)
