...
class MessageBus(ABC):
    ...
    @abstractmethod
    async def log_append(self, key: str, payload: dict, *, ttl_secs: int | None=None, max_len: int | None=None) -> str:
    ...
    @abstractmethod
    async def subscribe(self, key: str, *, on_ready: Callable[[], None] | None=None) -> AsyncGenerator[dict, None]:
    ...
    @abstractmethod
    async def registry_set(self, namespace: str, field: str, value: str, *, ttl_secs: int | None=None) -> None:
    ...
    @deprecated('Use acquire_lock(MessageBusKeys.session_lock(sid), ...) directly.')
    @asynccontextmanager
    async def session_run(self, session_id: str) -> AsyncGenerator[None, None]:
        ...
        async with self.acquire_lock(self._SESSION_LOCK_KEY.format(sid=session_id), ttl_secs=self._SESSION_RUN_TTL_SECS):
            try:
                yield
            finally:
                await self.log_trim(self._SESSION_EVENTS_KEY.format(sid=session_id))

    @deprecated('Use is_locked(MessageBusKeys.session_lock(sid)) directly.')
    async def session_is_running(self, session_id: str) -> bool:
        """Check whether some process holds the session lock."""
        return await self.is_locked(self._SESSION_LOCK_KEY.format(sid=session_id))

    @deprecated('Use publish_session_event(bus, sid, event) from agentscope.app._bus_ops directly.')
    async def session_publish_event(self, session_id: str, event: dict) -> str:
        ...
        entry_id = await self.log_append(key, event, max_len=self._SESSION_REPLAY_MAX_LEN)
        await self.publish(key, {**event, '_entry_id': entry_id})
        return entry_id

    @deprecated('Use log_read(MessageBusKeys.session_events(sid), ...) directly.')
    async def session_read_events(self, session_id: str, since: str | None=None, max_count: int=1000) -> list[tuple[str, dict]]:
        """Read events from the session's replay log."""
        return await self.log_read(self._SESSION_EVENTS_KEY.format(sid=session_id), since=since, max_count=max_count)

    @deprecated('Use subscribe(MessageBusKeys.session_events(sid), ...) directly, stripping _entry_id from each payload.')
    async def session_subscribe_events(self, session_id: str, *, on_ready: Callable[[], None] | None=None) -> AsyncGenerator[dict, None]:
    ...
    @deprecated("Use publish(MessageBusKeys.session_cancel_channel(), {'session_id': sid}) directly.")
    async def session_publish_cancel(self, session_id: str) -> None:
        """Broadcast a session cancel request."""
        await self.publish(self._SESSION_CANCEL_KEY, {'session_id': session_id})

    @deprecated('Use subscribe(MessageBusKeys.session_cancel_channel(), ...) directly, extracting session_id from the payload.')
    async def session_subscribe_cancel(self, *, on_ready: Callable[[], None] | None=None) -> AsyncGenerator[str, None]:
        """Subscribe to session cancel broadcasts, yielding session ids."""
        async for payload in self.subscribe(self._SESSION_CANCEL_KEY, on_ready=on_ready):
            sid = payload.get('session_id')
            if isinstance(sid, str):
                yield sid

    @deprecated('Call log_trim / queue_delete / registry_drop with MessageBusKeys directly.')
    ...
    @deprecated('Use queue_push(MessageBusKeys.inbox(sid), ...) directly.')
    async def inbox_push(self, session_id: str, msg: dict, *, ttl_secs: int | None=None) -> str:
        """Push a message to a session's inbox."""
        return await self.queue_push(self._INBOX_KEY.format(sid=session_id), msg, ttl_secs=ttl_secs)

    @deprecated('Use queue_drain(MessageBusKeys.inbox(sid), ...) directly.')
    async def inbox_drain(self, session_id: str, max_count: int=100) -> list[tuple[str, dict]]:
        """Drain pending inbox messages for a session."""
        return await self.queue_drain(self._INBOX_KEY.format(sid=session_id), max_count=max_count)

    @deprecated('Use enqueue_run_trigger(bus, ...) from agentscope.app._bus_ops directly.')
    async def enqueue_wakeup(self, user_id: str, session_id: str, agent_id: str) -> None:
        """Enqueue an idle-session wake-up."""
        await self.queue_push(self._WAKEUP_QUEUE_KEY, {'user_id': user_id, 'session_id': session_id, 'agent_id': agent_id, 'kind': MessageBusKeys.WAKEUP_KIND_WAKE, 'input': None})
        await self.publish(self._WAKEUP_SIGNAL_KEY, {})

    @deprecated('Use enqueue_run_trigger(bus, ...) from agentscope.app._bus_ops directly.')
    async def enqueue_input(self, user_id: str, session_id: str, agent_id: str, *, kind: str, inputs: dict | None=None) -> None:
        """Enqueue a typed run trigger."""
        await self.queue_push(self._WAKEUP_QUEUE_KEY, {'user_id': user_id, 'session_id': session_id, 'agent_id': agent_id, 'kind': kind, 'input': inputs})
        await self.publish(self._WAKEUP_SIGNAL_KEY, {})

    @deprecated('Use queue_drain(MessageBusKeys.wakeup_queue(), ...) directly.')
    async def dequeue_wakeups(self, max_count: int=64) -> list[dict]:
        """Drain pending run-trigger entries."""
        entries = await self.queue_drain(self._WAKEUP_QUEUE_KEY, max_count=max_count)
        return [payload for (_entry_id, payload) in entries]

    @deprecated('Use subscribe(MessageBusKeys.wakeup_signal(), ...) directly.')
    async def subscribe_wakeup_signal(self, *, on_ready: Callable[[], None] | None=None) -> AsyncGenerator[dict, None]:
        """Subscribe to the shared wake-up signal channel."""
        async for payload in self.subscribe(self._WAKEUP_SIGNAL_KEY, on_ready=on_ready):
            yield payload

    @deprecated('Use registry_set(MessageBusKeys.bg_tasks(sid), ...) directly.')
    async def bg_task_register(self, session_id: str, task_id: str, metadata: str) -> None:
        """Register a background task."""
        await self.registry_set(self._BG_TASKS_KEY.format(sid=session_id), task_id, metadata, ttl_secs=self._BG_TASKS_TTL_SECS)

    @deprecated('Use registry_del(MessageBusKeys.bg_tasks(sid), tid) directly.')
    async def bg_task_unregister(self, session_id: str, task_id: str) -> None:
        """Unregister a background task."""
        await self.registry_del(self._BG_TASKS_KEY.format(sid=session_id), task_id)

    @deprecated('Use registry_exists(MessageBusKeys.bg_tasks(sid), tid) directly.')
    async def bg_task_exists(self, session_id: str, task_id: str) -> bool:
        """Check whether a background task is registered."""
        return await self.registry_exists(self._BG_TASKS_KEY.format(sid=session_id), task_id)

    @deprecated('Use registry_getall(MessageBusKeys.bg_tasks(sid)) directly.')
    async def bg_task_list(self, session_id: str) -> dict[str, str]:
        """List all background tasks for a session."""
        return await self.registry_getall(self._BG_TASKS_KEY.format(sid=session_id))

    @deprecated('Use registry_drop(MessageBusKeys.bg_tasks(sid)) directly.')
    async def bg_task_purge(self, session_id: str) -> None:
        """Delete all background task entries for a session."""
        await self.registry_drop(self._BG_TASKS_KEY.format(sid=session_id))

    @deprecated("Use publish(MessageBusKeys.task_cancel_channel(), {'task_id': tid}) directly.")
    ...
    @deprecated('Use subscribe(MessageBusKeys.task_cancel_channel(), ...) directly, extracting task_id from the payload.')
    async def task_subscribe_cancel(self, *, on_ready: Callable[[], None] | None=None) -> AsyncGenerator[str, None]:
        """Subscribe to task cancel broadcasts, yielding task ids."""
        async for payload in self.subscribe(self._TASK_CANCEL_KEY, on_ready=on_ready):
