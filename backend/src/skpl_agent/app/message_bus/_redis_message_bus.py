...
class RedisMessageBus(MessageBus):
    ...
    def __init__(self, host: str='localhost', port: int=6379, db: int=0, password: str | None=None, connection_pool: ConnectionPool | None=None, **kwargs: Any) -> None:
    ...
    async def __aenter__(self) -> Self:
        ...
        try:
            ...
            raise ImportError("The 'redis' package is required for RedisMessageBus. Install it with: pip install redis[async]") from e
    ...
    async def queue_push(self, key: str, payload: dict, *, ttl_secs: int | None=None) -> str:
        ...
        """
        entry_id = await self._client.xadd(key, {'payload': json.dumps(payload, ensure_ascii=False)})
    ...
    async def log_append(self, key: str, payload: dict, *, ttl_secs: int | None=None, max_len: int | None=None) -> str:
        ...
        entry_id = await self._client.xadd(key, {'payload': json.dumps(payload, ensure_ascii=False)}, **kwargs)
        if ttl_secs is not None:
            await self._client.expire(key, ttl_secs)
        return entry_id
    ...
    async def log_read(self, key: str, since: str | None=None, max_count: int=100) -> list[tuple[str, dict]]:
        ...
        entries = await self._client.xrange(key, min=start, count=max_count)
        results: list[tuple[str, dict]] = []
        for (entry_id, fields) in entries:
            raw = fields.get('payload')
            if raw is None:
    ...
    async def publish(self, key: str, payload: dict) -> None:
        ...
        Args:
            ...
            payload (`dict`):
                ...
                message body.
        """
        await self._client.publish(key, json.dumps(payload, ensure_ascii=False))
    ...
    async def subscribe(self, key: str, *, on_ready: Callable[[], None] | None=None) -> AsyncGenerator[dict, None]:
        ...
        try:
            ...
            while True:
                try:
                    message = await pubsub.get_message(ignore_subscribe_messages=True, timeout=self._SUBSCRIBE_POLL_TIMEOUT_SECS)
    ...
    async def acquire_lock(self, key: str, *, ttl_secs: int=600) -> AsyncGenerator[None, None]:
        ...
        async def _heartbeat() -> None:
            while True:
                ...
                await self._client.expire(key, ttl_secs)
        hb_task = asyncio.create_task(_heartbeat(), name=f'lock-heartbeat:{key}')
        try:
            yield
        finally:
            hb_task.cancel()
