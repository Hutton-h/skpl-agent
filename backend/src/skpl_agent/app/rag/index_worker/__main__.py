...
def _resolve(dotted: str) -> Callable[[], dict[str, Any]]:
    ...
    """
    if ':' not in dotted:
        raise ValueError(f"AGENTSCOPE_WORKER_BOOTSTRAP must be in 'module:attr' form, got {dotted!r}.")
...
def main() -> None:
    ...
    if not bootstrap_path:
        sys.stderr.write("AGENTSCOPE_WORKER_BOOTSTRAP is required — set it to 'package.module:callable' that returns a kwargs dict for run_worker.\n")
        sys.exit(2)
