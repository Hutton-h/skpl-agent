...
class CollectionPerKbManager(KnowledgeBaseManagerBase):
    ...
    async def create_knowledge_base(self, user_id: str, name: str, description: str, embedding_model_config: 'EmbeddingModelConfig', is_public: bool = False) -> KnowledgeBaseRecord:
        ...
        """
        record = KnowledgeBaseRecord(user_id=user_id, data=KnowledgeBaseData(name=name, description=description, embedding_model_config=embedding_model_config, collection_name=''), is_public=is_public)
        record.data.collection_name = f'kb_{record.id}'
        await self._vector_store.create_collection(name=record.data.collection_name, dimensions=embedding_model_config.dimensions)
        try:
            return await self._storage.upsert_knowledge_base(user_id, record)
        except Exception:
            try:
                await self._vector_store.delete_collection(record.data.collection_name)
            except Exception:
                logger.exception('Failed to drop orphan collection %r after upsert_knowledge_base failed; collection leaked.', record.data.collection_name)
            raise
    ...
    async def delete_knowledge_base(self, user_id: str, knowledge_base_id: str) -> bool:
        ...
        """
        record = await self._storage.get_knowledge_base(user_id, knowledge_base_id)
        if record is None:
            return False
        if await self._vector_store.has_collection(record.data.collection_name):
            await self._vector_store.delete_collection(record.data.collection_name)
        return await self._storage.delete_knowledge_base(user_id, knowledge_base_id)

    async def get_knowledge(self, user_id: str, knowledge_base_id: str) -> KnowledgeBase:
        ...
        record = await self._storage.get_knowledge_base(user_id, knowledge_base_id)
        if record is None:
            raise KnowledgeBaseNotFoundError(f'Knowledge base {knowledge_base_id!r} not found.')
        credential_record = await self._storage.get_credential(record.user_id, record.data.embedding_model_config.credential_id)
        if credential_record is None:
            raise KnowledgeBaseNotFoundError(f'Credential {record.data.embedding_model_config.credential_id!r} for knowledge base {knowledge_base_id!r} not found.')
        embedding_model = build_embedding_model(credential_record=credential_record, config=record.data.embedding_model_config)
        return KnowledgeBase(name=record.data.name, description=record.data.description, embedding_model=embedding_model, vector_store=self._vector_store, collection=record.data.collection_name, metadata_filter=None)