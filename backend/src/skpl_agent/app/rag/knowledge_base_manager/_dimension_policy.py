...
class DimensionPolicy(BaseModel):
    ...
    kind: DimensionPolicyKind = Field(description='What constraint applies to the chosen dimension.')
    'The kind of constraint.'
    dimension: int | None = Field(default=None, description='The required dimension when ``kind`` is ``FIXED`` or ``LOCKED_BY_EXISTING``.  Always ``None`` for ``ANY``.')
    ...
    def _enforce_kind_dimension_invariant(self) -> 'DimensionPolicy':
        ...
        if self.kind is DimensionPolicyKind.ANY:
            if self.dimension is not None:
                raise ValueError(f'DimensionPolicy: kind=ANY requires dimension=None, got dimension={self.dimension!r}.')
        elif self.dimension is None or self.dimension <= 0:
            raise ValueError(f'DimensionPolicy: kind={self.kind.value} requires a positive dimension, got dimension={self.dimension!r}.')
        return self
    ...
    def filter_card(self, card: 'EmbeddingModelCard') -> 'EmbeddingModelCard | None':
        ...
        if target not in card.supported_dimensions:
            return None
        return card.model_copy(update={'dimensions': target, 'supported_dimensions': [target]})