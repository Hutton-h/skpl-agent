...
class S3BlobStore(BlobStoreBase):
    ...
    def __init__(self, bucket: str, *, region_name: str | None=None, endpoint_url: str | None=None, aws_access_key_id: str | None=None, aws_secret_access_key: str | None=None, session_token: str | None=None, use_ssl: bool=True, config: Any=None) -> None:
        ...
        try:
            ...
            raise ImportError('S3BlobStore requires the optional dependency ``aioboto3``. Install it with ``uv pip install aioboto3`` or with the ``[s3]`` extra.') from e
        self._bucket = bucket
    ...
    def _client(self) -> Any:
        ...
        if self._session is None:
            raise RuntimeError('S3BlobStore is not entered; use it inside ``async with`` before calling blob methods.')
        return self._session.client('s3', region_name=self._region_name, endpoint_url=self._endpoint_url, aws_access_key_id=self._aws_access_key_id, aws_secret_access_key=self._aws_secret_access_key, aws_session_token=self._session_token, use_ssl=self._use_ssl, config=self._config)
    ...
    def _key_from_uri(cls, uri: str, expected_bucket: str) -> str:
        ...
        if bucket != expected_bucket:
            raise ValueError(f'Bucket {bucket!r} in URI {uri!r} does not match configured bucket {expected_bucket!r}.')
        return key
