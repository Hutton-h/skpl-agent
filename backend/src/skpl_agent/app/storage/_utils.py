...
async def _ensure_team_members(storage: 'StorageBase', user_id: str, team: 'TeamRecord') -> list[TeamMember]:
    ...
    for agent_id in team.data.member_ids:
        ...
        if not sessions:
            continue
        migrated.append(TeamMember(owner_id=user_id, agent_id=agent_id, session_id=sessions[0].id, role='created'))
