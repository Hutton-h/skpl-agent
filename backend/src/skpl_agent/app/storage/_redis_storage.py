...
class RedisStorage(StorageBase):
    ...
    class KeyConfig(BaseModel):
        ...
        schedule_session_index: str = 'agentscope:user:{user_id}:schedule:{schedule_id}:sessions'
    ...
    def __init__(self, host: str='localhost', port: int=6379, db: int=0, password: str | None=None, connection_pool: ConnectionPool | None=None, key_ttl: int | None=None, key_config: 'RedisStorage.KeyConfig | None'=None, **kwargs: Any) -> None:
    ...
    async def __aenter__(self) -> Self:
        ...
        try:
            ...
            raise ImportError("The 'redis' package is required for RedisStorage. Install it with: pip install redis[async]") from e
        if self._external_pool is not None:
            pool = self._external_pool
        else:
            self._owned_pool = aioredis.ConnectionPool(host=self._host, port=self._port, db=self._db, password=self._password, decode_responses=True, **self._kwargs)
    ...
    async def upsert_credential(self, user_id: str, credential_data: CredentialBase) -> str:
        ...
        if credential_data.id:
            key = self._key(self.key_config.credential, user_id=user_id, credential_id=credential_data.id)
        ...
        key = self._key(self.key_config.credential, user_id=user_id, credential_id=record.id)
        index_key = self._key(self.key_config.credential_index, user_id=user_id)
    ...
    async def list_credentials(self, user_id: str) -> list[CredentialRecord]:
        ...
        records = []
        for cred_id in ids:
            raw = await self._client.get(self._key(self.key_config.credential, user_id=user_id, credential_id=cred_id))
            if raw:
    ...
    async def get_credential(self, user_id: str, credential_id: str) -> CredentialRecord | None:
        ...
        key = self._key(self.key_config.credential, user_id=user_id, credential_id=credential_id)
        raw = await self._client.get(key)
    ...
    async def upsert_agent(self, user_id: str, agent_record: AgentRecord) -> str:
        ...
        """
        key = self._key(self.key_config.agent, user_id=user_id, agent_id=agent_record.id)
    ...
    async def list_agents(self, user_id: str) -> list[AgentRecord]:
        ...
        records = []
        for agent_id in ids:
            raw = await self._client.get(self._key(self.key_config.agent, user_id=user_id, agent_id=agent_id))
            if raw:
    ...
    async def get_agent(self, user_id: str, agent_id: str) -> AgentRecord | None:
        ...
        key = self._key(self.key_config.agent, user_id=user_id, agent_id=agent_id)
        raw = await self._client.get(key)
    ...
    async def delete_agent(self, user_id: str, agent_id: str) -> bool:
        ...
        for team in teams:
            ...
            if agent_id in team.data.member_ids:
                team.data.member_ids = [mid for mid in team.data.member_ids if mid != agent_id]
                dirty = True
            filtered_members = [m for m in team.data.members if m.agent_id != agent_id]
    ...
    async def upsert_session(self, user_id: str, agent_id: str, config: SessionConfig, state: AgentState | None=None, session_id: str | None=None, source: SessionSource=SessionSource.USER, source_schedule_id: str | None=None) -> SessionRecord:
        ...
        """
        if session_id:
            key = self._key(self.key_config.session, user_id=user_id, session_id=session_id)
            raw = await self._client.get(key)
            if raw:
        ...
        key = self._key(self.key_config.session, user_id=user_id, session_id=record.id)
        index_key = self._key(self.key_config.session_index, user_id=user_id, agent_id=agent_id)
        await self._set_with_ttl(key, record.model_dump_json())
        await self._client.sadd(index_key, record.id)
        if source_schedule_id:
            schedule_session_key = self._key(self.key_config.schedule_session_index, user_id=user_id, schedule_id=source_schedule_id)
    ...
    async def list_sessions(self, user_id: str, agent_id: str) -> list[SessionRecord]:
        ...
        for session_id in ids:
            raw = await self._client.get(self._key(self.key_config.session, user_id=user_id, session_id=session_id))
            if raw:
    ...
    async def delete_session(self, user_id: str, agent_id: str, session_id: str) -> bool:
        ...
        index_key = self._key(self.key_config.session_index, user_id=user_id, agent_id=agent_id)
        msg_key = self._key(self.key_config.messages, user_id=user_id, session_id=session_id)
        await self._client.delete(key)
        await self._client.srem(index_key, session_id)
        await self._client.delete(msg_key)
        if record.source_schedule_id:
            schedule_session_key = self._key(self.key_config.schedule_session_index, user_id=user_id, schedule_id=record.source_schedule_id)
    ...
    async def list_sessions_by_schedule(self, user_id: str, schedule_id: str) -> list[SessionRecord]:
        ...
        schedule_session_key = self._key(self.key_config.schedule_session_index, user_id=user_id, schedule_id=schedule_id)
    ...
    async def upsert_schedule(self, user_id: str, record: ScheduleRecord) -> str:
        ...
        indexes."""
        key = self._key(self.key_config.schedule, user_id=user_id, schedule_id=record.id)
        index_key = self._key(self.key_config.schedule_index, user_id=user_id)
        await self._set_with_ttl(key, record.model_dump_json())
        await self._client.sadd(index_key, record.id)
        await self._client.sadd(self.key_config.schedule_global_index, f'{user_id}:{record.id}')
        return record.id

    async def get_schedule(self, user_id: str, schedule_id: str) -> ScheduleRecord | None:
        """Fetch a single cron task record by id."""
        key = self._key(self.key_config.schedule, user_id=user_id, schedule_id=schedule_id)
        raw = await self._client.get(key)
        if not raw:
            return None
    ...
    async def list_schedules(self, user_id: str) -> list[ScheduleRecord]:
        ...
        for schedule_id in ids:
            raw = await self._client.get(self._key(self.key_config.schedule, user_id=user_id, schedule_id=schedule_id))
            if raw:
    ...
    async def delete_schedule(self, user_id: str, schedule_id: str) -> bool:
        ...
        await self._client.srem(self.key_config.schedule_global_index, f'{user_id}:{schedule_id}')
        return True
    ...
    async def list_all_schedules(self) -> list[ScheduleRecord]:
        ...
        """
        entries = await self._client.smembers(self.key_config.schedule_global_index)
        records = []
    ...
    def _message_key(self, user_id: str, session_id: str) -> str:
        ...
        return self._key(self.key_config.messages, user_id=user_id, session_id=session_id)
    ...
    async def _find_message_index(self, key: str, message_id: str, chunk_size: int=100) -> int | None:
    ...
    async def upsert_team(self, user_id: str, record: TeamRecord) -> TeamRecord:
        ...
        """
        record.updated_at = datetime.now()
        key = self._key(self.key_config.team, user_id=user_id, team_id=record.id)
    ...
    async def get_team(self, user_id: str, team_id: str) -> TeamRecord | None:
        ...
        """
        key = self._key(self.key_config.team, user_id=user_id, team_id=team_id)
        raw = await self._client.get(key)
        if not raw:
            return None
    ...
    async def list_teams(self, user_id: str) -> list[TeamRecord]:
        ...
        for team_id in ids:
            raw = await self._client.get(self._key(self.key_config.team, user_id=user_id, team_id=team_id))
            if raw:
    ...
    async def delete_team(self, user_id: str, team_id: str) -> bool:
        ...
        for member in members:
            if member.role == 'created':
                ...
                await self.delete_session(member.owner_id, member.agent_id, member.session_id)
    ...
    async def upsert_knowledge_base(self, user_id: str, record: KnowledgeBaseRecord) -> KnowledgeBaseRecord:
        ...
        key = self._key(self.key_config.knowledge_base, user_id=user_id, knowledge_base_id=record.id)
    ...
    async def delete_knowledge_base(self, user_id: str, knowledge_base_id: str) -> bool:
        ...
        Returns:
            `bool`:
                ...
                ``False`` if not found.
        """
        documents = await self.list_knowledge_documents(user_id, knowledge_base_id)
    ...
    def _document_key(self, user_id: str, knowledge_base_id: str, document_id: str) -> str:
        ...
        return self._key(self.key_config.knowledge_document, user_id=user_id, knowledge_base_id=knowledge_base_id, document_id=document_id)

    def _document_index_key(self, user_id: str, knowledge_base_id: str) -> str:
        """Format the per-KB document index Set key."""
        return self._key(self.key_config.knowledge_document_index, user_id=user_id, knowledge_base_id=knowledge_base_id)

    def _document_global_token(self, user_id: str, knowledge_base_id: str, document_id: str) -> str:
    ...
    async def upsert_knowledge_document(self, user_id: str, record: KnowledgeDocumentRecord) -> KnowledgeDocumentRecord:
        ...
        if record.user_id != user_id:
            raise ValueError('record.user_id does not match the given user_id.')
        key = self._document_key(user_id, record.knowledge_base_id, record.id)
        existing_raw = await self._client.get(key)
        if existing_raw:
            existing = KnowledgeDocumentRecord.model_validate_json(existing_raw)
            record.created_at = existing.created_at
        record.updated_at = datetime.now()
        await self._set_with_ttl(key, record.model_dump_json())
        await self._client.sadd(self._document_index_key(user_id, record.knowledge_base_id), record.id)
        await self._client.sadd(self.key_config.knowledge_document_global_index, self._document_global_token(user_id, record.knowledge_base_id, record.id))
        return record
    ...
    async def get_knowledge_document(self, user_id: str, knowledge_base_id: str, document_id: str) -> KnowledgeDocumentRecord | None:
        ...
        raw = await self._client.get(self._document_key(user_id, knowledge_base_id, document_id))
        return KnowledgeDocumentRecord.model_validate_json(raw) if raw else None
    ...
    async def list_knowledge_documents(self, user_id: str, knowledge_base_id: str) -> list[KnowledgeDocumentRecord]:
        ...
        silently skipped.
        """
        ids = await self._client.smembers(self._document_index_key(user_id, knowledge_base_id))
        records: list[KnowledgeDocumentRecord] = []
        for document_id in ids:
            raw = await self._client.get(self._document_key(user_id, knowledge_base_id, document_id))
            if raw:
                records.append(KnowledgeDocumentRecord.model_validate_json(raw))
        return records
    ...
    async def delete_knowledge_document(self, user_id: str, knowledge_base_id: str, document_id: str) -> bool:
        ...
        deleted = await self._client.delete(key)
        await self._client.srem(self._document_index_key(user_id, knowledge_base_id), document_id)
        await self._client.srem(self.key_config.knowledge_document_global_index, self._document_global_token(user_id, knowledge_base_id, document_id))
        return deleted > 0
        ...
        async with self._client.pipeline(transaction=True) as pipe:
            for _ in range(3):
                try:
                    ...
                    if holder is not None and deadline is not None and (deadline > now):
                        await pipe.unwatch()
                        return False
    ...
    async def list_knowledge_documents_with_expired_lease(self, now: datetime | None=None) -> list[KnowledgeDocumentRecord]:
        ...
        tokens = await self._client.smembers(self.key_config.knowledge_document_global_index)
        records: list[KnowledgeDocumentRecord] = []
        for token in tokens:
            try:
                ...
                continue
            raw = await self._client.get(self._document_key(user_id, kb_id, document_id))
            if not raw:
                continue
            record = KnowledgeDocumentRecord.model_validate_json(raw)
