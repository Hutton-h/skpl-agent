...
KnowledgeDocumentStatus = Literal['pending', 'parsing', 'chunking', 'indexing', 'ready', 'error']
...
class KnowledgeDocumentData(BaseModel):
    ...
    """
    filename: str = Field(description='Original filename supplied by the uploader.')
    'The original filename — used both for citation and as the\n    ``source`` field on every chunk produced from this document.'
    size: int = Field(ge=0, description='Document size in bytes as observed at upload time.')
    'Byte length recorded at upload time.  Used by quota checks and\n    the UI; not authoritative for parsing (the worker reopens the\n    blob).'
    content_type: str | None = Field(default=None, description='IANA media type used to route the upload to a parser.')
    'IANA media type.  ``None`` lets the worker fall back to\n    ``mimetypes.guess_type(filename)``.'
    blob_uri: str = Field(description='URI returned by the blob store after the upload was streamed in (e.g. ``local://kb/.../uuid``).')
    ...
    error: str | None = Field(default=None, description="Human-readable failure reason when ``status == 'error'``. MUST NOT include stack traces or filesystem paths — it is rendered verbatim in the UI.")
    'Human-readable failure reason.  Surfaced directly to the user;\n    keep it short and free of sensitive content.'
    chunk_count: int = Field(default=0, ge=0, description='Number of chunks successfully indexed.')
...
class KnowledgeDocumentRecord(_RecordBase):
    ...
    knowledge_base_id: str = Field(description='The id of the knowledge base this document belongs to.')
    'The knowledge base the document is being indexed into.'
    processing_node: str | None = Field(default=None, description='Identifier of the worker process that currently holds the lease on this document.  ``None`` if no worker is processing it.')
    ...
    status: KnowledgeDocumentStatus = Field(default='pending', description='Current lifecycle state.')
    ...
    lease_expires_at: datetime | None = Field(default=None, description='Wall-clock deadline for the worker that currently holds this document.  ``None`` if no worker is processing it.')
