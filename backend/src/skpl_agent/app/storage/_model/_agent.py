...
class InviteConfig(BaseModel):
    ...
    """
    invitable: bool = Field(default=False, description="Whether this agent may be borrowed into another agent's team via the ``AgentInvite`` tool. Independent from :attr:`invite_description` so the user can preserve an authored blurb while temporarily disabling the toggle. ``invitable=True`` requires a non-empty :attr:`invite_description` (enforced by validator).", title='Invitable')
    invite_description: str | None = Field(default=None, description="Free-text blurb shown to a leader LLM in the ``AgentInvite`` tool description — used by the leader to decide whether to borrow this agent. Persisted across toggle off/on so the user's authored draft is not lost when :attr:`invitable` is temporarily disabled.", title='Invite Description', json_schema_extra={'format': 'textarea'})
    ...
    def _check_invitable_has_description(self) -> Self:
        ...
        if self.invitable and (not (self.invite_description or '').strip()):
            raise ValueError('invite_description must be non-empty when invitable=True')
        return self

class AgentData(BaseModel):
    """The agent data model."""
    id: SkipJsonSchema[str] = Field(description='Unique agent id', default_factory=_generate_id)
    ...
    system_prompt: str = Field(default="You're a helpful assistant.", description='The system prompt for the agent.', title='System Prompt', json_schema_extra={'format': 'textarea'})
    context_config: ContextConfig = Field(description='The context config for the agent.', title='Context Config')
    react_config: ReActConfig = Field(description='The react config for the agent.', title='React Config')
    plan_config: PlanConfig = Field(default_factory=PlanConfig, description='The planning phase config for the agent.', title='Plan Config')
    invite_config: InviteConfig = Field(default_factory=InviteConfig, description='The invite config for the agent.', title='Invite Config')
