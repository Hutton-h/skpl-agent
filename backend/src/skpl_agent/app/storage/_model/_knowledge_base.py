...
class KnowledgeBaseData(BaseModel):
    ...
    description: str = Field(default='', description='Free-form description of the knowledge base purpose.')
    'Free-form description shown in the UI.'
    embedding_model_config: EmbeddingModelConfig = Field(description='Embedding model configuration pinned at creation time. Cannot change for the lifetime of the record because the underlying collection is sized to its dimension.')
    'Embedding model configuration pinned at creation time.'
    collection_name: str = Field(description='The vector store collection that physically backs this knowledge base. Generated server-side; opaque to clients.')
...
class KnowledgeBaseRecord(_RecordBase):
    ...
    def _nest_legacy_flat_payload(cls, obj: Any) -> Any:
        ...
        if not isinstance(obj, dict) or 'data' in obj:
            return obj
        legacy_keys = ('name', 'description', 'embedding_model_config', 'collection_name')
