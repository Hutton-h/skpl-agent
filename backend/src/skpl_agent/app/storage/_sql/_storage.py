...
from .._model import AgentRecord, CredentialRecord, KnowledgeBaseRecord, KnowledgeDocumentRecord, KnowledgeDocumentStatus, ScheduleRecord, SessionRecord, SessionConfig, SessionSource, TeamRecord
from .._utils import _dump_with_secrets
from ._mappers import _from_record, _to_record
from ._tables import _Base, AgentRow, CredentialRow, KnowledgeBaseRow, KnowledgeDocumentRow, MessageRow, ScheduleRow, SessionRow, TeamRow
...
class AsyncSQLAlchemyStorage(StorageBase):
    ...
    def __init__(self, url: str, *, create_tables: bool=True, auto_migrate: bool=False, engine: 'AsyncEngine | None'=None, engine_kwargs: dict[str, Any] | None=None) -> None:
    ...
    async def __aenter__(self) -> Self:
        ...
        if self._external_engine is not None:
            ...
            self._engine = create_async_engine(self._url, future=True, **self._engine_kwargs)
    ...
    def _session(self) -> 'AsyncSession':
        ...
        if self._session_factory is None:
            raise RuntimeError('AsyncSQLAlchemyStorage is not initialised — use `async with storage:`.')
        return self._session_factory()

    def _upsert_stmt(self, row_cls: type, values: dict[str, Any], conflict_cols: list[str], update_cols: tuple[str, ...]) -> Any:
        ...
        if dialect in ('postgresql', 'sqlite'):
            ...
            return stmt.on_conflict_do_update(index_elements=conflict_cols, set_={c: getattr(stmt.excluded, c) for c in update_cols})
        if dialect in ('mysql', 'mariadb'):
            from sqlalchemy.dialects.mysql import insert as _insert
            stmt = _insert(table).values(**values)
            return stmt.on_duplicate_key_update(**{c: getattr(stmt.inserted, c) for c in update_cols})
        raise NotImplementedError(f'Atomic upsert is unsupported for the {dialect!r} dialect; supported dialects are postgresql, sqlite, mysql, mariadb.')

    async def _write_row(self, row_cls: type, record: Any, *, preserve_created_at: bool=True) -> Any:
        ...
        values = {col: getattr(new_row, col) for col in ('id', 'created_at', 'updated_at', 'payload') + indexed}
        update_cols = ('updated_at', 'payload') + indexed
        async with self._session() as sess:
            await sess.execute(self._upsert_stmt(row_cls, values, ['id'], update_cols))
            if preserve_created_at:
                record.created_at = (await sess.execute(select(row_cls.created_at).where(row_cls.id == record.id))).scalar_one()
            await sess.commit()
        return record

    async def _delete_session_impl(self, sess: 'AsyncSession', user_id: str, agent_id: str, session_id: str) -> bool:
        """Delete a session (+ its messages, + leader-team) on *sess*."""
        from sqlalchemy import delete, select
        _ = agent_id
        row = (await sess.execute(select(SessionRow).where(SessionRow.id == session_id))).scalar_one_or_none()
        if row is None or row.user_id != user_id:
            return False
        record = _to_record(row, SessionRecord)
        if record.team_id:
            team_row = (await sess.execute(select(TeamRow).where(TeamRow.id == record.team_id))).scalar_one_or_none()
            if team_row is not None and _to_record(team_row, TeamRecord).session_id == session_id:
                await self._delete_team_impl(sess, user_id, record.team_id)
        await sess.execute(delete(SessionRow).where(SessionRow.id == session_id, SessionRow.user_id == user_id))
        await sess.execute(delete(MessageRow).where(MessageRow.session_id == session_id))
        return True

    async def _delete_schedule_impl(self, sess: 'AsyncSession', user_id: str, schedule_id: str) -> bool:
        """Delete a schedule + cascade its execution sessions on *sess*."""
        from sqlalchemy import delete, select
        row = (await sess.execute(select(ScheduleRow).where(ScheduleRow.id == schedule_id))).scalar_one_or_none()
        if row is None or row.user_id != user_id:
            return False
        sessions = (await sess.execute(select(SessionRow).where(SessionRow.user_id == user_id, SessionRow.source_schedule_id == schedule_id))).scalars().all()
        for s in sessions:
            await self._delete_session_impl(sess, user_id, s.agent_id, s.id)
        await sess.execute(delete(ScheduleRow).where(ScheduleRow.id == schedule_id, ScheduleRow.user_id == user_id))
        return True

    async def _delete_agent_impl(self, sess: 'AsyncSession', user_id: str, agent_id: str) -> bool:
        """Delete an agent + cascade sessions/schedules/team refs on *sess*."""
        from sqlalchemy import delete, select, update
        sessions = (await sess.execute(select(SessionRow).where(SessionRow.user_id == user_id, SessionRow.agent_id == agent_id))).scalars().all()
        for s in sessions:
            await self._delete_session_impl(sess, user_id, agent_id, s.id)
        schedules = (await sess.execute(select(ScheduleRow).where(ScheduleRow.user_id == user_id, ScheduleRow.agent_id == agent_id))).scalars().all()
        for sch in schedules:
            await self._delete_schedule_impl(sess, user_id, sch.id)
        teams = (await sess.execute(select(TeamRow).where(TeamRow.user_id == user_id))).scalars().all()
        for team_row in teams:
            team_rec = _to_record(team_row, TeamRecord)
            dirty = False
            if agent_id in team_rec.data.member_ids:
                team_rec.data.member_ids = [m for m in team_rec.data.member_ids if m != agent_id]
                dirty = True
            filtered = [m for m in team_rec.data.members if m.agent_id != agent_id]
            ...
            if dirty:
                ...
                await sess.execute(update(TeamRow).where(TeamRow.id == team_rec.id).values(payload=new_row.payload, updated_at=new_row.updated_at))
        result = await sess.execute(delete(AgentRow).where(AgentRow.id == agent_id, AgentRow.user_id == user_id))
        return result.rowcount > 0

    async def _delete_team_impl(self, sess: 'AsyncSession', user_id: str, team_id: str) -> bool:
        """Role-aware team cleanup + leader-detach + delete, on *sess*."""
        from sqlalchemy import delete, select, update
        from .._model._team import TeamMember
        row = (await sess.execute(select(TeamRow).where(TeamRow.id == team_id))).scalar_one_or_none()
        if row is None or row.user_id != user_id:
            return False
        ...
        if not members and team.data.member_ids:
            for aid in team.data.member_ids:
                worker = (await sess.execute(select(SessionRow).where(SessionRow.user_id == user_id, SessionRow.agent_id == aid))).scalars().first()
                if worker is not None:
                    members.append(TeamMember(owner_id=user_id, agent_id=aid, session_id=worker.id, role='created'))
        for member in members:
            if member.role == 'created':
                await self._delete_agent_impl(sess, member.owner_id, member.agent_id)
            else:
                await self._delete_session_impl(sess, member.owner_id, member.agent_id, member.session_id)
        await sess.execute(update(SessionRow).where(SessionRow.id == team.session_id, SessionRow.user_id == user_id).values(team_id=None, updated_at=_utcnow()))
        await sess.execute(delete(TeamRow).where(TeamRow.id == team_id, TeamRow.user_id == user_id))
        return True

    async def _generate_credential_name(self, user_id: str, credential_data: CredentialBase) -> str:
        ...
        base_name = cred_type.removesuffix('_credential').replace('_', ' ').title()
        if not base_name:
            base_name = 'Credential'
        existing = await self.list_credentials(user_id)
        same_type_names = [c.data.get('name', '') for c in existing if c.data.get('type') == cred_type and c.id != credential_data.id]
    ...
    async def upsert_credential(self, user_id: str, credential_data: CredentialBase) -> str:
        ...
        if not credential_data.name:
            credential_data.name = await self._generate_credential_name(user_id, credential_data)
        data_dump = _dump_with_secrets(credential_data)
        if credential_data.id:
            from sqlalchemy import select
            async with self._session() as sess:
                existing = (await sess.execute(select(CredentialRow).where(CredentialRow.id == credential_data.id, CredentialRow.user_id == user_id))).scalar_one_or_none()
            if existing is not None:
                record = _to_record(existing, CredentialRecord)
                record.data = data_dump
                await self._write_row(CredentialRow, record)
                return record.id
            record = CredentialRecord(id=credential_data.id, user_id=user_id, data=data_dump)
        else:
            record = CredentialRecord(user_id=user_id, data=data_dump)
        record.created_at = _to_naive_utc(record.created_at)
        record.updated_at = _utcnow()
        async with self._session() as sess:
    ...
    async def list_credentials(self, user_id: str) -> list[CredentialRecord]:
        ...
        from sqlalchemy import select
        async with self._session() as sess:
            rows = (await sess.execute(select(CredentialRow).where(CredentialRow.user_id == user_id))).scalars().all()
    ...
    async def delete_credential(self, user_id: str, credential_id: str) -> bool:
        ...
        async with self._session() as sess:
            result = await sess.execute(delete(CredentialRow).where(CredentialRow.id == credential_id, CredentialRow.user_id == user_id))
            await sess.commit()
        return result.rowcount > 0
    ...
    async def list_agents(self, user_id: str) -> list[AgentRecord]:
        ...
        from sqlalchemy import select
        async with self._session() as sess:
            rows = (await sess.execute(select(AgentRow).where(AgentRow.user_id == user_id, AgentRow.source == 'user'))).scalars().all()
    ...
    async def upsert_session(self, user_id: str, agent_id: str, config: SessionConfig, state: AgentState | None=None, session_id: str | None=None, source: SessionSource=SessionSource.USER, source_schedule_id: str | None=None) -> SessionRecord:
        ...
        record = SessionRecord(user_id=user_id, agent_id=agent_id, config=config, source=source, source_schedule_id=source_schedule_id, state=state if state is not None else AgentState(), **new_id_kwargs)
        await self._write_row(SessionRow, record, preserve_created_at=False)
        return record

    async def set_session_team_id(self, user_id: str, session_id: str, team_id: str | None) -> None:
        """Single-column UPDATE — atomic on all supported dialects."""
        from sqlalchemy import update
        now = _utcnow()
        async with self._session() as sess:
            await sess.execute(update(SessionRow).where(SessionRow.id == session_id, SessionRow.user_id == user_id).values(team_id=team_id, updated_at=now))
            await sess.commit()

    async def update_session_state(self, user_id: str, agent_id: str, session_id: str, state: AgentState) -> None:
    ...
    async def list_sessions(self, user_id: str, agent_id: str) -> list[SessionRecord]:
        ...
        from sqlalchemy import select
        async with self._session() as sess:
            rows = (await sess.execute(select(SessionRow).where(SessionRow.user_id == user_id, SessionRow.agent_id == agent_id).order_by(SessionRow.created_at.desc()))).scalars().all()
        return [_to_record(r, SessionRecord) for r in rows]

    async def get_session(self, user_id: str, agent_id: str, session_id: str) -> SessionRecord | None:
    ...
    async def delete_session(self, user_id: str, agent_id: str, session_id: str) -> bool:
        ...
        """
        async with self._session() as sess:
            ok = await self._delete_session_impl(sess, user_id, agent_id, session_id)
            await sess.commit()
        return ok

    async def list_sessions_by_schedule(self, user_id: str, schedule_id: str) -> list[SessionRecord]:
        """Sessions created by *schedule_id* — newest first."""
        from sqlalchemy import select
        async with self._session() as sess:
            rows = (await sess.execute(select(SessionRow).where(SessionRow.user_id == user_id, SessionRow.source_schedule_id == schedule_id).order_by(SessionRow.created_at.desc()))).scalars().all()
        return [_to_record(r, SessionRecord) for r in rows]
    ...
    async def get_schedule(self, user_id: str, schedule_id: str) -> ScheduleRecord | None:
    ...
    async def list_schedules(self, user_id: str) -> list[ScheduleRecord]:
        ...
        from sqlalchemy import select
        async with self._session() as sess:
            rows = (await sess.execute(select(ScheduleRow).where(ScheduleRow.user_id == user_id))).scalars().all()
    ...
    async def upsert_message(self, user_id: str, session_id: str, msg: Msg) -> None:
        ...
        values = {'session_id': session_id, 'msg_id': msg.id, 'created_at': now, 'payload': payload}
        async with self._session() as sess:
            await sess.execute(self._upsert_stmt(MessageRow, values, ['session_id', 'msg_id'], ('payload',)))
            await sess.commit()
    ...
    async def get_message(self, user_id: str, session_id: str, message_id: str) -> Msg | None:
        ...
        from sqlalchemy import select
        async with self._session() as sess:
            row = (await sess.execute(select(MessageRow).where(MessageRow.session_id == session_id, MessageRow.msg_id == message_id))).scalar_one_or_none()
        if row is None:
            return None
        return Msg.model_validate(row.payload)

    async def list_messages(self, user_id: str, session_id: str, limit: int=50, before: str | None=None, **kwargs: Any) -> tuple[list[Msg], bool]:
        ...
        *limit* messages in chronological order together with a
        ...
        if 'offset' in kwargs:
            import warnings
            warnings.warn("The 'offset' parameter is deprecated and will be removed in a future version. Use 'before' for cursor-based pagination instead.", DeprecationWarning, stacklevel=2)
        from sqlalchemy import and_, or_, select
        async with self._session() as sess:
            cond = MessageRow.session_id == session_id
            if before is not None:
                cursor = (await sess.execute(select(MessageRow.created_at, MessageRow.msg_id).where(MessageRow.session_id == session_id, MessageRow.msg_id == before))).one_or_none()
                if cursor is None:
                    return ([], False)
                (c_created, c_msg) = cursor
                cond = and_(cond, or_(MessageRow.created_at < c_created, and_(MessageRow.created_at == c_created, MessageRow.msg_id < c_msg)))
            rows = list((await sess.execute(select(MessageRow).where(cond).order_by(MessageRow.created_at.desc(), MessageRow.msg_id.desc()).limit(limit + 1))).scalars().all())
    ...
    async def list_teams(self, user_id: str) -> list[TeamRecord]:
        ...
        from sqlalchemy import select
        async with self._session() as sess:
            rows = (await sess.execute(select(TeamRow).where(TeamRow.user_id == user_id))).scalars().all()
    ...
    async def upsert_knowledge_base(self, user_id: str, record: KnowledgeBaseRecord) -> KnowledgeBaseRecord:
        """Create or update a KB record; enforces ``record.user_id``."""
        if record.user_id != user_id:
            raise ValueError('record.user_id does not match the given user_id.')
    ...
    async def get_knowledge_base(self, user_id: str, knowledge_base_id: str) -> KnowledgeBaseRecord | None:
        ...
        if row is None:
            return None
        # Allow access if user is the owner OR the KB is public
    ...
    async def list_knowledge_bases(self, user_id: str) -> list[KnowledgeBaseRecord]:
        """All KBs for a user, including public knowledge bases."""
        from sqlalchemy import or_, select
        async with self._session() as sess:
            rows = (await sess.execute(select(KnowledgeBaseRow).where(or_(KnowledgeBaseRow.user_id == user_id, KnowledgeBaseRow.is_public == 1)))).scalars().all()
    ...
    async def delete_knowledge_base(self, user_id: str, knowledge_base_id: str) -> bool:
        ...
        from sqlalchemy import delete
        async with self._session() as sess:
            result = await sess.execute(delete(KnowledgeBaseRow).where(KnowledgeBaseRow.id == knowledge_base_id, KnowledgeBaseRow.user_id == user_id))
            await sess.commit()
        return result.rowcount > 0

    async def upsert_knowledge_document(self, user_id: str, record: KnowledgeDocumentRecord) -> KnowledgeDocumentRecord:
    ...
    async def get_knowledge_document(self, user_id: str, knowledge_base_id: str, document_id: str) -> KnowledgeDocumentRecord | None:
        """One document; owner + KB scoped."""
        async with self._session() as sess:
            row = await sess.get(KnowledgeDocumentRow, document_id)
        if row is None or row.user_id != user_id or row.knowledge_base_id != knowledge_base_id:
            return None
        return _to_record(row, KnowledgeDocumentRecord)

    async def list_knowledge_documents(self, user_id: str, knowledge_base_id: str) -> list[KnowledgeDocumentRecord]:
        """All documents inside a KB."""
        from sqlalchemy import select
        async with self._session() as sess:
            rows = (await sess.execute(select(KnowledgeDocumentRow).where(KnowledgeDocumentRow.user_id == user_id, KnowledgeDocumentRow.knowledge_base_id == knowledge_base_id))).scalars().all()
        return [_to_record(r, KnowledgeDocumentRecord) for r in rows]

    async def delete_knowledge_document(self, user_id: str, knowledge_base_id: str, document_id: str) -> bool:
        """Delete one document."""
        from sqlalchemy import delete
        async with self._session() as sess:
            result = await sess.execute(delete(KnowledgeDocumentRow).where(KnowledgeDocumentRow.id == document_id, KnowledgeDocumentRow.user_id == user_id, KnowledgeDocumentRow.knowledge_base_id == knowledge_base_id))
            await sess.commit()
        return result.rowcount > 0

    async def update_knowledge_document_status(self, user_id: str, knowledge_base_id: str, document_id: str, status: KnowledgeDocumentStatus, error: str | None=None, chunk_count: int | None=None) -> None:
    ...
    async def acquire_knowledge_document_lease(self, user_id: str, knowledge_base_id: str, document_id: str, processing_node: str, lease_ttl: timedelta, now: datetime | None=None) -> bool:
        ...
        async with self._session() as sess:
            result = await sess.execute(update(KnowledgeDocumentRow).where(KnowledgeDocumentRow.id == document_id, KnowledgeDocumentRow.user_id == user_id, KnowledgeDocumentRow.knowledge_base_id == knowledge_base_id, or_(KnowledgeDocumentRow.processing_node.is_(None), KnowledgeDocumentRow.lease_expires_at.is_(None), KnowledgeDocumentRow.lease_expires_at < now)).values(processing_node=processing_node, lease_expires_at=deadline, updated_at=now))
            await sess.commit()
        return result.rowcount > 0

    async def renew_knowledge_document_lease(self, user_id: str, knowledge_base_id: str, document_id: str, processing_node: str, lease_ttl: timedelta, now: datetime | None=None) -> bool:
        """Conditional UPDATE constrained to the current holder."""
        from sqlalchemy import update
        now = _to_naive_utc(now) if now is not None else _utcnow()
        deadline = now + lease_ttl
        async with self._session() as sess:
            result = await sess.execute(update(KnowledgeDocumentRow).where(KnowledgeDocumentRow.id == document_id, KnowledgeDocumentRow.user_id == user_id, KnowledgeDocumentRow.knowledge_base_id == knowledge_base_id, KnowledgeDocumentRow.processing_node == processing_node).values(lease_expires_at=deadline, updated_at=now))
            await sess.commit()
        return result.rowcount > 0

    async def release_knowledge_document_lease(self, user_id: str, knowledge_base_id: str, document_id: str, processing_node: str) -> None:
        """Conditional UPDATE constrained to the current holder."""
        from sqlalchemy import update
        now = _utcnow()
        async with self._session() as sess:
            await sess.execute(update(KnowledgeDocumentRow).where(KnowledgeDocumentRow.id == document_id, KnowledgeDocumentRow.user_id == user_id, KnowledgeDocumentRow.knowledge_base_id == knowledge_base_id, KnowledgeDocumentRow.processing_node == processing_node).values(processing_node=None, lease_expires_at=None, updated_at=now))
            await sess.commit()

    async def list_knowledge_documents_with_expired_lease(self, now: datetime | None=None) -> list[KnowledgeDocumentRecord]:
        """Documents past their lease deadline, non-terminal, held."""
        from sqlalchemy import select
        now = _to_naive_utc(now) if now is not None else _utcnow()
        terminal = ('ready', 'error')
        async with self._session() as sess:
            rows = (await sess.execute(select(KnowledgeDocumentRow).where(KnowledgeDocumentRow.status.notin_(terminal), KnowledgeDocumentRow.processing_node.is_not(None), KnowledgeDocumentRow.lease_expires_at.is_not(None), KnowledgeDocumentRow.lease_expires_at < now))).scalars().all()
        return [_to_record(r, KnowledgeDocumentRecord) for r in rows]

    async def list_knowledge_documents_pending_since(self, threshold: datetime) -> list[KnowledgeDocumentRecord]:
        """Documents stuck in ``pending`` before *threshold*."""
        from sqlalchemy import select
        threshold = _to_naive_utc(threshold)
        async with self._session() as sess:
            rows = (await sess.execute(select(KnowledgeDocumentRow).where(KnowledgeDocumentRow.status == 'pending', KnowledgeDocumentRow.created_at < threshold))).scalars().all()
        return [_to_record(r, KnowledgeDocumentRecord) for r in rows]