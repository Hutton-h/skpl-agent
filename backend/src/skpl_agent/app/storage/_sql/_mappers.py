...
def _from_record(row_cls: 'type[_JsonRecordMixin]', record: _RecordBase) -> '_JsonRecordMixin':
    ...
    return row_cls(id=record.id, created_at=record.created_at, updated_at=record.updated_at, payload=dump, **column_values)

def _to_record(row: '_JsonRecordMixin', record_cls: type[R]) -> R:
