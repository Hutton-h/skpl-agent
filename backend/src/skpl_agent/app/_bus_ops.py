...
if TYPE_CHECKING:
    ...
    from skpl_agent.event import ExternalExecutionResultEvent, UserConfirmResultEvent, UserInterruptEvent

async def publish_session_event(bus: 'MessageBus', session_id: str, event: dict) -> str:
    ...
    entry_id = await bus.log_append(key, event, max_len=MessageBusKeys.SESSION_REPLAY_MAX_LEN)
    await bus.publish(key, {**event, '_entry_id': entry_id})
    return entry_id

async def enqueue_run_trigger(bus: 'MessageBus', user_id: str, session_id: str, agent_id: str, *, kind: Literal['wake', 'resume']=MessageBusKeys.WAKEUP_KIND_WAKE, inputs: UserConfirmResultEvent | ExternalExecutionResultEvent | UserInterruptEvent | None=None) -> None:
    """Enqueue a typed run trigger and signal dispatchers.
    ...
    """
    await bus.queue_push(MessageBusKeys.wakeup_queue(), {'user_id': user_id, 'session_id': session_id, 'agent_id': agent_id, 'kind': kind, 'input': inputs.model_dump(mode='json') if inputs else None})
    await bus.publish(MessageBusKeys.wakeup_signal(), {})

async def enqueue_index_task(bus: 'MessageBus', user_id: str, knowledge_base_id: str, document_id: str) -> None:
    """Enqueue a knowledge-document indexing task and signal consumers.
    ...
    """
    await bus.queue_push(MessageBusKeys.index_tasks_queue(), {'user_id': user_id, 'knowledge_base_id': knowledge_base_id, 'document_id': document_id})
