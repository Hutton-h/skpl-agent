...
class ResourceRef(BaseModel):
    ...
    """
    kind: ResourceKind = Field(description='Resource kind: credential, agent, or knowledge_base.')
    owner_id: str = Field(description="Resource owner user id, matching the storage record's user_id.")
    resource_id: str = Field(description='Resource id under the owner, such as credential_id or agent_id.')
    permission: ResourcePermission = Field(default=ResourcePermission.READ, description='Granted permission level. Defaults to read.')
...
class ResourceAccessPolicyBase(ABC):
    ...
    @abstractmethod
    async def list_accessible(self, viewer_id: str, kind: ResourceKind, storage: StorageBase) -> list[ResourceRef]:
    ...
    async def can_edit(self, viewer_id: str, kind: ResourceKind, owner_id: str, resource_id: str, storage: StorageBase) -> bool:
        ...
        return any((ref.kind == kind and ref.owner_id == owner_id and (ref.resource_id == resource_id) and (ref.permission == ResourcePermission.EDIT) for ref in refs))
