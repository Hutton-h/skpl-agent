...
class TeamCreate(_TeamToolBase):
    ...
    async def __call__(self, name: str, description: str) -> ToolChunk:
        ...
        try:
            ...
            if session is None:
                return ToolChunk(content=[TextBlock(text=f'TeamCreate: session {self._session_id} not found.')], state=ToolResultState.ERROR)
            if session.team_id is not None:
                return ToolChunk(content=[TextBlock(text=f'TeamCreate: this session is already part of team {session.team_id}. A session can only lead one team at a time — dissolve the current one with TeamDelete first.')], state=ToolResultState.ERROR)
            team = TeamRecord(user_id=self._user_id, session_id=self._session_id, data=TeamData(name=name, description=description, member_ids=[]))
            await self._storage.upsert_team(self._user_id, team)
            await self._storage.set_session_team_id(self._user_id, self._session_id, team.id)
            return ToolChunk(content=[TextBlock(text=f'Team {team.id} ({team.data.name}) created. You are the leader. Use AgentCreate to add members, then TeamSay to coordinate them.')])
        except Exception as e:
            return ToolChunk(content=[TextBlock(text=f'TeamCreate failed: {e}')], state=ToolResultState.ERROR)