...
class _TeamToolBase(ToolBase):
    ...
    def __init__(self, storage: 'StorageBase', message_bus: 'MessageBus', workspace_manager: 'WorkspaceManagerBase', user_id: str, session_id: str, agent_id: str) -> None:
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        """
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message=f'{self.name} is always allowed when attached to the agent.')