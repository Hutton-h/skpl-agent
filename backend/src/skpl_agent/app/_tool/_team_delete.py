    ...
    async def __call__(self) -> ToolChunk:
        ...
        try:
            ...
            if session is None or session.team_id is None:
                return ToolChunk(content=[TextBlock(text='TeamDelete: this session is not in any team.')], state=ToolResultState.ERROR)
            team = await self._storage.get_team(self._user_id, session.team_id)
            if team is None:
                return ToolChunk(content=[TextBlock(text=f'TeamDelete: team {session.team_id} no longer exists.')], state=ToolResultState.ERROR)
            if team.session_id != self._session_id:
                return ToolChunk(content=[TextBlock(text='TeamDelete: only the team leader can dissolve the team; this session is a worker.')], state=ToolResultState.ERROR)
            ...
            return ToolChunk(content=[TextBlock(text=f'Team {team.id} dissolved. All members deleted; your session is no longer leading any team.')])
        except Exception as e:
            return ToolChunk(content=[TextBlock(text=f'TeamDelete failed: {e}')], state=ToolResultState.ERROR)