...
from ...workspace._docker._make_dockerfile import DEFAULT_BASE_IMAGE, DEFAULT_GATEWAY_PORT
...
class DockerWorkspaceManager(WorkspaceManagerBase):
    ...
    def __init__(self, basedir: str, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT, base_image: str=DEFAULT_BASE_IMAGE, node_version: str='20', extra_pip: list[str] | None=None, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, ttl: float=3600.0, sweep_interval: float=DEFAULT_SWEEP_INTERVAL) -> None:
    ...
    async def _build_and_start(self, *, workspace_id: str, user_id: str, agent_id: str) -> DockerWorkspace:
        ...
        ws = DockerWorkspace(workspace_id=workspace_id, workdir=workdir, base_image=self._base_image, node_version=self._node_version, extra_pip=self._extra_pip, gateway_port=self._gateway_port, env=self._env, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
        await ws.initialize()
        return ws

    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> DockerWorkspace:
    ...
    @deprecated('DockerWorkspaceManager.create_workspace is deprecated; use get_workspace(workspace_id=None) instead.', category=None)
    async def create_workspace(self, user_id: str, agent_id: str, session_id: str) -> DockerWorkspace:
        ...
        workdir = self._workdir_for(user_id, agent_id)
        os.makedirs(workdir, exist_ok=True)
        ws = DockerWorkspace(host_workdir=workdir, base_image=self._base_image, node_version=self._node_version, extra_pip=self._extra_pip, gateway_port=self._gateway_port, env=self._env, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
        await ws.initialize()
        async with self._lock:
    ...
    async def _safe_close(ws: DockerWorkspace) -> None:
        ...
        try:
            ...
            logger.exception('Failed to close DockerWorkspace %s', ws.workspace_id)