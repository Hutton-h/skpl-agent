...
class LocalWorkspaceManager(WorkspaceManagerBase):
    ...
    def __init__(self, basedir: str, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT, default_mcps: list | None=None, skill_paths: list[str] | None=None, ttl: float=3600.0) -> None:
    ...
    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> LocalWorkspace:
        ...
        if workspace_id is None:
            workspace_id = self.assign_workspace_id(user_id='', agent_id=agent_id, session_id='')
        async with self._lock:
            ...
            if cached is not None:
                ...
                hit = None
        if expired:
            await asyncio.gather(*(self._safe_close(ws) for ws in expired), return_exceptions=True)
        if hit is not None:
            return hit
        async with self._lock:
            cached = self._cache.get(workspace_id)
            ...
            ws = LocalWorkspace(workspace_id=workspace_id, workdir=workdir, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
            await ws.initialize()
            self._cache[workspace_id] = (ws, time.monotonic())
            return ws

    @deprecated('LocalWorkspaceManager.create_workspace is deprecated; use get_workspace(workspace_id=None) instead.', category=None)
    async def create_workspace(self, user_id: str, agent_id: str, session_id: str) -> LocalWorkspace:
        ...
        ws = LocalWorkspace(workdir=workdir, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
        await ws.initialize()
        async with self._lock:
