...
from ...workspace._k8s._constants import DEFAULT_GATEWAY_PORT, DEFAULT_IMAGE
...
class K8sWorkspaceManager(WorkspaceManagerBase):
    ...
    def __init__(self, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT, namespace: str='agentscope', kubeconfig: str | None=None, image: str=DEFAULT_IMAGE, image_pull_policy: str='IfNotPresent', image_pull_secrets: list[str] | None=None, resources: dict[str, Any] | None=None, node_selector: dict[str, str] | None=None, tolerations: list[dict[str, Any]] | None=None, service_account: str | None=None, gateway_port: int=DEFAULT_GATEWAY_PORT, extra_pip: list[str] | None=None, storage_class: str | None=None, storage_size: str='1Gi', env: dict[str, str] | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, ttl: float=3600.0, sweep_interval: float=DEFAULT_SWEEP_INTERVAL, delete_pvc_on_close: bool=False) -> None:
    ...
    async def _build_and_start(self, *, workspace_id: str | None) -> K8sWorkspace:
        """Construct a :class:`K8sWorkspace` and run ``initialize``."""
        ws = K8sWorkspace(workspace_id=workspace_id, kubeconfig=self._kubeconfig, namespace=self._namespace, image=self._image, image_pull_policy=self._image_pull_policy, image_pull_secrets=self._image_pull_secrets, resources=self._resources, node_selector=self._node_selector, tolerations=self._tolerations, service_account=self._service_account, gateway_port=self._gateway_port, extra_pip=self._extra_pip, storage_class=self._storage_class, storage_size=self._storage_size, delete_pvc_on_close=self._delete_pvc_on_close, env=self._env, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
        await ws.initialize()
        return ws

    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> K8sWorkspace:
        ...
        if workspace_id is None:
            workspace_id = self.assign_workspace_id(user_id=user_id, agent_id=agent_id, session_id='')
    ...
    @deprecated('K8sWorkspaceManager.create_workspace is deprecated; use get_workspace(workspace_id=None) instead.', category=None)
    async def create_workspace(self, user_id: str, agent_id: str, session_id: str) -> K8sWorkspace:
    ...
    async def close_all(self) -> None:
        ...
        if not entries:
            return
        await asyncio.gather(*(self._safe_close(ws) for (ws, _) in entries), return_exceptions=True)
    ...
    async def _sweep_once(self) -> None:
        ...
        now = time.monotonic()
        async with self._lock:
            expired_ids = [wid for (wid, (_, ts)) in self._cache.items() if now - ts > self._ttl]
            evicted = [self._cache.pop(wid)[0] for wid in expired_ids]
        if not evicted:
            return
        await asyncio.gather(*(self._safe_close(ws) for ws in evicted), return_exceptions=True)
    ...
    async def _safe_close(ws: K8sWorkspace) -> None:
        ...
        try:
            ...
            logger.exception('Failed to close K8sWorkspace %s', ws.workspace_id)