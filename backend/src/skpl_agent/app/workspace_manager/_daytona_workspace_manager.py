...
from ...workspace._daytona._constants import DEFAULT_GATEWAY_PORT, DEFAULT_SWEEP_INTERVAL, DEFAULT_TIMEOUT
...
class DaytonaWorkspaceManager(WorkspaceManagerBase):
    ...
    def __init__(self, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT, api_key: str='', api_url: str='', target: str='', timeout_seconds: int=DEFAULT_TIMEOUT, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, sandbox_metadata: dict[str, str] | None=None, extra_pip: list[str] | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, os_user: str | None=None, ttl: float=3600.0, sweep_interval: float=DEFAULT_SWEEP_INTERVAL) -> None:
    ...
    async def _build_and_start(self, *, workspace_id: str | None, user_id: str, agent_id: str) -> DaytonaWorkspace:
        ...
        """
        ws = DaytonaWorkspace(workspace_id=workspace_id, api_key=self._api_key, api_url=self._api_url, target=self._target, timeout_seconds=self._timeout_seconds, gateway_port=self._gateway_port, env=self._env, sandbox_metadata=self._metadata_for(user_id, agent_id), extra_pip=self._extra_pip, default_mcps=self._default_mcps, skill_paths=self._skill_paths, os_user=self._os_user)
        await ws.initialize()
        return ws

    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> DaytonaWorkspace:
    ...
    async def _safe_close(ws: DaytonaWorkspace) -> None:
        ...
        try:
            ...
            logger.exception('Failed to close DaytonaWorkspace %s', ws.workspace_id)