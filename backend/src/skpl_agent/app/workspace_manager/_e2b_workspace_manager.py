...
from ...workspace._e2b._constants import DEFAULT_GATEWAY_PORT, DEFAULT_TEMPLATE, DEFAULT_TIMEOUT
...
class E2BWorkspaceManager(WorkspaceManagerBase):
    ...
    def __init__(self, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT, template: str=DEFAULT_TEMPLATE, api_key: str='', domain: str='', timeout_seconds: int=DEFAULT_TIMEOUT, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, sandbox_metadata: dict[str, str] | None=None, extra_pip: list[str] | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, ttl: float=3600.0, sweep_interval: float=DEFAULT_SWEEP_INTERVAL) -> None:
    ...
    def _metadata_for(self, user_id: str, agent_id: str) -> dict[str, str]:
        ...
        """
        return {'skpl_agent.user.id': user_id, 'skpl_agent.agent.id': agent_id, **self._sandbox_metadata}

    async def _build_and_start(self, *, workspace_id: str | None, user_id: str, agent_id: str) -> E2BWorkspace:
        ...
        the second call.
        """
        ws = E2BWorkspace(workspace_id=workspace_id, template=self._template, api_key=self._api_key, domain=self._domain, timeout_seconds=self._timeout_seconds, gateway_port=self._gateway_port, env=self._env, sandbox_metadata=self._metadata_for(user_id, agent_id), extra_pip=self._extra_pip, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
        await ws.initialize()
        return ws

    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> E2BWorkspace:
    ...
    @deprecated('E2BWorkspaceManager.create_workspace is deprecated; use get_workspace(workspace_id=None) instead.', category=None)
    async def create_workspace(self, user_id: str, agent_id: str, session_id: str) -> E2BWorkspace:
        ...
        """
        del session_id
        ws = await self._build_and_start(workspace_id=None, user_id=user_id, agent_id=agent_id)
        async with self._lock:
    ...
    async def _safe_close(ws: E2BWorkspace) -> None:
        ...
        try:
            ...
            logger.exception('Failed to close E2BWorkspace %s', ws.workspace_id)