...
class BubblewrapWorkspaceManager(WorkspaceManagerBase):
    ...
    def __init__(self, basedir: str, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT, gateway_port: int | None=DEFAULT_GATEWAY_PORT, share_net: bool=True, env: dict[str, str] | None=None, extra_pip: list[str] | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, ttl: float=3600.0, sweep_interval: float=DEFAULT_SWEEP_INTERVAL) -> None:
        ...
        if not share_net:
            raise ValueError('BubblewrapWorkspaceManager currently requires share_net=True because BubblewrapWorkspace uses a TCP MCP gateway across separate bwrap executions.')
    ...
    def _workdir_for(self, user_id: str, workspace_id: str) -> str:
        ...
        path = os.path.join(self._basedir, _safe_component(user_id), _safe_component(workspace_id))
    ...
    async def _build_and_start(self, *, workspace_id: str, user_id: str, agent_id: str) -> BubblewrapWorkspace:
        """Construct and initialize a Bubblewrap workspace."""
        del agent_id
        workdir = self._workdir_for(user_id, workspace_id)
        os.makedirs(workdir, mode=448, exist_ok=True)
        os.chmod(workdir, 448)
        ws = BubblewrapWorkspace(workspace_id=workspace_id, host_workdir=workdir, gateway_port=self._gateway_port, share_net=self._share_net, env=self._env, extra_pip=self._extra_pip, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
        await ws.initialize()
        return ws

    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> BubblewrapWorkspace:
        """Return an initialized workspace, creating it on cache miss."""
        if workspace_id is None:
            workspace_id = self.assign_workspace_id(user_id=user_id, agent_id=agent_id, session_id=session_id)
    ...
    @deprecated('BubblewrapWorkspaceManager.create_workspace is deprecated; use get_workspace(workspace_id=None) instead.', category=None)
    async def create_workspace(self, user_id: str, agent_id: str, session_id: str) -> BubblewrapWorkspace:
    ...
    async def _safe_close(ws: BubblewrapWorkspace) -> None:
        ...
        try:
            ...
            logger.exception('Failed to close BubblewrapWorkspace %s', ws.workspace_id)