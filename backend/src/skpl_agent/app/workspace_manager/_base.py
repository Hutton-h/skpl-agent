...
class WorkspaceManagerBase(ABC):
    ...
    def __init__(self, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT) -> None:
    ...
    def assign_workspace_id(self, *, user_id: str, agent_id: str, session_id: str) -> str:
        ...
        * ``PER_USER`` → deterministic BLAKE2b of ``user::``.
        ...
        if self._isolation is IsolationPolicy.PER_AGENT:
            return hashlib.blake2b(f'{user_id}::{agent_id}'.encode('utf-8'), digest_size=8).hexdigest()
        if self._isolation is IsolationPolicy.PER_USER:
            return hashlib.blake2b(f'user::{user_id}'.encode('utf-8'), digest_size=8).hexdigest()
        return _generate_id()

    @abstractmethod
    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> WorkspaceBase:
