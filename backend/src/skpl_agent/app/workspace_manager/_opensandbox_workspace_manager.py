...
from ...workspace._opensandbox._constants import DEFAULT_GATEWAY_PORT, DEFAULT_IMAGE, DEFAULT_REQUEST_TIMEOUT, DEFAULT_TIMEOUT
...
class OpenSandboxWorkspaceManager(WorkspaceManagerBase):
    ...
    def __init__(self, *, isolation: IsolationPolicy=IsolationPolicy.PER_AGENT, image: str=DEFAULT_IMAGE, api_key: str='', domain: str='', protocol: Literal['http', 'https']='http', request_timeout_seconds: float | None=DEFAULT_REQUEST_TIMEOUT, timeout_seconds: int=DEFAULT_TIMEOUT, gateway_port: int=DEFAULT_GATEWAY_PORT, env: dict[str, str] | None=None, sandbox_metadata: dict[str, str] | None=None, resource: dict[str, str] | None=None, entrypoint: list[str] | None=None, network_policy: Any | None=None, extra_pip: list[str] | None=None, default_mcps: list[MCPClient] | None=None, skill_paths: list[str] | None=None, ttl: float=3600.0, sweep_interval: float=DEFAULT_SWEEP_INTERVAL) -> None:
    ...
    async def _build_and_start(self, *, workspace_id: str, user_id: str, agent_id: str) -> OpenSandboxWorkspace:
        ...
        next cache miss.
        """
        ws = OpenSandboxWorkspace(workspace_id=workspace_id, image=self._image, api_key=self._api_key, domain=self._domain, protocol=self._protocol, request_timeout_seconds=self._request_timeout_seconds, timeout_seconds=self._timeout_seconds, gateway_port=self._gateway_port, env=self._env, sandbox_metadata={'skpl_agent.user.id': user_id, 'skpl_agent.agent.id': agent_id, **self._sandbox_metadata}, resource=self._resource, entrypoint=self._entrypoint, network_policy=self._network_policy, extra_pip=self._extra_pip, default_mcps=self._default_mcps, skill_paths=self._skill_paths)
        await ws.initialize()
        return ws

    async def get_workspace(self, user_id: str, agent_id: str, session_id: str, workspace_id: str | None=None) -> OpenSandboxWorkspace:
        ...
        async with self._lock:
            ...
            if cached is not None:
                ...
                self._cache[workspace_id] = (ws, time.monotonic())
                return ws
            ws = await self._build_and_start(workspace_id=workspace_id, user_id=user_id, agent_id=agent_id)
            self._cache[workspace_id] = (ws, time.monotonic())
            return ws
    ...
    async def _safe_close(ws: OpenSandboxWorkspace) -> None:
        ...
        try:
            ...
            logger.exception('Failed to close OpenSandboxWorkspace %s', ws.workspace_id)