...
class InboxMiddleware(MiddlewareBase):
    ...
    def __init__(self, message_bus: MessageBus, max_count: int=100) -> None:
    ...
    async def on_reasoning(self, agent: Agent, input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator[Any, None]:
        ...
        """
        entries = await self._bus.queue_drain(MessageBusKeys.inbox(agent.state.session_id), max_count=self._max_count)
        if entries:
            hint_blocks = [HintBlock.model_validate(payload) for (_entry_id, payload) in entries]
            logger.info('InboxMiddleware: injecting %d HintBlock(s) into context for session %s', len(hint_blocks), agent.state.session_id)
            if len(agent.state.context) > 0:
                ...
                if last_msg.role == 'assistant' and last_msg.name == agent.name:
                    ...
                    agent.state.context.append(AssistantMsg(id=agent.state.reply_id, name=agent.name, content=list(hint_blocks)))
            else:
                agent.state.context.append(AssistantMsg(id=agent.state.reply_id, name=agent.name, content=list(hint_blocks)))
