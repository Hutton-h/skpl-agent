...
from ...message import DataBlock, HintBlock, TextBlock, ToolResultState
...
class ToolOffloadMiddleware(MiddlewareBase):
    ...
    def __init__(self, bg_manager: BackgroundTaskManager, message_bus: MessageBus, user_id: str, agent_id: str, timeout_secs: float=10.0) -> None:
        """Bind dependencies.
    ...
    async def on_acting(self, agent: Agent, input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        logger.info("Tool '%s' timed out after %.1fs, offloading to background: session_id=%s, agent_id=%s", tool_name, self._timeout_secs, session_id, agent.name)
        ...
        async def _deliver_when_done() -> None:
            ...
            try:
                ...
                logger.info("Background tool '%s' cancelled, skipping delivery: session_id=%s", tool_name, session_id)
                return
            except Exception:
                logger.warning("Background tool '%s' failed, skipping delivery: session_id=%s", tool_name, session_id, exc_info=True)
                return
            ...
            response: ToolResponse | None = next((i for i in all_items if isinstance(i, ToolResponse)), None)
            tool_call_id = tool_call.id
            hint_source = json.dumps({'label': 'tool_output', 'sublabel': f'{tool_name} · {tool_call_id}'}, ensure_ascii=False)
            if response is None or len(response.content) == 0:
                hint = HintBlock(hint=f"<system-notification>Tool '{tool_name}' running in background (id={tool_call_id}) has completed with no output.</system-notification>", source=hint_source)
            else:
                content_blocks: list[TextBlock | DataBlock] = deepcopy(response.content)
                prefix = f"<system-notification>Tool '{tool_name}' running in background (id={tool_call_id}) has completed.\n\nResult:\n\n"
                ...
                hint = HintBlock(hint=content_blocks, source=hint_source)
            logger.info("Background tool '%s' completed, pushing result to inbox and enqueueing wakeup: session_id=%s", tool_name, session_id)
            await self._message_bus.queue_push(MessageBusKeys.inbox(session_id), hint.model_dump(mode='json'))
            await enqueue_run_trigger(self._message_bus, user_id=self._user_id, session_id=session_id, agent_id=self._agent_id)
        asyncio.create_task(_deliver_when_done())
        task_id = await self._bg_manager.register_task(asyncio_task=drain_task, session_id=session_id, agent_id=self._agent_id, user_id=self._user_id, tool_name=tool_name)
        logger.info("Synthetic ToolResponse yielded for offloaded tool '%s': task_id=%s, session_id=%s, agent_id=%s", tool_name, task_id, session_id, agent.name)
        placeholder_text = f"<system-reminder>Tool '{tool_name}' is running in background (id={task_id}) for over {self._timeout_secs}s. You will be notified automatically when it finishes, so **DO NOT** poll, query, or wait for the result yourself. **DO NOT** call any waiting tool such as `bash sleep`. You have exactly two valid options:\n1. Continue with other independent tasks and ignore this tool for now; or\n2. If there is nothing else to do, simply give a text reply without calling any tool, which ends the current reasoning loop — just do nothing and end this run.\n</system-reminder>"
        yield ToolChunk(content=[TextBlock(text=placeholder_text)], state=ToolResultState.SUCCESS)
        yield ToolResponse(content=[TextBlock(text=placeholder_text)], state=ToolResultState.SUCCESS, id=tool_call.id)