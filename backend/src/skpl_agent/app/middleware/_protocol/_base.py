...
from skpl_agent.event import AgentEvent
...
class ProtocolMiddlewareBase(BaseHTTPMiddleware, ABC):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if content_type.startswith('text/event-stream') and body_iterator is not None:
            converted_stream = self._convert_stream(body_iterator)
            return StreamingResponse(content=converted_stream, status_code=response.status_code, headers=dict(response.headers), media_type=response.media_type)
        return response

    async def _convert_stream(self, original_stream: AsyncGenerator) -> AsyncGenerator[bytes, None]:
    ...
    def _convert_event_json(self, chunk_str: str) -> bytes | None:
        ...
        try:
            ...
            return json.dumps(protocol_data, ensure_ascii=False).encode('utf-8') + b'\n'
    ...
    def _deserialize_event(self, event_dict: dict) -> AgentEvent:
        ...
        adapter = TypeAdapter(Annotated[AgentEvent, Field(discriminator='type')])
