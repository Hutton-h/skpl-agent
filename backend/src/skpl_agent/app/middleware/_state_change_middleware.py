...
_TEAM_TOOL_NAMES = frozenset({'TeamCreate', 'AgentCreate', 'AgentInvite', 'TeamDelete'})
...
class StateChangeMiddleware(MiddlewareBase):
    ...
    def _state_hash(agent: Any) -> str:
        ...
        Returns:
            ...
            change.
        """
        raw = agent.state.tasks_context.model_dump_json() + agent.state.permission_context.model_dump_json()
    ...
    async def _publish_state(self, agent: Any) -> None:
        ...
        """
        event = CustomEvent(name='state_updated', value={'tasks_context': agent.state.tasks_context.model_dump(mode='json'), 'permission_context': agent.state.permission_context.model_dump(mode='json')})
    ...
    async def on_acting(self, agent: Any, input_kwargs: dict, next_handler: Callable[..., AsyncGenerator]) -> AsyncGenerator:
        ...
        if tool_name in _TEAM_TOOL_NAMES:
            event = CustomEvent(name='team_updated', value={})
            await publish_session_event(self._bus, self._session_id, event.model_dump(mode='json'))