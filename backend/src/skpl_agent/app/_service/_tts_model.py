...
async def get_tts_model(user_id: str, config: TTSModelConfig, access: ResourceAccessService) -> TTSModelBase:
    ...
    if not tts_classes:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Provider {config.type!r} does not support TTS models.')
    tts_cls = _resolve_tts_class(tts_classes, config.model)
    parameters = tts_cls.Parameters(**config.parameters) if config.parameters else None
    return tts_cls(credential=credential, model=config.model, parameters=parameters)

def _resolve_tts_class(classes: list[Type[TTSModelBase]], model: str) -> Type[TTSModelBase]:
