...
from ....event import RequireUserConfirmEvent, RequireExternalExecutionEvent, UserConfirmResultEvent, ExternalExecutionResultEvent, ReplyEndEvent
if TYPE_CHECKING:
...
class SubagentHitlProjector:
    ...
    async def maybe_project(self, user_id: str, session_record: 'SessionRecord', agent_record: 'AgentRecord', event: 'AgentEvent', projection: 'SessionProjection') -> None:
        ...
        if not session_record.team_id:
            return
        if not isinstance(event, (RequireUserConfirmEvent, RequireExternalExecutionEvent, UserConfirmResultEvent, ExternalExecutionResultEvent, ReplyEndEvent)):
            return
        ...
        if isinstance(event, (RequireUserConfirmEvent, RequireExternalExecutionEvent)):
            payload = {'worker_session_id': session_record.id, 'worker_agent_id': agent_record.id, 'worker_agent_name': agent_record.data.name, 'reply_id': event.reply_id, 'event_type': 'require_user_confirm' if isinstance(event, RequireUserConfirmEvent) else 'require_external_execution', 'event': event.model_dump(mode='json'), 'created_at': datetime.now().isoformat()}
            await projection.upsert(leader_sid, self.KIND, self.entry_id(session_record.id, event.reply_id), payload)
            await projection.publish(leader_sid, self.EVT_REQUIRE, payload)
        else:
            await projection.delete(leader_sid, self.KIND, self.entry_id(session_record.id, event.reply_id))
            await projection.publish(leader_sid, self.EVT_RESULT, {'worker_session_id': session_record.id, 'reply_id': event.reply_id})

    @classmethod
    async def resolve(cls, projection: 'SessionProjection', leader_sid: str, reply_id: str) -> dict | None:
    ...
    @classmethod
    async def purge(cls, projection: 'SessionProjection', leader_sid: str) -> None:
    ...
    @classmethod
    async def drop_worker(cls, projection: 'SessionProjection', leader_sid: str, worker_sid: str) -> None:
        ...
        for entry in await projection.list(leader_sid, cls.KIND):
            if entry.get('worker_session_id') == worker_sid:
                await projection.delete(leader_sid, cls.KIND, cls.entry_id(worker_sid, entry['reply_id']))