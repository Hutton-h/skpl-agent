...
class SessionProjection:
    ...
    async def upsert(self, target_sid: str, kind: str, entry_id: str, payload: dict) -> None:
        ...
        """
        await self._bus.registry_set(MessageBusKeys.projection_namespace(target_sid), MessageBusKeys.projection_field(kind, entry_id), json.dumps(payload, ensure_ascii=False))

    async def delete(self, target_sid: str, kind: str, entry_id: str) -> None:
        ...
        Args:
            ...
            entry_id (`str`):
                Identity of the entry within the feed.
        """
        await self._bus.registry_del(MessageBusKeys.projection_namespace(target_sid), MessageBusKeys.projection_field(kind, entry_id))
    ...
    async def list(self, target_sid: str, kind: str) -> list[dict]:
        ...
        """
        raw = await self._bus.registry_getall(MessageBusKeys.projection_namespace(target_sid))
        prefix = MessageBusKeys.projection_field_prefix(kind)
        return [json.loads(value) for (field, value) in raw.items() if field.startswith(prefix)]
    ...
    async def purge(self, target_sid: str, kind: str | None=None) -> None:
        ...
        """
        if kind is None:
            await self._bus.registry_drop(MessageBusKeys.projection_namespace(target_sid))
            return
    ...
    async def publish(self, target_sid: str, event_name: str, value: dict) -> None:
        ...
        await publish_session_event(self._bus, target_sid, custom.model_dump(mode='json'))