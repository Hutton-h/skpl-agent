...
from ...tool import TaskCreate, TaskGet, TaskList, TaskUpdate, Toolkit, ToolGroup
from ...workspace import WorkspaceBase
from ..access import ResourceKind
from ._access import ResourceAccessService

async def get_toolkit(*, storage: StorageBase, workspace: WorkspaceBase, workspace_manager: WorkspaceManagerBase, scheduler_manager: SchedulerManager, background_task_manager: BackgroundTaskManager, message_bus: MessageBus, middlewares: list[MiddlewareBase], user_id: str, agent_record: AgentRecord, session_record: SessionRecord, resource_access_service: ResourceAccessService, extra_factory: AgentToolFactory | None=None, sub_agent_templates: dict[str, SubAgentTemplate] | None=None) -> Toolkit:
    ...
    6. Notification tool (:class:`SendNotification`) — always attached;
       it resolves the caller's WhatsApp / SMTP credentials from storage
       at call time.
    7. Visual publishing tool (:class:`PublishVisual`) — always attached;
       it pushes ``CustomEvent(name='visual')`` onto the session's event
       stream via the message bus.
    8. Caller-supplied extras (``extra_factory``)
    ...
    tools += await background_task_manager.list_tools(session_id=session_record.id)
    if session_record.config.chat_model_config is not None:
        tool_groups.append(ToolGroup(name='schedule_tools', description='Tools for managing cron schedules. A cron schedule is a recurring task that fires at a specified time — at that point, a new session is created and an agent will be invoked to complete the given task autonomously.\n\n## When to Use This Tool Group\n- When you need to create a new cron schedule that triggers at a specific time or interval"\n- When you\'re asked to list, inspect, stop, or delete existing cron schedules\n', tools=await scheduler_manager.list_tools(user_id=user_id, agent_id=agent_record.id, chat_model_config=session_record.config.chat_model_config)))
    team_tool_kwargs: dict[str, Any] = {'storage': storage, 'message_bus': message_bus, 'workspace_manager': workspace_manager, 'user_id': user_id, 'session_id': session_record.id, 'agent_id': agent_record.id}
    ...
    if session_record.team_id is not None:
        ...
        if team is not None:
            team_role = 'leader' if team.session_id == session_record.id else 'worker'
    if team_role == 'worker':
        tools.append(TeamSay(**team_tool_kwargs, role='worker'))
    else:
        tools += [TeamCreate(**team_tool_kwargs), AgentCreate(**team_tool_kwargs, sub_agent_templates=sub_agent_templates or {}), TeamSay(**team_tool_kwargs, role='leader'), TeamDelete(**team_tool_kwargs)]
        visible_agents = await resource_access_service.list_resource(user_id, ResourceKind.AGENT)
        invitable_pool = [view for view in visible_agents if view.data.invite_config.invitable and (view.data.invite_config.invite_description or '').strip()]
        if invitable_pool:
    ...
    return Toolkit(tools=tools, skills_or_loaders=await workspace.list_skills(), mcps=await workspace.list_mcps(), tool_groups=tool_groups)
