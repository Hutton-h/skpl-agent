...
from ..middleware import InboxMiddleware, StateChangeMiddleware, ToolOffloadMiddleware
from ...middleware import TTSMiddleware, RAGMiddleware
from ...rag import KnowledgeBase
from .._types import AgentMiddlewareFactory, AgentToolFactory, EventProjector, SubAgentTemplate
...
from ...event import AgentEvent, ReplyStartEvent, ReplyEndEvent, ReplyFinishedReason, UserConfirmResultEvent, ExternalExecutionResultEvent, UserInterruptEvent
...
class ChatService:
    ...
    def __init__(self, storage: StorageBase, workspace_manager: WorkspaceManagerBase, scheduler_manager: SchedulerManager, background_task_manager: BackgroundTaskManager, message_bus: MessageBus, resource_access_service: ResourceAccessService, knowledge_base_manager: KnowledgeBaseManagerBase | None=None, extra_agent_middlewares: AgentMiddlewareFactory | None=None, extra_agent_tools: AgentToolFactory | None=None, custom_subagent_templates: dict[str, SubAgentTemplate] | None=None, custom_agent_cls: type[Agent] | None=None, extra_projectors: list[EventProjector] | None=None) -> None:
        ...
        Args:
            ...
            custom_subagent_templates (`dict[str, SubAgentTemplate] | None`,             optional):
        ...
        self._projectors: list[EventProjector] = [SubagentHitlProjector(storage), *(extra_projectors or [])]

    async def run(self, user_id: str, session_id: str, agent_id: str, input_msg: Msg | list[Msg] | UserConfirmResultEvent | ExternalExecutionResultEvent | UserInterruptEvent | None=None) -> None:
        ...
        try:
            ...
            logger.exception('ChatService.run failed for user_id=%s session_id=%s agent_id=%s, error=%s', user_id, session_id, agent_id, str(e))
    ...
    async def interrupt(self, user_id: str, session_id: str, agent_id: str) -> None:
        ...
        if await self._message_bus.is_locked(MessageBusKeys.session_lock(session_id)):
            await self._message_bus.publish(MessageBusKeys.session_interrupt_channel(), {'session_id': session_id})
            return
        await enqueue_run_trigger(self._message_bus, user_id=user_id, session_id=session_id, agent_id=agent_id, kind=MessageBusKeys.WAKEUP_KIND_RESUME, inputs=UserInterruptEvent(reply_id=session.state.reply_id))

    async def _run_impl(self, user_id: str, session_id: str, agent_id: str, input_msg: Msg | list[Msg] | UserConfirmResultEvent | ExternalExecutionResultEvent | UserInterruptEvent | None) -> None:
        ...
        session_record = await self._storage.get_session(user_id, agent_id, session_id)
        if session_record is None:
            raise HTTPException(status_code=404, detail=f'Session {session_id!r} not found for agent {agent_id!r}.')
        workspace = await self._workspace_manager.get_workspace(user_id, agent_id, session_id, session_record.config.workspace_id)
        if workspace.workdir not in session_record.state.permission_context.working_directories:
            session_record.state.permission_context.working_directories[workspace.workdir] = AdditionalWorkingDirectory(path=workspace.workdir, source='session')
        middlewares: list = [InboxMiddleware(self._message_bus), StateChangeMiddleware(message_bus=self._message_bus, session_id=session_id), ToolOffloadMiddleware(bg_manager=self._background_task_manager, message_bus=self._message_bus, user_id=user_id, agent_id=agent_id)]
        if self._extra_agent_middlewares is not None:
            middlewares.extend(await self._extra_agent_middlewares(user_id, agent_id, session_id))
        tts_cfg = session_record.config.tts_model_config
        if tts_cfg is not None:
            tts_model = await get_tts_model(user_id, tts_cfg, self._access)
            middlewares.append(TTSMiddleware(tts_model))
        kb_cfg = session_record.config.knowledge_config
        if kb_cfg is not None and kb_cfg.knowledge_base_ids and (self._knowledge_base_manager is not None):
            knowledges: list[KnowledgeBase] = []
            for kb_id in kb_cfg.knowledge_base_ids:
                try:
                    kb_record = await self._access.resolve_knowledge_base(user_id, kb_id)
                    knowledge = await self._knowledge_base_manager.get_knowledge(kb_record.user_id, kb_id)
                except Exception:
                    logger.exception('Skipping knowledge base %r for session %r: failed to resolve runtime handle.', kb_id, session_id)
                    continue
                knowledges.append(knowledge)
            if knowledges:
                middlewares.append(RAGMiddleware(knowledge_bases=knowledges, parameters=RAGMiddleware.Parameters(**kb_cfg.parameters or {})))
        toolkit = await get_toolkit(storage=self._storage, workspace=workspace, workspace_manager=self._workspace_manager, scheduler_manager=self._scheduler_manager, background_task_manager=self._background_task_manager, message_bus=self._message_bus, middlewares=middlewares, user_id=user_id, agent_record=agent_record, session_record=session_record, resource_access_service=self._access, extra_factory=self._extra_agent_tools, sub_agent_templates=self._sub_agent_templates)
        model_cfg = session_record.config.chat_model_config
        if not model_cfg:
            raise HTTPException(status_code=404, detail=f'No model configuration found for agent {agent_id}')
        model = await get_model(user_id, model_cfg, self._access)
        fallback_cfg = session_record.config.fallback_chat_model_config
        fallback_model = await get_model(user_id, fallback_cfg, self._access) if fallback_cfg is not None else None
        agent_state = session_record.state
        agent_state.session_id = session_id
        agent = self._agent_cls(name=agent_record.data.name, system_prompt=agent_record.data.system_prompt, model=model, toolkit=toolkit, model_config=ModelConfig(fallback_model=fallback_model), context_config=agent_record.data.context_config, react_config=agent_record.data.react_config, plan_config=agent_record.data.plan_config, state=agent_state, middlewares=middlewares, offloader=workspace)
        if input_msg is None and agent.state.context:
            last_msg = agent.state.context[-1]
            if last_msg.role == 'assistant' and last_msg.name == agent.name:
                awaiting = [tc for tc in last_msg.get_content_blocks('tool_call') if tc.state in (ToolCallState.ASKING, ToolCallState.SUBMITTED)]
                if awaiting:
                    logger.info('Skipping wake-up for session %s: agent is parked on %d awaiting tool call(s); inbox messages will be drained when the agent resumes.', session_id, len(awaiting))
                    return
        lock_key = MessageBusKeys.session_lock(session_id)
        events_key = MessageBusKeys.session_events(session_id)
        async with self._message_bus.acquire_lock(lock_key, ttl_secs=MessageBusKeys.SESSION_RUN_TTL_SECS):
            reply_msg: Msg | None = None
            try:
                if input_msg is None or isinstance(input_msg, (Msg, list)):
                    if isinstance(input_msg, (Msg, list)):
                        input_msgs = [input_msg] if isinstance(input_msg, Msg) else input_msg
                    ...
                    async for event in agent.reply_stream(inputs=input_msg):
                        if isinstance(event, ReplyStartEvent):
                            reply_msg = AssistantMsg(id=event.reply_id, name=event.name, content=[])
                        elif reply_msg is not None:
                            reply_msg.append_event(event)
                        try:
                            await publish_session_event(self._message_bus, session_id, event.model_dump(mode='json'))
                            await self._project_event(user_id, session_record, agent_record, event)
                        except asyncio.CancelledError:
                            current = asyncio.current_task()
                            if current is not None:
                                current.cancel()
                else:
                    reply_msg = await self._storage.get_message(user_id, session_id, agent.state.reply_id)
                    if reply_msg is None:
                        logger.warning('Reply message %r not found in storage for session %r; tool-call state changes from the incoming event will not be persisted.', agent.state.reply_id, session_id)
                    elif input_msg:
                ...
                if reply_msg is not None and reply_msg.finished_reason is None:
                    end_event = ReplyEndEvent(session_id=session_id, reply_id=reply_msg.id, finished_reason=ReplyFinishedReason.ERROR, error=_classify_error(e))
                    reply_msg.append_event(end_event)
                    await publish_session_event(self._message_bus, session_id, end_event.model_dump(mode='json'))
                raise
            finally:
                ...
                async def _persist() -> None:
                    ...
                    await self._storage.update_session_state(user_id=user_id, agent_id=agent_id, session_id=session_id, state=agent.state)
                    # NOTE: Do NOT call log_trim(events_key) here — it deletes the entire
                    # replay log when before_id is None, causing SSE clients to receive 0
                    # events. The log_append call in publish_session_event() already enforces
                    # SESSION_REPLAY_MAX_LEN (1000) to prevent unbounded growth.
    ...
    async def _project_event(self, user_id: str, session_record: SessionRecord, agent_record: AgentRecord, event: AgentEvent) -> None:
        ...
        for projector in self._projectors:
            try:
                await projector.maybe_project(user_id, session_record, agent_record, event, self._projection)
            except Exception as e:
                logger.warning('Projector %s failed on event %s from session %s: %s', type(projector).__name__, type(event).__name__, session_record.id, str(e))