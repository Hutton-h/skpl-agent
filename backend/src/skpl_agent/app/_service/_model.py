...
async def get_model(user_id: str, config: ChatModelConfig, access: ResourceAccessService) -> ChatModelBase:
    ...
    parameters = model_cls.Parameters(**config.parameters) if config.parameters else None
    return model_cls(credential=credential, model=config.model, parameters=parameters)