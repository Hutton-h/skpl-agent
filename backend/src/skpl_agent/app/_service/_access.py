...
from ..access import ResourceAccessPolicyBase, ResourceKind, ResourcePermission, ResourceRef
from ..storage import AgentRecord, CredentialRecord, EmbeddingModelConfig, KnowledgeBaseRecord, StorageBase
...
class AgentView(AgentRecord):
    ...
    """
    editable: bool = Field(description='Whether the current viewer may PATCH/DELETE this agent.')
...
class CredentialView(CredentialRecord):
    ...
    """
    editable: bool = Field(description='Whether the current viewer may PATCH/DELETE this credential.')
...
class KnowledgeBaseView(BaseModel):
    ...
    description: str = Field(default='', description='Free-form description of the knowledge base.')
    embedding_model_config: EmbeddingModelConfig = Field(description='Embedding model configuration pinned at creation.')
    created_at: datetime = Field(description='Creation timestamp.')
    updated_at: datetime = Field(description='Last-update timestamp.')
    editable: bool = Field(description='Whether the current viewer may modify this knowledge base.')
    is_public: bool = Field(default=False, description='Whether this knowledge base is publicly visible.')
    user_id: str = Field(default='', description='The owner user id.')
...
class ResourceAccessService:
    ...
    def __init__(self, storage: StorageBase, policy: ResourceAccessPolicyBase) -> None:
    ...
    @overload
    async def list_resource(self, viewer_id: str, kind: Literal[ResourceKind.CREDENTIAL]) -> list[CredentialView]:
        ...

    @overload
    async def list_resource(self, viewer_id: str, kind: Literal[ResourceKind.AGENT]) -> list[AgentView]:
        ...

    @overload
    async def list_resource(self, viewer_id: str, kind: Literal[ResourceKind.KNOWLEDGE_BASE]) -> list[KnowledgeBaseView]:
        ...

    async def list_resource(self, viewer_id: str, kind: ResourceKind) -> list[BaseModel]:
        ...
        if kind is ResourceKind.CREDENTIAL:
            ...
            own = [record for record in await self._storage.list_agents(viewer_id) if record.source != 'team']
        else:
            own = await self._storage.list_knowledge_bases(viewer_id)
        views: list[BaseModel] = [self._build_view(record, viewer_id, True) for record in own]
        ...
        for ref in await self._list_refs(viewer_id, kind):
            ...
            if isinstance(record, AgentRecord) and record.source == 'team':
                continue
            views.append(self._build_view(record, viewer_id, ref.permission == ResourcePermission.EDIT))
            seen.add(key)
        return views

    @overload
    async def get_resource(self, viewer_id: str, kind: Literal[ResourceKind.CREDENTIAL], resource_id: str) -> CredentialView:
        ...

    @overload
    async def get_resource(self, viewer_id: str, kind: Literal[ResourceKind.AGENT], resource_id: str) -> AgentView:
        ...

    @overload
    async def get_resource(self, viewer_id: str, kind: Literal[ResourceKind.KNOWLEDGE_BASE], resource_id: str) -> KnowledgeBaseView:
        ...

    async def get_resource(self, viewer_id: str, kind: ResourceKind, resource_id: str) -> BaseModel:
        ...
        for ref in await self._list_refs(viewer_id, kind):
            ...
            if isinstance(record, AgentRecord) and record.source == 'team':
                continue
            return self._build_view(record, viewer_id, ref.permission == ResourcePermission.EDIT)
        raise self._not_found(kind, resource_id)

    async def resolve_credential(self, viewer_id: str, credential_id: str) -> CredentialRecord:
        ...
        for ref in await self._list_refs(viewer_id, ResourceKind.CREDENTIAL):
            if ref.resource_id != credential_id:
                continue
            record = await self._storage.get_credential(ref.owner_id, ref.resource_id)
            if record is not None:
                return record
        raise self._not_found(ResourceKind.CREDENTIAL, credential_id)

    async def resolve_agent(self, viewer_id: str, agent_id: str) -> AgentRecord:
        ...
        for ref in await self._list_refs(viewer_id, ResourceKind.AGENT):
            if ref.resource_id != agent_id:
                continue
            record = await self._storage.get_agent(ref.owner_id, ref.resource_id)
            if record is not None and record.source != 'team':
                return record
        raise self._not_found(ResourceKind.AGENT, agent_id)

    async def resolve_knowledge_base(self, viewer_id: str, knowledge_base_id: str) -> KnowledgeBaseRecord:
        ...
        record = await self._storage.get_knowledge_base(viewer_id, knowledge_base_id)
        if record is not None:
            return record
        for ref in await self._list_refs(viewer_id, ResourceKind.KNOWLEDGE_BASE):
            if ref.resource_id != knowledge_base_id:
                continue
            record = await self._storage.get_knowledge_base(ref.owner_id, ref.resource_id)
            if record is not None:
                return record
        raise self._not_found(ResourceKind.KNOWLEDGE_BASE, knowledge_base_id)

    async def resolve_for_edit(self, viewer_id: str, kind: ResourceKind, resource_id: str) -> tuple[str, Any]:
        ...
        own = await self._get_owned(kind, viewer_id, resource_id)
        if own is not None:
            # For public resources, verify the viewer is the actual owner
            if hasattr(own, 'user_id') and own.user_id != viewer_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"{kind.value.replace('_', ' ').title()} '{resource_id}' is read-only for this viewer.",
                )
        ...
        for ref in await self._list_refs(viewer_id, kind):
            ...
            if ref.permission != ResourcePermission.EDIT:
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"{kind.value.replace('_', ' ').title()} '{resource_id}' is read-only for this viewer.")
            return (ref.owner_id, record)
        raise self._not_found(kind, resource_id)

    async def _get_owned(self, kind: ResourceKind, owner_id: str, resource_id: str) -> Any:
    ...
    def _build_view(self, record: Any, viewer_id: str, editable: bool) -> BaseModel:
        ...
        if isinstance(record, CredentialRecord):
            ...
            if record.user_id != viewer_id:
                payload['data'] = {k: payload['data'][k] for k in ('type', 'name') if k in payload['data']}
            payload['editable'] = editable
            return CredentialView.model_validate(payload)
        if isinstance(record, AgentRecord):
            return AgentView.model_validate({**record.model_dump(), 'editable': editable})
        if isinstance(record, KnowledgeBaseRecord):
            # For knowledge bases, editable only if the viewer is the actual owner
            # Public KBs from other users are visible but not editable
            is_owner = record.user_id == viewer_id
            return KnowledgeBaseView.model_validate({**record.model_dump(), 'editable': editable and is_owner})
        return KnowledgeBaseView.model_validate({**record.model_dump(), 'editable': editable})

    async def _list_refs(self, viewer_id: str, kind: ResourceKind) -> list[ResourceRef]:
        """Return policy refs for ``kind`` and drop mismatched refs."""
        refs = await self._policy.list_accessible(viewer_id, kind, self._storage)
    ...
    def _not_found(kind: ResourceKind, resource_id: str) -> HTTPException:
        ...
        return HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"{label.title()} '{resource_id}' not found.")