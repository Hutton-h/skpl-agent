...
class SessionService:
    ...
    async def get_session_status(self, user_id: str, agent_id: str, session_id: str) -> SessionStatus | None:
    ...
    async def cancel_session_run(self, session_id: str, *, timeout: float=10.0) -> bool:
        ...
        Returns:
            `bool`:
                ...
                expired.
        """
        await self._bus.publish(MessageBusKeys.session_cancel_channel(), {'session_id': session_id})
        deadline = asyncio.get_event_loop().time() + timeout
        while True:
            if not await self._bus.is_locked(MessageBusKeys.session_lock(session_id)):
                return True
            if asyncio.get_event_loop().time() >= deadline:
                logger.warning('Session %s did not release its run-lock within %.1fs after cancel; proceeding anyway.', session_id, timeout)
                return False
        ...
        for session in await self._storage.list_sessions(user_id, agent_id):
            if session.team_id is not None:
                ...
                if team is not None and team.session_id != session.id:
                    members = await _ensure_team_members(self._storage, user_id, team)
                    filtered = [m for m in members if m.session_id != session.id]
    ...
    async def delete_schedule(self, user_id: str, schedule_id: str) -> bool:
        ...
        """
        for session in await self._storage.list_sessions_by_schedule(user_id, schedule_id):
    ...
    async def _team_worker_session_ids(self, user_id: str, agent_id: str, session_id: str) -> list[str]:
    ...
    async def _cancel_runs(self, session_ids: list[str]) -> None:
        ...
        if not session_ids:
            return
        await asyncio.gather(*(self.cancel_session_run(sid) for sid in session_ids))
    ...
    async def _purge_bus(self, session_ids: list[str]) -> None:
        ...
        if not session_ids:
            return
        await asyncio.gather(*(self._purge_session_bus(sid) for sid in session_ids))
    ...
    async def _purge_subagent_hitl(self, user_id: str, agent_id: str, session_id: str) -> None:
        ...
        try:
            ...
            if team.session_id == session_id:
                ...
                await SubagentHitlProjector.drop_worker(self._projection, team.session_id, session_id)
        except Exception as e:
            logger.warning('Failed to purge subagent HITL projection for session %s: %s', session_id, str(e))