...
def _build_parser_registry(parsers: 'list[ParserBase] | dict[str, ParserBase]') -> 'dict[str, ParserBase]':
    ...
    if isinstance(parsers, dict):
        for (media_type, parser) in parsers.items():
            ...
            if declared and media_type not in declared:
                logger.warning('Parser %s registered for media type %r but it only declares %s — proceeding with the caller-supplied mapping.', type(parser).__name__, media_type, list(declared))
        return dict(parsers)
    ...
    for parser in parsers:
        for media_type in parser.supported_media_types:
            ...
            if previous is not None and previous is not parser:
                logger.warning('Parser %s overrides %s for media type %r — later entries in `parsers` win.  Pass a `dict[str, ParserBase]` if you want explicit routing.', type(parser).__name__, type(previous).__name__, media_type)
...
class IndexWorker:
    ...
    def __init__(self, storage: 'StorageBase', blob_store: 'BlobStoreBase', knowledge_base_manager: 'KnowledgeBaseManagerBase', parsers: 'list[ParserBase] | dict[str, ParserBase]', chunker: 'ChunkerBase', node_id: str, max_concurrency: int=4, lease_ttl: timedelta=timedelta(seconds=90), parser_executor: ProcessPoolExecutor | None=None) -> None:
    ...
    async def process(self, user_id: str, knowledge_base_id: str, document_id: str) -> None:
        ...
        """
        acquired = await self._storage.acquire_knowledge_document_lease(user_id=user_id, knowledge_base_id=knowledge_base_id, document_id=document_id, processing_node=self._node_id, lease_ttl=self._lease_ttl)
        if not acquired:
            logger.debug('Skipping %s — another worker holds the lease.', document_id)
            return
        pipeline_task = asyncio.create_task(self._guarded_pipeline(user_id, knowledge_base_id, document_id), name=f'pipeline:{document_id}')
        heartbeat_task = asyncio.create_task(self._heartbeat(user_id, knowledge_base_id, document_id), name=f'lease-renew:{document_id}')
        try:
            await asyncio.wait({pipeline_task, heartbeat_task}, return_when=asyncio.FIRST_COMPLETED)
            if not pipeline_task.done():
                pipeline_task.cancel()
                with contextlib.suppress(asyncio.CancelledError, Exception):
                    await pipeline_task
                raise RuntimeError(f'Lost lease on {document_id} during processing; another worker has taken over.')
            heartbeat_task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await heartbeat_task
            await pipeline_task
        except Exception as exc:
            await self._mark_error(user_id, knowledge_base_id, document_id, exc)
        finally:
            await self._storage.release_knowledge_document_lease(user_id=user_id, knowledge_base_id=knowledge_base_id, document_id=document_id, processing_node=self._node_id)

    async def _guarded_pipeline(self, user_id: str, knowledge_base_id: str, document_id: str) -> None:
        """Run the throttled pipeline inside the per-worker semaphore."""
        async with self._sem:
            await self._run_pipeline(user_id, knowledge_base_id, document_id)

    async def _run_pipeline(self, user_id: str, knowledge_base_id: str, document_id: str) -> None:
        """Walk the document through parse → chunk → index."""
        record = await self._storage.get_knowledge_document(user_id, knowledge_base_id, document_id)
        if record is None:
        ...
        if not media_type:
            raise ValueError(f'Cannot determine media type for {data.filename!r}.')
        parser = self._parsers_by_media_type.get(media_type)
        if parser is None:
            raise ValueError(f'No parser registered for media type {media_type!r}.')
        await self._storage.update_knowledge_document_status(user_id, knowledge_base_id, document_id, 'parsing')
        file_bytes = await self._read_blob(data.blob_uri)
        sections = await self._parse(parser, file_bytes, data.filename)
        await self._storage.update_knowledge_document_status(user_id, knowledge_base_id, document_id, 'chunking')
        chunks = await self._chunker.chunk(sections)
        await self._storage.update_knowledge_document_status(user_id, knowledge_base_id, document_id, 'indexing')
        knowledge = await self._manager.get_knowledge(user_id, knowledge_base_id)
        await knowledge.insert_document(chunks=chunks, document_id=document_id, document_metadata={'filename': data.filename, 'media_type': media_type, 'size_bytes': data.size})
        await self._storage.update_knowledge_document_status(user_id, knowledge_base_id, document_id, 'ready', chunk_count=len(chunks))

    async def _parse(self, parser: 'ParserBase', file_bytes: bytes, filename: str) -> 'list[Section]':
        ...
        return await loop.run_in_executor(self._parser_executor, _run_parser_sync, parser, file_bytes, filename)
    ...
    async def _heartbeat(self, user_id: str, knowledge_base_id: str, document_id: str) -> None:
        ...
        while True:
            try:
                ...
                return
            ok = await self._storage.renew_knowledge_document_lease(user_id=user_id, knowledge_base_id=knowledge_base_id, document_id=document_id, processing_node=self._node_id, lease_ttl=self._lease_ttl)
            if not ok:
                logger.warning('Lost lease on %s while processing.', document_id)
                return

    async def _mark_error(self, user_id: str, knowledge_base_id: str, document_id: str, exc: BaseException) -> None:
        """Persist a sanitised error and mark the document failed."""
        logger.exception('Indexing failed for %s/%s', knowledge_base_id, document_id, exc_info=exc)
        message = _sanitise_error(exc)
        try:
            await self._storage.update_knowledge_document_status(user_id, knowledge_base_id, document_id, 'error', error=message)
        except Exception:
            logger.exception('Failed to persist error status for %s', document_id)

def _run_parser_sync(parser: 'ParserBase', file_bytes: bytes, filename: str) -> 'list[Section]':
