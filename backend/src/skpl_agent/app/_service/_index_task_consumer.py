...
class IndexTaskConsumer:
    ...
    def __init__(self, message_bus: 'MessageBus', worker: 'IndexWorker', max_batch: int=32) -> None:
    ...
    async def __aenter__(self) -> Self:
        ...
        """
        ready = asyncio.Event()
        self._task = asyncio.create_task(self._loop(ready), name='index-task-consumer')
        await ready.wait()
    ...
    async def _loop(self, ready: asyncio.Event) -> None:
        ...
        """
        try:
            async for _signal in self._bus.subscribe(MessageBusKeys.index_tasks_signal(), on_ready=ready.set):
                await self._drain_and_dispatch()
        except Exception:
            logger.exception('IndexTaskConsumer loop crashed; subscription ended.')
        finally:
            ready.set()
    ...
    async def _drain_and_dispatch(self) -> None:
        ...
        try:
            entries = await self._bus.queue_drain(MessageBusKeys.index_tasks_queue(), max_count=self._max_batch)
        except Exception:
        ...
        for (_entry_id, payload) in entries:
            try:
                ...
                logger.warning('IndexTaskConsumer: skipping malformed entry %r', payload)
                continue
            self._spawn(user_id=user_id, knowledge_base_id=knowledge_base_id, document_id=document_id)

    def _spawn(self, *, user_id: str, knowledge_base_id: str, document_id: str) -> None:
        ...
        """
        task = asyncio.create_task(self._worker.process(user_id=user_id, knowledge_base_id=knowledge_base_id, document_id=document_id), name=f'index-task:{knowledge_base_id}:{document_id}')
    ...
    def _on_done(self, task: asyncio.Task[Any]) -> None:
        ...
        exc = task.exception()
        if exc is not None:
            logger.exception('IndexTaskConsumer: worker.process(%s) raised', task.get_name(), exc_info=exc)