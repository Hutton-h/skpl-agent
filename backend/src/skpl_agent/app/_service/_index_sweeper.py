...
class IndexSweeper:
    ...
    def __init__(self, storage: 'StorageBase', message_bus: 'MessageBus', interval: timedelta=timedelta(seconds=60), pending_grace: timedelta=timedelta(minutes=5)) -> None:
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(self._loop(), name='kb-index-sweeper')
    ...
    async def _sweep_once(self) -> None:
        ...
        seen: set[str] = set()
        stuck = await self._storage.list_knowledge_documents_with_expired_lease(now=now)
        orphans = await self._storage.list_knowledge_documents_pending_since(threshold=pending_threshold)
        for record in (*stuck, *orphans):
            if record.id in seen:
                continue
            seen.add(record.id)
            try:
                await enqueue_index_task(self._bus, user_id=record.user_id, knowledge_base_id=record.knowledge_base_id, document_id=record.id)
            except Exception:
                logger.exception('Failed to re-enqueue document %s', record.id)
        if seen:
