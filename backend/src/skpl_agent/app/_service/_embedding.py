...
def build_embedding_model(credential_record: CredentialRecord, config: EmbeddingModelConfig) -> EmbeddingModelBase:
    ...
    if credential_cls is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Provider {config.type!r} not found.')
    embedding_cls = credential_cls.get_embedding_model_class()
    if embedding_cls is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Provider {config.type!r} does not support embedding models.')
    context_size: int | None = None
    for card in embedding_cls.list_models():
        if card.name == config.model:
            context_size = card.context_size
            break
    parameters = embedding_cls.Parameters(**config.parameters) if config.parameters else None
    kwargs: dict = {'credential': credential, 'model': config.model, 'dimensions': config.dimensions, 'parameters': parameters}
    if context_size is not None:
        kwargs['context_size'] = context_size
    # Detect Voyage AI and disable encoding_format (Voyage only accepts base64)
    base_url = getattr(credential, 'base_url', '') or ''
    if 'voyageai.com' in base_url:
        kwargs['pass_encoding_format'] = False
        kwargs['pass_dimensions'] = False
    return embedding_cls(**kwargs)

async def get_embedding_model(user_id: str, config: EmbeddingModelConfig, access: ResourceAccessService) -> EmbeddingModelBase:
    ...
    Raises:
        `HTTPException`:
            ...
            400 if the provider does not support embedding.
    """
    credential_record = await access.resolve_credential(user_id, config.credential_id)
