...
from ..rag.knowledge_base_manager import DimensionPolicyError, KnowledgeBaseNotFoundError
from ..storage import KnowledgeDocumentData, KnowledgeDocumentRecord
...
if TYPE_CHECKING:
    ...
    from ..storage import EmbeddingModelConfig, KnowledgeBaseRecord, StorageBase
...
class KnowledgeBaseService:
    ...
    def __init__(self, storage: 'StorageBase', knowledge_base_manager: 'KnowledgeBaseManagerBase', blob_store: 'BlobStoreBase', message_bus: 'MessageBus', resource_access_service: 'ResourceAccessService') -> None:
    ...
    async def create_knowledge_base(self, user_id: str, name: str, description: str, embedding_model_config: 'EmbeddingModelConfig', is_public: bool = False) -> 'KnowledgeBaseRecord':
        ...
        """
        try:
            return await self._manager.create_knowledge_base(user_id=user_id, name=name, description=description, embedding_model_config=embedding_model_config, is_public=is_public)
    ...
    async def delete_knowledge_base(self, user_id: str, knowledge_base_id: str) -> None:
        ...
        """
        owner_id = await self._require_edit(user_id, knowledge_base_id)
        documents = await self._storage.list_knowledge_documents(owner_id, knowledge_base_id)
        for document in documents:
            await self._delete_blob_quietly(document.data.blob_uri)
        deleted = await self._manager.delete_knowledge_base(owner_id, knowledge_base_id)
        if not deleted:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Knowledge base {knowledge_base_id!r} not found.')

    async def register_document(self, user_id: str, knowledge_base_id: str, filename: str, stream: IO[bytes], size: int, content_type: str | None=None) -> KnowledgeDocumentRecord:
        ...
        blob_uri = await self._blob_store.write_stream(key=f'kb/{knowledge_base_id}/{document_id}', stream=stream)
        record = KnowledgeDocumentRecord(id=document_id, user_id=owner_id, knowledge_base_id=knowledge_base_id, data=KnowledgeDocumentData(filename=filename, size=size, content_type=content_type, blob_uri=blob_uri))
        try:
            stored = await self._storage.upsert_knowledge_document(owner_id, record)
        except Exception:
            await self._delete_blob_quietly(blob_uri)
            raise
        await enqueue_index_task(self._bus, user_id=owner_id, knowledge_base_id=knowledge_base_id, document_id=document_id)
        return stored

    async def list_documents(self, user_id: str, knowledge_base_id: str) -> list[KnowledgeDocumentRecord]:
        ...
        Raises:
            `HTTPException`:
                ...
                caller.
        """
        record = await self._access.resolve_knowledge_base(user_id, knowledge_base_id)
        return await self._storage.list_knowledge_documents(record.user_id, knowledge_base_id)

    async def get_document_status(self, user_id: str, knowledge_base_id: str, document_ids: list[str]) -> list[KnowledgeDocumentRecord]:
        ...
        for document_id in document_ids:
            record_doc = await self._storage.get_knowledge_document(record.user_id, knowledge_base_id, document_id)
    ...
    async def delete_document(self, user_id: str, knowledge_base_id: str, document_id: str) -> None:
        ...
        """
        owner_id = await self._require_edit(user_id, knowledge_base_id)
        record = await self._storage.get_knowledge_document(owner_id, knowledge_base_id, document_id)
        if record is None:
            return
        knowledge = await self._resolve_knowledge(user_id, knowledge_base_id)
        await knowledge.delete_document(document_id)
        await self._storage.delete_knowledge_document(owner_id, knowledge_base_id, document_id)
        await self._delete_blob_quietly(record.data.blob_uri)

    async def search(self, user_id: str, knowledge_base_id: str, query: str, top_k: int=5) -> 'list[VectorSearchResult]':
    ...
    async def _require_edit(self, user_id: str, knowledge_base_id: str) -> str:
        ...
        readable.
        """
        (owner_id, _) = await self._access.resolve_for_edit(user_id, ResourceKind.KNOWLEDGE_BASE, knowledge_base_id)
        return owner_id

    async def _resolve_knowledge(self, user_id: str, knowledge_base_id: str) -> 'object':
        ...
        record = await self._access.resolve_knowledge_base(user_id, knowledge_base_id)
        try:
            return await self._manager.get_knowledge(record.user_id, knowledge_base_id)
    ...
    async def _delete_blob_quietly(self, blob_uri: str) -> None:
        ...
        try:
            ...
            logger.exception('Failed to delete blob %s', blob_uri)