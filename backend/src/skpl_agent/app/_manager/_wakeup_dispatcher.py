...
from ...event import ExternalExecutionResultEvent, UserConfirmResultEvent, UserInterruptEvent
...
_RESUME_INPUT_ADAPTER: TypeAdapter = TypeAdapter(UserConfirmResultEvent | ExternalExecutionResultEvent | UserInterruptEvent)
...
class WakeupDispatcher:
    ...
    def __init__(self, message_bus: 'MessageBus', storage: 'StorageBase', chat_service: 'ChatService', chat_run_registry: 'ChatRunRegistry') -> None:
        """Bind dependencies.
    ...
    async def __aenter__(self) -> Self:
        ...
        """
        ready = asyncio.Event()
        self._task = asyncio.create_task(self._loop(ready), name='wakeup-dispatcher')
        await ready.wait()
    ...
    async def _loop(self, ready: asyncio.Event) -> None:
        ...
        """
        try:
            async for _signal in self._bus.subscribe(MessageBusKeys.wakeup_signal(), on_ready=ready.set):
                await self._drain_and_dispatch()
        except Exception:
            logger.exception('WakeupDispatcher loop crashed; subscription ended.')

    async def _drain_and_dispatch(self) -> None:
        """Read up to a batch of trigger entries and dispatch each."""
        try:
            raw_entries = await self._bus.queue_drain(MessageBusKeys.wakeup_queue(), max_count=64)
        ...
        for payload in entries:
            try:
                ...
                logger.warning('WakeupDispatcher: skipping malformed trigger entry %r', payload)
                continue
            kind = payload.get('kind', MessageBusKeys.WAKEUP_KIND_WAKE)
            await self._dispatch_one(user_id=user_id, session_id=session_id, agent_id=agent_id, kind=kind, raw_input=payload.get('input'))

    async def _dispatch_one(self, user_id: str, session_id: str, agent_id: str, kind: str, raw_input: dict | None) -> None:
        ...
        if is_resume:
            if raw_input is None:
                logger.warning('WakeupDispatcher: dropping resume trigger for session %s — no input event carried.', session_id)
                return
            try:
                input_msg = _RESUME_INPUT_ADAPTER.validate_python(raw_input)
            except Exception:
                logger.exception('WakeupDispatcher: dropping resume trigger for session %s — input event failed to parse: %r', session_id, raw_input)
                return
        if await self._bus.is_locked(MessageBusKeys.session_lock(session_id)):
            if is_resume:
                self._schedule_resume_retry(user_id, session_id, agent_id, input_msg)
            return
        if await self._storage.get_session(user_id, agent_id, session_id) is None:
            logger.warning('WakeupDispatcher: dropping %s trigger for session %s (agent %s, user %s) — session no longer exists in storage; it was likely enqueued before the session was deleted.', kind, session_id, agent_id, user_id)
            return
        try:
            self._registry.spawn(self._chat_service.run(user_id=user_id, session_id=session_id, agent_id=agent_id, input_msg=input_msg), session_id=session_id, name=f'{kind}-run:{session_id}')
        except RuntimeError:
            if is_resume:
                self._schedule_resume_retry(user_id, session_id, agent_id, input_msg)
            else:
                logger.debug('WakeupDispatcher: skipping wake trigger for session %s; a local run is already registered.', session_id)

    def _schedule_resume_retry(self, user_id: str, session_id: str, agent_id: str, input_msg: UserConfirmResultEvent | ExternalExecutionResultEvent | None) -> None:
        ...
        async def _retry() -> None:
            try:
                ...
                await enqueue_run_trigger(self._bus, user_id=user_id, session_id=session_id, agent_id=agent_id, kind=MessageBusKeys.WAKEUP_KIND_RESUME, inputs=input_msg)
            except asyncio.CancelledError:
                pass
            except Exception:
                logger.exception('WakeupDispatcher: failed to re-enqueue resume trigger for session %s.', session_id)
        task = asyncio.create_task(_retry(), name=f'resume-retry:{session_id}')
