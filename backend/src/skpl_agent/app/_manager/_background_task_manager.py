...
from skpl_agent._logging import logger
...
class _ToolStopParams(BaseModel):
    ...
    task_id: str = Field(description='The task id of the background tool to stop.')

class ToolStop(ToolBase):
    """A tool to stop a running background tool execution."""
    name: str = 'ToolStop'
    'The tool name.'
    description: str = 'Stop a background tool execution by its task id. Use this when you want to cancel a previously offloaded tool that is still running in the background.'
    ...
    def __init__(self, background_tasks: dict[str, BackgroundTask], message_bus: 'MessageBus', session_id: str) -> None:
    ...
    async def __call__(self, task_id: str) -> ToolChunk:
        ...
        if local_task is not None and local_task.session_id == self._session_id:
            self.background_tasks.pop(task_id, None)
            local_task.asyncio_task.cancel()
            logger.info('Background task stopped via ToolStop (local): task_id=%s, session_id=%s, agent_id=%s', task_id, local_task.session_id, local_task.agent_id)
            return ToolChunk(content=[TextBlock(text=f'Task {task_id} stopped successfully.')], state=ToolResultState.SUCCESS)
        if await self._message_bus.registry_exists(MessageBusKeys.bg_tasks(self._session_id), task_id):
            await self._message_bus.publish(MessageBusKeys.task_cancel_channel(), {'task_id': task_id})
            logger.info('Background task cancel broadcast via ToolStop (remote): task_id=%s, session_id=%s', task_id, self._session_id)
            return ToolChunk(content=[TextBlock(text=f'Cancel request sent for task {task_id}. The owning worker will stop it shortly.')], state=ToolResultState.SUCCESS)
        return ToolChunk(content=[TextBlock(text=f'TaskNotFoundError: The task {task_id} does not exist.')], state=ToolResultState.ERROR)
...
class BackgroundTaskManager:
    ...
    async def register_task(self, asyncio_task: asyncio.Task, session_id: str, agent_id: str, user_id: str, tool_name: str='') -> str:
        ...
        """
        bg_task = BackgroundTask(asyncio_task=asyncio_task, session_id=session_id, agent_id=agent_id, user_id=user_id, tool_name=tool_name)
        task_id = bg_task.id
        self.tasks[task_id] = bg_task
        metadata = json.dumps({'tool_name': tool_name, 'agent_id': agent_id, 'started_at': time.time()}, ensure_ascii=False)
        await self._message_bus.registry_set(MessageBusKeys.bg_tasks(session_id), task_id, metadata, ttl_secs=MessageBusKeys.BG_TASKS_TTL_SECS)
        logger.info('Background task registered: task_id=%s, session_id=%s, agent_id=%s, tool_name=%s', task_id, session_id, agent_id, tool_name)

        def _on_done(_t: asyncio.Task) -> None:
            self.tasks.pop(task_id, None)
            try:
                asyncio.ensure_future(self._safe_bg_task_unregister(session_id, task_id))
            except RuntimeError:
                pass
        asyncio_task.add_done_callback(_on_done)
        return task_id

    async def _safe_bg_task_unregister(self, session_id: str, task_id: str) -> None:
        ...
        """
        try:
            await self._message_bus.registry_del(MessageBusKeys.bg_tasks(session_id), task_id)
        except Exception as e:
            logger.exception('Failed to unregister background task from the global registry: task_id=%s, session_id=%s, error=%s', task_id, session_id, str(e))
    ...
    def cancel_session_tasks(self, session_id: str) -> int:
        ...
        for bg_task in list(self.tasks.values()):
            if bg_task.session_id != session_id:
                continue
            logger.info('Cancelling background task for session cancel: task_id=%s, session_id=%s, agent_id=%s', bg_task.id, bg_task.session_id, bg_task.agent_id)
    ...
    def cancel_task(self, task_id: str) -> bool:
        ...
        if bg_task is None:
            return False
        logger.info('Cancelling background task via bus signal: task_id=%s, session_id=%s, agent_id=%s', task_id, bg_task.session_id, bg_task.agent_id)
        bg_task.asyncio_task.cancel()
        return True
    ...
    async def __aexit__(self, *exc: object) -> None:
        ...
        logger.info('Shutting down BackgroundTaskManager: cancelling %d task(s).', count)
        for bg_task in list(self.tasks.values()):
            logger.info('Cancelling background task on shutdown: task_id=%s, session_id=%s, agent_id=%s', bg_task.id, bg_task.session_id, bg_task.agent_id)
            bg_task.asyncio_task.cancel()
        self.tasks.clear()