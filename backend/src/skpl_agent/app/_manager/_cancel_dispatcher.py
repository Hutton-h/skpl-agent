...
class CancelDispatcher:
    ...
    def __init__(self, message_bus: 'MessageBus', registry: 'ChatRunRegistry', bg_manager: 'BackgroundTaskManager') -> None:
        """Bind dependencies.
    ...
    async def __aenter__(self) -> Self:
        ...
        self._session_task = asyncio.create_task(self._session_cancel_loop(session_ready), name='cancel-dispatcher:session')
        self._task_cancel_task = asyncio.create_task(self._task_cancel_loop(task_ready), name='cancel-dispatcher:task')
        self._interrupt_task = asyncio.create_task(self._interrupt_loop(interrupt_ready), name='cancel-dispatcher:interrupt')
    ...
    async def __aexit__(self, *exc: object) -> None:
        ...
        for task in (self._session_task, self._task_cancel_task, self._interrupt_task):
            if task is None:
                continue
            task.cancel()
            try:
    ...
    async def _session_cancel_loop(self, ready: asyncio.Event) -> None:
        ...
        """
        try:
            async for payload in self._bus.subscribe(MessageBusKeys.session_cancel_channel(), on_ready=ready.set):
                sid = payload.get('session_id')
                if isinstance(sid, str):
                    self._cancel_session(sid)
        except Exception:
            logger.exception('CancelDispatcher session-cancel loop crashed.')
        finally:
            ready.set()
    ...
    def _cancel_session(self, session_id: str) -> None:
        ...
        if task is not None and (not task.done()):
            logger.info('CancelDispatcher: cancelling local chat run for session %s', session_id)
            task.cancel()
        bg_cancelled = self._bg_manager.cancel_session_tasks(session_id)
        if bg_cancelled:
            logger.info('CancelDispatcher: cancelled %d local BG task(s) for session %s', bg_cancelled, session_id)
    ...
    async def _task_cancel_loop(self, ready: asyncio.Event) -> None:
        ...
        """
        try:
            async for payload in self._bus.subscribe(MessageBusKeys.task_cancel_channel(), on_ready=ready.set):
                task_id = payload.get('task_id')
                if not isinstance(task_id, str):
                    continue
                cancelled = self._bg_manager.cancel_task(task_id)
                if cancelled:
                    logger.info('CancelDispatcher: cancelled local BG task %s via task-level broadcast.', task_id)
        except Exception:
            logger.exception('CancelDispatcher task-cancel loop crashed.')
        finally:
            ready.set()
    ...
    async def _interrupt_loop(self, ready: asyncio.Event) -> None:
        ...
        """
        try:
            async for payload in self._bus.subscribe(MessageBusKeys.session_interrupt_channel(), on_ready=ready.set):
                sid = payload.get('session_id')
                if isinstance(sid, str):
                    self._interrupt_session(sid)
        except Exception:
            logger.exception('CancelDispatcher interrupt loop crashed.')
        finally:
            ready.set()
