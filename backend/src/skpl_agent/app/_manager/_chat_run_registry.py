...
class ChatRunRegistry:
    ...
    def spawn(self, coro: Coroutine, *, session_id: str, name: str | None=None) -> asyncio.Task:
        ...
        if existing is not None and (not existing.done()):
            raise RuntimeError(f'Session {session_id!r} already has an active chat run in this process.')
        task = asyncio.create_task(coro, name=name or f'chat-run:{session_id}')
    ...
    async def __aexit__(self, *exc: object) -> None:
        ...
        if not self._tasks:
            return
        logger.info('ChatRunRegistry shutdown: cancelling %d in-flight chat run(s).', len(self._tasks))
