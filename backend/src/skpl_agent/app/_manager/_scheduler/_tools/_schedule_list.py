...
class ScheduleList(ToolBase):
    ...
    description: str = 'List all scheduled tasks for the current user. Shows schedule ID, name, cron expression, timezone, next run time, enabled/disabled status, and whether the schedule is stateful.'
    ...
    async def __call__(self) -> ToolChunk:
        ...
        if not records:
            return ToolChunk(content=[TextBlock(text='No scheduled tasks found.')], state=ToolResultState.SUCCESS)
        next_run_map: dict[str, str] = {job.id: str(job.next_run_time) for job in self._scheduler.get_jobs()}
        ...
        for record in records:
            ...
            lines.append(f'- [{enabled_str}] {record.data.name!r}  (ID: {record.id})\n  Cron:      {record.data.cron_expression} ({record.data.timezone})\n  Next run:  {next_run}\n  Stateful:  {record.data.stateful}  |  Agent: {record.agent_id}\n  Source:    {record.data.source.value}\n')
        return ToolChunk(content=[TextBlock(text='\n'.join(lines))], state=ToolResultState.SUCCESS)