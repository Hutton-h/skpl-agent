...
class ScheduleView(ToolBase):
    ...
    description: str = 'View the full details of a scheduled task by its schedule ID, including cron expression, timezone, stateful flag, permission mode, and the next scheduled run time.'
    ...
    def __init__(self, user_id: str, scheduler: Any, storage: StorageBase) -> None:
    ...
    async def __call__(self, schedule_id: str) -> ToolChunk:
        ...
        if record is None:
            return ToolChunk(content=[TextBlock(text=f'ScheduleNotFoundError: Schedule with id {schedule_id!r} not found.')], state=ToolResultState.ERROR)
        job = self._scheduler.get_job(schedule_id)
        next_run = str(job.next_run_time) if job is not None else 'not in scheduler (may be disabled)'
        enabled_str = 'enabled' if record.data.enabled else 'disabled'
        text = f"Schedule ID:     {record.id}\nName:            {record.data.name}\nDescription:     {record.data.description or '(none)'}\nStatus:          {enabled_str}\nCron:            {record.data.cron_expression} (timezone: {record.data.timezone})\nNext run:        {next_run}\nStateful:        {record.data.stateful}\nPermission mode: {record.data.permission_mode.value}\nSource:          {record.data.source.value}\nSource session:  {record.data.source_session_id or '(none)'}\nAgent ID:        {record.agent_id}\nCreated at:      {record.created_at}\nUpdated at:      {record.updated_at}\n"
