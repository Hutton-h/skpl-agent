...
class _ScheduleDeleteParams(BaseModel):
    ...
    schedule_id: str = Field(description='The schedule ID to delete (permanently remove).')
...
class ScheduleDelete(ToolBase):
    ...
    description: str = 'Permanently delete a scheduled task by its schedule ID. After this call the task will no longer be executed and its record will be deleted from storage.'
    ...
    def __init__(self, user_id: str, scheduler: Any, storage: StorageBase, message_bus: MessageBus) -> None:
    ...
    async def __call__(self, schedule_id: str) -> ToolChunk:
        ...
        session_service = SessionService(storage=self._storage, message_bus=self._message_bus)
        deleted = await session_service.delete_schedule(self._user_id, schedule_id)
        if not deleted:
            return ToolChunk(content=[TextBlock(text=f'ScheduleNotFoundError: Schedule with id {schedule_id!r} not found in storage.')], state=ToolResultState.ERROR)
        return ToolChunk(content=[TextBlock(text=f'Schedule {schedule_id!r} has been permanently deleted.')], state=ToolResultState.SUCCESS)