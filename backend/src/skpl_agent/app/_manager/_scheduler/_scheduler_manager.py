...
from ...storage import StorageBase, ScheduleRecord, ChatModelConfig, SessionConfig, SessionSource
...
class SchedulerManager:
    ...
    def __init__(self, storage: StorageBase, message_bus: MessageBus) -> None:
    ...
    def _build_trigger(self, record: ScheduleRecord) -> Callable[[], Coroutine]:
        ...
        async def _trigger() -> None:
            logger.info('[Schedule:%s(%s)] Trigger fired', record.id, record.data.name)
            if not record.data.enabled:
                logger.info('[Schedule:%s(%s)] Skipped — schedule disabled', record.id, record.data.name)
                return
            try:
                if record.data.stateful:
                    stateful_session_id = f'{record.id}_stateful'
                    logger.info('[Schedule:%s(%s)] Stateful mode, looking up session %s', record.id, record.data.name, stateful_session_id)
                    session = await storage.get_session(record.user_id, record.agent_id, stateful_session_id)
                    if session is None:
                        logger.info('[Schedule:%s(%s)] First fire, creating stateful session', record.id, record.data.name)
                        state = AgentState()
                        state.permission_context = PermissionContext(mode=record.data.permission_mode)
                        session_config = SessionConfig(workspace_id='', chat_model_config=record.data.chat_model_config)
                        session = await storage.upsert_session(user_id=record.user_id, agent_id=record.agent_id, config=session_config, state=state, session_id=stateful_session_id, source=SessionSource.SCHEDULE, source_schedule_id=record.id)
                    else:
                        logger.info('[Schedule:%s(%s)] Reusing existing stateful session %s', record.id, record.data.name, session.id)
                else:
                    logger.info('[Schedule:%s(%s)] Non-stateful mode, creating fresh session', record.id, record.data.name)
                    state = AgentState()
                    state.permission_context = PermissionContext(mode=record.data.permission_mode)
                    session = await storage.upsert_session(user_id=record.user_id, agent_id=record.agent_id, config=SessionConfig(workspace_id='', chat_model_config=record.data.chat_model_config), state=state, source=SessionSource.SCHEDULE, source_schedule_id=record.id)
                logger.info('[Schedule:%s(%s)] Session ready: %s, delivering prompt via inbox + wakeup', record.id, record.data.name, session.id)
                hint = HintBlock(hint=f'<scheduled-task>\n{record.data.description}\n</scheduled-task>', source=json.dumps({'label': 'schedule', 'sublabel': record.data.name}, ensure_ascii=False))
                await message_bus.queue_push(MessageBusKeys.inbox(session.id), hint.model_dump(mode='json'))
                await enqueue_run_trigger(message_bus, user_id=record.user_id, session_id=session.id, agent_id=record.agent_id)
                logger.info('[Schedule:%s(%s)] Wakeup enqueued for session %s', record.id, record.data.name, session.id)
            except Exception:
                logger.exception('[Schedule:%s(%s)] Trigger failed', record.id, record.data.name)
        return _trigger
    ...
    async def register_schedule(self, record: ScheduleRecord) -> str:
        ...
        logger.info('Registering schedule %s(%s) cron=%s tz=%s', record.id, record.data.name, record.data.cron_expression, record.data.timezone)
        fields = record.data.cron_expression.split()
        if len(fields) != 5:
            raise ValueError(f'Expected a 5-field cron expression, got {record.data.cron_expression!r}')
        (minute, hour, day, month, day_of_week) = fields
        trigger = self._build_trigger(record)
        job = self._scheduler.add_job(trigger, trigger=CronTrigger(minute=minute, hour=hour, day=day, month=month, day_of_week=day_of_week, timezone=record.data.timezone, start_date=record.data.started_at, end_date=record.data.ended_at), id=record.id, name=record.data.name, misfire_grace_time=300)
        logger.info('Schedule %s(%s) registered, next_run=%s', record.id, record.data.name, job.next_run_time)
        return job.id
    ...
    async def restore(self, records: list[ScheduleRecord]) -> None:
        ...
        logger.info('Restoring schedules: %d total, %d enabled', len(records), len(enabled))
    ...
    async def list_tasks(self) -> list[dict]:
        ...
        """
        return [{'id': job.id, 'name': job.name, 'next_run': job.next_run_time} for job in self._scheduler.get_jobs()]

    async def list_tools(self, user_id: str, agent_id: str, chat_model_config: ChatModelConfig) -> list[ToolBase]:
        ...
        Returns:
            `list[ToolBase]`:
                ...
                :class:`ScheduleList`.
        """
        return [ScheduleCreate(user_id=user_id, agent_id=agent_id, chat_model_config=chat_model_config, storage=self._storage, scheduler_manager=self), ScheduleView(user_id=user_id, scheduler=self._scheduler, storage=self._storage), ScheduleDelete(user_id=user_id, scheduler=self._scheduler, storage=self._storage, message_bus=self._message_bus), ScheduleList(user_id=user_id, scheduler=self._scheduler, storage=self._storage)]