...
class DashScopeCredential(CredentialBase):
    ...
    api_key: SecretStr = Field(description='The DashScope API key.', title='API Key')
    base_url: str = Field(default=_DASHSCOPE_BASE_URL, title='API Base URL', description='The base URL for the DashScope OpenAI-compatible API endpoint.')
    ...
    def get_tts_model_classes(cls) -> list[Type['TTSModelBase']]:
        ...
        from ..tts import DashScopeCosyVoiceTTSModel, DashScopeRealtimeTTSModel, DashScopeTTSModel
