...
class CredentialFactory:
    ...
    """
    _classes: list[Type[CredentialBase]] = [AnthropicCredential, DashScopeCredential, DeepSeekCredential, GeminiCredential, MoonshotCredential, OllamaCredential, OpenAICredential, SMTPCredential, WhatsAppCloudCredential, XAICredential]
    _adapter: TypeAdapter[CredentialBase] | None = None

    @classmethod
    def _get_adapter(cls) -> TypeAdapter[CredentialBase]:
        if cls._adapter is None:
            union = Annotated[Union[tuple(cls._classes)], Field(discriminator='type')]
