...
def _build_streaming_wav_header(sample_rate: int=24000, channels: int=1, bits_per_sample: int=16) -> bytes:
    ...
    return b'RIFF' + struct.pack('<I', 4294967295) + b'WAVE' + b'fmt ' + struct.pack('<I', 16) + struct.pack('<HHIIHH', 1, channels, sample_rate, byte_rate, block_align, bits_per_sample) + b'data' + struct.pack('<I', 4294967295)