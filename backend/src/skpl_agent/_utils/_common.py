...
def set_id_factory(factory: Callable[[], str]) -> None:
    ...
    if not callable(factory):
        raise TypeError(f'factory must be a callable, got {type(factory).__name__}')
    global _id_factory
    _id_factory = factory
...
def _json_loads_with_repair(json_str: str, schema: dict | None=None) -> dict:
    ...
    try:
        ...
        if isinstance(res, dict):
            return res
        error_message = f'Error: Your argument string is decoded into a {type(res)} object, but a dict object is expected!'
    except json.JSONDecodeError as e:
        error_message = f'Error: When decoding your tool arguments from JSON format to a Python dictionary, a JSONDecodeError was raised with message: {str(e)}.'
    try:
...
async def _is_async_func(func: Callable) -> bool:
    ...
    """
    return inspect.iscoroutinefunction(func) or inspect.isasyncgenfunction(func) or isinstance(func, types.CoroutineType) or (isinstance(func, types.GeneratorType) and asyncio.iscoroutine(func)) or (isinstance(func, functools.partial) and await _is_async_func(func.func))

async def _execute_async_or_sync_func(func: Callable, *args: Any, **kwargs: Any) -> Any:
...
def _get_bytes_from_web_url(url: str, max_retries: int=3) -> str:
    ...
    for _ in range(max_retries):
        try:
            ...
            logger.info('Failed to fetch bytes from URL %s. Error %s. Retrying...', url, str(e))
    raise RuntimeError(f'Failed to fetch bytes from URL `{url}` after {max_retries} retries.')
...
def _flatten_json_schema(schema: dict) -> dict:
    ...
    """
    has_defs = isinstance(schema.get('$defs'), dict) or isinstance(schema.get('definitions'), dict)
    if not has_defs:
        return schema
    ...
    def _resolve_ref(obj: Any, visited: frozenset=frozenset()) -> Any:
        ...
        if '$ref' in obj:
            ref_path = obj['$ref']
            if isinstance(ref_path, str) and (ref_path.startswith('#/$defs/') or ref_path.startswith('#/definitions/')):
                def_name = ref_path.split('/')[-1]
                if def_name in visited:
                    logger.warning("Circular reference detected for '%s' in tool schema", def_name)
                    return {'type': 'object', 'description': f'(circular: {def_name})'}
                if def_name in defs:
                    resolved = _resolve_ref(defs[def_name], visited | {def_name})
                    for (key, value) in obj.items():
                        if key != '$ref':
                            resolved[key] = _resolve_ref(value, visited | {def_name})
                    return resolved
            return obj
        result: dict[str, Any] = {}
        for (key, value) in obj.items():
