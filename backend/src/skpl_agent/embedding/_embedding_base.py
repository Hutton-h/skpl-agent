...
class EmbeddingModelBase(Generic[InputT]):
    ...
    def __init__(self, credential: CredentialBase, model: str, dimensions: int | None, parameters: BaseModel | None, context_size: int, batch_size: int, max_retries: int, retry_delay: float) -> None:
        ...
        if dimensions is None:
            if legacy_dimensions is None:
                raise ValueError('dimensions is required: pass it explicitly to EmbeddingModelBase.__init__ or include it in the legacy `parameters` mapping.')
    ...
    async def __call__(self, inputs: list[InputT], **kwargs: Any) -> EmbeddingResponse:
        ...
        """
        if not inputs:
            return EmbeddingResponse(embeddings=[], usage=EmbeddingUsage(tokens=0, time=0))
        normalized: list[Any] = [item.text if isinstance(item, TextBlock) else item for item in inputs]
        batches = [normalized[i:i + self.batch_size] for i in range(0, len(normalized), self.batch_size)]
        if len(batches) > 1:
            logger.info('Embedding %d inputs in %d batches (batch_size=%d) for model %s.', len(normalized), len(batches), self.batch_size, self.model)
    ...
    def _merge_responses(responses: list[EmbeddingResponse]) -> EmbeddingResponse:
        ...
        return EmbeddingResponse(embeddings=all_embeddings, usage=EmbeddingUsage(tokens=total_tokens, time=total_time), source='api')

    async def _call_with_retry(self, inputs: list[Any], **kwargs: Any) -> EmbeddingResponse:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    logger.warning('Batch attempt %d failed for embedding model %s: %s. Retrying in %.1fs...', attempt + 1, self.model, str(e), self.retry_delay)
                    await asyncio.sleep(self.retry_delay)
                else:
                    logger.warning('All %d attempt(s) failed for a batch of embedding model %s.', self.max_retries + 1, self.model)
        if last_error is not None:
            raise last_error
        raise RuntimeError(f'Failed to call embedding model {self.model} after {self.max_retries + 1} retries.')
    ...
    @classmethod
    def list_models(cls, custom_yaml_dir: str | None=None) -> list[EmbeddingModelCard]:
        ...
        for yaml_file in yaml_files:
            try:
                card = EmbeddingModelCard.from_yaml(yaml_path=str(yaml_file), parameter_class=cls.Parameters)
                model_cards.append(card)
            except Exception as e:
                logger.warning('Failed to load embedding model card %s: %s', yaml_file, str(e))
                continue
        return model_cards