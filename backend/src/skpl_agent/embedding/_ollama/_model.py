...
class OllamaEmbeddingModel(EmbeddingModelBase[str | TextBlock]):
    ...
    def __init__(self, credential: CredentialBase, model: str, dimensions: int | None, parameters: 'OllamaEmbeddingModel.Parameters | None'=None, embedding_cache: EmbeddingCacheBase | None=None, context_size: int=8192, max_retries: int=3, retry_delay: float=1.0) -> None:
        ...
        Args:
            ...
            parameters (`OllamaEmbeddingModel.Parameters | None`,             defaults to ``None``):
    ...
    async def _call_api(self, inputs: list[str], **kwargs: Any) -> EmbeddingResponse:
        ...
        """
        api_kwargs: dict[str, Any] = {'input': inputs, 'model': self.model, 'dimensions': self.dimensions, **kwargs}
        ...
        if self.embedding_cache:
            await self.embedding_cache.store(identifier=api_kwargs, embeddings=response.embeddings)
        return EmbeddingResponse(embeddings=response.embeddings, usage=EmbeddingUsage(time=time))