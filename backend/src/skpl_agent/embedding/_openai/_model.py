...
class OpenAIEmbeddingModel(EmbeddingModelBase[str | TextBlock]):
    ...
    def __init__(self, credential: CredentialBase, model: str, dimensions: int | None, parameters: 'OpenAIEmbeddingModel.Parameters | None'=None, pass_dimensions: bool=True, pass_encoding_format: bool=True, embedding_cache: EmbeddingCacheBase | None=None, context_size: int=8191, max_retries: int=3, retry_delay: float=1.0) -> None:
        ...
        Args:
            ...
            parameters (`OpenAIEmbeddingModel.Parameters | None`,             defaults to ``None``):
            ...
            pass_encoding_format (`bool`, defaults to `True`):
                Whether to pass the ``encoding_format`` parameter to the API.
                Set to ``False`` for Voyage AI and other providers that only
        ...
        self.client: openai.AsyncClient = openai.AsyncClient(api_key=credential.api_key.get_secret_value(), **client_kwargs)
        self.pass_dimensions = pass_dimensions
        self.pass_encoding_format = pass_encoding_format
    ...
    async def _call_api(self, inputs: list[str], **kwargs: Any) -> EmbeddingResponse:
        ...
        """
        api_kwargs: dict[str, Any] = {'input': inputs, 'model': self.model, **kwargs}
        if self.pass_encoding_format:
            api_kwargs['encoding_format'] = 'float'
        ...
        for (pos, item) in enumerate(response.data):
            ...
            if 0 <= index < len(inputs):
                embeddings[index] = item.embedding or getattr(item, 'dense_embedding', None)
        if self.embedding_cache:
