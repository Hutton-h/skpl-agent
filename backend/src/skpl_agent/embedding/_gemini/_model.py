...
_MODEL_LIMITS: dict[str, _MultimodalLimits] = {'gemini-embedding-2': _MultimodalLimits(max_elements=20, max_images=6, max_videos=1, max_audios=1, max_pdfs=1)}
...
class GeminiEmbeddingModel(EmbeddingModelBase[str | TextBlock | DataBlock]):
    ...
    def __init__(self, credential: CredentialBase, model: str, dimensions: int | None, parameters: 'GeminiEmbeddingModel.Parameters | None'=None, embedding_cache: EmbeddingCacheBase | None=None, context_size: int=8192, max_retries: int=3, retry_delay: float=1.0) -> None:
        ...
        Args:
            ...
            parameters (`GeminiEmbeddingModel.Parameters | None`,             defaults to ``None``):
        ...
        self.client: genai.Client = genai.Client(api_key=credential.api_key.get_secret_value())
    ...
    async def __call__(self, inputs: list[str | TextBlock | DataBlock], **kwargs: Any) -> EmbeddingResponse:
        ...
        if len(batches) > 1:
            logger.info('Embedding %d multimodal inputs in %d batches for model %s.', len(normalized), len(batches), self.model)
    ...
    def _split_multimodal_batches(self, inputs: list[str | DataBlock]) -> list[list[str | DataBlock]]:
        ...
        for item in inputs:
            ...
            would_exceed = n_elem + 1 > limits.max_elements or (is_img and n_img + 1 > limits.max_images) or (is_vid and n_vid + 1 > limits.max_videos) or (is_aud and n_aud + 1 > limits.max_audios) or (is_pdf and n_pdf + 1 > limits.max_pdfs)
    ...
    async def _call_text(self, inputs: list[str | DataBlock], **kwargs: Any) -> EmbeddingResponse:
        ...
        for item in inputs:
            ...
            texts.append(item)
        config = types.EmbedContentConfig(output_dimensionality=self.dimensions, **kwargs)
        cache_key = {'model': self.model, 'contents': texts, 'output_dimensionality': self.dimensions, **kwargs}
        ...
        if self.embedding_cache:
            await self.embedding_cache.store(identifier=cache_key, embeddings=embeddings)
        return EmbeddingResponse(embeddings=embeddings, usage=EmbeddingUsage(time=time))
    ...
    async def _call_multimodal(self, inputs: list[str | DataBlock], **kwargs: Any) -> EmbeddingResponse:
        ...
        for item in inputs:
            if isinstance(item, str):
                contents.append(types.Content(parts=[types.Part.from_text(text=item)]))
            elif isinstance(item, DataBlock):
                contents.append(types.Content(parts=[self._data_block_to_part(item)]))
            else:
                raise ValueError(f'Invalid input: {item!r}. Expected str or DataBlock.')
        config = types.EmbedContentConfig(output_dimensionality=self.dimensions, **kwargs)
        start_time = datetime.now()
        response = self.client.models.embed_content(model=self.model, contents=contents, config=config)
        time = (datetime.now() - start_time).total_seconds()
        embeddings = [item.values for item in response.embeddings]
        return EmbeddingResponse(embeddings=embeddings, usage=EmbeddingUsage(time=time))
    ...
    def _data_block_to_part(block: DataBlock) -> Any:
        ...
        if isinstance(source, Base64Source):
            import base64
            return types.Part.from_bytes(data=base64.b64decode(source.data), mime_type=source.media_type)
        if isinstance(source, URLSource):
            raise ValueError(f'Gemini embedding API requires inline data (Base64Source). URLSource is not directly supported for embedding. Got URL: {source.url}')
        raise ValueError(f'Unsupported source type {type(source).__name__} in DataBlock.')