...
_MULTIMODAL_PREFIXES = ('multimodal-embedding-', 'tongyi-embedding-vision-', 'qwen3-vl-embedding', 'qwen2.5-vl-embedding')
...
_MODEL_LIMITS: dict[str, _MultimodalLimits] = {'qwen3-vl-embedding': _MultimodalLimits(max_elements=20, max_images=5, max_videos=1), 'qwen2.5-vl-embedding': _MultimodalLimits(max_elements=20, max_images=5, max_videos=1), 'tongyi-embedding-vision-plus': _MultimodalLimits(max_elements=20, max_images=64, max_videos=8), 'tongyi-embedding-vision-flash': _MultimodalLimits(max_elements=20, max_images=64, max_videos=8), 'multimodal-embedding-v1': _MultimodalLimits(max_elements=20, max_images=1, max_videos=1)}
_DEFAULT_LIMITS = _MultimodalLimits(max_elements=20, max_images=1, max_videos=1)
...
class DashScopeEmbeddingModel(EmbeddingModelBase[str | TextBlock | DataBlock]):
    ...
    def __init__(self, credential: CredentialBase, model: str, dimensions: int | None, parameters: 'DashScopeEmbeddingModel.Parameters | None'=None, embedding_cache: EmbeddingCacheBase | None=None, context_size: int=8192, max_retries: int=3, retry_delay: float=1.0) -> None:
        ...
        Args:
            ...
            parameters (`DashScopeEmbeddingModel.Parameters | None`,             defaults to ``None``):
        ...
        super().__init__(credential=credential, model=model, dimensions=dimensions, parameters=parameters, context_size=context_size, batch_size=self._TEXT_BATCH_SIZE, max_retries=max_retries, retry_delay=retry_delay)
    ...
    async def __call__(self, inputs: list[str | TextBlock | DataBlock], **kwargs: Any) -> EmbeddingResponse:
        ...
        """
        normalized: list[str | DataBlock] = [item.text if isinstance(item, TextBlock) else item for item in inputs]
        if not self._is_multimodal:
            return await super().__call__(normalized, **kwargs)
        batches = self._split_multimodal_batches(normalized)
        if len(batches) > 1:
            logger.info('Embedding %d multimodal inputs in %d batches for model %s (limits: elements=%d, images=%d, videos=%d).', len(normalized), len(batches), self.model, self._limits.max_elements, self._limits.max_images, self._limits.max_videos)
        results: list[EmbeddingResponse] = await asyncio.gather(*(self._call_with_retry(batch, **kwargs) for batch in batches))
        return self._merge_responses(results)

    def _split_multimodal_batches(self, inputs: list[str | DataBlock]) -> list[list[str | DataBlock]]:
        ...
        for item in inputs:
            ...
            would_exceed = n_elements + 1 > limits.max_elements or (is_image and n_images + 1 > limits.max_images) or (is_video and n_videos + 1 > limits.max_videos)
    ...
    async def _call_api(self, inputs: list[str | DataBlock], **kwargs: Any) -> EmbeddingResponse:
    ...
    async def _call_text(self, inputs: list[str | DataBlock], **kwargs: Any) -> EmbeddingResponse:
        ...
        for item in inputs:
            if not isinstance(item, str):
                raise ValueError(f'Text embedding model {self.model!r} only accepts str inputs, got {type(item).__name__}.')
            texts.append(item)
        api_kwargs: dict[str, Any] = {'input': texts, 'model': self.model, 'dimension': self.dimensions, **kwargs}
        if self.embedding_cache:
            cached = await self.embedding_cache.retrieve(identifier=api_kwargs)
            if cached:
                return EmbeddingResponse(embeddings=cached, usage=EmbeddingUsage(tokens=0, time=0), source='cache')
        import dashscope
        start_time = datetime.now()
        response = dashscope.embeddings.TextEmbedding.call(api_key=self.api_key, **api_kwargs)
        time = (datetime.now() - start_time).total_seconds()
        if response.status_code != 200:
            raise RuntimeError(f'DashScope text embedding API error: {response}')
        embeddings = [entry['embedding'] for entry in response.output['embeddings']]
        if self.embedding_cache:
            await self.embedding_cache.store(identifier=api_kwargs, embeddings=embeddings)
        return EmbeddingResponse(embeddings=embeddings, usage=EmbeddingUsage(tokens=response.usage['total_tokens'], time=time))

    async def _call_multimodal(self, inputs: list[str | DataBlock], **kwargs: Any) -> EmbeddingResponse:
        ...
        for item in inputs:
            if isinstance(item, str):
                ...
                raise ValueError(f'Invalid input: {item!r}. Expected str or DataBlock.')
        api_kwargs: dict[str, Any] = {'input': formatted, 'model': self.model, 'api_key': self.api_key, **kwargs}
        cache_identifier = {k: v for (k, v) in api_kwargs.items() if k != 'api_key'}
        if self.embedding_cache:
            cached = await self.embedding_cache.retrieve(identifier=cache_identifier)
            if cached:
                return EmbeddingResponse(embeddings=cached, usage=EmbeddingUsage(tokens=0, time=0), source='cache')
        import dashscope
        start_time = datetime.now()
        ...
        if res.status_code != 200:
            raise RuntimeError(f'DashScope multimodal embedding API error: {res}')
        embeddings = [entry['embedding'] for entry in res.output['embeddings']]
        if self.embedding_cache:
            await self.embedding_cache.store(identifier=cache_identifier, embeddings=embeddings)
        return EmbeddingResponse(embeddings=embeddings, usage=EmbeddingUsage(tokens=res.usage.get('image_tokens', 0) + res.usage.get('input_tokens', 0), time=time), source='api')
    ...
    def _format_data_block(block: DataBlock) -> dict[str, str]:
        ...
        if media_type.startswith('video/'):
            if not isinstance(source, URLSource):
                raise ValueError(f'Multimodal embedding API only supports URL input for video data, got {type(source).__name__}.')
            return {'video': str(source.url)}
        if media_type.startswith('image/'):
            if isinstance(source, Base64Source):
                return {'image': f'data:{media_type};base64,{source.data}'}
            if isinstance(source, URLSource):
                return {'image': str(source.url)}
        raise ValueError(f'Unsupported media type {media_type!r} in DataBlock. Expected image/* or video/*.')