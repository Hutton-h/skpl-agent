...
class FileEmbeddingCache(EmbeddingCacheBase):
    ...
    def __init__(self, cache_dir: str='./.cache/embeddings', max_file_number: int | None=None, max_cache_size: int | None=None) -> None:
    ...
    async def store(self, embeddings: List[Embedding], identifier: JSONSerializableObject, overwrite: bool=False, **kwargs: Any) -> None:
        ...
        if os.path.exists(path_file):
            if not os.path.isfile(path_file):
                raise RuntimeError(f'Path {path_file} exists but is not a file.')
            if overwrite:
    ...
    async def _maintain_cache_dir(self) -> None:
        ...
        maximum size."""
        files = [(_.name, _.stat().st_mtime) for _ in os.scandir(self.cache_dir) if _.is_file() and _.name.endswith('.npy')]
        files.sort(key=lambda x: x[1])
        if self.max_file_number and len(files) > self.max_file_number:
            for (file_name, _) in files[:0 - self.max_file_number]:
                os.remove(os.path.join(self.cache_dir, file_name))
                logger.info('Remove cached embedding file %s for limited number of files (%d).', file_name, self.max_file_number)
            files = files[0 - self.max_file_number:]
        if self.max_cache_size is not None and self._get_cache_size() > self.max_cache_size:
            removed_files = []
            for (filename, _) in files:
                ...
                if self._get_cache_size() <= self.max_cache_size:
                    break
            if removed_files:
                logger.info('Remove %d cached embedding file(s) for limited cache size (%d MB).', len(removed_files), self.max_cache_size)