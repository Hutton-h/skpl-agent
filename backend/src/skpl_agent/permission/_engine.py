...
class PermissionEngine:
    ...
    async def _check_default(self, tool: ToolBase, tool_input: dict[str, Any]) -> PermissionDecision:
        ...
        if ask:
            ask.suggested_rules = await self._generate_suggestions(tool, tool_input)
            return ask
        ...
        if allow:
            return allow
        default = PermissionDecision(behavior=PermissionBehavior.ASK, message=f'Permission required for {tool.name}', decision_reason=f'Mode: {self.context.mode.value}')
        default.suggested_rules = await self._generate_suggestions(tool, tool_input)
        return default

    async def _check_explore(self, tool: ToolBase, tool_input: dict[str, Any]) -> PermissionDecision:
        ...
        if read_only:
            return read_only
        return PermissionDecision(behavior=PermissionBehavior.DENY, message=f'Permission denied for {tool.name} (explore mode is read-only)', decision_reason='Explore mode does not allow modifications')

    async def _check_accept_edits(self, tool: ToolBase, tool_input: dict[str, Any]) -> PermissionDecision:
    ...
    async def _check_bypass(self, tool: ToolBase, tool_input: dict[str, Any]) -> PermissionDecision:
        ...
        if allow:
            return allow
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message=f'Permission granted for {tool.name} (bypass mode)', decision_reason='Bypass mode allows all operations')

    async def _check_dont_ask(self, tool: ToolBase, tool_input: dict[str, Any]) -> PermissionDecision:
        ...
        if allow:
            return allow
        return PermissionDecision(behavior=PermissionBehavior.DENY, message=f'Permission denied for {tool.name} (dont_ask mode - user not available)', decision_reason='User is not available to answer permission prompts')

    @staticmethod
    def _convert_ask_to_deny(tool: ToolBase, ask_decision: PermissionDecision) -> PermissionDecision:
        ...
        """
        return PermissionDecision(behavior=PermissionBehavior.DENY, message=f'Permission denied for {tool.name} (dont_ask mode - ASK converted to DENY, user not available)', decision_reason=f'DONT_ASK mode converted ASK to DENY. Original reason: {ask_decision.decision_reason}', suggested_rules=ask_decision.suggested_rules)
    ...
    async def _check_read_only_fast_path(self, tool: ToolBase, tool_input: dict[str, Any]) -> PermissionDecision | None:
        ...
        if await tool.check_read_only(tool_input):
            return PermissionDecision(behavior=PermissionBehavior.ALLOW, message=f'Permission granted for {tool.name} (read-only invocation)', decision_reason='Read-only operations are auto-allowed')
        return None

    async def _check_deny_rules(self, tool: ToolBase, input_data: dict[str, Any]) -> PermissionDecision | None:
        ...
        for rule in rules:
            if await self._rule_matches(tool, rule, input_data):
                return PermissionDecision(behavior=PermissionBehavior.DENY, message=f'Permission to use {tool.name} has been denied', decision_reason=f'Rule: {rule.rule_content}')
        return None

    async def _check_ask_rules(self, tool: ToolBase, input_data: dict[str, Any]) -> PermissionDecision | None:
        ...
        for rule in rules:
            if await self._rule_matches(tool, rule, input_data):
                return PermissionDecision(behavior=PermissionBehavior.ASK, message=f'Permission required for {tool.name}', decision_reason=f'Rule: {rule.rule_content}')
        return None

    async def _check_allow_rules(self, tool: ToolBase, input_data: dict[str, Any]) -> PermissionDecision | None:
        ...
        for rule in rules:
            if await self._rule_matches(tool, rule, input_data):
                return PermissionDecision(behavior=PermissionBehavior.ALLOW, message=f'Permission granted for {tool.name}', updated_input=input_data)
        return None

    async def _rule_matches(self, tool: ToolBase, rule: PermissionRule, input_data: dict[str, Any]) -> bool:
        ...
        if not rule.rule_content:
            return True
        return await _execute_async_or_sync_func(tool.match_rule, rule.rule_content, input_data)

    async def _generate_suggestions(self, tool: ToolBase, tool_input: dict[str, Any]) -> List[PermissionRule]:
        ...
        Returns:
            `List[PermissionRule]`:
                ...
                compound commands)
        """
        return await _execute_async_or_sync_func(tool.generate_suggestions, tool_input)