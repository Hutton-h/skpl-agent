...
from typing import Literal, Type, Any, TypeAlias, Coroutine, AsyncGenerator, Generator, Awaitable, Callable
...
class RegisteredTool:
    ...
    def __post_init__(self) -> None:
        ...
        if self.tool.input_schema is not None:
            if not (isinstance(self.tool.input_schema, dict) and self.tool.input_schema.get('type') == 'object' and isinstance(self.tool.input_schema.get('properties'), dict)):
                raise ValueError(f'Invalid input_schema: {self.tool.input_schema}. ')

    def get_tool_schema(self, extended_model: Type[BaseModel] | None=None) -> dict:
        ...
        function_schema: dict = {'type': 'function', 'function': {'name': self.tool.name, 'description': self.tool.description, 'parameters': input_schema}}
        ...
        for (key, value) in extended_schema['properties'].items():
            if key in function_schema['function']['parameters']['properties']:
                raise ValueError(f'The field `{key}` already exists in the original function schema of `{self.tool.name}`. Try to use a different name.')
        ...
        if '$defs' in extended_schema:
            ...
            for (def_key, def_value) in extended_schema['$defs'].items():
                ...
                _remove_title_field(def_value_copy)
                if def_key in merged_params['$defs']:
                    existing_def_copy = deepcopy(merged_params['$defs'][def_key])
                    _remove_title_field(existing_def_copy)
                    if existing_def_copy != def_value_copy:
                        raise ValueError(f'The $defs key `{def_key}` conflicts with existing definition in function schema of `{self.tool.name}`.')
                    continue
                merged_params['$defs'][def_key] = def_value_copy
        return function_schema
Function: TypeAlias = Callable[..., ToolChunk] | Callable[..., Awaitable[ToolChunk]] | Callable[..., Generator[ToolChunk, None, None]] | Callable[..., AsyncGenerator[ToolChunk, None]] | Callable[..., Coroutine[Any, Any, AsyncGenerator[ToolChunk, None]]] | Callable[..., Coroutine[Any, Any, Generator[ToolChunk, None, None]]]
