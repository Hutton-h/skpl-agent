...
class SkillViewer(ToolBase):
    ...
    description = 'Retrieve a skill within the conversation. When users asks you to perform tasks, check if any of the available skills match. Skills provide specialized capabilities and domain knowledge.'
    'The tool description of the skill viewer tool to the agent.'
    input_schema: dict[str, Any] = {'type': 'object', 'properties': {'skill': {'type': 'string', 'description': 'The exact name of the skill to view. '}}, 'required': ['skill']}
    ...
    def __init__(self, get_skills_method: Callable[..., Awaitable[dict[str, Skill]]], middlewares: List[ToolMiddlewareBase] | None=None) -> None:
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message='The skill viewer is always allowed to be called.')
    ...
    async def call(self, skill: str, _agent_state: AgentState) -> ToolChunk:
        ...
        if not isinstance(_agent_state, AgentState):
            raise DeveloperOrientedException(f'Expected AgentState but got {type(_agent_state)} instead for the Skill viewer tool.')
        skills = await self._get_skills_method(_agent_state.tool_context.activated_groups)
        target_skill = skills.get(skill)
        if not target_skill:
            return ToolChunk(content=[TextBlock(text=f"SkillNotFoundError: Skill '{skill}' not found.")], state=ToolResultState.ERROR)
