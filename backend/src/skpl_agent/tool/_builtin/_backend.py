...
class BackendBase(ABC):
    ...
    async def list_dir(self, path: str, *, recursive: bool=False) -> list[str]:
        ...
        if recursive:
            ...
            command = ['find', path, '-mindepth', '1', '-maxdepth', '1', '-printf', '%f\\0']
        result = await self.exec_shell(command)
        if not result.ok():
            return []
        return [part.decode('utf-8', errors='surrogateescape') for part in result.stdout.split(b'\x00') if part]
    ...
    async def stat_mtime(self, path: str) -> float | None:
        ...
        script = f'stat -c %Y {quoted} 2>/dev/null || stat -f %m {quoted} 2>/dev/null'
...
def _subprocess_creation_kwargs() -> dict[str, Any]:
    ...
    if os.name != 'nt':
        return {}
    import subprocess
    return {'creationflags': getattr(subprocess, 'CREATE_NO_WINDOW', 134217728)}
...
class LocalBackend(BackendBase):
    ...
    async def exec_shell(self, command: list[str], *, cwd: str | None=None, timeout: float | None=None) -> ExecResult:
        ...
        if cwd is not None:
            kwargs['cwd'] = cwd
        try:
            process = await asyncio.create_subprocess_exec(*command, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, **kwargs)
