...
from ...permission import PermissionBehavior, PermissionContext, PermissionDecision, PermissionRule
...
class PowerShell(ToolBase):
    ...
    input_schema: dict[str, Any] = {'type': 'object', 'properties': {'command': {'type': 'string', 'description': 'The PowerShell command to execute.'}, 'description': {'type': 'string', 'description': 'Clear, concise description of what this command does.'}, 'timeout': {'type': 'integer', 'description': 'Optional timeout in milliseconds (default: 120000, max: 600000)', 'default': 120000, 'maximum': 600000, 'minimum': 0}}, 'required': ['command']}
    is_mcp: bool = False
    ...
    def __init__(self, cwd: str | os.PathLike[str] | None=None, middlewares: List[ToolMiddlewareBase] | None=None, backend: BackendBase | None=None) -> None:
    ...
    async def _resolve_executable(self) -> str:
        ...
        if self._executable is None:
            for candidate in _SHELL_CANDIDATES:
                probe = await self._backend.exec_shell([candidate, '-NoLogo', '-NoProfile', '-NonInteractive', '-Command', 'exit 0'], timeout=10.0)
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        """
        return PermissionDecision(behavior=PermissionBehavior.ASK, message='Execute PowerShell command', decision_reason='PowerShell command validation is not enabled')
    ...
    async def call(self, command: str, description: str='', timeout: int=120000) -> AsyncGenerator[ToolChunk, None]:
        ...
        encoded_user_command = base64.b64encode(command.encode('utf-16-le')).decode('ascii')
        powershell_script = f"$ProgressPreference = [System.Management.Automation.ActionPreference]::SilentlyContinue\n$OutputEncoding = [Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false)\n$AgentScopeCommand = [System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String('{encoded_user_command}'))\n& ([ScriptBlock]::Create($AgentScopeCommand))"
        encoded_command = base64.b64encode(powershell_script.encode('utf-16-le')).decode('ascii')
        try:
            executable = await self._resolve_executable()
            result = await self._backend.exec_shell([executable, '-NoLogo', '-NoProfile', '-NonInteractive', '-EncodedCommand', encoded_command], cwd=self._cwd, timeout=timeout_ms / 1000.0)
        except Exception as exc:
            yield ToolChunk(content=[TextBlock(text=f'Command failed: {command}\nError: {exc}')], state=ToolResultState.ERROR, is_last=True)
            return
        stdout = _normalize_newlines(result.stdout.decode('utf-8', errors='replace'))
        stderr = _normalize_newlines(result.stderr.decode('utf-8', errors='replace'))
        if result.exit_code == -1 and result.stderr == b'timed out':
            yield ToolChunk(content=[TextBlock(text=f'Command timed out after {timeout_ms}ms: {command}')], state=ToolResultState.ERROR, is_last=True)
            return
        if not result.ok():
