...
class Write(ToolBase):
    ...
    input_schema: dict[str, Any] = {'type': 'object', 'properties': {'file_path': {'type': 'string', 'description': 'The absolute path to the file to write (must be absolute, not relative)'}, 'content': {'type': 'string', 'description': 'The content to write to the file'}}, 'required': ['file_path', 'content']}
    is_mcp: bool = False
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        if self._is_dangerous_path(file_path):
            return PermissionDecision(behavior=PermissionBehavior.ASK, message=f'Permission required: Write operation on sensitive file {file_path}', decision_reason='Safety check: dangerous file or directory', bypass_immune=True)
        if context.mode in (PermissionMode.ACCEPT_EDITS, PermissionMode.DONT_ASK):
            if self._path_in_allowed_working_path(file_path, context):
                return PermissionDecision(behavior=PermissionBehavior.ALLOW, message=f'Permission granted for writing {file_path} (in working directory)', decision_reason='File is in working directory and not a dangerous path')
    ...
    async def call(self, file_path: str, content: str, _agent_state: AgentState | None=None) -> ToolChunk:
        ...
        if await self._backend.file_exists(file_path) and _agent_state is not None:
            ...
            if cache is None:
                return ToolChunk(content=[TextBlock(text=f'Error: File {file_path} exists but has not been read yet. You must read the file first before writing to it.')], state=ToolResultState.ERROR, is_last=True)
        file_existed = await self._backend.file_exists(file_path)
        previous_content = ''
        if file_existed:
            try:
                previous_content = (await self._backend.read_file(file_path)).decode('utf-8')
            except Exception:
                previous_content = ''
        parent_dir = Path(file_path).parent
        await self._backend.exec_shell(['mkdir', '-p', str(parent_dir)])
        await self._backend.write_file(file_path, content.encode('utf-8'))
        line_count = len(content.split('\n'))
        diff_text = ''.join(difflib.unified_diff(previous_content.splitlines(keepends=True), content.splitlines(keepends=True), fromfile='/dev/null' if not file_existed else f'a/{file_path}', tofile=f'b/{file_path}', n=3))
        return ToolChunk(content=[TextBlock(text=f'The file {file_path} has been written successfully ({line_count} lines).')], state=ToolResultState.RUNNING, is_last=True, metadata={'diff': diff_text, 'file_path': file_path, 'occurrences': 1})