...
class Grep(ToolBase):
    ...
    def __init__(self, middlewares: List[ToolMiddlewareBase] | None=None, backend: BackendBase | None=None) -> None:
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        return PermissionDecision(behavior=PermissionBehavior.PASSTHROUGH, message='Grep search is read-only.')
    ...
    def _apply_head_limit(self, items: list[str], limit: int | None, offset: int=0) -> tuple[list[str], int | None]:
    ...
    async def _run_ripgrep(self, args: list[str], search_path: str, timeout: int=30) -> list[str]:
        ...
        result = await self._backend.exec_shell(command, timeout=float(timeout))
        if result.exit_code == -1 and result.stderr == b'timed out':
            raise RipgrepTimeoutError(f'Ripgrep search timed out after {timeout} seconds. Try searching a more specific path or pattern.', [])
        if result.exit_code not in (0, 1):
            error_msg = result.stderr.decode('utf-8', errors='ignore').strip()
            raise RuntimeError(f'ripgrep error (code {result.exit_code}): {error_msg}')
        raw = result.stdout.decode('utf-8', errors='ignore')
        lines = [line.rstrip('\r') for line in raw.split('\n') if line.rstrip('\r')]
        return lines

    async def call(self, pattern: str, path: str | None=None, output_mode: Literal['content', 'files_with_matches', 'count']='files_with_matches', glob: str | None=None, type: str | None=None, i: bool=False, case_insensitive: bool=False, context: int | None=None, multiline: bool=False, head_limit: int | None=None, offset: int=0, n: bool=True, **kwargs: Any) -> ToolChunk:
        ...
        if head_limit is not None and head_limit < 0:
            return ToolChunk(content=[TextBlock(text='Error: head_limit must be non-negative.')], state=ToolResultState.ERROR, is_last=True)
        if offset < 0:
            return ToolChunk(content=[TextBlock(text='Error: offset must be non-negative.')], state=ToolResultState.ERROR, is_last=True)
        ...
        try:
            ...
            return ToolChunk(content=[TextBlock(text=str(e))], state=ToolResultState.ERROR, is_last=True)
        except RuntimeError as e:
            return ToolChunk(content=[TextBlock(text=str(e))], state=ToolResultState.ERROR, is_last=True)
        if not results:
            return ToolChunk(content=[TextBlock(text=f'No matches found for pattern: {pattern}')], state=ToolResultState.SUCCESS, is_last=True)
        ...
        if applied_limit is not None:
            ...
            suffix += ']'
        return ToolChunk(content=[TextBlock(text='\n'.join(limited) + suffix)], state=ToolResultState.SUCCESS, is_last=True)