...
def _match_parts(parts: list[str], part_index: int, current_dir: str, results: list[str]) -> None:
                ...
                with os.scandir(current_dir) as entries:
                    for entry in entries:
                        if entry.is_dir(follow_symlinks=False):
                            _match_parts(parts, part_index, entry.path, results)
            except (PermissionError, OSError):
                pass
    else:
                ...
                for entry in entries:
                    if regex.match(entry.name):
                        ...
                        if is_last:
                            ...
                            _match_parts(parts, part_index + 1, full_path, results)
        except (PermissionError, OSError):
            pass
...
def glob_match(pattern: str, base_dir: str) -> list[str]:
    ...
    """
    results: list[str] = []
    parts = [p for p in re.split('[\\\\/]+', pattern) if p]
...
def main() -> None:
    ...
    """
    parser = argparse.ArgumentParser(description='Glob file matching with mtime sorting.')
    parser.add_argument('--pattern', required=True, help="Glob pattern (e.g. '**/*.py')")
    parser.add_argument('--base-dir', required=True, help='Base directory to search from')
