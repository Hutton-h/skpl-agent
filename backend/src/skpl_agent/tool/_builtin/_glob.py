...
def _default_glob_helper_path() -> str:
    ...
    ref = _res.files('skpl_agent.tool._builtin._scripts').joinpath('_glob_helper.py')
    return str(ref)
...
class Glob(ToolBase):
    ...
    input_schema: dict[str, Any] = {'type': 'object', 'properties': {'pattern': {'type': 'string', 'description': "The glob pattern to match against (e.g., '**/*.py', 'src/**/*.ts')"}, 'path': {'type': 'string', 'description': 'The base directory to search from (defaults to current working directory)'}}, 'required': ['pattern']}
    is_mcp: bool = False
    ...
    def __init__(self, backend: BackendBase | None=None, glob_helper_path: str | None=None, middlewares: List[ToolMiddlewareBase] | None=None) -> None:
        ...
        self._glob_helper_path = glob_helper_path if glob_helper_path is not None else _default_glob_helper_path()
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        """
        return PermissionDecision(behavior=PermissionBehavior.PASSTHROUGH, message='Glob pattern matching is read-only.')
    ...
    async def call(self, pattern: str, path: str | None=None) -> ToolChunk:
        ...
        if not await self._backend.is_dir(base_dir):
            return ToolChunk(content=[TextBlock(text=f'Directory not found: {base_dir}')], state=ToolResultState.ERROR, is_last=True)
        python = sys.executable if self._is_local else 'python3'
        command = [python, self._glob_helper_path, '--pattern', pattern, '--base-dir', base_dir]
        result = await self._backend.exec_shell(command, timeout=30.0)
        if not result.ok():
            stderr = result.stderr.decode('utf-8', errors='replace').strip()
            return ToolChunk(content=[TextBlock(text=f'Glob helper failed: {stderr}' if stderr else 'Glob helper failed with no error output.')], state=ToolResultState.ERROR, is_last=True)
        try:
            matches = json.loads(result.stdout.decode('utf-8', errors='replace'))
        except (json.JSONDecodeError, ValueError):
            matches = []
        if len(matches) == 0:
            return ToolChunk(content=[TextBlock(text=f'No files found matching pattern: {pattern}')], state=ToolResultState.RUNNING, is_last=True)
        return ToolChunk(content=[TextBlock(text='\n'.join(matches))], state=ToolResultState.RUNNING, is_last=True)