...
class ResetTools(ToolBase):
    ...
    description: str = 'This tool allows you to reset your equipped tools based on your current task requirements. These tools are organized into different groups, and you can activate/deactivate them by specifying the boolean values for each group in the input.\n\n**Important: The input booleans are the final state of their corresponding tool groups, not incremental changes.** Any group not explicitly set to True will be deactivated, regardless of its previous state.\n\n**Best practice**: Actively manage your tool groups——activate only what you need for the current task, and promptly deactivate groups as soon as they are no longer needed to conserve context space.\n\nThis tool will return the usage instructions for the activated tool groups, which you **MUST pay attention to and follow**. You can also reuse this tool to re-check the instructions.'
    is_mcp: bool = False
    ...
    def __init__(self, groups: list[ToolGroup], response_template: str, middlewares: List[ToolMiddlewareBase] | None=None) -> None:
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message='The meta tool is always allowed to be called.')
    ...
    async def call(self, _agent_state: AgentState, **kwargs: Any) -> ToolChunk:
        ...
        if _agent_state is None:
            raise DeveloperOrientedException('Error: ResetTools requires state to be provided.')
        _agent_state.tool_context.activated_groups.clear()
        to_activate = []
        for (key, value) in kwargs.items():
            if not isinstance(value, bool):
                return ToolChunk(content=[TextBlock(text=f'Invalid arguments: the argument {key} should be a bool value, but got {type(value)}.')])
            if value:
        ...
        return ToolChunk(content=[TextBlock(text=template.render(groups=activated_groups))])