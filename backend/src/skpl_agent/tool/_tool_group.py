...
class ToolGroup:
    ...
    def __init__(self, name: Literal['basic'] | str, description: str | None=None, instructions: str | None=None, tools: list[ToolBase] | None=None, skills_or_loaders: Sequence[str | Skill | SkillLoaderBase] | None=None, mcps: list[MCPClient] | None=None) -> None:
        ...
        if name != 'basic' and description is None:
            raise ValueError(f"The tool group description is required for tool group '{name}' (Only the 'basic' tool group can have an optional description).")
        self.name = name
        ...
        for _ in skills_or_loaders or []:
            if isinstance(_, str):
                ...
                raise TypeError(f'Invalid skill or loader: {_}. Must be a skill, skill loader, or directory path.')
