...
from typing import AsyncGenerator, Type, Generator, Sequence
import mcp
...
class Toolkit:
    ...
    def __init__(self, tools: list[ToolBase] | None=None, skills_or_loaders: Sequence[str | Skill | SkillLoaderBase] | None=None, mcps: list[MCPClient] | None=None, tool_groups: list[ToolGroup] | None=None, meta_tool_response_template: str=DEFAULT_META_TOOL_RESPONSE_TEMPLATE, skill_instruction_template: str=DEFAULT_SKILL_INSTRUCTION) -> None:
        """Initialize the toolkit.

        Args:
            tools (`list[ToolBase] | None`, optional):
                The tool objects that belong to the "basic" tool group.
            skills_or_loaders (`list[str | Skill | SkillLoaderBase] | None`,             optional):
        ...
        """
        if tool_groups is not None and any((_.name == 'basic' for _ in tool_groups)):
            raise ValueError("The 'basic' tool group is reserved for the default tool group. Don't include 'basic' in the tool_groups argument when you also provide tools, skills or mcps in the constructor.")
        self.tool_groups = [ToolGroup(name='basic', tools=tools or [], skills_or_loaders=skills_or_loaders or [], mcps=mcps or [])] + (tool_groups or [])
        if len(set((_.name for _ in self.tool_groups))) != len(self.tool_groups):
            raise ValueError('Tool groups must not contain duplicate tool groups.')
        for group in self.tool_groups:
            for client in group.mcps:
                if client.is_stateful and (not client.is_connected):
                    raise ValueError(f"The MCP client '{client.name}' is stateful, but not connected.")
        self.meta_tool_response_template = meta_tool_response_template
        self.skill_instruction_template = skill_instruction_template
        self.builtin_meta_tool = RegisteredTool(tool=ResetTools(groups=self.tool_groups, response_template=meta_tool_response_template))
        self.builtin_skill_viewer = RegisteredTool(tool=SkillViewer(get_skills_method=self._get_available_skills))
    ...
    async def call_tool(self, tool_call: ToolCallBlock, state: AgentState) -> AsyncGenerator[ToolChunk | ToolResponse, None]:
        ...
        if tool_call.name not in available_tools:
            all_tools = await self._get_available_tools(groups=[_.name for _ in self.tool_groups])
            if tool_call.name in all_tools:
                group_name = all_tools[tool_call.name].group
                chunk = ToolChunk(content=[TextBlock(text=f"ToolGroupInactiveError: The tool '{tool_call.name}' in group '{group_name}' is currently inactive. You should first activate the group by calling the '{self.builtin_meta_tool.tool.name}' tool.")], state=ToolResultState.ERROR)
                yield chunk
                yield tool_response.append_chunk(chunk)
                return
            chunk = ToolChunk(content=[TextBlock(text=f"ToolNotFoundError: The tool named '{tool_call.name}' doesn't exist.")], state=ToolResultState.ERROR)
            yield chunk
            yield tool_response.append_chunk(chunk)
            return
        ...
        try:
            ...
            if tool_func.is_state_injected and (not tool_func.is_mcp) and (not tool_func.is_external_tool):
            ...
            if isinstance(res, ToolChunk):
                ...
                raise DeveloperOrientedException(f'The tool function must return a ToolChunk object, or an AsyncGenerator/Generator of ToolChunk objects, but got {type(res)}.')
        except mcp.shared.exceptions.McpError as e:
            chunk = ToolChunk(content=[TextBlock(type='text', text=f'Error occurred when calling MCP tool: {e}')], state=ToolResultState.ERROR)
            yield chunk
            tool_response.append_chunk(chunk)
        except Exception as e:
            if isinstance(e, DeveloperOrientedException):
                raise e from None
            chunk = ToolChunk(content=[TextBlock(type='text', text=str(e))], state=ToolResultState.ERROR)
            yield chunk
            tool_response.append_chunk(chunk)
        except asyncio.CancelledError:
            chunk = ToolChunk(content=[TextBlock(type='text', text='<system-reminder>The tool call has been interrupted by the user.</system-reminder>')], state=ToolResultState.INTERRUPTED)
            yield chunk
            tool_response.append_chunk(chunk)
        finally:
            yield tool_response

    async def _get_available_skills(self, groups: list[str] | None=None) -> dict[str, Skill]:
        ...
        for group in self.tool_groups:
            ...
            for skill in await group.list_skills():
                if skill.name in skills:
                    logger.warning("Duplicate skill name '%s' found in group '%s', overwriting it.", skill.name, group.name)
                skills[skill.name] = skill
        return skills

    async def get_skill_instructions(self, activated_groups: list[str] | None=None) -> str | None:
        ...
        skills = await self._get_available_skills(activated_groups)
        if len(skills) == 0:
            return None
        template = Template(self.skill_instruction_template)
        return template.render(skills=skills.values(), skill_viewer=self.builtin_skill_viewer.tool.name)

    async def _get_available_tools(self, groups: list[str] | None) -> dict[str, RegisteredTool]:
        ...
        if len(self.tool_groups) == 1 and self.tool_groups[0].name != 'basic' or len(self.tool_groups) > 1:
            available_tools[self.builtin_meta_tool.tool.name] = self.builtin_meta_tool
        groups_filter = ['basic'] + (groups or [])
        for group in self.tool_groups:
            if group.name not in groups_filter:
                continue
            ...
            for tool in cache_tools:
                if tool.name in available_tools:
                    logger.warning("Duplicate tool name '%s' found in group '%s', overwriting it.", tool.name, group.name)
                available_tools[tool.name] = RegisteredTool(tool=tool, group=group.name)
        return available_tools

    async def check_tool_available(self, tool_name: str, activated_groups: list[str]) -> ToolBase:
        ...
        if tool_name not in tools:
            raise ToolNotFoundError(f"ToolNotFoundError: The tool named '{tool_name}' doesn't exist.")
        group_name = tools[tool_name].group
        if group_name != 'basic' and group_name not in activated_groups:
            raise ToolGroupInactiveError(f"ToolGroupInactiveError: The tool '{tool_name}' in group '{group_name}' is currently inactive. You should first activate the group by calling the '{self.builtin_meta_tool.tool.name}' tool.")
    ...
    async def get_tool(self, name: str) -> ToolBase | None:
        ...
        """
        tools = await self._get_available_tools([_.name for _ in self.tool_groups])
    ...
    def _get_meta_tool_schema(self) -> Type[BaseModel]:
        ...
        for group in self.tool_groups:
            if group.name == 'basic':
                continue
            fields[group.name] = (bool, Field(default=False, description=group.description))
            ...
            if group.name == group_name:
                ...
                for new_tool in new_tools:
                    if new_tool.name in existing_tools:
                        logger.warning("Duplicate tool name '%s' found in group '%s', overwriting it.", new_tool.name, group.name)
                        group.tools = [t for t in group.tools if t.name != new_tool.name] + [new_tool]
                    else:
                        group.tools.append(new_tool)
                        existing_tools.add(new_tool.name)
                return
        raise ValueError(f"Cannot find group '{group_name}' in toolkit, only {[_.name for _ in self.tool_groups]} are available.")
    ...
    async def remove_tool(self, tool_name: str | list[str]) -> None:
        ...
        if isinstance(tool_name, str):
            tool_name = [tool_name]
        for group in self.tool_groups:
            group.tools = [tool for tool in group.tools if tool.name not in tool_name]