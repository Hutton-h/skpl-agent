...
def _extract_input_schema(tool_func: Callable, include_var_positional: bool=False, include_var_keyword: bool=False) -> dict:
    ...
    for (name, param) in inspect.signature(tool_func).parameters.items():
        ...
        if param.kind == inspect.Parameter.VAR_KEYWORD:
            if not include_var_keyword:
                continue
            fields[name] = (Dict[str, Any] if param.annotation == inspect.Parameter.empty else Dict[str, param.annotation], Field(description=params_docstring.get(f'**{name}', params_docstring.get(name, None)), default={} if param.default is param.empty else param.default))
        elif param.kind == inspect.Parameter.VAR_POSITIONAL:
            if not include_var_positional:
                continue
            fields[name] = (list[Any] if param.annotation == inspect.Parameter.empty else list[param.annotation], Field(description=params_docstring.get(f'*{name}', params_docstring.get(name, None)), default=[] if param.default is param.empty else param.default))
        else:
            fields[name] = (Any if param.annotation == inspect.Parameter.empty else param.annotation, Field(description=params_docstring.get(name, None), default=... if param.default is param.empty else param.default))
    base_model = create_model('_StructuredOutputDynamicClass', __config__=ConfigDict(arbitrary_types_allowed=True), **fields)
