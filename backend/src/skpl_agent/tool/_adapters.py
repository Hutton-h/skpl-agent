...
from ..message import TextBlock, DataBlock, Base64Source, URLSource, ToolResultState
...
class FunctionTool(ToolBase):
    ...
    def __init__(self, func: Function, name: str | None=None, description: str | None=None, is_concurrency_safe: bool=True, is_read_only: bool=False, is_state_injected: bool=False, middlewares: list[ToolMiddlewareBase] | None=None) -> None:
    ...
    async def check_permissions(self, *_args: Any, **_kwargs: Any) -> PermissionDecision:
        ...
        """
        return PermissionDecision(behavior=PermissionBehavior.ASK, message='Custom function tools must be explicitly allowed by the user.')
    ...
    @staticmethod
    def _convert_func_result_to_chunk(result: Any) -> ToolChunk:
        ...
        if isinstance(result, str):
            ...
            try:
                ...
                text = str(result)
        return ToolChunk(content=[TextBlock(text=text)], state=ToolResultState.RUNNING)
...
class MCPTool(ToolBase):
    ...
    def __init__(self, mcp_name: str, tool: mcp.types.Tool, client_gen: Callable[..., _AsyncGeneratorContextManager[Any]] | None=None, session: Any | None=None, timeout: float | None=None, middlewares: list[ToolMiddlewareBase] | None=None) -> None:
        ...
        Args:
            ...
            client_gen (`Callable[..., _AsyncGeneratorContextManager[Any]]             | None`, optional):
        ...
        sanitized_tool = re.sub('[^a-zA-Z0-9_-]', 'x', tool.name)
        self.name = f'mcp__{mcp_name}__{sanitized_tool}'
        if sanitized_tool != tool.name:
            logger.debug("MCP tool name sanitized: '%s' -> '%s'.", tool.name, self.name)
    ...
    async def call(self, **kwargs: Any) -> ToolChunk:
        ...
        return ToolChunk(content=self._convert_mcp_content_to_blocks(result.content), state=ToolResultState.ERROR if result.isError else ToolResultState.RUNNING)

    @staticmethod
    def _convert_mcp_content_to_blocks(mcp_content_blocks: list) -> list[TextBlock | DataBlock]:
        ...
        for content in mcp_content_blocks:
            ...
            elif isinstance(content, (mcp.types.ImageContent, mcp.types.AudioContent)):
                as_content.append(DataBlock(source=Base64Source(type='base64', media_type=content.mimeType, data=content.data)))
            elif isinstance(content, mcp.types.EmbeddedResource):
                if isinstance(content.resource, mcp.types.TextResourceContents):
                    as_content.append(TextBlock(text=content.resource.model_dump_json(indent=2)))
                else:
                    logger.error('Unsupported EmbeddedResource content type: %s. Skipping this content.', type(content.resource))
            elif isinstance(content, mcp.types.ResourceContents):
                as_content.append(DataBlock(source=URLSource(media_type=content.mimeType, url=content.uri)))
            else:
                logger.warning('Unsupported content type: %s. Skipping this content.', type(content))
        return as_content