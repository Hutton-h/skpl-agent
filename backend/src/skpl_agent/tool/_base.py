...
from ..permission import PermissionContext, PermissionDecision, PermissionRule, PermissionBehavior
...
class ToolMiddlewareBase(ABC):
    ...
    @abstractmethod
    async def on_tool_call(self, tool: 'ToolBase', input_kwargs: dict[str, Any], next_handler: Callable[..., AsyncGenerator[ToolChunk, None]]) -> AsyncGenerator[ToolChunk, None]:
...
class ToolBase(ABC):
    ...
    def __init__(self, middlewares: List['ToolMiddlewareBase'] | None=None) -> None:
        ...
        """
        self._middlewares: List['ToolMiddlewareBase'] = middlewares if middlewares is not None else []

    async def call(self, *args: Any, **kwargs: Any) -> ToolChunk | AsyncGenerator[ToolChunk, None]:
        ...
        if not self.is_external_tool:
            raise NotImplementedError(f'{self.__class__.__name__} does not implement call')
        raise RuntimeError(f'{self.__class__.__name__} is an external tool and should not be called directly')

    async def __call__(self, *args: Any, **kwargs: Any) -> ToolChunk | AsyncGenerator[ToolChunk, None]:
        ...
        """
        if args:
            raise TypeError(f'{type(self).__name__} must be called with keyword arguments only, but got {len(args)} positional argument(s).')
        ...
        async def execute_chain(index: int=0, **chain_kwargs: Any) -> AsyncGenerator[ToolChunk, None]:
            ...
            if index >= len(middlewares):
                ...
                async def next_handler(**kw: Any) -> AsyncGenerator[ToolChunk, None]:
                    async for chunk in execute_chain(index + 1, **kw):
                        yield chunk
                async for chunk in mw.on_tool_call(tool=self, input_kwargs=input_kwargs, next_handler=next_handler):
                    yield chunk
    ...
    async def generate_suggestions(self, tool_input: dict[str, Any]) -> List[PermissionRule]:
        ...
        Returns:
            `List[PermissionRule]`:
                ...
                compound operations)
        """
        return [PermissionRule(tool_name=self.name, rule_content=None, behavior=PermissionBehavior.ALLOW, source='suggested')]

    def _path_in_allowed_working_path(self, file_path: str, context: PermissionContext) -> bool:
        ...
        for working_dir in all_working_dirs:
            abs_working_dir = os.path.realpath(os.path.expanduser(working_dir))
            try:
                os.path.relpath(abs_file_path, abs_working_dir)
                if abs_file_path.startswith(abs_working_dir + os.sep) or abs_file_path == abs_working_dir:
                    return True
            except ValueError:
                continue
        return False
