...
class TaskList(_TaskToolBase):
    ...
    async def call(self, _agent_state: AgentState) -> ToolChunk:
        ...
        if not isinstance(_agent_state, AgentState):
            raise DeveloperOrientedException(f'Error: TaskList requires AgentState to be provided, got {_agent_state} instead.')
        if len(_agent_state.tasks_context.tasks) == 0:
            return ToolChunk(content=[TextBlock(text='No tasks available.')])
        tasks = []
        for task in _agent_state.tasks_context.tasks:
            owner = f'({task.owner})' if task.owner else ''
            blocked = f"[blocked by {', '.join(task.blocked_by)}]" if task.blocked_by else ''
            tasks.append(f'{task.id} [{task.state}] {task.subject}{owner}{blocked}')
