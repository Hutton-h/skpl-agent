...
class _TaskUpdateParams(BaseModel):
    ...
    status: Literal['pending', 'in_progress', 'completed', 'deleted'] | None = Field(default=None, description='New status for the task')
    add_blocked_by: list[str] | None = Field(default=None, description='Task IDs that block this task')
    owner: str | None = Field(default=None, description='New owner for the task')
    metadata: dict | None = Field(default=None, description='Metadata keys to merge into the task. Set a key to null to delete it.')
...
class TaskUpdate(_TaskToolBase):
    ...
    async def call(self, _agent_state: AgentState, task_id: str, subject: str | None=None, description: str | None=None, add_blocks: list[str] | None=None, status: Literal['pending', 'completed', 'in_progress', 'deleted'] | None=None, add_blocked_by: list[str] | None=None, owner: str | None=None, metadata: dict | None=None) -> ToolChunk:
        """Update the agent task."""
        if not isinstance(_agent_state, AgentState):
            raise DeveloperOrientedException(f'Error: {self.name} requires AgentState to be provided, got {_agent_state} instead.')
        index = None
        for (i, task) in enumerate(_agent_state.tasks_context.tasks):
            if task.id == task_id:
                index = i
        if index is None:
            return ToolChunk(content=[TextBlock(text=f'TaskNotFoundError: The task (id={task_id}) does not exist.')], state=ToolResultState.ERROR)
        updated_fields = []
        if subject:
        ...
        if add_blocks:
            ...
            new_blocks = [_ for _ in add_blocks if _ not in current_blocks and _ in existed_ids]
            if new_blocks:
        ...
        if add_blocked_by is not None:
            current_blocked_by = _agent_state.tasks_context.tasks[index].blocked_by
            new_blocked_by = [_ for _ in add_blocked_by if _ not in current_blocked_by and _ in existed_ids]
            if new_blocked_by:
        ...
        if status:
            if status == 'deleted':
                ...
                return ToolChunk(content=[TextBlock(text=f'Task (id={task_id}) has been deleted.')])
        ...
        if metadata:
            ...
            for (k, v) in metadata.items():
                if v is None:
                    _agent_state.tasks_context.tasks[index].metadata.pop(k, None)
                else:
                    _agent_state.tasks_context.tasks[index].metadata[k] = v
        if updated_fields:
            res = f"Update task (id={task_id}) {', '.join(updated_fields)}."
        else:
            res = f'No updates were made to the task (id={task_id}). Make sure you provided at least one field to update and the values are correct.'
        if _agent_state.tasks_context.tasks[index].state == 'completed':
            res += '\n\nTask completed. Call TaskList now to find your next available task or see if your work unblocked others.'
        return ToolChunk(content=[TextBlock(text=res)])

    @staticmethod
    def _update_block_relation(block_id: str, blocked_by_id: str, _agent_state: AgentState) -> None:
