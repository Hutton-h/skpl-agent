...
def _extend_with_default(validator_class: Any) -> Any:
    ...
    def set_defaults(validator: Any, properties: dict, instance: Any, schema: dict) -> Generator[Any, None, None]:
...
class _GenerateStructuredOutput(ToolBase):
    ...
    def __init__(self, schema: Type[BaseModel] | dict, middlewares: List[ToolMiddlewareBase] | None=None) -> None:
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        to be called."""
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message=f'{self.name} is always allowed.')
    ...
    async def call(self, _agent_state: AgentState, **kwargs: Any) -> ToolChunk:
        ...
        if not schema:
            return ToolChunk(content=[TextBlock(text='No structured output is required for now.')], state=ToolResultState.SUCCESS)
        if isinstance(schema, type):
            try:
                validated = schema.model_validate(kwargs)
            except Exception as e:
                if isinstance(e, ValidationError):
                    message = '; '.join((f"{'.'.join((str(_) for _ in err['loc']))}: {err['msg']}" for err in e.errors()))
                else:
                    message = str(e)
                return ToolChunk(content=[TextBlock(text=f'ValidationError: Structured output validation failed with error: {message}')], state=ToolResultState.ERROR)
            _agent_state.reply_context.structured_output = validated.model_dump(mode='json')
        else:
            validator = _DefaultFillingValidator(schema)
            errors = list(validator.iter_errors(kwargs))
            if errors:
                messages = '; '.join((f'{_.json_path}: {_.message}' for _ in errors))
                return ToolChunk(content=[TextBlock(text=f'ValidationError: Structured output validation failed with error: {messages}')], state=ToolResultState.ERROR)
            _agent_state.reply_context.structured_output = kwargs
        return ToolChunk(content=[TextBlock(text='Structured output generated successfully.')], state=ToolResultState.SUCCESS)