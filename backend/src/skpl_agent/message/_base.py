...
from typing import Literal, List, overload, Sequence, TYPE_CHECKING, Any
from typing_extensions import Self
from pydantic import BaseModel, Field, model_validator
from .._utils._common import _generate_id
from ._block import TextBlock, ThinkingBlock, HintBlock, DataBlock, Base64Source, URLSource, ToolCallBlock, ToolCallState, ToolResultBlock, ToolResultState, ContentBlock, ContentBlockTypes
...
def _assert_system_content_blocks(content: Sequence[ContentBlock]) -> None:
...
class Msg(BaseModel):
    ...
    def has_content_blocks(self, block_type: ContentBlockTypes | list[ContentBlockTypes] | None=None) -> bool:
        ...
        Args:
            block_type (`ContentBlockTypes | list[ContentBlockTypes] | None`,             optional):
    ...
    @overload
    def get_content_blocks(self, block_type: Literal['text']) -> list[TextBlock]:
        ...

    @overload
    def get_content_blocks(self, block_type: Literal['thinking']) -> list[ThinkingBlock]:
        ...

    @overload
    def get_content_blocks(self, block_type: Literal['tool_call']) -> list[ToolCallBlock]:
        ...

    @overload
    def get_content_blocks(self, block_type: Literal['tool_result']) -> list[ToolResultBlock]:
        ...

    @overload
    def get_content_blocks(self, block_type: Literal['data']) -> list[DataBlock]:
        ...

    @overload
    def get_content_blocks(self, block_type: None=None) -> list[ContentBlock]:
        ...

    @overload
    def get_content_blocks(self, block_type: Literal['hint']) -> list[HintBlock]:
        ...

    def get_content_blocks(self, block_type: ContentBlockTypes | List[ContentBlockTypes] | None=None) -> Sequence[ContentBlock]:
    ...
    def append_event(self, event: AgentEvent) -> Self:
        ...
        if event_reply_id != self.id:
            if event_reply_id is not None:
                logger.warning('Event %s with reply_id %r does not match message id %r, skipping.', event.__class__.__name__, event_reply_id, self.id)
            return self
        match event.type:
            ...
            case EventType.MODEL_CALL_END:
                if self.usage is None:
                    self.usage = Usage(input_tokens=event.input_tokens, output_tokens=event.output_tokens)
                else:
            ...
            case EventType.DATA_BLOCK_START:
                self.content.append(DataBlock(id=event.block_id, source=Base64Source(data='', media_type=event.media_type)))
            case EventType.DATA_BLOCK_DELTA:
                block = self._find_block('data', event.block_id)
                if block is None:
                    logger.warning('DataBlock %s not found, skipping.', event.block_id)
                elif event.data:
                    existing = base64.b64decode(block.source.data) if block.source.data else b''
                    incoming = base64.b64decode(event.data)
                    block.source.data = base64.b64encode(existing + incoming).decode('ascii')
            ...
            case EventType.HINT_BLOCK:
                self.content.append(HintBlock(id=event.block_id, source=event.source, hint=event.hint))
            case EventType.TOOL_CALL_START:
                self.content.append(ToolCallBlock(id=event.tool_call_id, name=event.tool_call_name, input=''))
            case EventType.TOOL_CALL_DELTA:
                block = self._find_block('tool_call', event.tool_call_id)
                if block is None:
                    logger.warning('ToolCallBlock %r not found, skipping.', event.tool_call_id)
                else:
            ...
            case EventType.TOOL_RESULT_START:
                self.content.append(ToolResultBlock(id=event.tool_call_id, name=event.tool_call_name, output=[], state=ToolResultState.RUNNING))
            case EventType.TOOL_RESULT_TEXT_DELTA:
                block = self._find_block('tool_result', event.tool_call_id)
                if block is None:
                    logger.warning('ToolResultBlock %r not found, skipping.', event.tool_call_id)
                else:
            ...
            case EventType.TOOL_RESULT_DATA_DELTA:
                ...
                if block is None:
                    ...
                    if isinstance(block.output, str):
                        block.output = [TextBlock(text=block.output)]
                    src = Base64Source(data=event.data, media_type=event.media_type) if event.data is not None else URLSource(url=str(event.url), media_type=event.media_type)
            ...
            case EventType.USER_CONFIRM_RESULT:
                for result in event.confirm_results:
                    ...
                    if b is not None and b.state == ToolCallState.ASKING:
                        assert isinstance(b, ToolCallBlock)
                        b.state = ToolCallState.ALLOWED if result.confirmed else ToolCallState.FINISHED
            ...
            case EventType.EXTERNAL_EXECUTION_RESULT:
                existing_ids = {b.id for b in self.content if isinstance(b, ToolResultBlock)}
...
def UserMsg(name: str, content: str | list[TextBlock | DataBlock], metadata: dict | None=None, created_at: str | None=None, finished_at: str | None=None, finished_reason: ReplyFinishedReason | None=None, id: str | None=None) -> Msg:
    ...
    return Msg(name=name, content=_to_blocks(content), role='user', metadata=metadata or {}, created_at=created_at, finished_at=finished_at, finished_reason=finished_reason, id=id or _generate_id())

def AssistantMsg(name: str, content: str | list[ContentBlock], metadata: dict | None=None, created_at: str | None=None, finished_at: str | None=None, finished_reason: ReplyFinishedReason | None=None, structured_output: dict | None=None, id: str | None=None, usage: Usage | None=None) -> Msg:
    ...
    """
    return Msg(name=name, content=_to_blocks(content), role='assistant', metadata=metadata or {}, created_at=created_at or datetime.now().isoformat(), finished_at=finished_at, finished_reason=finished_reason, structured_output=structured_output, id=id or _generate_id(), usage=usage)

def SystemMsg(name: str, content: str | list[TextBlock], metadata: dict | None=None, created_at: str | None=None, finished_at: str | None=None, finished_reason: ReplyFinishedReason | None=None, id: str | None=None) -> Msg:
    ...
    if finished_at is None:
        finished_at = created_at
    return Msg(name=name, content=_to_blocks(content), role='system', metadata=metadata or {}, created_at=created_at, finished_at=finished_at, finished_reason=finished_reason, id=id or _generate_id())