...
import logging
_DEFAULT_FORMAT = '%(asctime)s | %(levelname)-7s | %(module)s:%(funcName)s:%(lineno)s - %(message)s'
...
def setup_logger(level: str, filepath: str | None=None) -> None:
    ...
    if level not in ['INFO', 'DEBUG', 'WARNING', 'ERROR', 'CRITICAL']:
        raise ValueError(f"Invalid logging level: {level}. Must be one of 'INFO', 'DEBUG', 'WARNING', 'ERROR', 'CRITICAL'.")
