...
_MEDIA_TYPES = {'mp3': 'audio/mpeg', 'opus': 'audio/opus', 'aac': 'audio/aac', 'flac': 'audio/flac', 'wav': 'audio/wav', 'pcm': 'audio/pcm'}
...
def _parse_usage(usage: Any, elapsed: float) -> TTSUsage | None:
    ...
    if usage is None:
        return None
    return TTSUsage(input_tokens=getattr(usage, 'input_tokens', 0) or 0, output_tokens=getattr(usage, 'output_tokens', 0) or 0, time=elapsed)
...
class OpenAITTSModel(TTSModelBase):
    ...
    class Parameters(BaseModel):
        ...
        voice: str = Field(default='alloy', title='Voice', description='The voice to use for synthesis.')
        response_format: Literal['mp3', 'opus', 'aac', 'flac', 'wav', 'pcm'] = Field(default=_DEFAULT_RESPONSE_FORMAT, title='Response Format', description='The audio format of the synthesized speech.')
        instructions: str | None = Field(default=None, title='Instructions', description='Additional instructions for controlling the voice (only supported by some models, e.g. gpt-4o-mini-tts).')
    type: Literal['openai_tts'] = 'openai_tts'
    'The type of the TTS model.'
    realtime: bool = False

    def __init__(self, credential: OpenAICredential, model: str='tts-1', parameters: 'OpenAITTSModel.Parameters | None'=None, stream: bool=True) -> None:
        ...
        Args:
            ...
            parameters (`OpenAITTSModel.Parameters | None`, defaults to             `None`):
    ...
    async def synthesize(self, text: str | None=None, **kwargs: Any) -> TTSResponse | AsyncGenerator[TTSResponse, None]:
        ...
        import openai
        client = openai.AsyncClient(api_key=self.credential.api_key.get_secret_value(), organization=self.credential.organization, base_url=self.credential.base_url)
        media_type = _MEDIA_TYPES.get(self.parameters.response_format, _MEDIA_TYPES[_DEFAULT_RESPONSE_FORMAT])
        request_kwargs: dict[str, Any] = {'model': self.model, 'voice': self.parameters.voice, 'input': text, 'response_format': self.parameters.response_format, **kwargs}
    ...
    @staticmethod
    async def _aggregate(client: 'AsyncOpenAI', media_type: str, **request_kwargs: Any) -> TTSResponse:
        ...
        return TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(audio_bytes).decode('ascii'), media_type=media_type)), usage=_parse_usage(None, elapsed))

    @staticmethod
    async def _stream(client: 'AsyncOpenAI', media_type: str, **request_kwargs: Any) -> AsyncGenerator[TTSResponse, None]:
        ...
        async with client.audio.speech.with_streaming_response.create(**request_kwargs) as response:
            async for delta_bytes in response.iter_bytes():
                ...
                if pending is not None:
                    yield pending
                pending = TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(delta_bytes).decode('ascii'), media_type=media_type)), is_last=False)
        elapsed = (datetime.now() - start_datetime).total_seconds()
        if pending is not None:
            ...
            yield TTSResponse(content=None, is_last=True, usage=_parse_usage(None, elapsed))