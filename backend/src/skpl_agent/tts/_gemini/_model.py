...
from typing import Any, AsyncGenerator, AsyncIterator, Literal, TYPE_CHECKING
...
def _parse_usage(usage_metadata: Any, elapsed: float) -> TTSUsage | None:
    """Extract a TTSUsage from the Gemini ``usage_metadata`` object, or
    ``None``."""
    if not usage_metadata:
        return None
    return TTSUsage(input_tokens=getattr(usage_metadata, 'prompt_token_count', 0) or 0, output_tokens=getattr(usage_metadata, 'candidates_token_count', 0) or 0, time=elapsed)
...
class GeminiTTSModel(TTSModelBase):
    ...
    class Parameters(BaseModel):
        ...
        voice: str = Field(default='Kore', title='Voice', description='The voice to use for synthesis.')
    type: Literal['gemini_tts'] = 'gemini_tts'
    'The type of the TTS model.'
    realtime: bool = False

    def __init__(self, credential: GeminiCredential, model: str='gemini-2.5-flash-preview-tts', parameters: 'GeminiTTSModel.Parameters | None'=None, stream: bool=False, client_kwargs: dict[str, Any] | None=None) -> None:
        ...
        Args:
            ...
            parameters (`GeminiTTSModel.Parameters | None`, defaults to             `None`):
    ...
    async def synthesize(self, text: str | None=None, **kwargs: Any) -> TTSResponse | AsyncGenerator[TTSResponse, None]:
        ...
        config: dict[str, Any] = {'response_modalities': ['AUDIO'], 'speech_config': {'voice_config': {'prebuilt_voice_config': {'voice_name': self.parameters.voice}}}, **kwargs}
        if self.stream:
            stream = await client.aio.models.generate_content_stream(model=self.model, contents=text, config=config)
            return self._parse_stream_into_async_generator(stream)
        start_datetime = datetime.now()
        response = await client.aio.models.generate_content(model=self.model, contents=text, config=config)
        elapsed = (datetime.now() - start_datetime).total_seconds()
        return self._parse_response(response, elapsed)

    @staticmethod
    def _parse_response(response: 'GenerateContentResponse', elapsed: float) -> TTSResponse:
        ...
        """
        usage = _parse_usage(getattr(response, 'usage_metadata', None), elapsed)
        ...
        return TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(buf.getvalue()).decode('ascii'), media_type=_DEFAULT_MEDIA_TYPE)), usage=usage)

    @staticmethod
    async def _parse_stream_into_async_generator(stream: AsyncIterator['GenerateContentResponse']) -> AsyncGenerator[TTSResponse, None]:
            ...
            for candidate in candidates:
                ...
                for part in parts:
                    ...
                    if not data:
                        continue
                    if not header_sent:
                        payload = _build_streaming_wav_header(sample_rate=_TTS_SAMPLE_RATE, channels=_TTS_CHANNELS, bits_per_sample=_TTS_BITS_PER_SAMPLE) + data
                        header_sent = True
                    else:
                        payload = data
                    if pending is not None:
                        yield pending
                    pending = TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(payload).decode('ascii'), media_type=_DEFAULT_MEDIA_TYPE)), is_last=False)
        elapsed = (datetime.now() - start_datetime).total_seconds()
        if pending is not None:
            ...
            yield TTSResponse(content=None, is_last=True, usage=_parse_usage(usage_metadata, elapsed))