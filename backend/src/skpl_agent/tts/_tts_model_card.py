...
class TTSModelCard(BaseModel):
    ...
    status: Literal['active', 'deprecated', 'sunset'] = Field(default='active', title='Status', description='The model status')
    'The model status.'
    ...
    output_types: list[str] = Field(description='The supported model output types.', title='Output types', default=['audio/wav'])
    'The model supported output types.'
    realtime: bool = Field(default=False, description='Whether the model supports streaming input (realtime).', title='Realtime')
