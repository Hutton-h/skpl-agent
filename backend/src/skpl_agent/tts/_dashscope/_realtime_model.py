    ...
    class _Callback(QwenTtsRealtimeCallback):
        ...
        def on_close(self, close_status_code: int, close_msg: str) -> None:
            ...
            if close_status_code:
                logger.warning('TTS WebSocket closed with code %s: %s', close_status_code, close_msg)
        ...
        def _take_delta(self, header: bool=False) -> bytes | None:
            ...
            if header:
                return _build_streaming_wav_header(sample_rate=_SAMPLE_RATE, channels=_CHANNELS, bits_per_sample=_BITS_PER_SAMPLE) + bytes(new_data)
        ...
        def get_audio_response(self, block: bool) -> TTSResponse:
            ...
            if delta:
                return TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(delta).decode('ascii'), media_type=_MEDIA_TYPE)))
        ...
        async def get_audio_chunks(self) -> AsyncGenerator[TTSResponse, None]:
            ...
            while True:
                if self.finish_event.is_set():
                    ...
                    if delta:
                        yield TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(delta).decode('ascii'), media_type=_MEDIA_TYPE)), is_last=True)
                    else:
                ...
                if delta:
                    header_sent = True
                    yield TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(delta).decode('ascii'), media_type=_MEDIA_TYPE)), is_last=False)
...
class DashScopeRealtimeTTSModel(TTSModelBase):
    ...
    class Parameters(BaseModel):
        ...
        voice: str = Field(default='Cherry', title='Voice', description='The voice to use for synthesis.')
    type: Literal['dashscope_realtime_tts'] = 'dashscope_realtime_tts'
    realtime: bool = True

    def __init__(self, credential: DashScopeCredential, model: str='qwen3-tts-flash-realtime', parameters: 'DashScopeRealtimeTTSModel.Parameters | None'=None, stream: bool=True, cold_start_length: int | None=None, cold_start_words: int | None=None, max_retries: int=3, retry_delay: float=5.0) -> None:
    ...
    def _create_client(self) -> None:
        ...
        self._tts_client = QwenTtsRealtime(model=self.model, callback=self._callback)
    ...
    async def connect(self) -> None:
        ...
        self._tts_client.update_session(voice=self.parameters.voice, mode='server_commit')
    ...
    async def push(self, text: str, **kwargs: Any) -> TTSResponse:
        ...
        if not self._cold_start_done:
            ...
            ready = True
            if self.cold_start_length and len(self._cold_start_buffer) < self.cold_start_length:
                ready = False
            if ready and self.cold_start_words and (len(self._cold_start_buffer.split()) < self.cold_start_words):
                ready = False
            if ready:
                try:
    ...
    async def synthesize(self, text: str | None=None, **kwargs: Any) -> TTSResponse | AsyncGenerator[TTSResponse, None]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                await asyncio.to_thread(self._callback.finish_event.wait)
                if full_text and (not self._callback.has_audio_data()):
                    if attempt < self.max_retries - 1:
                        logger.warning('TTS: no audio received, retrying (%d/%d) in %.1fs...', attempt + 1, self.max_retries, delay)
                        ...
                        continue
                    self._reset_state()
                    raise RuntimeError(f'TTS synthesis failed: no audio after {self.max_retries} attempts')
                break
            except WebSocketConnectionClosedException:
                if attempt < self.max_retries - 1:
                    logger.warning('TTS WebSocket closed, retrying (%d/%d) in %.1fs...', attempt + 1, self.max_retries, delay)
                    await asyncio.sleep(delay)
                    await self._reconnect()
                    unsent = full_text
                    delay *= 2
