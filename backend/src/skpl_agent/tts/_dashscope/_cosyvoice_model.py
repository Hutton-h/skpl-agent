...
from ._cosyvoice_utils import _BITS_PER_SAMPLE, _CHANNELS, _make_cosyvoice_callback_class, _MEDIA_TYPE, _SAMPLE_RATE
...
class DashScopeCosyVoiceTTSModel(TTSModelBase):
    ...
    class Parameters(BaseModel):
        ...
        voice: str = Field(default='longanhuan', title='Voice', description='The voice to use for synthesis.')
        realtime: bool = Field(default=False, title='Realtime', description='Whether to enable streaming input mode.')
    ...
    def __init__(self, credential: DashScopeCredential, model: str='cosyvoice-v3-flash', parameters: 'DashScopeCosyVoiceTTSModel.Parameters | None'=None, stream: bool=True, cold_start_length: int | None=None, cold_start_words: int | None=None, max_retries: int=3, retry_delay: float=5.0) -> None:
        ...
        Args:
            ...
            parameters (`DashScopeCosyVoiceTTSModel.Parameters | None`,             defaults to `None`):
        ...
        """
        super().__init__(credential=credential, model=model, parameters=parameters, stream=stream)
    ...
    @classmethod
    def list_models(cls, custom_yaml_dir: str | None=None) -> list[TTSModelCard]:
        """List CosyVoice model cards from the dedicated card directory."""
        return super().list_models(custom_yaml_dir=custom_yaml_dir or cls._MODELS_DIR)

    async def synthesize(self, text: str | None=None, **kwargs: Any) -> TTSResponse | AsyncGenerator[TTSResponse, None]:
    ...
    def _synthesize_once(self, text: str | None=None) -> TTSResponse | AsyncGenerator[TTSResponse, None]:
        """Synthesize text with the non-realtime one-shot SDK path."""
        if not text:
            return TTSResponse(content=None)
        (synthesizer, callback) = self._create_synthesizer(callback=self.stream)
        if self.stream:
    ...
    def _create_synthesizer(self, *, callback: bool) -> tuple['SpeechSynthesizer', 'ResultCallback | None']:
        ...
        synthesizer = SpeechSynthesizer(model=self.model, voice=self.parameters.voice, format=AudioFormat.PCM_24000HZ_MONO_16BIT, callback=callback_instance)
    ...
    async def connect(self) -> None:
        ...
        if self._connected:
            return
        (self._synthesizer, self._callback) = self._create_synthesizer(callback=True)
    ...
    async def push(self, text: str, **kwargs: Any) -> TTSResponse:
        """Push an incremental text delta for realtime synthesis."""
        del kwargs
        if not self.realtime:
            return await super().push(text)
        if not self._connected:
            raise RuntimeError('TTS model is not connected. Call `connect()` first.')
        if not text:
            return TTSResponse(content=None)
        ...
        if self._cold_start_done:
            ...
            if self.cold_start_length and len(self._cold_start_buffer) < self.cold_start_length or (self.cold_start_words and len(self._cold_start_buffer.split()) < self.cold_start_words):
    ...
    async def _synthesize_realtime(self, text: str | None=None) -> TTSResponse | AsyncGenerator[TTSResponse, None]:
        ...
        try:
            if not full_text and (not unsent):
                if self.stream:
                    ...
                    async def _empty_gen() -> AsyncGenerator[TTSResponse, None]:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    finished = await asyncio.to_thread(self._callback.finish_event.wait, 30)
                    if not finished:
                        logger.warning('CosyVoice TTS: timed out waiting for synthesis completion (30s)')
                        if attempt < self.max_retries - 1:
                            ...
                            delay *= 2
                            continue
                        raise RuntimeError('CosyVoice TTS synthesis timed out after 30s')
                    if full_text and (not self._callback.has_audio_data()):
                        if attempt < self.max_retries - 1:
                            logger.warning('CosyVoice TTS: no audio received, retrying (%d/%d) in %.1fs...', attempt + 1, self.max_retries, delay)
                            await asyncio.sleep(delay)
                            await self._reconnect()
                            unsent = full_text
                            delay *= 2
                            continue
                        raise RuntimeError(f'CosyVoice TTS synthesis failed: no audio after {self.max_retries} attempts')
                    break
                except RuntimeError:
                    raise
                except Exception as e:
                    if attempt < self.max_retries - 1:
                        logger.warning('CosyVoice TTS error, retrying (%d/%d) in %.1fs: %s', attempt + 1, self.max_retries, delay, e)
                        await asyncio.sleep(delay)
                        await self._reconnect()
                        unsent = full_text
                        delay *= 2
    ...
    @staticmethod
    def _build_wav_response(audio_data: bytes) -> TTSResponse:
        ...
        return TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(buf.getvalue()).decode('ascii'), media_type=_MEDIA_TYPE)))