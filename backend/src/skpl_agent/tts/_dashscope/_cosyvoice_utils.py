...
def _build_audio_response(audio_data: bytes, media_type: str, *, is_last: bool=True) -> TTSResponse:
    """Build a TTSResponse containing base64-encoded audio data."""
    return TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(audio_data).decode('ascii'), media_type=media_type)), is_last=is_last)
        ...
        async def get_audio_chunks(self) -> AsyncGenerator[TTSResponse, None]:
            ...
            while True:
                if self.finish_event.is_set():
                    ...
                    if delta:
                        yield _build_audio_response(delta, _MEDIA_TYPE, is_last=True)
                    else:
                ...
                if delta:
                    header_sent = True
                    yield _build_audio_response(delta, _MEDIA_TYPE, is_last=False)
