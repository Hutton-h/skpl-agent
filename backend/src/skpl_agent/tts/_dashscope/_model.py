...
from typing import Any, AsyncGenerator, Generator, Literal, TYPE_CHECKING
...
class DashScopeTTSModel(TTSModelBase):
    ...
    def __init__(self, credential: DashScopeCredential, model: str='qwen3-tts-flash', parameters: 'DashScopeTTSModel.Parameters | None'=None, stream: bool=True) -> None:
        ...
        Args:
            ...
            parameters (`DashScopeTTSModel.Parameters | None`, defaults to             `None`):
    ...
    async def synthesize(self, text: str | None=None, **kwargs: Any) -> TTSResponse | AsyncGenerator[TTSResponse, None]:
        ...
        import dashscope
        response = dashscope.MultiModalConversation.call(model=self.model, api_key=self.credential.api_key.get_secret_value(), text=text, voice=self.parameters.voice, stream=True, **kwargs)
        if self.stream:
            return self._parse_into_async_generator(response)
        return self._aggregate_sync(response)

    @staticmethod
    def _aggregate_sync(response: Generator['MultiModalConversationResponse', None, None]) -> TTSResponse:
        ...
        return TTSResponse(content=DataBlock(source=Base64Source(data=base64.b64encode(buf.getvalue()).decode('ascii'), media_type=_DEFAULT_MEDIA_TYPE)), usage=_parse_usage(usage, elapsed))

    @staticmethod
    async def _parse_into_async_generator(response: Generator['MultiModalConversationResponse', None, None]) -> AsyncGenerator[TTSResponse, None]:
        ...
        while True:
            ...
            if not delta_bytes:
                continue
            if not header_sent:
                payload = _build_streaming_wav_header(sample_rate=_TTS_SAMPLE_RATE, channels=_TTS_CHANNELS, bits_per_sample=_TTS_BITS_PER_SAMPLE) + delta_bytes
                header_sent = True
            else:
                payload = delta_bytes
        ...
        if pending is not None:
            ...
            yield TTSResponse(content=None, is_last=True, usage=_parse_usage(usage, elapsed))