...
import type { ChatModelConfig, SessionRecord } from '@/api';
...
const ChatPageInner = () => {
	...
	const { agents, loading: agentsLoading, refetch: refetchAgents, remove: removeAgent } = useAgents();
	const {
		sessions,
	...
	const { isMobile, setOpen: _setOpen, setOpenMobile } = useSidebar();
	...
	// Persist the active session ID so Dashboard / Context / BugLog
	// pages can use it without hardcoding 'default'.
	useEffect(() => {
		if (urlSessionId) {
			localStorage.setItem('active_session_id', urlSessionId);
		}
		if (urlAgentId) {
			localStorage.setItem('active_agent_id', urlAgentId);
		}
	}, [urlSessionId, urlAgentId]);
	...
	const handleCreateSession = async () => {
		...
		// When no existing session provides a seed model config (first-time
		// user), pick the first available model so the new session is usable
		// immediately instead of waiting for ChatViewport's auto-patch.
		const fallbackModel = (!seedConfig?.chat_model_config) ? getFirstAvailableModel() : null;
	...
	const handleDeleteSession = async (sessionId: string) => {
		...
		// The redirect effect below will pick the next available session
		// after refetch() updates the list — no explicit navigate needed.
		// Avoids double-navigation and the ERR_ABORTED on the DELETE request.
	};
			...
			<div className="flex h-full w-full items-center justify-center">
				<Empty className="border-none">
					<EmptyHeader>
						...
						<EmptyTitle>{t('chat.agent.emptyTitle')}</EmptyTitle>
						<EmptyDescription>{t('chat.agent.emptyDescription')}</EmptyDescription>
					</EmptyHeader>
					<EmptyContent>
