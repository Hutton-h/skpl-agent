...
import { Camera, Paperclip, Send, Loader2, Square, X, type LucideIcon } from 'lucide-react';
import React, {
	useState,
	useRef,
	useMemo,
...
import { SKPL_ASK_EVENT, takePendingPrompt } from '@/lib/vscodeBridge';
...
export const TextInput = forwardRef<TextInputRef, TextInputProps>(
	(
		...
		const handleScreenshot = async () => {
			try {
				...
				// User cancelled or browser doesn't support screen capture
		...
		// VS Code extension bridge. Two intake paths, both fill-only — the user
		// always presses send themselves (人工确认门, no auto-send):
		//   1. Mount drain — the extension wrote ``skpl.pendingPrompt`` before
		//      this input mounted (extension first, page later); take it once.
		//   2. Live event — ``skpl:ask`` CustomEvent dispatched by the app-root
		//      postMessage listener while this input is already mounted.
		// The stored copy is cleared on consume so later mounts never refill.
		useEffect(() => {
			const applyPrompt = (text: string | null) => {
				if (!text) return;
				takePendingPrompt();
				// Append when the user has an in-progress draft, never overwrite.
				setValue((prev) => (prev.trim() ? `${prev}\n\n${text}` : text));
				textareaRef.current?.focus();
			};
			applyPrompt(takePendingPrompt());
			const onAsk = (e: Event) => applyPrompt((e as CustomEvent<string>).detail);
			window.addEventListener(SKPL_ASK_EVENT, onAsk);
			return () => window.removeEventListener(SKPL_ASK_EVENT, onAsk);
		}, []);
	...
	// Shared file processing helper
		...
		const handleDrop = (e: React.DragEvent) => {
			...
			e.stopPropagation();
			setIsDragOver(false);
			if (attachDisabled || !e.dataTransfer.files.length) return;
		...
		return (
			<div
				...
				<div
					className={cn(
						"flex w-full flex-col gap-2 rounded-[28px] border bg-background px-2",
