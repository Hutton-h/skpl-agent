...
const ChatContentComponent: React.FC<ChatContentProps> = ({
	...
	return (
		<div
			className={cn(
				'flex flex-col min-h-0 w-full items-center p-2 gap-4',
				isEmpty ? 'justify-center' : 'flex-1',
				className,
			)}
		>
			<div
			...
			>
				<div
					...
					className={cn(
						...
						isEmpty ? 'w-full' : 'h-full',
					)}
				>
					<div
						className={cn(
							'flex flex-col gap-4 max-w-full',
							isEmpty ? 'w-full' : 'min-h-full w-full',
						)}
					>
						{isEmpty ? (
			...
			<TextInput
				className="shrink-0 min-w-full max-w-full w-full"
				onSend={onSend}
				disabled={disabled}
