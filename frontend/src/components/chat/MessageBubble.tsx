...
import type { PlanBlock, VisualBlock } from './custom-blocks';
...
type ExtendedContentBlock = ContentBlock | ToolCallGroupBlock | VisualBlock | PlanBlock;

/** Generate a stable React key for a content block, avoiding array-index
 *  collisions during streaming updates. */
function getBlockKey(block: ExtendedContentBlock, index: number): string {
	if ('id' in block && block.id) return block.id;
	if (block.type === 'tool_call_group') {
		return block.calls.map((c) => c.call.id).join('|') || `tool_group_${index}`;
	}
	return `${block.type}_${index}`;
}
...
function renderBlock(
	...
	switch (block.type) {
		...
		case 'tool_call_group': {
			...
			return (
				<div key={key} className="flex flex-col gap-y-4 text-muted-foreground">
		...
		case 'text':
			return (
				<div key={key} className="prose w-full min-w-full">
					<ReactMarkdown
		...
		case 'thinking':
			return (
				<details key={key} className="text-muted-foreground">
				...
				case 'image':
					return (
						<img
							key={key}
							src={data}
							alt={block.name || t('messageBubble.uploadedImage')}
		...
		case 'hint': {
			...
			const items: (TextBlock | DataBlock)[] =
				typeof block.hint === 'string'
					...
					: block.hint;
			return (
				<Item key={key} variant={'outline'} className="max-w-full">
