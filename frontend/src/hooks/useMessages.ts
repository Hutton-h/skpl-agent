...
import type {
	...
	VisualBlock,
	VisualEventValue,
} from '@/components/chat/custom-blocks';
...
/**
 * Wrap a visual/plan block that arrived outside any in-flight reply in a
 * standalone, already-finished assistant Msg so it still renders inline
 * in the conversation (custom events are not part of the SDK reply
 * stream, so without a carrier Msg they would never be displayed).
 */
const makeStandaloneMsg = (event: CustomEvent, block: VisualBlock | PlanBlock): Msg => ({
	id: `${block.type}_${event.id}`,
	name: 'assistant',
	role: 'assistant',
	content: [block as unknown as ContentBlock],
	metadata: {},
	created_at: event.created_at ?? new Date().toISOString(),
	finished_at: event.created_at ?? new Date().toISOString(),
});
...
export function useMessages(
	...
	/**
	 * Append a visual/plan block to the message stream. When a reply is
	 * in flight the block attaches to it (so a ``publish_visual`` tool
	 * call renders inside its own assistant bubble); otherwise it becomes
	 * a standalone finished Msg.
	 */
	const appendCustomBlock = useCallback(
		(event: CustomEvent, block: VisualBlock | PlanBlock) => {
			const target = currentReplyRef.current;
			if (target && !target.finished_at) {
				target.content.push(block as unknown as ContentBlock);
			} else {
				msgsRef.current = [...msgsRef.current, makeStandaloneMsg(event, block)];
			}
			scheduleUpdate();
		},
		[scheduleUpdate],
	...
	/** Render a ``CustomEvent(name="visual")`` payload as a VisualCard block. */
	const appendVisualBlock = useCallback(
		(event: CustomEvent) => {
			const v = event.value as VisualEventValue | undefined;
			if (!v) return;
			// Allow blocks with only structured data (no html) for dedicated frontend components.
			const hasHtml = typeof v.html === 'string' && v.html.length > 0;
			const hasData = v.data != null && typeof v.data === 'object';
			if (!hasHtml && !hasData) return;
			const block: VisualBlock = {
				type: 'visual',
				id: `visual_${event.id}`,
				title: v.title ?? '',
				visualType: v.visual_type ?? 'chart',
				html: v.html ?? '',
				summary: v.summary,
				data: v.data ?? null,
			};
			// Dedup on SSE replay: the same event id may be pushed again
			// when the stream reconnects.
			const exists = msgsRef.current.some((m) =>
				m.content.some((b) => (b as { id?: string }).id === block.id),
			);
			if (exists) return;
			appendCustomBlock(event, block);
		},
		[appendCustomBlock],
	);

	/**
	 * Render a ``CustomEvent(name="plan")`` payload as a PlanCard block.
	 * A plan for an already-seen ``reply_id`` is updated in place (the
	 * backend re-pushes it when the confirmation state flips).
	 */
	const appendPlanBlock = useCallback(
		(event: CustomEvent) => {
			const v = event.value as PlanEventValue | undefined;
			const steps = Array.isArray(v?.steps)
				? v.steps.filter((s): s is string => typeof s === 'string')
				: [];
			if (!v || steps.length === 0) return;
			const replyId = v.reply_id ?? '';
			if (replyId) {
				for (const m of msgsRef.current) {
					for (const b of m.content) {
						const pb = b as unknown as PlanBlock;
						if (pb.type === 'plan' && pb.replyId === replyId) {
							pb.steps = steps;
							pb.needsConfirmation = !!v.needs_confirmation;
							scheduleUpdate();
							return;
						}
					}
			...
			appendCustomBlock(event, {
				type: 'plan',
				id: `plan_${event.id}`,
				replyId,
				steps,
				needsConfirmation: !!v.needs_confirmation,
			});
		},
		[appendCustomBlock, scheduleUpdate],
	);
	...
	const processEvent = useCallback(
		(event: AgentEvent) => {
			...
			if (event.type === EventType.CUSTOM) {
				...
				} else if (custom.name === 'visual') {
					appendVisualBlock(custom);
				} else if (custom.name === 'plan') {
					appendPlanBlock(custom);
				}
				return;
			}
		...
		[scheduleUpdate, audioManager, clearInterruptTimer, appendVisualBlock, appendPlanBlock],
	);
		...
		(async () => {
			...
			try {
				...
				if ((e as Error).name !== 'AbortError' && !cancelled) {
					// 404 → session doesn't exist on backend; clear stale
					// localStorage entry so other pages don't keep hitting it.
